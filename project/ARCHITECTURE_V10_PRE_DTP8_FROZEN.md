...
async def cleanup_clients():
    """Close persistent httpx clients to prevent connection leaks."""
    await close_embed_client()
    logger.info("Persistent httpx clients closed.")
```
...
# AUDIT FIX (P7-13): Asyncio lock for pool flush serialization.
# During TEI crash, up to 8 concurrent embed_text() coroutines hit ConnectError.
# Without a lock, multiple coroutines enter the flush handler simultaneously:
# coroutine A calls aclose(), yields at await; coroutine B calls aclose() on the
# same (closing/closed) client, raising AttributeError or RuntimeError.
...
from backend.shared.tokenizer import TOKENIZER, count_tokens  # AUDIT FIX (P8-11, P2-03, P7-01)
...
async def chat(request: Request):
    ...
    try:
        ...
        try:
            await asyncio.wait_for(EMBED_SEMAPHORE.acquire(), timeout=5.0)
...
def hybrid_search(
    ...
    # Dynamic threshold: keep chunks within 30% of top score
    ...
    # AUDIT FIX (DT-P6-05): Mode-adaptive dynamic ratio.
    # In cosine space (dense-only), scores range widely (0.3–1.0). A 0.70 ratio
    # on a top score of 0.90 = threshold 0.63, brutally discarding chunks at 0.60
    # that are still highly relevant. Disable ratio in dense-only mode — let the
    # absolute floor (P6-04: 0.50) and top_k handle filtering.
    effective_ratio = min_score_ratio if len(prefetch_list) >= 2 else 0.0
...
def build_context(
    ...
    # Typical (with ledger, no chat, 2 messages): 32768 - 900 - 2550 - 2000 - 10 = 27,308 tokens for RAG
    # Typical (with ledger + chat ~1000, ~8 messages): 32768 - 900 - 2550 - 2000 - 1000 - 50 - 50 = 26,218 tokens
    ...
    # AUDIT FIX (P5-11): Cache separator token cost. The "\n\n---\n\n".join() between
...
│   = (message_count + 2) × 5                        │
│ ────────────────────────────────────────────────── │
│ RAG Context Budget                    ~24,218 tokens│
...
V9 comparison:
  ...
  V10 RAG budget: ~24,218 tokens (typical, with chat)
  Improvement: ~15.1× more context for diagnostic reasoning
```
...
# AUDIT FIX (P8-05): Asyncio lock for LLM pool flush serialization, matching P7-13 TEI pattern.
# Without a lock, concurrent ConnectError handlers (browser tab duplicates, frontend retry-on-timeout,
# stale requests queued behind 120s vLLM timeout) can double-close the httpx client.
...
MAX_USER_QUERY = 8000
FRAMEWORK_OVERHEAD = 250  # P7-17: conservative bound

def validate(path: str) -> bool:
    with open(path, 'r') as f:
        content = f.read()
    count = count_tokens(content)  # AUDIT FIX (P7-01)
    # AUDIT FIX (P7-10): Show both no-chat and worst-case budgets.
    # Original formula omitted chat_history (8000), user_query (8000), and
    # FRAMEWORK_OVERHEAD (250) — showing 27,318 available when worst-case
    # runtime is only 11,068. Operators reading the validator output assumed
    # the full 27,318 was available for RAG context.
    remaining_no_chat = 32768 - 900 - count - 2000 - FRAMEWORK_OVERHEAD
    remaining_worst_case = remaining_no_chat - MAX_CHAT_HISTORY - MAX_USER_QUERY
    
    print(f"Ledger tokens (Qwen2.5 native): {count}")
    print(f"Adjusted cap (15% safety): {ADJUSTED_CAP}")
    print(f"Budget remaining (no chat): {remaining_no_chat}")
