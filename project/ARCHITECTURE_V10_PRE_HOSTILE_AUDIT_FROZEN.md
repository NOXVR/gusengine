...
# Load native Qwen2.5 tokenizer for exact token counting
# AUDIT FIX (Opus + Gemini DT): Use AutoTokenizer, NOT tiktoken or linear regression
...
def index_chunk(client: QdrantClient, chunk: dict, chunk_id: int,
...
async def ingest_pdf(pdf_path: str, client: QdrantClient, start_id: int = 0) -> int:
...
> [!NOTE]
> **Query-time embedding:** In `backend/routes/chat.py`, user queries are embedded the same way before search:
> ```python
...
def hybrid_search(
    ...
    results = client.query_points(
        ...
        prefetch=[
            ...
            Prefetch(
                query=SparseVector(
                    # AUDIT FIX: Guard against None sparse vector if TEI
                    # /embed_sparse endpoint degrades (embed_text returns None).
...
def build_context(
    ...
    # Default (chat_history_tokens=0): 32768 - 900 - 2550 - 2000 = 27,318 tokens for RAG
    # Typical case (with ~1000 chat history): 26,318 tokens — see budget diagram below
    ...
    for i, chunk in enumerate(chunks):
        ...
        # Add formatting overhead (~20 tokens for header + separator)
        total_cost = chunk_tokens + 20
...
| GPU Memory Utilization | 0.85 | Leaves 15% headroom per GPU |
...
3. CITATION RULES (V10 — STRUCTURAL):
   ...
   b) Use the "page_numbers" field from chunk metadata for page citations. If multiple pages, cite the range.
