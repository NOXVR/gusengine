...
**VERSION:** V10 PROTOTYPE (RunPod Cloud — Single A100 80GB)
**CLASSIFICATION:** ZERO-TOLERANCE / LIFE-SAFETY ADJACENT
**HARDWARE TARGET:** Single NVIDIA A100 80GB (RunPod Cloud Pod)
**DATE:** 2026-02-28
**PREDECESSOR:** `ARCHITECTURE_V10.md` (2618 lines, 199 verified audit findings across 11 phases)

**STATUS:** PROTOTYPE — Cloud deployment on RunPod

> [!IMPORTANT]
> **This is the PROTOTYPE deployment document.** It deploys the fully-audited V10 architecture
> on a single NVIDIA A100 80GB GPU via RunPod cloud. The A100 80GB has MORE VRAM (80 GB) than
> the production spec's dual 4090s (48 GB), so this prototype runs the full 32B model at full
> 32K context — closer to production than any local hardware could achieve.
>
> **Key differences from production V10:**
> | Component | Production V10 | RunPod Prototype |
> |:----------|:---------------|:-----------------|
> | GPU | 2× RTX 4090 (48 GB) | 1× A100 80GB (80 GB) |
> | LLM | Qwen2.5-32B-AWQ (vLLM, TP=2) | Qwen2.5-32B-AWQ (vLLM, TP=1) |
> | Inference Server | vLLM (Docker, TP=2) | vLLM (Docker, TP=1) |
> | Embedding | TEI on GPU 0 | TEI on GPU |
> | Host OS | Ubuntu Server (bare metal) | Ubuntu (RunPod template) |
> | Context Window | 32K | 32K |
> | All other components | — | Identical to production V10 |
...
2. **Air-Gap Mandatory:** Zero external API calls at inference time. All models run inside the RunPod pod.
...
- Inference server added: vLLM with single A100 80GB (TP=1)
- Parser replaced: PyMuPDF flat text → Docling (IBM) with EasyOCR for scanned PDFs
- Embedding replaced: Voyage AI (API) → BGE-M3 via HuggingFace TEI (local, GPU)
...
- Context budget expanded: 4K total → 32K total (~25K usable RAG budget)
...
- Deployment: Docker Compose on RunPod cloud (vLLM, TEI, Qdrant, backend, frontend)
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    RUNPOD POD (Ubuntu 22.04)                       │
│                     A100 80GB SXM (80 GB VRAM)                      │
│                                                                     │
│  ┌──────────────────────────── Docker Compose ────────────────────┐ │
│  │                                                                │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐ │ │
...
│  │  │  TP=1 (A100)  │  │  Server       │  │  Dense + Sparse      │ │ │
│  │  │  Port: 8000   │  │  (GPU)        │  │  (CPU + Disk)        │ │ │
│  │  └──────────────┘  │  Port: 8080   │  │  Port: 6333          │ │ │
│  │                    └──────────────┘  └──────────────────────┘ │ │
│  │                                                                │ │
│  │  ┌──────────────────────────────────────────────────────────┐  │ │
...
> [!CAUTION]
> **RUNPOD A100 80GB:** Single A100 80GB has MORE VRAM than production V10's dual 4090s (48 GB).
> Qwen2.5-32B-AWQ fits comfortably with 51.5 GB headroom. No tensor parallelism needed.

### VRAM Allocation (Single A100 80GB)

| Component | VRAM (GB) | Notes |
|:----------|:---------:|:------|
| Qwen2.5-32B-Instruct-AWQ weights | ~18 | 4-bit AWQ quantization |
| KV Cache @ 32K context | ~8 | 64 layers × 8 KV heads × 128 dim × 32K × 2 (K+V) × FP16 |
| vLLM overhead (CUDA kernels, buffers) | ~2 | PagedAttention, scheduler |
| **Subtotal: LLM** | **~28** | **35% of 80 GB** |
| BGE-M3 embedding model | ~2 | 568M params, FP16 |
| TEI runtime overhead | ~0.5 | |
| **Subtotal: Embedding** | **~2.5** | |
| **TOTAL GPU** | **~30.5** | **38% of 80 GB** |
| **Headroom** | **~49.5** | OS, CUDA context, burst batches |

> [!TIP]
> **VRAM is extremely comfortable.** With only 38% used, 49.5 GB remains free.
> This is more headroom than the production spec (17.5 GB on dual 4090s).
> Future context expansion to 64K is feasible on this hardware.

### Future Scale-Up Path

| Target | Context | Hardware Required | Strategy |
|:-------|:-------:|:-----------------|:---------|
| **Prototype (THIS DOC)** | 32K | 1× A100 80GB (RunPod) | AWQ-4bit, TP=1 |
| **Extended Prototype** | 64K | 1× A100 80GB (RunPod) | AWQ-4bit, TP=1, tight budget |
| **Production V10** | 32K | 2× RTX 4090 (48 GB) | AWQ-4bit, TP=2 |
| **MVP Cloud** | 128K | 1× A100-80GB or 2× A6000-48GB | FP16 or AWQ |
| **Production Cloud** | 128K+ | 2× A100-80GB (160 GB) | Full precision, maximum context |

---

## RUNPOD SETUP (PROTOTYPE-SPECIFIC)

> [!IMPORTANT]
> **This section is PROTOTYPE-SPECIFIC.** Production V10 deploys on bare-metal Ubuntu.
> The prototype runs on a RunPod cloud pod with A100 80GB GPU.
> See `RUNPOD_DEPLOY.md` for the complete step-by-step deployment guide.

### Pod Configuration

| Setting | Value | Notes |
|:--------|:------|:------|
| **GPU** | 1× A100 80GB SXM | ~$0.79/hr on-demand |
| **Template** | RunPod PyTorch 2.1 | Ubuntu 22.04, CUDA 12.1, Docker pre-installed |
| **Volume** | 100 GB Network Volume | Persists models + PDFs across pod restarts |
| **Exposed Ports** | 80 (HTTP), 8888 (API) | Via RunPod proxy |
...
# Step 1: Verify GPU
nvidia-smi
# Expected: A100-SXM4-80GB, 81920 MiB memory

# Step 2: Clone repo to pod
git clone <your-repo-url> /workspace/GusEngine
cd /workspace/GusEngine

# Step 3: Download models (first time only — saved to network volume)
./scripts/download_models.sh   # See Model Pre-Download Commands below

# Step 4: Upload FSM PDFs
# From your Windows machine:
# scp -r J:\GusEngine\storage\pdfs user@<pod-ip>:/workspace/GusEngine/storage/pdfs/
...
> [!TIP]
> **Cost Management:** Stop your pod when not actively using GusEngine.
> Stopped pods cost $0/hr — only the network volume storage remains
> (~$0.07/GB/mo = ~$7/mo for 100 GB). Restart takes ~2 minutes.
...
```yaml
# docker-compose.yml — GusEngine V10 PROTOTYPE (RunPod A100 80GB)
# Pre-requisite: RunPod pod with A100 80GB GPU
...
services:
    ...
    deploy:
      resources:
        reservations:
          devices:
            - driver: nvidia
              count: 1  # PROTOTYPE: single A100 80GB
              capabilities: [gpu]
    command: >
  ...
  # PROTOTYPE: HTTP-only for RunPod (no TLS certs yet)
  frontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile
...
> [!NOTE]
> **Why BGE-M3 over bge-small-en-v1.5:** BGE-M3 provides: (1) 1024d vs 384d vectors for better semantic separation, (2) native sparse vector output for hybrid search, (3) 8192 token input window vs 512 for bge-small. Running on CPU in prototype — ~2 GB RAM cost is negligible with 64 GB system RAM.
...
|:----------|:------|:----------|
| Model | Qwen2.5-32B-Instruct-AWQ | Full production model on A100 80GB |
| Quantization | AWQ (4-bit) | Reduces 28GB FP16 → ~7.5GB |
| Tensor Parallelism | 1 | Single RTX 4080 |
| Max Model Length | 32768 | Proven feasible (~14.5 GB total) |
| GPU Memory Utilization | 0.90 | PROTOTYPE: TEI on CPU, more VRAM available |
