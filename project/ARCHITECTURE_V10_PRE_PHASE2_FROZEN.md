...
async def ingest_pdf(pdf_path: str, client: QdrantClient) -> int:
    ...
    for i, chunk in enumerate(chunks):
        ...
        chunk_id = uuid.uuid5(uuid.NAMESPACE_URL, f"{pdf_path}_{i}").hex
...
>     background_tasks.add_task(ingest_pdf, pdf_path, qdrant_client)
...
router = APIRouter()
SYSTEM_PROMPT = open("/app/config/system_prompt.txt").read()
...
async def chat(request: Request):
    ...
    try:
        ...
        results = hybrid_search(qdrant_client, query_dense, query_sparse)
    except Exception as e:
...
def hybrid_search(
    ...
    min_absolute_score: float = 0.005,  # AUDIT FIX: absolute RRF floor
) -> list[dict]:
...
// import DOMPurify from 'dompurify';  // bundled via npm pack, no CDN
...
function renderGusResponse(gus, containerEl, textInputEl) {
  ...
  // Mechanic instructions (LLM-generated HTML → MUST sanitize)
  if (gus.mechanic_instructions) {
    const instructions = document.createElement('div');
    instructions.className = 'gus-instructions';
    instructions.innerHTML = DOMPurify.sanitize(gus.mechanic_instructions);
    containerEl.appendChild(instructions);
  }
  
  // Diagnostic reasoning (LLM-generated → sanitize)
  if (gus.diagnostic_reasoning) {
    const reasoning = document.createElement('div');
    reasoning.className = 'gus-reasoning';
    reasoning.innerHTML = DOMPurify.sanitize(gus.diagnostic_reasoning);
