...
**VERSION:** V9 (Phase 9 Gap Recovery — 16 restorations from VFINAL, verified by 3 hostile audits)
...
sed -i '/^INTERNAL_API_KEY=/d' $HOME/diagnostic_engine/.env
...
**Goal:** Automate 20GB+ VMDK extraction with double-verified kernel locking, 5-page physical chunking, and 1MB I/O hashing. *(V9.2: Absolute page watermarking deprecated — see V9.2 Citation Strategy note in Phase 8.)*
...
def chunk_pdf(input_path, output_dir, file_hash, base_name, chunk_size=5):
    """Chunk PDF into 5-page segments.
    ...
    V9.2 DEPRECATION: Absolute page watermarks ([[ABSOLUTE_PAGE: N]])
    are no longer burned into the text layer. Production testing showed:
    (1) zero diagnostic accuracy benefit, (2) 2hr+ ingestion bottleneck,
    (3) AnythingLLM's 400-char text splitter breaks watermarks away from
    content. Document-level citation via sources[].title is sufficient.
    The watermark code below is preserved but SKIPPED at runtime.
    """
...
> [!NOTE]
> **V9 RECOVERY (D-4): Archive Ledger Lifecycle.** When the ledger exceeds the token cap, old entries are moved to a non-pinned `MASTER_LEDGER_ARCHIVE.md`. This archive file is still uploaded and embedded in the workspace — it participates in vector search (so historical tribal knowledge remains discoverable via RAG retrieval) but is NOT pinned, meaning it does not consume the fixed context token budget on every query. This preserves historical tribal knowledge while freeing pinned context budget for active fault signatures. The archive concept was documented in VFINAL Phase 10 (lines 516–518) but dropped during consolidation.
...
> [!IMPORTANT]
> **V9 RECOVERY (D-5): API Key Insertion Guide.** The following UI navigation paths are required to insert third-party API keys. These were dropped during consolidation because V8 assumes the operator already knows the AnythingLLM UI:
>
> | Provider | UI Navigation Path | Key Required |
> |:---------|:-------------------|:-------------|
> | **Anthropic** (LLM) | Settings → AI Providers → **Anthropic** → Insert API Key | Anthropic API key for `claude-3-5-sonnet-latest` |
> | **Voyage AI** (Embedding) | Settings → Embedding Preference → **Voyage AI** → Insert API Key | Voyage AI API key for `voyage-3-large` |
> | **Mistral** (OCR/Document Handling) | Settings → Document Handling → **Mistral OCR** | Mistral API key (enables table-preserving OCR during ingestion) |
> | **Cohere** (Reranking) | Settings → AI Providers → Reranking → **Cohere** → Insert API Key | Cohere API key for `rerank-english-v3.0` |
>
> **Source:** VFINAL Part 1, Phase 4, lines 404–412.
...
> ```
> System Prompt:        ~750 tokens (V9: +150 from R-1 through R-4 restorations)
> Pinned Ledger (cap):  ~1275 tokens (safety-adjusted cap)
> RAG Chunks (4 × 400): 1600 tokens
> TOTAL INPUT:          ~3625 tokens
> Response Budget:      ~375 tokens (Gus JSON is ~200 tokens — safe but tighter margin)
> ```
...
> [!IMPORTANT]
> **V9.2 CITATION STRATEGY (REPLACES V2 DUAL-LAYER):**
> The original V2 strategy burned `[[ABSOLUTE_PAGE: N]]` watermarks into every PDF page via PyMuPDF, then instructed the LLM to extract page numbers from watermarks. **This was deprecated in V9.2** after production testing revealed:
> 1. **Zero diagnostic accuracy benefit** — watermarks are purely citation UX; the LLM reasons from semantic content, not page numbers.
> 2. **Ingestion bottleneck** — watermarking + re-ingestion required 2+ hours per corpus update, creating an unacceptable production bottleneck.
> 3. **Chunk boundary problem** — AnythingLLM's text splitter (400 chars) breaks content at arbitrary boundaries, causing watermark tags to land in separate chunks from the content they annotate.
>
> **V9.2 RESOLUTION:** Document-level citations using the native `sources[].title` field from AnythingLLM. Mercedes FSM documents average 4.7 pages (514 PDFs / 2,442 pages), making document-name citation sufficient for mechanic reference. The document naming convention (e.g., `07.4.1-411 Checkup of Electronically Controlled Gasoline Injection System.pdf`) already encodes the FSM section number, which serves as a locator.
>
> **V2 DAG BEHAVIOR:**
...
> [!WARNING]
> **V9.3 OPEN INVESTIGATION — RETRIEVAL ACCURACY:**
> The diagnostic engine's accuracy is ultimately bounded by the quality of chunks the vector search returns. If AnythingLLM retrieves irrelevant or low-quality chunks, the LLM will reason from bad data regardless of how good the system prompt is. The following RAG pipeline parameters need empirical testing against known-answer diagnostic queries:
>
> | Parameter | Current Setting | Investigation Needed |
> |:----------|:---------------|:---------------------|
> | Embedding Model | Voyage AI | Is Voyage optimal for technical/automotive OCR text? Compare against alternatives |
> | Chunk Size | 400 chars | Too small? FSM procedures span multiple paragraphs. Test 800, 1200 |
> | Chunk Overlap | 20 chars | Likely too low — context lost at boundaries. Test 50, 100, 200 |
> | TopN Results | 4 | Is 4 enough? Some diagnostics span multiple documents. Test 6, 8 |
> | Reranking | Cohere | Is reranking actually improving relevance for FSM content? A/B test |
> | Similarity Threshold | Default | Should low-confidence chunks be filtered out? |
>
> **Methodology:** Build a test suite of 10-20 known-answer diagnostic queries (e.g., "K-Jetronic warm control pressure spec" → must return 07.4.1-411). Measure retrieval precision/recall across parameter combinations. This is the single highest-leverage optimization for production accuracy.
...
{
  "name": "Manual-Status", "hubId": "manual-status", "version": "1.0.0",
  "schema": "skill-1.0.0",
  "imported": true,
  "active": true,
...
> [!NOTE]
> The 3 additional agent skills (VIN-Lookup, Purchase-Router, Draft-Tribal-Knowledge) are provided in **Appendix A: Agent Skill Definitions** below. These skills do NOT require `$INTERNAL_KEY` — only @Manual-Status calls the internal API. Each skill uses a quoted heredoc (`cat << 'EOF'`) for simple file creation, plus `mkdir -p` for directory setup and `docker restart` after deployment. Toggle all skills ON in **Settings → Agent Skills**.
...
> [!IMPORTANT]
> The container is stopped before backup to prevent torn copies of LanceDB/SQLite databases. Downtime is ~2-5 minutes at 2 AM. Backup cron job uses absolute path `/usr/bin/docker` to prevent `command not found` failures under cron's minimal PATH environment. Semicolons (`;`) guarantee the container restart fires regardless of the tar exit code.
...
## V9.3 — RAG PIPELINE REASSESSMENT & DEPRECATION NOTICE

> [!CAUTION]
> **CRITICAL ARCHITECTURAL FINDING (2026-02-24):**
> Production testing and competitive benchmarking against Google NotebookLM revealed that the V9 ingestion, chunking, and retrieval pipeline is **fundamentally inadequate** for the stated goal of production-grade diagnostic accuracy. The findings below deprecate the RAG pipeline assumptions in Phases 4, 5, 7, and 8. The infrastructure (Phases 1-3), system prompt logic (Phase 8 DAG/state machine), agent skills (Phase 9), and operational tooling (Phases 10-12) remain valid and are NOT deprecated.

### Discovery Context

A production-identical NotebookLM instance was built with the same 514 FSM PDFs, a simplified natural-language system prompt, and shared with working Mercedes mechanics who confirmed its diagnostic accuracy "beyond a doubt." When the same diagnostic query was run against both systems, the NotebookLM Gus demonstrated capabilities that the V9 AnythingLLM pipeline cannot replicate.

**Test Query:** `"Hot start vapor lock, cranks but won't catch."`

| Capability | NotebookLM (Benchmark) | V9 AnythingLLM | Gap |
|:-----------|:-----------------------|:---------------|:----|
| **Documents retrieved per query** | Multiple sections from 3+ documents simultaneously | 4 chunks of ~400 chars from potentially 1-2 documents | CRITICAL |
| **Chunking strategy** | Structural/semantic segmentation following headings, figures, logical sections | Blind RecursiveTextSplitter at 400 chars, 20 overlap | CRITICAL |
| **Image/diagram support** | Preserved during ingestion, displayed in citation hovers, original PDF rendering available | Discarded during Mistral OCR — all diagrams, schematics, wiring diagrams lost | CRITICAL |
| **Cross-document reasoning** | Retrieved C.I.S. Diagnosis Chart + Static Pressure Test docs + K-Jetronic manual in single turn | No multi-document awareness — vector search returns nearest chunks regardless of source | CRITICAL |
| **Citation granularity** | Section-level semantics ("Electric fuel pump", "Fuel accumulator", "Air-flow sensor") with hoverable original PDF rendering | Document filename only (V9.2) or watermark page number (V9, deprecated) | SIGNIFICANT |
| **Diagnostic flowchart following** | Identified Test 3, 4, 6 from C.I.S. Problem Diagnosis Chart; each answer path maps to a component elimination pathway | No awareness of FSM diagnostic flowcharts as structured decision trees | CRITICAL |
| **Corpus utilization** | Gemini 1.5 Pro with 1M token context — can hold significant corpus portions simultaneously | 4 × 400 chars = 1,600 chars of context per query out of multi-GB corpus | CRITICAL |

### Architectural Findings

#### Finding 1: Flat Text Chunking Destroys Document Intelligence

The RecursiveTextSplitter (400 chars, 20 overlap) has zero awareness of document structure. It splits mid-sentence, mid-procedure, mid-specification. A diagnostic procedure that spans 3 paragraphs across a page gets fragmented into 6+ chunks, each losing the context of the whole. NotebookLM's structural segmentation preserves logical document units — headings, figures, complete procedures, specification tables — as atomic retrieval units.

**Impact:** The LLM receives context-free text fragments instead of complete diagnostic information. It cannot follow multi-step procedures because steps are split across chunks that may not all be retrieved.

#### Finding 2: Image/Diagram Loss Eliminates Critical Diagnostic Data

Mistral OCR converts PDFs to plain text and discards all visual content. For an automotive diagnostic system, this means:
- **Wiring diagrams** — lost. A mechanic troubleshooting electrical issues cannot see the circuit.
- **Component illustrations** — lost. Exploded views showing part locations are critical for physical reference.
- **Specification tables** — partially preserved as text but lose formatting context.
- **Flowcharts** — the FSM diagnostic flowcharts (e.g., C.I.S. Problem Diagnosis Chart) are images that get reduced to OCR garbage or partial text extraction.

NotebookLM preserves the original PDF rendering and displays it in citation hovers. The LLM operates on extracted text, but the user can see the original PDF source including all images. This dual-layer architecture (text for LLM, original for display) is a requirement for production.

**Impact:** The system cannot provide visual references that mechanics rely on daily. A wiring diagram description is not a substitute for the diagram itself.

#### Finding 3: TopN=4 Is Insufficient for Multi-Document Diagnostics

Real diagnostic reasoning requires correlating information across multiple FSM documents:
1. **Symptom → Diagnosis Chart** (which tests to run)
2. **Test Procedure** (how to run the test)
3. **Component Specification** (what values are correct)
4. **System Overview** (how the component fits in the whole)

With TopN=4 and 400-char chunks, the system may retrieve 4 fragments from one document and miss the other three entirely. NotebookLM retrieved from all three relevant sources simultaneously in a single query.

**Impact:** Diagnostics that require cross-referencing multiple manuals will fail or produce incomplete guidance.

#### Finding 4: No Autonomous Cross-Referencing Capability

NotebookLM's response demonstrated autonomous cross-referencing: after identifying the symptom in the Diagnosis Chart, it pulled the Static Pressure Test procedure, then verified the accumulator's purpose in the K-Jetronic Technical manual. Each source informed the next retrieval.

The V9 pipeline has no retrieval chaining. Each user turn triggers one vector search, returns 4 chunks, and the LLM must reason from those alone. The diagnostic funnel (PHASE_A → PHASE_B → PHASE_C) uses user selections to drive new queries, but the LLM cannot autonomously decide "I need to also check the wiring diagram for this circuit before I answer."

**Impact:** The system's diagnostic depth is limited to what a single 4-chunk retrieval happens to contain.

#### Finding 5: NotebookLM's Dual-Layer Architecture

Interrogation of NotebookLM revealed it operates with two parallel systems:
- **LLM Layer:** Receives "raw, sequential plain-text strings divided into numerical excerpts" — i.e., indexed text segments, NOT fixed-size chunks. The segmentation appears to follow document structure.
- **Display Layer:** Preserves the original PDF rendering with images, formatting, and page layouts. Citation hovers render the original PDF page, not the extracted text.

This means NotebookLM's ingestion pipeline produces:
1. Structurally-segmented text for the LLM to reason with
2. Preserved original documents for human reference

Our V9 pipeline produces only (1), and does so with inferior segmentation.

### Deprecated Components

> [!WARNING]
> The following V9 components are **DEPRECATED** pending V10 pipeline redesign. They remain in the architecture document for historical reference but should NOT be used for production ingestion.

| Component | Phase | Reason for Deprecation |
|:----------|:------|:-----------------------|
| `chunk_pdf()` with 5-page physical chunking | Phase 4 | Arbitrary page-count splitting ignores document structure |
| `[[ABSOLUTE_PAGE: N]]` watermarking | Phase 4 | Zero accuracy benefit, ingestion bottleneck (V9.2) |
| `sync_ingest.py` flat file upload | Phase 5 | No structural parsing, no image preservation, no metadata |
| RecursiveTextSplitter 400/20 | Phase 7 | Blind character-count splitting destroys document intelligence |
| Voyage AI embedding (as sole retrieval) | Phase 7 | May not be optimal for technical/automotive OCR text (untested) |
| TopN=4 | Phase 7 | Insufficient for multi-document diagnostic reasoning |
| Cohere reranking (unvalidated) | Phase 7 | Never empirically tested for FSM retrieval quality |
| Dual-layer watermark citation rules | Phase 8 | Replaced by V9.2 document-level citations, then deprecated entirely |

### V10 Investigation Roadmap

The following capabilities must be evaluated for the replacement pipeline:

**1. Structural Document Parsing**
- Replace flat OCR with a parser that segments by headings, sections, figures, and logical boundaries
- Candidates: Google Document AI, LlamaParse, Docling, custom PyMuPDF structural extractor
- Output: hierarchical document representation, not flat text

**2. Multimodal Ingestion**
- Preserve images/diagrams from PDFs as separate indexed assets
- Link images to their surrounding text context
- Enable citation hovers to display original PDF rendering
- Candidates: multimodal embeddings (CLIP, SigLIP), Document AI with image extraction

**3. Intelligent Retrieval**
- Increase retrieval depth (TopN ≥ 8, test up to 16)
- Implement hybrid search (vector + keyword/BM25)
- Test retrieval chaining (initial retrieval → reasoning → secondary retrieval)
- Consider parent-child chunk relationships (retrieve a small chunk, inject its parent section)

**4. Expanded Context Utilization**
- Evaluate models with larger effective context windows
- Test whether injecting full document sections (vs. small chunks) improves diagnostic accuracy
- Quantify the relationship between context size and diagnostic quality

**5. Dual-Layer Architecture**
- Implement a display layer that preserves original PDFs for citation rendering
- Map text segments back to PDF page/position coordinates
- Enable "click citation → see original PDF page with diagrams"

**6. Empirical Validation Framework**
- Build a test suite of 10-20 known-answer diagnostic queries
- Measure retrieval precision/recall against the NotebookLM benchmark
- A/B test every pipeline parameter change with ground-truth queries
- The NotebookLM instance serves as the accuracy oracle

> [!IMPORTANT]
> **DESIGN PRINCIPLE:** The goal is not to replicate NotebookLM. The goal is to **surpass it** by an order of magnitude — with faster retrieval, deeper cross-referencing, richer citation rendering, and diagnostic accuracy that a master mechanic would trust with their reputation. NotebookLM is the floor, not the ceiling.

---

## APPENDIX A: AGENT SKILL DEFINITIONS

> [!NOTE]
> **V9 RECOVERY (I-3, I-4, I-5):** These three agent skills were restored from VFINAL Phase 7 (lines 83–192). They were silently dropped during the VFINAL → Phase 6 consolidation and never existed past VFINAL. None of these skills call the internal API — they do NOT use `$INTERNAL_KEY` or the `sed` + `REPLACE_ME_KEY` injection pattern. VIN-Lookup calls the public NHTSA API. Purchase-Router and Draft-Tribal-Knowledge require no API calls.

### Skill: @VIN-Lookup

Decodes 17-digit VINs via the NHTSA public API. For classic chassis codes (< 17 chars), bypasses the API and returns a manual lock instruction.

```bash
mkdir -p $ENGINE_DIR/plugins/agent-skills/vin-lookup

cat << 'EOF' > $ENGINE_DIR/plugins/agent-skills/vin-lookup/plugin.json
{
  "name": "VIN-Lookup",
  "hubId": "vin-lookup",
  "version": "1.0.0",
...
cat << 'EOF' > $ENGINE_DIR/plugins/agent-skills/vin-lookup/handler.js
module.exports.runtime = {
...
### Skill: @Purchase-Router

Generates vendor search links when Gus determines that required FSM documentation is missing from the workspace.

```bash
mkdir -p $ENGINE_DIR/plugins/agent-skills/purchase-router

cat << 'EOF' > $ENGINE_DIR/plugins/agent-skills/purchase-router/plugin.json
{
...
cat << 'EOF' > $ENGINE_DIR/plugins/agent-skills/purchase-router/handler.js
module.exports.runtime = {
...
### Skill: @Draft-Tribal-Knowledge

Formats undocumented shop fixes into the FAULT SIGNATURE structure required by `MASTER_LEDGER.md`. Output requires Shop Manager approval before pinning.

```bash
mkdir -p $ENGINE_DIR/plugins/agent-skills/draft-tribal-knowledge

cat << 'EOF' > $ENGINE_DIR/plugins/agent-skills/draft-tribal-knowledge/plugin.json
{
...
cat << 'EOF' > $ENGINE_DIR/plugins/agent-skills/draft-tribal-knowledge/handler.js
module.exports.runtime = {
...
After deploying all three skills, go to AnythingLLM **Settings → Agent Skills** and toggle VIN-Lookup, Purchase-Router, and Draft-Tribal-Knowledge to **ON**.

---

## APPENDIX B: FRONTEND REFERENCE IMPLEMENTATION

> [!NOTE]
> **V9 RECOVERY (D-1, D-2, D-3):** The deployed Gus system uses AnythingLLM's built-in chat UI, which handles message rendering and API communication natively. However, a **custom frontend** (such as a shop dashboard) MUST implement the rendering and API integration logic below. These functions were the complete reference implementation in VFINAL Phase 9 (lines 281–371) but were dropped during the VFINAL → Phase 6 consolidation because the scope was reduced from "Reference Implementation" to "UI Logic Integration." The `parseGusResponse()` and `buildUserMessage()` functions from Phase 11 handle JSON parsing and state transition injection — this appendix completes the loop by showing how to **render** Gus's output and **send** messages to the API.

### D-1: `renderGusResponse()` — UI Rendering Function

This function renders the Gus JSON response into the shop dashboard UI: state badge, mechanic instructions, citation bubbles, answer-path buttons, text input control, and completion state.

**Source:** `SECTION_B_VFINAL_BLUEPRINT_PART2.md`, lines 281–346.

> [!WARNING]
> **Security: XSS Risk.** This reference implementation uses `.innerHTML` to render `gus.mechanic_instructions` and `gus.current_state` (lines 1716, 1730). These values originate from LLM JSON output derived from RAG-retrieved documents. If a malicious or corrupted document contains HTML/JavaScript payloads, they will execute in the browser. **Any custom frontend deployment MUST replace `.innerHTML` with a sanitized rendering method** — either `DOMPurify.sanitize()` before innerHTML assignment, or `.textContent` / `document.createElement()` for all LLM-controlled strings. The deployed Gus system uses AnythingLLM's built-in chat UI, which handles its own output sanitization.
...
### D-2: `sendToAnythingLLM()` — API Integration Function

This function sends messages to the AnythingLLM workspace chat API, parses the response, and triggers a re-render of the Gus UI.

**Source:** `SECTION_B_VFINAL_BLUEPRINT_PART2.md`, lines 348–371.

> [!WARNING]
> `YOUR_SERVER_IP` and `YOUR_INTERNAL_API_KEY` in the code below are placeholders. Replace them with your actual server address and the API key generated during Phase 3.
...
The following CSS classes and DOM element IDs are used by `renderGusResponse()` and `sendToAnythingLLM()`. Any custom frontend implementation MUST define styles for these selectors.

**Source:** Extracted from `SECTION_B_VFINAL_BLUEPRINT_PART2.md`, lines 281–346.

#### CSS Classes

| Class Name | Element | Purpose |
|:-----------|:--------|:--------|
| `gus-error` | `<div>` | Wraps error/retrieval failure state display (RETRIEVAL_FAILURE, PHASE_ERROR) |
| `gus-state-badge` | `<div>` | Displays the current DAG state (PHASE_A_TRIAGE, PHASE_B_FUNNEL, etc.) |
| `gus-instructions` | `<div>` | Contains the `mechanic_instructions` text with heading |
| `gus-citations` | `<div>` | Container for source citation bubbles |
| `gus-citation-bubble` | `<span>` | Individual citation bubble showing source document and page number |
| `gus-buttons` | `<div>` | Container for answer-path prompt buttons |
| `gus-answer-btn` | `<button>` | Individual answer-path button (tap target for mechanic input) |
| `gus-complete` | `<div>` | Diagnostic completion indicator (shown when `requires_input` is false) |

#### DOM Element IDs

| Element ID | Element | Purpose |
|:-----------|:--------|:--------|
| `gus-container` | `<div>` | Main container element passed to `renderGusResponse()` — all Gus output renders here |
| `symptom-input` | `<input>` | Text input field for free-text symptom entry — disabled during button-mode diagnostics, re-enabled at PHASE_D completion |
