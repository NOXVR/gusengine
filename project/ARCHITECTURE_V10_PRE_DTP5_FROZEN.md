...
async def close_embed_client():
    """AUDIT FIX (P5-05): Graceful shutdown — close persistent httpx client."""
    global _http_client
...
>     # AUDIT FIX (DT-P4-04): The host daemon sends absolute HOST paths
>     # (e.g. /home/user/storage/pdfs/manual.pdf). The container volume mount
>     # maps that directory to /app/pdfs. Extract filename and construct
>     # the container-relative path before validation.
>     filename = os.path.basename(body["pdf_path"])
>     pdf_path = os.path.realpath(os.path.join(ALLOWED_PDF_DIR, filename))
...
async def chat(request: Request):
    ...
    if chat_history_tokens > MAX_CHAT_HISTORY_TOKENS:
        original_history = list(chat_history)  # AUDIT FIX (P5-02): snapshot for gap detection
        truncated = []
        running = 0
        for msg in reversed(chat_history):
            msg_tokens = len(TOKENIZER.encode(msg["content"]))
            if running + msg_tokens > MAX_CHAT_HISTORY_TOKENS and truncated:
                continue  # AUDIT FIX (DT-P4-05): Skip oversized messages, keep salvaging older ones
        ...
        # AUDIT FIX (P5-02): Detect non-contiguous eviction — when `continue` skips
        # mid-history messages, the LLM may see a jumbled conversation with missing
        # context. Also flag consecutive same-role messages (violates chat template).
        if len(truncated) < len(original_history):
            kept_ids = set(id(m) for m in truncated)
            skipped_mid = False
            for j, m in enumerate(original_history):
                if id(m) not in kept_ids:
                    if j > 0 and j < len(original_history) - 1:
                        if id(original_history[j-1]) in kept_ids and \
                           id(original_history[j+1]) in kept_ids:
                            skipped_mid = True
            if skipped_mid:
                logger.warning("Non-contiguous eviction: mid-history messages skipped (P5-02)")
        # Check for consecutive same-role messages (may violate chat template)
        for j in range(len(truncated) - 1):
            if truncated[j]["role"] == truncated[j+1]["role"]:
                logger.warning(
                    f"Eviction produced consecutive {truncated[j]['role']} messages "
                    f"at positions {j},{j+1} — may violate Qwen2.5 chat template (P5-02)"
                )
        
        # AUDIT FIX (P4-02 + P5-06): If P3-02 kept a single oversized message, its actual
        # tokens exceed MAX_CHAT_HISTORY_TOKENS. Force-truncate the content to fit.
        # P5-06: Append truncation marker so the LLM doesn't try to "complete" the cut text.
        if chat_history_tokens > MAX_CHAT_HISTORY_TOKENS and len(chat_history) == 1:
            ...
            if len(tokens) > MAX_CHAT_HISTORY_TOKENS:
                msg["content"] = TOKENIZER.decode(
                    tokens[:MAX_CHAT_HISTORY_TOKENS - 10]
...
def hybrid_search(
    ...
    # AUDIT FIX (P5-04): Mode-adaptive absolute score floor.
    # RRF scores cluster in [0.0125, 0.0164] — 0.013 catches off-topic queries.
    # Dense cosine scores range [0, 1] — 0.013 is trivially passed, leaving no
    # off-topic rejection. Use 0.35 for dense-only mode to reject garbage results.
    effective_min_absolute = min_absolute_score if len(prefetch_list) >= 2 else 0.35
