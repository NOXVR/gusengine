...
def parse_and_chunk(pdf_path: str, max_tokens: int = 512) -> list[dict]:
    ...
    class LockedTokenizer:
        ...
        def __getattr__(self, name):
            return getattr(self.tok, name)  # Delegate read-only attributes
...
async def cleanup_clients():
    ...
    logger.info("All httpx clients and thread pools closed.")
```
...
def build_context(
    ...
    # Typical (with ledger, no chat, 2 messages): 32768 - 900 - 2550 - 2000 - 10 - 15 = 27,293 tokens for RAG
...
│   = (message_count + 2) × 5                        │
│ ────────────────────────────────────────────────── │
│ RAG Context Budget                    ~26,218 tokens│
...
V9 comparison:
  ...
  V10 RAG budget: ~26,218 tokens (typical, with chat)
