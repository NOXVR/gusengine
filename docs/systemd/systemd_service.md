...
## AUTOMATIC DEPENDENCIES          top
   ...
   Implicit Dependencies
       ...
       â¢   Services with Type=dbus set automatically acquire dependencies
           of type Requires= and After= on dbus.socket.

       â¢   Socket activated services are automatically ordered after
   ...
   Default Dependencies
       ...
       â¢   Service units will have dependencies of type Requires= and
       ...
       â¢   Instanced service units (i.e. service units with an "@" in
...
## OPTIONS          top
       ...
       Type=
           ...
           â¢   If set to simple (the default if ExecStart= is specified
           ...
           â¢   The exec type is similar to simple, but the service
           ...
           â¢   If set to forking, the manager will consider the unit
           ...
           â¢   Behavior of oneshot is similar to exec; however, the
           ...
           â¢   Behavior of dbus is similar to simple; however, units of
           ...
           â¢   Behavior of notify is similar to exec; however, it is
           ...
           â¢   Behavior of notify-reload is similar to notify, with one
               ...
               CLOCK_MONOTONIC in clock_gettime(2)) in Î¼s, formatted as
           ...
           â¢   Behavior of idle is very similar to simple; however,
           ...
           is only established by the service itself â in contrast to
           ...
           service at the appropriate time) â if it is not supported,
       ...
       ExitType=
           ...
           â¢   If set to main (the default), the service manager will
           ...
           â¢   If set to cgroup, the service will be considered running
       ...
       PIDFile=
           ...
           below /run/. If a relative path is specified for system
           service, then it is hence prefixed with /run/, and prefixed
           with $XDG_RUNTIME_DIR if specified in a user service. The
           service manager will read the PID of the main process of the
           service from this file after start-up of the service. The
           service manager will not write to the file configured here,
           although it will remove the file after the service has shut
           down if it still exists. The PID file does not need to be
           owned by a privileged user, but if it is owned by an
           unprivileged user additional safety restrictions are enforced:
           the file may not be a symlink to a file owned by a different
           user (neither directly nor indirectly), and the PID file must
       ...
       ExecReload=
           ...
           service. This setting may take multiple command lines,
           ...
           without completion notification (as is the case with the
           example line above) is usually not a good choice, because this
           is an asynchronous operation and hence not suitable when
           ordering reloads of multiple services against each other. It
           is thus strongly recommended to either use Type=notify-reload,
           or to set ExecReload= to a command that not only triggers a
           ...
           This setting can be combined with Type=notify-reload, in which
           case the service main process is signaled after all specified
           command lines finish execution. Specially, if "RELOADING=1"
           notification is received before ExecReload= completes, the
           signaling is skipped and the service manager immediately
           starts listening for "READY=1".

       ExecReloadPost=
           Commands to execute after a successful reload operation.
           Syntax for this setting is exactly the same as ExecReload=.
       ...
       ExecStopPost=
           ...
           supported. Note that â unlike ExecStop= â commands specified
       ...
       RestartSteps=
           Configures the number of exponential steps to take to increase
           the interval of auto-restarts from RestartSec= to
       ...
       Restart=
           ...
           â¢   for types other than Type=oneshot, one of the signals
               SIGHUP, SIGINT, SIGTERM, or SIGPIPE;

           â¢   exit statuses and signals specified in SuccessExitStatus=.
           ...
           â Restart       â no â always â on-success â on-failure â on-abnormal â on-abort â on-watchdog â
           â settings/Exit â    â        â            â            â             â          â             â
           â causes        â    â        â            â            â             â          â             â
           âââââââââââââââââ¼âââââ¼âââââââââ¼âââââââââââââ¼âââââââââââââ¼ââââââââââââââ¼âââââââââââ¼ââââââââââââââ¤
           â Clean exit    â    â X      â X          â            â             â          â             â
           â code or       â    â        â            â            â             â          â             â
           â signal        â    â        â            â            â             â          â             â
           âââââââââââââââââ¼âââââ¼âââââââââ¼âââââââââââââ¼âââââââââââââ¼ââââââââââââââ¼âââââââââââ¼ââââââââââââââ¤
           â Unclean exit  â    â X      â            â X          â             â          â             â
           â code          â    â        â            â            â             â          â             â
           âââââââââââââââââ¼âââââ¼âââââââââ¼âââââââââââââ¼âââââââââââââ¼ââââââââââââââ¼âââââââââââ¼ââââââââââââââ¤
           â Unclean       â    â X      â            â X          â X           â X        â             â
           â signal        â    â        â            â            â             â          â             â
           âââââââââââââââââ¼âââââ¼âââââââââ¼âââââââââââââ¼âââââââââââââ¼ââââââââââââââ¼âââââââââââ¼ââââââââââââââ¤
           â Timeout       â    â X      â            â X          â X           â          â             â
           âââââââââââââââââ¼âââââ¼âââââââââ¼âââââââââââââ¼âââââââââââââ¼ââââââââââââââ¼âââââââââââ¼ââââââââââââââ¤
           â Watchdog      â    â X      â            â X          â X           â          â X           â
           âââââââââââââââââ¼âââââ¼âââââââââ¼âââââââââââââ¼âââââââââââââ¼ââââââââââââââ¼âââââââââââ¼ââââââââââââââ¤
           â Termination   â    â X      â            â X          â X           â          â             â
           â due to OOM    â    â        â            â            â             â          â             â
           âââââââââââââââââ´âââââ´âââââââââ´âââââââââââââ´âââââââââââââ´ââââââââââââââ´âââââââââââ´ââââââââââââââ
       ...
       RestartMode=
           ...
           â¢   If set to normal (the default), the service restarts by
               going through a failed/inactive state.

               Added in version 254.

           â¢   If set to direct, the service transitions to the
           ...
           â¢   If set to debug, the service manager will log messages
       ...
       RootDirectoryStartOnly=
           ...
           ExecStartPost=, ExecReload=, ExecReloadPost=, ExecStop=, and
       ...
       OOMPolicy=
           ...
           to stop the event is logged and the unit's processes are
           terminated cleanly by the service manager. If set to kill and
           one of the unit's processes is killed by the OOM killer the
           kernel is instructed to kill all remaining processes of the
           unit too, by setting the memory.oom.group attribute to 1; also
           see kernel page Control Group v2[3]. In case of both stop and
           kill, the service ultimately ends up in the oom-kill failed
       ...
       OpenFile=
           ...
           â¢   "path" is a path to a file or an AF_UNIX socket in the
               file system;

           â¢   "fd-name" is a name that will be associated with the file
           ...
           â¢   "options" is a comma-separated list of access options;
...
## COMMAND LINES          top
       ...
       â Prefix â Effect                      â
       ââââââââââ¼ââââââââââââââââââââââââââââââ¤
       â "@"    â If the executable path      â
       â        â is prefixed with "@",       â
       â        â the second specified        â
       â        â token will be passed as     â
       â        â argv[0] to the executed     â
       â        â process (instead of the     â
       â        â actual filename),           â
       â        â followed by the further     â
       â        â arguments specified,        â
       â        â unless "|" is also          â
       â        â specified, in which case    â
       â        â it enables login shell      â
       â        â semantics for the shell     â
       â        â spawned by prefixing "-"    â
       â        â to argv[0].                 â
       ââââââââââ¼ââââââââââââââââââââââââââââââ¤
       â "-"    â If the executable path      â
       â        â is prefixed with "-", an    â
       â        â exit code of the command    â
       â        â normally considered a       â
       â        â failure (i.e. non-zero      â
       â        â exit status or abnormal     â
       â        â exit due to signal) is      â
       â        â recorded, but has no        â
       â        â further effect and is       â
       â        â considered equivalent to    â
       â        â success.                    â
       ââââââââââ¼ââââââââââââââââââââââââââââââ¤
       â ":"    â If the executable path      â
       â        â is prefixed with ":",       â
       â        â environment variable        â
       â        â substitution (as            â
       â        â described below this        â
       â        â table) is not applied.      â
       ââââââââââ¼ââââââââââââââââââââââââââââââ¤
       â "+"    â If the executable path      â
       â        â is prefixed with "+"        â
       â        â then the process is         â
       â        â executed with full          â
       â        â privileges. In this mode    â
       â        â privilege restrictions      â
       â        â configured with User=,      â
       â        â Group=,                     â
       â        â CapabilityBoundingSet=      â
       â        â or the various file         â
       â        â system namespacing          â
       â        â options (such as            â
       â        â PrivateDevices=,            â
       â        â PrivateTmp=) are not        â
       â        â applied to the invoked      â
       â        â command line (but still     â
       â        â affect any other            â
       â        â ExecStart=, ExecStop=,      â
       â        â ... lines). However,        â
       â        â note that this will not     â
       â        â bypass options that         â
       â        â apply to the whole          â
       â        â control group, such as      â
       â        â DevicePolicy=, see          â
       â        â systemd.resource-control(5) â
       â        â for the full list.          â
       ââââââââââ¼ââââââââââââââââââââââââââââââ¤
       â "!"    â Similar to the "+"          â
       â        â character discussed above   â
       â        â this permits invoking       â
       â        â command lines with elevated â
       â        â privileges. However, unlike â
       â        â "+" the "!" character       â
       â        â exclusively alters the      â
       â        â effect of User=, Group= and â
       â        â SupplementaryGroups=, i.e.  â
       â        â only the stanzas that       â
       â        â affect user and group       â
       â        â credentials. Note that this â
       â        â setting may be combined     â
       â        â with DynamicUser=, in which â
       â        â case a dynamic user/group   â
       â        â pair is allocated before    â
       â        â the command is invoked, but â
       â        â credential changing is left â
       â        â to the executed process     â
       â        â itself.                     â
       ââââââââââ¼ââââââââââââââââââââââââââââââ¤
       â "|"    â If "|" is specified         â
       â        â standalone as executable    â
       â        â path, invoke the default    â
       â        â shell of User=. If          â
       â        â specified as a prefix, use  â
       â        â the shell ("-c") to spawn   â
       â        â the executable. When "@" is â
       â        â used in conjunction,        â
       â        â argv[0] of shell will be    â
       â        â prefixed with "-" to enable â
       â        â login shell semantics.      â
       ââââââââââ´ââââââââââââââââââââââââââââââ

       "@", "|", "-", ":", and one of "+"/"!"  may be used together and
       they can appear in any order. However, "+" and "!"  may not be
       ...
       Unless for commands with the special executable prefix ":", to
       pass a literal dollar sign, use "$$". Variables whose value is not
       known at expansion time are treated as empty strings. Note that
       ...
       invokes the user's default shell which isn't deterministic. It's
       recommended to specify a shell implementation explicitly if
       portability is desired. Example:
...
systemd 260~devel                                      SYSTEMD.SERVICE(5)
