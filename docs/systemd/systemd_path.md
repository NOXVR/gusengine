# Source: https://man7.org/linux/man-pages/man5/systemd.path.5.html
...
## DESCRIPTION          top
       ...
       foo.service. The unit to activate may be controlled by Unit= (see
       below).
       ...
       unit, see StartLimitIntervalSec= and StartLimitBurst= in
...
## AUTOMATIC DEPENDENCIES          top
   ...
   Implicit Dependencies
       ...
       â¢   If a path unit is beneath another mount unit in the file
           system hierarchy, both a requirement and an ordering
           dependency between both units are created automatically.

       â¢   An implicit Before= dependency is added between a path unit
   ...
   Default Dependencies
       ...
       â¢   Path units will automatically have dependencies of type
...
## OPTIONS          top
       ...
       DirectoryNotEmpty=
           ...
           PathExistsGlob= works similarly, but checks for the existence
           ...
           PathModified= is similar, but additionally it is activated
           also on simple writes to the watched file.  DirectoryNotEmpty=
           ...
           of DirectoryNotEmpty=) at the time the path unit is activated,
           then the configured unit is immediately activated as well.
           Something similar does not apply to PathChanged= and
           PathModified=.
...
systemd 260~devel                                         SYSTEMD.PATH(5)
