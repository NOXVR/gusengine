# Source: https://man7.org/linux/man-pages/man5/systemd.resource-control.5.html
...
## DESCRIPTION          top
   ...
   Enabling and disabling controllers
       ...
       systemd (in the user@.service unit), it will repeat the same logic
       ...
       the child slices beneath user@1000.service. Assuming that there is
...
## IMPLICIT DEPENDENCIES          top
       ...
       â¢   Units with the Slice= setting set automatically acquire
...
## OPTIONS          top
   ...
   CPU Control
       CPUWeight=weight, StartupCPUWeight=weight
           ...
           â¢   If set to an integer value, assign the specified CPU time
           ...
           â¢   If set to the special string "idle", mark the cgroup for
   ...
   Process Accounting and Control
       ...
       TasksMax=N
           ...
           attribute. For details about this control group attribute, see
           the pids controller[7]. The effective configuration is
           reported as EffectiveTasksMax=.
   ...
   Network Accounting and Control
       IPAccounting=
           ...
           the socket unit â and never to the service unit it might have
       ...
       IPAddressDeny=ADDRESS[/PREFIXLENGTH]...
           ...
           â¢   Access is granted when the checked IP address matches an
               entry in the IPAddressAllow= list.

           â¢   Otherwise, access is denied when the checked IP address
               matches an entry in the IPAddressDeny= list.

           â¢   Otherwise, access is granted.
           ...
           slice containing all system services system.slice â see
           ...
           â Symbolic Name â Definition     â Meaning            â
           âââââââââââââââââ¼âââââââââââââââââ¼âââââââââââââââââââââ¤
           â any           â 0.0.0.0/0 ::/0 â Any host           â
           âââââââââââââââââ¼âââââââââââââââââ¼âââââââââââââââââââââ¤
           â localhost     â 127.0.0.0/8    â All addresses on   â
           â               â ::1/128        â the local loopback â
           âââââââââââââââââ¼âââââââââââââââââ¼âââââââââââââââââââââ¤
           â link-local    â 169.254.0.0/16 â All link-local IP  â
           â               â fe80::/64      â addresses          â
           âââââââââââââââââ¼âââââââââââââââââ¼âââââââââââââââââââââ¤
           â multicast     â 224.0.0.0/4    â All IP             â
           â               â ff00::/8       â multicasting       â
           â               â                â addresses          â
           âââââââââââââââââ´âââââââââââââââââ´âââââââââââââââââââââ
       ...
       SocketBindAllow=bind-rule, SocketBindDeny=bind-rule
           ...
           â¢   Binding to a socket is allowed when a socket address
               matches an entry in the SocketBindAllow= list.

           â¢   Otherwise, binding is denied when the socket address
               matches an entry in the SocketBindDeny= list.

           â¢   Otherwise, binding is allowed.
       ...
       BindNetworkInterface=
           Takes the name of a network interface. This option causes
           every socket created by processes of this unit to be bound to
           the specified network interface.

           It is specially useful to confine a process to a VRF, when the
           program does not offer native support for it. It is equivalent
           to running the program using ip vrf exec.

           In systems using nss-resolve, the interface used for DNS
           resolution can be chosen by using the
           SYSTEMD_NSS_RESOLVE_IFINDEX environment variable.

           The feature is implemented with cgroup/sock_create cgroup-bpf
           hooks.
       ...
       NFTSet=family:table:set
           ...
           sets must conform to lexicographic restrictions of NFT table
           names. The type of the element used in the NFT filter must
           match the type implied by the directive ("cgroup", "user" or
           "group") as shown in the table below. When a control group or
           a unit is realized, the corresponding ID will be appended to
           the NFT sets and it will be be removed when the control group
           or unit is removed.  systemd only inserts elements to (or
           removes from) the sets, so the related NFT rules, tables and
           sets must be prepared elsewhere in advance. Failures to manage
           the sets will be ignored.

           Table 2. Defined source type values
           âââââââââââââââ¬âââââââââââââââââââ¬ââââââââââââââââââââ
           â Source type â Description      â Corresponding NFT â
           â             â                  â type name         â
           âââââââââââââââ¼âââââââââââââââââââ¼ââââââââââââââââââââ¤
           â "cgroup"    â control group ID â "cgroupsv2"       â
           âââââââââââââââ¼âââââââââââââââââââ¼ââââââââââââââââââââ¤
           â "user"      â user ID          â "meta skuid"      â
           âââââââââââââââ¼âââââââââââââââââââ¼ââââââââââââââââââââ¤
           â "group"     â group ID         â "meta skgid"      â
           âââââââââââââââ´âââââââââââââââââââ´ââââââââââââââââââââ
   ...
   Memory Pressure Control
       ...
       ManagedOOMMemoryPressureDurationSec=
           ...
           supports a time unit such as "ms" or "Î¼s", see systemd.time(7)
       ...
       ManagedOOMPreference=none|avoid|omit
           ...
           /user.slice/user-1000.slice/user@1000.service/ will be ignored
       ...
       MemoryPressureThresholdSec=
           ...
           specified value expects a time unit such as "ms" or "Î¼s", see
...
systemd 260~devel                                SYSTEMD....CE-CONTROL(5)
