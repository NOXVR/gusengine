...
sqlite3 â DB-API 2.0 interface for SQLite databases — Python 3.14.3 documentation
...
# `sqlite3` â DB-API 2.0 interface for SQLite databases[Â¶](#module-sqlite3)

**Source code:** [Lib/sqlite3/](https://github.com/python/cpython/tree/3.14/Lib/sqlite3/)

SQLite is a C library that provides a lightweight disk-based database that
doesnât require a separate server process and allows accessing the database
using a nonstandard variant of the SQL query language. Some applications can use
SQLite for internal data storage.  Itâs also possible to prototype an
application using SQLite and then port the code to a larger database such as
PostgreSQL or Oracle.

The `sqlite3` module was written by Gerhard HÃ¤ring.  It provides an SQL interface
...
Now that weâve got a database connection and a cursor,
we can create a database table `movie` with columns for title,
release year, and review score.
For simplicity, we can just use column names in the table declaration â
...
as the query returns a [`tuple`](stdtypes.html#tuple) containing the tableâs name.
...
The result is a [`list`](stdtypes.html#list) of two `tuple`s, one per row,
each containing that rowâs `score` value.
...
Each row is a two-item [`tuple`](stdtypes.html#tuple) of `(year, title)`,
...
Youâve now created an SQLite database using the `sqlite3` module,
...
### Module functions[Â¶](#module-functions)

sqlite3.connect(database, timeout=5.0, detect_types=0, isolation_level='DEFERRED', check_same_thread=True, factory=sqlite3.Connection, cached_statements=128, uri=False, *, autocommit=sqlite3.LEGACY_TRANSACTION_CONTROL)[Â¶](#sqlite3.connect)
...
- 
**database** ([path-like object](../glossary.html#term-path-like-object)) â The path to the database file to be opened.
You can pass `":memory:"` to create an SQLite database existing only
in memory, and open a connection
to it.

- 
**timeout** ([*float*](functions.html#float)) â How many seconds the connection should wait before raising
...
- 
**detect_types** ([*int*](functions.html#int)) â Control whether and how data types not
...
- 
**isolation_level** ([*str*](stdtypes.html#str)* | **None*) â Control legacy transaction handling behaviour.
...
- 
**check_same_thread** ([*bool*](functions.html#bool)) â If `True` (default), [`ProgrammingError`](#sqlite3.ProgrammingError) will be raised
...
- 
**factory** ([*Connection*](#sqlite3.Connection)) â A custom subclass of [`Connection`](#sqlite3.Connection) to create the connection with,
if not the default [`Connection`](#sqlite3.Connection) class.

- 
**cached_statements** ([*int*](functions.html#int)) â The number of statements that `sqlite3`
should internally cache for this connection, to avoid parsing overhead.
By default, 128 statements.

- 
**uri** ([*bool*](functions.html#bool)) â If set to `True`, *database* is interpreted as a
...
- 
**autocommit** ([*bool*](functions.html#bool)) â Control [**PEP 249**](https://peps.python.org/pep-0249/) transaction handling behaviour.
...
sqlite3.complete_statement(statement)[Â¶](#sqlite3.complete_statement)
...
See `runsource()` in [Lib/sqlite3/__main__.py](https://github.com/python/cpython/tree/3.14/Lib/sqlite3/__main__.py)
for real-world use.

sqlite3.enable_callback_tracebacks(flag, /)[Â¶](#sqlite3.enable_callback_tracebacks)
...
will get tracebacks from callbacks on [`sys.stderr`](sys.html#sys.stderr). Use `False`
to disable the feature again.

Note

Errors in user-defined function callbacks are logged as unraisable exceptions.
Use an [`unraisable hook handler`](sys.html#sys.unraisablehook) for
introspection of the failed callback.

sqlite3.register_adapter(type, adapter, /)[Â¶](#sqlite3.register_adapter)
...
sqlite3.register_converter(typename, converter, /)[Â¶](#sqlite3.register_converter)
...
it is passed a [`bytes`](stdtypes.html#bytes) object and should return an object of the
desired Python type.
...
### Module constants[Â¶](#module-constants)

sqlite3.LEGACY_TRANSACTION_CONTROL[Â¶](#sqlite3.LEGACY_TRANSACTION_CONTROL)
...
sqlite3.PARSE_DECLTYPES[Â¶](#sqlite3.PARSE_DECLTYPES)
...
Generated fields (for example `MAX(p)`) are returned as [`str`](stdtypes.html#str).
Use `PARSE_COLNAMES` to enforce types for such queries.

sqlite3.PARSE_COLNAMES[Â¶](#sqlite3.PARSE_COLNAMES)
...
sqlite3.SQLITE_OK[Â¶](#sqlite3.SQLITE_OK)

sqlite3.SQLITE_DENY[Â¶](#sqlite3.SQLITE_DENY)

sqlite3.SQLITE_IGNORE[Â¶](#sqlite3.SQLITE_IGNORE)

Flags that should be returned by the *authorizer_callback* [callable](../glossary.html#term-callable)
...
sqlite3.apilevel[Â¶](#sqlite3.apilevel)

String constant stating the supported DB-API level. Required by the DB-API.
Hard-coded to `"2.0"`.

sqlite3.paramstyle[Â¶](#sqlite3.paramstyle)
...
sqlite3.sqlite_version[Â¶](#sqlite3.sqlite_version)

Version number of the runtime SQLite library as a [`string`](stdtypes.html#str).

sqlite3.sqlite_version_info[Â¶](#sqlite3.sqlite_version_info)

Version number of the runtime SQLite library as a [`tuple`](stdtypes.html#tuple) of
[`integers`](functions.html#int).

sqlite3.threadsafety[Â¶](#sqlite3.threadsafety)
...
[SQLITE_THREADSAFE](https://sqlite.org/compile.html#threadsafe)
...
sqlite3.SQLITE_DBCONFIG_DEFENSIVE[Â¶](#sqlite3.SQLITE_DBCONFIG_DEFENSIVE)

sqlite3.SQLITE_DBCONFIG_DQS_DDL[Â¶](#sqlite3.SQLITE_DBCONFIG_DQS_DDL)

sqlite3.SQLITE_DBCONFIG_DQS_DML[Â¶](#sqlite3.SQLITE_DBCONFIG_DQS_DML)

sqlite3.SQLITE_DBCONFIG_ENABLE_FKEY[Â¶](#sqlite3.SQLITE_DBCONFIG_ENABLE_FKEY)

sqlite3.SQLITE_DBCONFIG_ENABLE_FTS3_TOKENIZER[Â¶](#sqlite3.SQLITE_DBCONFIG_ENABLE_FTS3_TOKENIZER)

sqlite3.SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION[Â¶](#sqlite3.SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION)

sqlite3.SQLITE_DBCONFIG_ENABLE_QPSG[Â¶](#sqlite3.SQLITE_DBCONFIG_ENABLE_QPSG)

sqlite3.SQLITE_DBCONFIG_ENABLE_TRIGGER[Â¶](#sqlite3.SQLITE_DBCONFIG_ENABLE_TRIGGER)

sqlite3.SQLITE_DBCONFIG_ENABLE_VIEW[Â¶](#sqlite3.SQLITE_DBCONFIG_ENABLE_VIEW)

sqlite3.SQLITE_DBCONFIG_LEGACY_ALTER_TABLE[Â¶](#sqlite3.SQLITE_DBCONFIG_LEGACY_ALTER_TABLE)

sqlite3.SQLITE_DBCONFIG_LEGACY_FILE_FORMAT[Â¶](#sqlite3.SQLITE_DBCONFIG_LEGACY_FILE_FORMAT)

sqlite3.SQLITE_DBCONFIG_NO_CKPT_ON_CLOSE[Â¶](#sqlite3.SQLITE_DBCONFIG_NO_CKPT_ON_CLOSE)

sqlite3.SQLITE_DBCONFIG_RESET_DATABASE[Â¶](#sqlite3.SQLITE_DBCONFIG_RESET_DATABASE)

sqlite3.SQLITE_DBCONFIG_TRIGGER_EQP[Â¶](#sqlite3.SQLITE_DBCONFIG_TRIGGER_EQP)

sqlite3.SQLITE_DBCONFIG_TRUSTED_SCHEMA[Â¶](#sqlite3.SQLITE_DBCONFIG_TRUSTED_SCHEMA)

sqlite3.SQLITE_DBCONFIG_WRITABLE_SCHEMA[Â¶](#sqlite3.SQLITE_DBCONFIG_WRITABLE_SCHEMA)
...
class sqlite3.Connection[Â¶](#sqlite3.Connection)
...
Changed in version 3.13: A [`ResourceWarning`](exceptions.html#ResourceWarning) is emitted if [`close()`](#sqlite3.Connection.close) is not called before
a `Connection` object is deleted.

An SQLite database connection has the following attributes and methods:

cursor(factory=Cursor)[Â¶](#sqlite3.Connection.cursor)
...
blobopen(table, column, rowid, /, *, readonly=False, name='main')[Â¶](#sqlite3.Connection.blobopen)
...
- 
**table** ([*str*](stdtypes.html#str)) â The name of the table where the blob is located.

- 
**column** ([*str*](stdtypes.html#str)) â The name of the column where the blob is located.

- 
**rowid** ([*int*](functions.html#int)) â The row id where the blob is located.

- 
**readonly** ([*bool*](functions.html#bool)) â Set to `True` if the blob should be opened without write
permissions.
Defaults to `False`.

- 
**name** ([*str*](stdtypes.html#str)) â The name of the database where the blob is located.
Defaults to `"main"`.

Raises:

[**OperationalError**](#sqlite3.OperationalError) â When trying to open a blob in a `WITHOUT ROWID` table.
...
commit()[Â¶](#sqlite3.Connection.commit)
...
rollback()[Â¶](#sqlite3.Connection.rollback)
...
close()[Â¶](#sqlite3.Connection.close)
...
execute(sql, parameters=(), /)[Â¶](#sqlite3.Connection.execute)
...
executemany(sql, parameters, /)[Â¶](#sqlite3.Connection.executemany)

Create a new [`Cursor`](#sqlite3.Cursor) object and call
[`executemany()`](#sqlite3.Cursor.executemany) on it with the given *sql* and *parameters*.
Return the new cursor object.

executescript(sql_script, /)[Â¶](#sqlite3.Connection.executescript)

Create a new [`Cursor`](#sqlite3.Cursor) object and call
[`executescript()`](#sqlite3.Cursor.executescript) on it with the given *sql_script*.
Return the new cursor object.

create_function(name, narg, func, *, deterministic=False)[Â¶](#sqlite3.Connection.create_function)
...
- 
**name** ([*str*](stdtypes.html#str)) â The name of the SQL function.

- 
**narg** ([*int*](functions.html#int)) â The number of arguments the SQL function can accept.
If `-1`, it may take any number of arguments.

- 
**func** ([callback](../glossary.html#term-callback) | None) â A [callable](../glossary.html#term-callable) that is called when the SQL function is invoked.
The callable must return [a type natively supported by SQLite](#sqlite3-types).
Set to `None` to remove an existing SQL function.

- 
**deterministic** ([*bool*](functions.html#bool)) â If `True`, the created SQL function is marked as
...
create_aggregate(name, n_arg, aggregate_class)[Â¶](#sqlite3.Connection.create_aggregate)
...
- 
**name** ([*str*](stdtypes.html#str)) â The name of the SQL aggregate function.

- 
**n_arg** ([*int*](functions.html#int)) â The number of arguments the SQL aggregate function can accept.
If `-1`, it may take any number of arguments.

- 
**aggregate_class** ([class](../glossary.html#term-class) | None) â 
...
create_window_function(name, num_params, aggregate_class, /)[Â¶](#sqlite3.Connection.create_window_function)
...
- 
**name** ([*str*](stdtypes.html#str)) â The name of the SQL aggregate window function to create or remove.

- 
**num_params** ([*int*](functions.html#int)) â The number of arguments the SQL aggregate window function can accept.
If `-1`, it may take any number of arguments.
...
[**NotSupportedError**](#sqlite3.NotSupportedError) â If used with a version of SQLite older than 3.25.0,
...
create_collation(name, callable, /)[Â¶](#sqlite3.Connection.create_collation)

Create a collation named *name* using the collating function *callable*.
*callable* is passed two [`string`](stdtypes.html#str) arguments,
and it should return an [`integer`](functions.html#int):
...
interrupt()[Â¶](#sqlite3.Connection.interrupt)
...
set_authorizer(authorizer_callback)[Â¶](#sqlite3.Connection.set_authorizer)

Register [callable](../glossary.html#term-callable) *authorizer_callback* to be invoked
...
(âmainâ, âtempâ, etc.) if applicable. The 5th argument is the name of the
...
set_progress_handler(progress_handler, n)[Â¶](#sqlite3.Connection.set_progress_handler)

Register [callable](../glossary.html#term-callable) *progress_handler* to be invoked for every *n*
...
set_trace_callback(trace_callback)[Â¶](#sqlite3.Connection.set_trace_callback)

Register [callable](../glossary.html#term-callable) *trace_callback* to be invoked
for each SQL statement that is actually executed by the SQLite backend.

The only argument passed to the callback is the statement (as
[`str`](stdtypes.html#str)) that is being executed. The return value of the callback is
...
enable_load_extension(enabled, /)[Â¶](#sqlite3.Connection.enable_load_extension)
...
load_extension(path, /, *, entrypoint=None)[Â¶](#sqlite3.Connection.load_extension)
...
- 
**path** ([*str*](stdtypes.html#str)) â The path to the SQLite extension.

- 
**entrypoint** ([*str*](stdtypes.html#str)* | **None*) â Entry point name.
If `None` (the default),
...
iterdump(*, filter=None)[Â¶](#sqlite3.Connection.iterdump)
...
**filter** ([*str*](stdtypes.html#str)* | **None*) â An optional `LIKE` pattern for database objects to dump, e.g. `prefix_%`.
...
backup(target, *, pages=-1, progress=None, name='main', sleep=0.250)[Â¶](#sqlite3.Connection.backup)
...
- 
**target** ([*Connection*](#sqlite3.Connection)) â The database connection to save the backup to.

- 
**pages** ([*int*](functions.html#int)) â The number of pages to copy at a time.
If equal to or less than `0`,
the entire database is copied in a single step.
Defaults to `-1`.

- 
**progress** ([callback](../glossary.html#term-callback) | None) â If set to a [callable](../glossary.html#term-callable),
...
- 
**name** ([*str*](stdtypes.html#str)) â The name of the database to back up.
...
- 
**sleep** ([*float*](functions.html#float)) â The number of seconds to sleep between successive attempts
...
getlimit(category, /)[Â¶](#sqlite3.Connection.getlimit)

Get a connection runtime limit.

Parameters:

**category** ([*int*](functions.html#int)) â The [SQLite limit category](https://www.sqlite.org/c3ref/c_limit_attached.html) to be queried.

Return type:

[int](functions.html#int)

Raises:

[**ProgrammingError**](#sqlite3.ProgrammingError) â If *category* is not recognised by the underlying SQLite library.
...
setlimit(category, limit, /)[Â¶](#sqlite3.Connection.setlimit)
...
- 
**category** ([*int*](functions.html#int)) â The [SQLite limit category](https://www.sqlite.org/c3ref/c_limit_attached.html) to be set.

- 
**limit** ([*int*](functions.html#int)) â The value of the new limit.
...
getconfig(op, /)[Â¶](#sqlite3.Connection.getconfig)

Query a boolean connection configuration option.

Parameters:

**op** ([*int*](functions.html#int)) â A [SQLITE_DBCONFIG code](#sqlite3-dbconfig-constants).

Return type:

[bool](functions.html#bool)

Added in version 3.12.

setconfig(op, enable=True, /)[Â¶](#sqlite3.Connection.setconfig)
...
- 
**enable** ([*bool*](functions.html#bool)) â `True` if the configuration option should be enabled (default);
`False` if it should be disabled.

Added in version 3.12.

serialize(*, name='main')[Â¶](#sqlite3.Connection.serialize)

Serialize a database into a [`bytes`](stdtypes.html#bytes) object.  For an
ordinary on-disk database file, the serialization is just a copy of the
disk file.  For an in-memory database or a âtempâ database, the
serialization is the same sequence of bytes which would be written to
disk if that database were backed up to disk.

Parameters:

**name** ([*str*](stdtypes.html#str)) â The database name to be serialized.
Defaults to `"main"`.
...
deserialize(data, /, *, name='main')[Â¶](#sqlite3.Connection.deserialize)
...
- 
**data** ([*bytes*](stdtypes.html#bytes)) â A serialized database.

- 
**name** ([*str*](stdtypes.html#str)) â The database name to deserialize into.
Defaults to `"main"`.

Raises:

- 
[**OperationalError**](#sqlite3.OperationalError) â If the database connection is currently involved in a read
transaction or a backup operation.

- 
[**DatabaseError**](#sqlite3.DatabaseError) â If *data* does not contain a valid SQLite database.

- 
[**OverflowError**](exceptions.html#OverflowError) â If [`len(data)`](functions.html#len) is larger than `2**63 - 1`.
...
autocommit[Â¶](#sqlite3.Connection.autocommit)
...
- 
`True`: Use SQLiteâs [autocommit mode](https://www.sqlite.org/lang_transaction.html#implicit_versus_explicit_transactions).
...
in_transaction[Â¶](#sqlite3.Connection.in_transaction)
...
isolation_level[Â¶](#sqlite3.Connection.isolation_level)
...
row_factory[Â¶](#sqlite3.Connection.row_factory)
...
Is `None` by default,
meaning each row is returned as a [`tuple`](stdtypes.html#tuple).

See [How to create and use row factories](#sqlite3-howto-row-factory) for more details.

text_factory[Â¶](#sqlite3.Connection.text_factory)

A [callable](../glossary.html#term-callable) that accepts a [`bytes`](stdtypes.html#bytes) parameter
and returns a text representation of it.
The callable is invoked for SQLite values with the `TEXT` data type.
By default, this attribute is set to [`str`](stdtypes.html#str).

See [How to handle non-UTF-8 text encodings](#sqlite3-howto-encoding) for more details.

total_changes[Â¶](#sqlite3.Connection.total_changes)

Return the total number of database rows that have been modified, inserted, or
deleted since the database connection was opened.

### Cursor objects[Â¶](#cursor-objects)
...
class sqlite3.Cursor[Â¶](#sqlite3.Cursor)

A [`Cursor`](#sqlite3.Cursor) instance has the following attributes and methods.

execute(sql, parameters=(), /)[Â¶](#sqlite3.Cursor.execute)
...
- 
**sql** ([*str*](stdtypes.html#str)) â A single SQL statement.

- 
**parameters** ([`dict`](stdtypes.html#dict) | [sequence](../glossary.html#term-sequence)) â Python values to bind to placeholders in *sql*.
...
[**ProgrammingError**](#sqlite3.ProgrammingError) â When *sql* contains more than one SQL statement.
When [named placeholders](#sqlite3-placeholders) are used
and *parameters* is a sequence instead of a [`dict`](stdtypes.html#dict).
...
executemany(sql, parameters, /)[Â¶](#sqlite3.Cursor.executemany)
...
- 
**sql** ([*str*](stdtypes.html#str)) â A single SQL DML statement.

- 
**parameters** ([iterable](../glossary.html#term-iterable)) â An iterable of parameters to bind with
...
When [named placeholders](#sqlite3-placeholders) are used
and the items in *parameters* are sequences instead of [`dict`](stdtypes.html#dict)s.
...
instead of [`dict`](stdtypes.html#dict)s.

executescript(sql_script, /)[Â¶](#sqlite3.Cursor.executescript)
...
*sql_script* must be a [`string`](stdtypes.html#str).
...
fetchone()[Â¶](#sqlite3.Cursor.fetchone)

If [`row_factory`](#sqlite3.Cursor.row_factory) is `None`,
return the next row query result set as a [`tuple`](stdtypes.html#tuple).
Else, pass it to the row factory and return its result.
Return `None` if no more data is available.

fetchmany(size=cursor.arraysize)[Â¶](#sqlite3.Cursor.fetchmany)

Return the next set of rows of a query result as a [`list`](stdtypes.html#list).
...
Changed in version 3.14.1: Negative *size* values are rejected by raising [`ValueError`](exceptions.html#ValueError).

fetchall()[Â¶](#sqlite3.Cursor.fetchall)

Return all (remaining) rows of a query result as a [`list`](stdtypes.html#list).
...
setinputsizes(sizes, /)[Â¶](#sqlite3.Cursor.setinputsizes)

Required by the DB-API. Does nothing in `sqlite3`.

setoutputsize(size, column=None, /)[Â¶](#sqlite3.Cursor.setoutputsize)

Required by the DB-API. Does nothing in `sqlite3`.

arraysize[Â¶](#sqlite3.Cursor.arraysize)

Read/write attribute that controls the number of rows returned by [`fetchmany()`](#sqlite3.Cursor.fetchmany).
The default value is 1 which means a single row would be fetched per call.

Changed in version 3.14.1: Negative values are rejected by raising [`ValueError`](exceptions.html#ValueError).

connection[Â¶](#sqlite3.Cursor.connection)
...
description[Â¶](#sqlite3.Cursor.description)
...
lastrowid[Â¶](#sqlite3.Cursor.lastrowid)
...
rowcount[Â¶](#sqlite3.Cursor.rowcount)
...
row_factory[Â¶](#sqlite3.Cursor.row_factory)

Control how a row fetched from this `Cursor` is represented.
If `None`, a row is represented as a [`tuple`](stdtypes.html#tuple).
...
### Row objects[Â¶](#row-objects)

class sqlite3.Row[Â¶](#sqlite3.Row)
...
It supports iteration, equality testing, [`len()`](functions.html#len),
...
keys()[Â¶](#sqlite3.Row.keys)

Return a [`list`](stdtypes.html#list) of column names as [`strings`](stdtypes.html#str).
Immediately after a query,
it is the first member of each tuple in [`Cursor.description`](#sqlite3.Cursor.description).

Changed in version 3.5: Added support of slicing.

### Blob objects[Â¶](#blob-objects)

class sqlite3.Blob[Â¶](#sqlite3.Blob)

Added in version 3.11.

A [`Blob`](#sqlite3.Blob) instance is a [file-like object](../glossary.html#term-file-like-object)
that can read and write data in an SQLite BLOB.
Call [`len(blob)`](functions.html#len) to get the size (number of bytes) of the blob.
Use indices and [slices](../glossary.html#term-slice) for direct access to the blob data.

Use the [`Blob`](#sqlite3.Blob) as a [context manager](../glossary.html#term-context-manager) to ensure that the blob
...
read(length=-1, /)[Â¶](#sqlite3.Blob.read)
...
write(data, /)[Â¶](#sqlite3.Blob.write)
...
tell()[Â¶](#sqlite3.Blob.tell)

Return the current access position of the blob.

seek(offset, origin=os.SEEK_SET, /)[Â¶](#sqlite3.Blob.seek)

Set the current access position of the blob to *offset*.  The *origin*
argument defaults to [`os.SEEK_SET`](os.html#os.SEEK_SET) (absolute blob positioning).
Other values for *origin* are [`os.SEEK_CUR`](os.html#os.SEEK_CUR) (seek relative to the
current position) and [`os.SEEK_END`](os.html#os.SEEK_END) (seek relative to the blobâs
end).

### PrepareProtocol objects[Â¶](#prepareprotocol-objects)

class sqlite3.PrepareProtocol[Â¶](#sqlite3.PrepareProtocol)

The PrepareProtocol typeâs single purpose is to act as a [**PEP 246**](https://peps.python.org/pep-0246/) style
...
exception sqlite3.Warning[Â¶](#sqlite3.Warning)
...
sqlite_errorname[Â¶](#sqlite3.Error.sqlite_errorname)
...
exception sqlite3.InterfaceError[Â¶](#sqlite3.InterfaceError)
...
exception sqlite3.DatabaseError[Â¶](#sqlite3.DatabaseError)
...
exception sqlite3.DataError[Â¶](#sqlite3.DataError)
...
exception sqlite3.OperationalError[Â¶](#sqlite3.OperationalError)
...
exception sqlite3.IntegrityError[Â¶](#sqlite3.IntegrityError)

Exception raised when the relational integrity of the database is affected,
e.g. a foreign key check fails.  It is a subclass of [`DatabaseError`](#sqlite3.DatabaseError).

exception sqlite3.InternalError[Â¶](#sqlite3.InternalError)
...
exception sqlite3.ProgrammingError[Â¶](#sqlite3.ProgrammingError)
...
exception sqlite3.NotSupportedError[Â¶](#sqlite3.NotSupportedError)
...
### SQLite and Python types[Â¶](#sqlite-and-python-types)
...
[`float`](functions.html#float)

`REAL`

[`str`](stdtypes.html#str)
...
depends on [`text_factory`](#sqlite3.Connection.text_factory),
[`str`](stdtypes.html#str) by default
...
### Default adapters and converters (deprecated)[Â¶](#default-adapters-and-converters-deprecated)
...
- 
An adapter for [`datetime.date`](datetime.html#datetime.date) objects to [`strings`](stdtypes.html#str) in
[ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format.

- 
An adapter for [`datetime.datetime`](datetime.html#datetime.datetime) objects to strings in
ISO 8601 format.

- 
A converter for [declared](#sqlite3-converters) âdateâ types to
[`datetime.date`](datetime.html#datetime.date) objects.

- 
A converter for declared âtimestampâ types to
[`datetime.datetime`](datetime.html#datetime.datetime) objects.
Fractional parts will be truncated to 6 digits (microsecond precision).

Note

The default âtimestampâ converter ignores UTC offsets in the database and
always returns a naive [`datetime.datetime`](datetime.html#datetime.datetime) object. To preserve UTC
...
using the interpreterâs [`-m`](../using/cmdline.html#cmdoption-m) switch,
...
-h, --help[Â¶](#cmdoption-python-m-sqlite3-h-v-filename-sql-h)

Print CLI help.

-v, --version[Â¶](#cmdoption-python-m-sqlite3-h-v-filename-sql-v)

Print underlying SQLite library version.

Added in version 3.12.

## How-to guides[Â¶](#how-to-guides)

### How to use placeholders to bind values in SQL queries[Â¶](#how-to-use-placeholders-to-bind-values-in-sql-queries)

SQL operations usually need to use values from Python variables. However,
beware of using Pythonâs string operations to assemble queries, as they
...
Instead, use the DB-APIâs parameter substitution. To insert a variable into a
query string, use a placeholder in the string, and substitute the actual values
into the query by providing them as a [`tuple`](stdtypes.html#tuple) of values to the second
argument of the cursorâs [`execute()`](#sqlite3.Cursor.execute) method.
...
an instance of a [`dict`](stdtypes.html#dict) (or a subclass),
which must contain keys for all named parameters;
any extra items are ignored.
Hereâs an example of both styles:
...
### How to adapt custom Python types to SQLite values[Â¶](#how-to-adapt-custom-python-types-to-sqlite-values)
...
#### How to write adaptable objects[Â¶](#how-to-write-adaptable-objects)
...
#### How to register adapter callables[Â¶](#how-to-register-adapter-callables)
...
### How to convert SQLite values to custom Python types[Â¶](#how-to-convert-sqlite-values-to-custom-python-types)
...
Letâs go back to the `Point` class. We stored the x and y coordinates
separated via semicolons as strings in SQLite.

First, weâll define a converter function that accepts the string as a parameter
and constructs a `Point` object from it.

Note

Converter functions are **always** passed a [`bytes`](stdtypes.html#bytes) object,
...
### Adapter and converter recipes[Â¶](#adapter-and-converter-recipes)
...
### How to use connection shortcut methods[Â¶](#how-to-use-connection-shortcut-methods)
...
be written more concisely because you donât have to create the (often
...
### How to use the connection context manager[Â¶](#how-to-use-the-connection-context-manager)
...
using [`contextlib.closing()`](contextlib.html#contextlib.closing).
...
### How to work with SQLite URIs[Â¶](#how-to-work-with-sqlite-uris)
...
### How to create and use row factories[Â¶](#how-to-create-and-use-row-factories)

By default, `sqlite3` represents each row as a [`tuple`](stdtypes.html#tuple).
...
that returns each row as a [`dict`](stdtypes.html#dict), with column names mapped to values:
...
[`dataclass`](dataclasses.html#dataclasses.dataclass), or any other custom class,
instead of a [`namedtuple`](collections.html#collections.namedtuple).

### How to handle non-UTF-8 text encodings[Â¶](#how-to-handle-non-utf-8-text-encodings)

By default, `sqlite3` uses [`str`](stdtypes.html#str) to adapt SQLite values
...
Because of SQLiteâs [flexible typing](https://www.sqlite.org/flextypegood.html), it is not uncommon to encounter table
columns with the `TEXT` data type containing non-UTF-8 encodings,
or even arbitrary data.
To demonstrate, letâs assume we have a database with ISO-8859-2 (Latin-2)
...
### Transaction control[Â¶](#transaction-control)
...
#### Transaction control via the `autocommit` attribute[Â¶](#transaction-control-via-the-autocommit-attribute)
...
Set *autocommit* to `True` to enable SQLiteâs [autocommit mode](https://www.sqlite.org/lang_transaction.html#implicit_versus_explicit_transactions).
In this mode, [`Connection.commit()`](#sqlite3.Connection.commit) and [`Connection.rollback()`](#sqlite3.Connection.rollback)
have no effect.
Note that SQLiteâs autocommit mode is distinct from
...
#### Transaction control via the `isolation_level` attribute[Â¶](#transaction-control-via-the-isolation-level-attribute)
...
You can choose the underlying [SQLite transaction behaviour](https://www.sqlite.org/lang_transaction.html#deferred_immediate_and_exclusive_transactions) â
...
[`dbm` â Interfaces to Unix âdatabasesâ](dbm.html)
