...
multiprocessing â Process-based parallelism — Python 3.14.3 documentation
...
- 
          [next](multiprocessing.shared_memory.html) |
        
- 
          [previous](threading.html) |
...
# `multiprocessing` â Process-based parallelism[Â¶](#module-multiprocessing)
...
API similar to the [`threading`](threading.html#module-threading) module.  The `multiprocessing` package
...
analogs in the [`threading`](threading.html#module-threading) module, like the ability to [`terminate`](#multiprocessing.Process.terminate), [`interrupt`](#multiprocessing.Process.interrupt) or [`kill`](#multiprocessing.Process.kill) a running process.

See also

[`concurrent.futures.ProcessPoolExecutor`](concurrent.futures.html#concurrent.futures.ProcessPoolExecutor) offers a higher level interface
...
interface directly, the [`concurrent.futures`](concurrent.futures.html#module-concurrent.futures) API more readily allows
the submission of work to the underlying process pool to be separated from
waiting for the results.

### The [`Process`](#multiprocessing.Process) class[Â¶](#the-process-class)
...
follows the API of [`threading.Thread`](threading.html#threading.Thread).  A trivial example of a
...
could lead to an [`AttributeError`](exceptions.html#AttributeError) in the child process trying to locate the
*f* function in the `__main__` module.

### Contexts and start methods[Â¶](#contexts-and-start-methods)
...
the process objectâs [`run()`](#multiprocessing.Process.run) method.  In particular,
...
*fork*
The parent process uses [`os.fork()`](os.html#os.fork) to fork the Python
...
[`os.fork()`](os.html#os.fork) function that this start method calls internally will
raise a [`DeprecationWarning`](exceptions.html#DeprecationWarning). Use a different start method.
See the [`os.fork()`](os.html#os.fork) documentation for further explanation.
...
side-effect so it is generally safe for it to use [`os.fork()`](os.html#os.fork).
...
[`SharedMemory`](multiprocessing.shared_memory.html#multiprocessing.shared_memory.SharedMemory) objects) created
...
there may be some âleakedâ resources.  (Neither leaked semaphores nor shared
...
[`ProcessPoolExecutor`](concurrent.futures.html#concurrent.futures.ProcessPoolExecutor) should be designed to allow
...
rest of the library userâs application.  Always document if your library
requires a specific start method.

Warning

The `'spawn'` and `'forkserver'` start methods generally cannot
be used with âfrozenâ executables (i.e., binaries produced by
packages like **PyInstaller** and **cx_Freeze**) on POSIX systems.
The `'fork'` start method may work if code does not use threads.

### Exchanging objects between processes[Â¶](#exchanging-objects-between-processes)
...
The [`Queue`](#multiprocessing.Queue) class is a near clone of [`queue.Queue`](queue.html#queue.Queue).  For
example:
...
### Synchronization between processes[Â¶](#synchronization-between-processes)

`multiprocessing` contains equivalents of all the synchronization
primitives from [`threading`](threading.html#module-threading).  For instance one can use a lock to ensure
...
### Sharing state between processes[Â¶](#sharing-state-between-processes)
...
typecodes of the kind used by the [`array`](array.html#module-array) module: `'d'` indicates a
...
[`list`](stdtypes.html#list), [`dict`](stdtypes.html#dict), [`set`](stdtypes.html#set), [`Namespace`](#multiprocessing.managers.Namespace), [`Lock`](#multiprocessing.Lock),
...
### Using a pool of workers[Â¶](#using-a-pool-of-workers)
...
[`threading`](threading.html#module-threading) module.

### Global start method[Â¶](#global-start-method)
...
certain objects will implicitly set the global start method to the systemâs default,
if it hasnât been set already. The global start method can only be set once.
...
### [`Process`](#multiprocessing.Process) and exceptions[Â¶](#process-and-exceptions)

class multiprocessing.Process(group=None, target=None, name=None, args=(), kwargs={}, *, daemon=None)[Â¶](#multiprocessing.Process)

Process objects represent activity that is run in a separate process. The
[`Process`](#multiprocessing.Process) class has equivalents of all the methods of
[`threading.Thread`](threading.html#threading.Thread).

The constructor should always be called with keyword arguments. *group*
should always be `None`; it exists solely for compatibility with
[`threading.Thread`](threading.html#threading.Thread).  *target* is the callable object to be invoked by
...
[`concurrent.futures.ProcessPoolExecutor`](concurrent.futures.html#concurrent.futures.ProcessPoolExecutor) from a REPL with a
locally defined *target* function.

Passing a callable object defined in the current REPL session causes the
child process to die via an uncaught [`AttributeError`](exceptions.html#AttributeError) exception when
...
run()[Â¶](#multiprocessing.Process.run)
...
method invokes the callable object passed to the objectâs constructor as
...
start()[Â¶](#multiprocessing.Process.start)

Start the processâs activity.

This must be called at most once per process object.  It arranges for the
objectâs [`run()`](#multiprocessing.Process.run) method to be invoked in a separate process.

join([timeout])[Â¶](#multiprocessing.Process.join)
...
method times out.  Check the processâs [`exitcode`](#multiprocessing.Process.exitcode) to determine if
it terminated.
...
name[Â¶](#multiprocessing.Process.name)

The processâs name.  The name is a string used for identification purposes
...
âProcess-N1:N2:â¦:Nkâ is constructed, where
each Nk is the N-th child of its parent.

is_alive()[Â¶](#multiprocessing.Process.is_alive)
...
daemon[Â¶](#multiprocessing.Process.daemon)
...
In addition to the  [`threading.Thread`](threading.html#threading.Thread) API, [`Process`](#multiprocessing.Process) objects
also support the following attributes and methods:

pid[Â¶](#multiprocessing.Process.pid)

Return the process ID.  Before the process is spawned, this will be
`None`.

exitcode[Â¶](#multiprocessing.Process.exitcode)

The childâs exit code.  This will be `None` if the process has not yet
terminated.

If the childâs [`run()`](#multiprocessing.Process.run) method returned normally, the exit code
will be 0.  If it terminated via [`sys.exit()`](sys.html#sys.exit) with an integer
...
sentinel[Â¶](#multiprocessing.Process.sentinel)

A numeric handle of a system object which will become âreadyâ when
the process ends.
...
a file descriptor usable with primitives from the [`select`](select.html#module-select) module.

Added in version 3.3.

interrupt()[Â¶](#multiprocessing.Process.interrupt)

Terminate the process. Works on POSIX using the [`SIGINT`](signal.html#signal.SIGINT) signal.
Behavior on Windows is undefined.

By default, this terminates the child process by raising [`KeyboardInterrupt`](exceptions.html#KeyboardInterrupt).
This behavior can be altered by setting the respective signal handler in the child
process [`signal.signal()`](signal.html#signal.signal) for [`SIGINT`](signal.html#signal.SIGINT).

Note: if the child process catches and discards [`KeyboardInterrupt`](exceptions.html#KeyboardInterrupt), the
...
[`exitcode`](#multiprocessing.Process.exitcode) you may simply catch [`KeyboardInterrupt`](exceptions.html#KeyboardInterrupt) and call
`exit(your_code)`.

Added in version 3.14.

terminate()[Â¶](#multiprocessing.Process.terminate)

Terminate the process.  On POSIX this is done using the [`SIGTERM`](signal.html#signal.SIGTERM) signal;
on Windows `TerminateProcess()` is used.  Note that exit handlers and
finally clauses, etc., will not be executed.

Note that descendant processes of the process will *not* be terminated â
...
raise [`ValueError`](exceptions.html#ValueError).
...
exception multiprocessing.ProcessError[Â¶](#multiprocessing.ProcessError)

The base class of all `multiprocessing` exceptions.

exception multiprocessing.BufferTooShort[Â¶](#multiprocessing.BufferTooShort)
...
exception multiprocessing.AuthenticationError[Â¶](#multiprocessing.AuthenticationError)

Raised when there is an authentication error.

exception multiprocessing.TimeoutError[Â¶](#multiprocessing.TimeoutError)

Raised by methods with a timeout when the timeout expires.

### Pipes and Queues[Â¶](#pipes-and-queues)
...
queues modelled on the [`queue.Queue`](queue.html#queue.Queue) class in the
standard library.  They differ in that [`Queue`](#multiprocessing.Queue) lacks the
[`task_done()`](queue.html#queue.Queue.task_done) and [`join()`](queue.html#queue.Queue.join) methods introduced
into Python 2.5âs [`queue.Queue`](queue.html#queue.Queue) class.
...
queues serializes all objects that are put into them using [`pickle`](pickle.html#module-pickle).
...
`multiprocessing` uses the usual [`queue.Empty`](queue.html#queue.Empty) and
[`queue.Full`](queue.html#queue.Full) exceptions to signal a timeout.  They are not available in
the `multiprocessing` namespace so you need to import them from
[`queue`](queue.html#module-queue).
...
infinitesimal delay before the queueâs [`empty()`](#multiprocessing.Queue.empty)
method returns [`False`](constants.html#False) and [`get_nowait()`](#multiprocessing.Queue.get_nowait) can
return without raising [`queue.Empty`](queue.html#queue.Empty).
...
If a process is killed using [`Process.terminate()`](#multiprocessing.Process.terminate) or [`os.kill()`](os.html#os.kill)
...
multiprocessing.Pipe(duplex=True)[Â¶](#multiprocessing.Pipe)
...
[`pickle`](pickle.html#module-pickle) and the `recv()` re-creates the object.

class multiprocessing.Queue([maxsize])[Â¶](#multiprocessing.Queue)
...
The usual [`queue.Empty`](queue.html#queue.Empty) and [`queue.Full`](queue.html#queue.Full) exceptions from the
standard libraryâs [`queue`](queue.html#module-queue) module are raised to signal timeouts.

[`Queue`](#multiprocessing.Queue) implements all the methods of [`queue.Queue`](queue.html#queue.Queue) except for
[`task_done()`](queue.html#queue.Queue.task_done) and [`join()`](queue.html#queue.Queue.join).

qsize()[Â¶](#multiprocessing.Queue.qsize)

Return the approximate size of the queue.  Because of
multithreading/multiprocessing semantics, this number is not reliable.

Note that this may raise [`NotImplementedError`](exceptions.html#NotImplementedError) on platforms like
macOS where `sem_getvalue()` is not implemented.

empty()[Â¶](#multiprocessing.Queue.empty)

Return `True` if the queue is empty, `False` otherwise.  Because of
multithreading/multiprocessing semantics, this is not reliable.

May raise an [`OSError`](exceptions.html#OSError) on closed queues. (not guaranteed)

full()[Â¶](#multiprocessing.Queue.full)

Return `True` if the queue is full, `False` otherwise.  Because of
multithreading/multiprocessing semantics, this is not reliable.

put(obj[, block[, timeout]])[Â¶](#multiprocessing.Queue.put)

Put obj into the queue.  If the optional argument *block* is `True`
(the default) and *timeout* is `None` (the default), block if necessary until
a free slot is available.  If *timeout* is a positive number, it blocks at
most *timeout* seconds and raises the [`queue.Full`](queue.html#queue.Full) exception if no
free slot was available within that time.  Otherwise (*block* is
`False`), put an item on the queue if a free slot is immediately
available, else raise the [`queue.Full`](queue.html#queue.Full) exception (*timeout* is
ignored in that case).

Changed in version 3.8: If the queue is closed, [`ValueError`](exceptions.html#ValueError) is raised instead of
[`AssertionError`](exceptions.html#AssertionError).

put_nowait(obj)[Â¶](#multiprocessing.Queue.put_nowait)

Equivalent to `put(obj, False)`.

get([block[, timeout]])[Â¶](#multiprocessing.Queue.get)

Remove and return an item from the queue.  If optional args *block* is
`True` (the default) and *timeout* is `None` (the default), block if
necessary until an item is available.  If *timeout* is a positive number,
it blocks at most *timeout* seconds and raises the [`queue.Empty`](queue.html#queue.Empty)
exception if no item was available within that time.  Otherwise (block is
`False`), return an item if one is immediately available, else raise the
[`queue.Empty`](queue.html#queue.Empty) exception (*timeout* is ignored in that case).

Changed in version 3.8: If the queue is closed, [`ValueError`](exceptions.html#ValueError) is raised instead of
[`OSError`](exceptions.html#OSError).

get_nowait()[Â¶](#multiprocessing.Queue.get_nowait)

Equivalent to `get(False)`.

[`multiprocessing.Queue`](#multiprocessing.Queue) has a few additional methods not found in
[`queue.Queue`](queue.html#queue.Queue).  These methods are usually unnecessary for most
code:

close()[Â¶](#multiprocessing.Queue.close)
...
join_thread()[Â¶](#multiprocessing.Queue.join_thread)
...
will attempt to join the queueâs background thread.  The process can call
...
cancel_join_thread()[Â¶](#multiprocessing.Queue.cancel_join_thread)

Prevent [`join_thread()`](#multiprocessing.Queue.join_thread) from blocking.  In particular, this prevents
the background thread from being joined automatically when the process
exits â see [`join_thread()`](#multiprocessing.Queue.join_thread).
...
underlying pipe, and you donât care about lost data.

Note

This classâs functionality requires a functioning shared semaphore
implementation on the host operating system. Without one, the
functionality in this class will be disabled, and attempts to
instantiate a [`Queue`](#multiprocessing.Queue) will result in an [`ImportError`](exceptions.html#ImportError). See
...
class multiprocessing.SimpleQueue[Â¶](#multiprocessing.SimpleQueue)
...
close()[Â¶](#multiprocessing.SimpleQueue.close)
...
empty()[Â¶](#multiprocessing.SimpleQueue.empty)

Return `True` if the queue is empty, `False` otherwise.

Always raises an [`OSError`](exceptions.html#OSError) if the SimpleQueue is closed.

get()[Â¶](#multiprocessing.SimpleQueue.get)

Remove and return an item from the queue.

put(item)[Â¶](#multiprocessing.SimpleQueue.put)

Put *item* into the queue.

class multiprocessing.JoinableQueue([maxsize])[Â¶](#multiprocessing.JoinableQueue)
...
task_done()[Â¶](#multiprocessing.JoinableQueue.task_done)
...
If a [`join()`](queue.html#queue.Queue.join) is currently blocking, it will resume when all
...
Raises a [`ValueError`](exceptions.html#ValueError) if called more times than there were items
placed in the queue.

join()[Â¶](#multiprocessing.JoinableQueue.join)
...
[`join()`](queue.html#queue.Queue.join) unblocks.

### Miscellaneous[Â¶](#miscellaneous)

multiprocessing.active_children()[Â¶](#multiprocessing.active_children)

Return list of all live children of the current process.

Calling this has the side effect of âjoiningâ any processes which have
already finished.

multiprocessing.cpu_count()[Â¶](#multiprocessing.cpu_count)

Return the number of CPUs in the system.

This number is not equivalent to the number of CPUs the current process can
use.  The number of usable CPUs can be obtained with
[`os.process_cpu_count()`](os.html#os.process_cpu_count) (or `len(os.sched_getaffinity(0))`).

When the number of CPUs cannot be determined a [`NotImplementedError`](exceptions.html#NotImplementedError)
is raised.

See also

[`os.cpu_count()`](os.html#os.cpu_count)
[`os.process_cpu_count()`](os.html#os.process_cpu_count)
...
merely a wrapper around the [`os`](os.html#module-os) cpu count APIs.

multiprocessing.current_process()[Â¶](#multiprocessing.current_process)

Return the [`Process`](#multiprocessing.Process) object corresponding to the current process.

An analogue of [`threading.current_thread()`](threading.html#threading.current_thread).

multiprocessing.parent_process()[Â¶](#multiprocessing.parent_process)
...
multiprocessing.freeze_support()[Â¶](#multiprocessing.freeze_support)
...
executable will raise [`RuntimeError`](exceptions.html#RuntimeError).
...
multiprocessing.get_all_start_methods()[Â¶](#multiprocessing.get_all_start_methods)
...
multiprocessing.get_context(method=None)[Â¶](#multiprocessing.get_context)
...
`'forkserver'`.  [`ValueError`](exceptions.html#ValueError) is raised if the specified
start method is not available.  See [Contexts and start methods](#multiprocessing-start-methods).

Added in version 3.4.

multiprocessing.get_start_method(allow_none=False)[Â¶](#multiprocessing.get_start_method)
...
multiprocessing.set_executable(executable)[Â¶](#multiprocessing.set_executable)

Set the path of the Python interpreter to use when starting a child process.
(By default [`sys.executable`](sys.html#sys.executable) is used).  Embedders will probably need to
do some thing like
...
multiprocessing.set_forkserver_preload(module_names)[Â¶](#multiprocessing.set_forkserver_preload)

Set a list of module names for the forkserver main process to attempt to
import so that their already imported state is inherited by forked
processes. Any [`ImportError`](exceptions.html#ImportError) when doing so is silently ignored.
...
multiprocessing.set_start_method(method, force=False)[Â¶](#multiprocessing.set_start_method)

Set the method which should be used to start child processes.
The *method* argument can be `'fork'`, `'spawn'` or `'forkserver'`.
Raises [`RuntimeError`](exceptions.html#RuntimeError) if the start method has already been set and *force*
...
[`threading.active_count()`](threading.html#threading.active_count), [`threading.enumerate()`](threading.html#threading.enumerate),
[`threading.settrace()`](threading.html#threading.settrace), [`threading.setprofile()`](threading.html#threading.setprofile),
[`threading.Timer`](threading.html#threading.Timer), or [`threading.local`](threading.html#threading.local).

### Connection Objects[Â¶](#connection-objects)

Connection objects allow the sending and receiving of picklable objects or
strings.  They can be thought of as message oriented connected sockets.

Connection objects are usually created using
[`Pipe`](#multiprocessing.Pipe) â see also
[Listeners and Clients](#multiprocessing-listeners-clients).

class multiprocessing.connection.Connection[Â¶](#multiprocessing.connection.Connection)

send(obj)[Â¶](#multiprocessing.connection.Connection.send)

Send an object to the other end of the connection which should be read
using [`recv()`](#multiprocessing.connection.Connection.recv).

The object must be picklable.  Very large pickles (approximately 32 MiB+,
though it depends on the OS) may raise a [`ValueError`](exceptions.html#ValueError) exception.

recv()[Â¶](#multiprocessing.connection.Connection.recv)

Return an object sent from the other end of the connection using
[`send()`](#multiprocessing.connection.Connection.send).  Blocks until there is something to receive.  Raises
[`EOFError`](exceptions.html#EOFError) if there is nothing left to receive
and the other end was closed.

fileno()[Â¶](#multiprocessing.connection.Connection.fileno)

Return the file descriptor or handle used by the connection.

close()[Â¶](#multiprocessing.connection.Connection.close)

Close the connection.

This is called automatically when the connection is garbage collected.

poll([timeout])[Â¶](#multiprocessing.connection.Connection.poll)
...
send_bytes(buffer[, offset[, size]])[Â¶](#multiprocessing.connection.Connection.send_bytes)
...
[`ValueError`](exceptions.html#ValueError) exception

recv_bytes([maxlength])[Â¶](#multiprocessing.connection.Connection.recv_bytes)

Return a complete message of byte data sent from the other end of the
connection as a string.  Blocks until there is something to receive.
Raises [`EOFError`](exceptions.html#EOFError) if there is nothing left
to receive and the other end has closed.

If *maxlength* is specified and the message is longer than *maxlength*
then [`OSError`](exceptions.html#OSError) is raised and the connection will no longer be
readable.

Changed in version 3.3: This function used to raise [`IOError`](exceptions.html#IOError), which is now an
alias of [`OSError`](exceptions.html#OSError).

recv_bytes_into(buffer[, offset])[Â¶](#multiprocessing.connection.Connection.recv_bytes_into)

Read into *buffer* a complete message of byte data sent from the other end
of the connection and return the number of bytes in the message.  Blocks
until there is something to receive.  Raises
[`EOFError`](exceptions.html#EOFError) if there is nothing left to receive and the other end was
closed.
...
[Context Manager Types](stdtypes.html#typecontextmanager).  [`__enter__()`](stdtypes.html#contextmanager.__enter__) returns the
connection object, and [`__exit__()`](stdtypes.html#contextmanager.__exit__) calls [`close()`](#multiprocessing.connection.Connection.close).
...
### Synchronization primitives[Â¶](#synchronization-primitives)
...
object â see [Managers](#multiprocessing-managers).

class multiprocessing.Barrier(parties[, action[, timeout]])[Â¶](#multiprocessing.Barrier)

A barrier object: a clone of [`threading.Barrier`](threading.html#threading.Barrier).
...
class multiprocessing.BoundedSemaphore([value])[Â¶](#multiprocessing.BoundedSemaphore)

A bounded semaphore object: a close analog of
[`threading.BoundedSemaphore`](threading.html#threading.BoundedSemaphore).

Instantiating this class may set the global start method. See
[Global start method](#global-start-method) for more details.

A solitary difference from its close analog exists: its `acquire` methodâs
first argument is named *block*, as is consistent with [`Lock.acquire()`](#multiprocessing.Lock.acquire).

locked()[Â¶](#multiprocessing.BoundedSemaphore.locked)
...
class multiprocessing.Condition([lock])[Â¶](#multiprocessing.Condition)

A condition variable: an alias for [`threading.Condition`](threading.html#threading.Condition).
...
Changed in version 3.3: The [`wait_for()`](threading.html#threading.Condition.wait_for) method was added.

class multiprocessing.Event[Â¶](#multiprocessing.Event)

A clone of [`threading.Event`](threading.html#threading.Event).

Instantiating this class may set the global start method. See
[Global start method](#global-start-method) for more details.

class multiprocessing.Lock[Â¶](#multiprocessing.Lock)

A non-recursive lock object: a close analog of [`threading.Lock`](threading.html#threading.Lock).
...
[`threading.Lock`](threading.html#threading.Lock) as it applies to threads are replicated here in
...
[`Lock`](#multiprocessing.Lock) supports the [context manager](../glossary.html#term-context-manager) protocol and thus may be
used in [`with`](../reference/compound_stmts.html#with) statements.

acquire(block=True, timeout=None)[Â¶](#multiprocessing.Lock.acquire)
...
from that in [`threading.Lock.acquire()`](threading.html#threading.Lock.acquire).
...
[`threading.Lock.acquire()`](threading.html#threading.Lock.acquire).  The *timeout* argument has no practical
...
release()[Â¶](#multiprocessing.Lock.release)
...
Behavior is the same as in [`threading.Lock.release()`](threading.html#threading.Lock.release) except that
when invoked on an unlocked lock, a [`ValueError`](exceptions.html#ValueError) is raised.

locked()[Â¶](#multiprocessing.Lock.locked)

Return a boolean indicating whether this object is locked right now.

Added in version 3.14.

class multiprocessing.RLock[Â¶](#multiprocessing.RLock)

A recursive lock object: a close analog of [`threading.RLock`](threading.html#threading.RLock).  A
...
[`RLock`](#multiprocessing.RLock) supports the [context manager](../glossary.html#term-context-manager) protocol and thus may be
used in [`with`](../reference/compound_stmts.html#with) statements.

acquire(block=True, timeout=None)[Â¶](#multiprocessing.RLock.acquire)
...
several differences in this first argumentâs behavior compared to the
implementation of [`threading.RLock.acquire()`](threading.html#threading.RLock.acquire), starting with the name
...
differ from the implemented behaviors in [`threading.RLock.acquire()`](threading.html#threading.RLock.acquire).

release()[Â¶](#multiprocessing.RLock.release)
...
An [`AssertionError`](exceptions.html#AssertionError) is raised if this method is called by a process
...
differs from the implemented behavior in [`threading.RLock.release()`](threading.html#threading.RLock.release).

locked()[Â¶](#multiprocessing.RLock.locked)

Return a boolean indicating whether this object is locked right now.

Added in version 3.14.

class multiprocessing.Semaphore([value])[Â¶](#multiprocessing.Semaphore)

A semaphore object: a close analog of [`threading.Semaphore`](threading.html#threading.Semaphore).
...
get_value()[Â¶](#multiprocessing.Semaphore.get_value)
...
locked()[Â¶](#multiprocessing.Semaphore.locked)
...
a timeout will emulate that functionâs behavior using a sleeping loop.

Note

Some of this packageâs functionality requires a functioning shared semaphore
implementation on the host operating system. Without one, the
`multiprocessing.synchronize` module will be disabled, and attempts to
import it will result in an [`ImportError`](exceptions.html#ImportError). See
[bpo-3770](https://bugs.python.org/issue?@action=redirect&bpo=3770) for additional information.

### Shared [`ctypes`](ctypes.html#module-ctypes) Objects[Â¶](#shared-ctypes-objects)

It is possible to create shared objects using shared memory which can be
inherited by child processes.

multiprocessing.Value(typecode_or_type, *args, lock=True)[Â¶](#multiprocessing.Value)

Return a [`ctypes`](ctypes.html#module-ctypes) object allocated from shared memory.  By default the
return value is actually a synchronized wrapper for the object.  The object
itself can be accessed via the *value* attribute of a [`Value`](#multiprocessing.Value).

*typecode_or_type* determines the type of the returned object: it is either a
ctypes type or a one character typecode of the kind used by the [`array`](array.html#module-array)
...
by a lock, so it will not necessarily be âprocess-safeâ.
...
multiprocessing.Array(typecode_or_type, size_or_initializer, *, lock=True)[Â¶](#multiprocessing.Array)
...
character typecode of the kind used by the [`array`](array.html#module-array) module with the
exception of `'w'`, which is not supported.  In addition, the `'c'`
typecode is an alias for [`ctypes.c_char`](ctypes.html#ctypes.c_char).  If *size_or_initializer*
...
Note that an array of [`ctypes.c_char`](ctypes.html#ctypes.c_char) has *value* and *raw*
attributes which allow one to use it to store and retrieve strings.

#### The `multiprocessing.sharedctypes` module[Â¶](#module-multiprocessing.sharedctypes)

The `multiprocessing.sharedctypes` module provides functions for allocating
[`ctypes`](ctypes.html#module-ctypes) objects from shared memory which can be inherited by child
processes.
...
multiprocessing.sharedctypes.RawArray(typecode_or_type, size_or_initializer)[Â¶](#multiprocessing.sharedctypes.RawArray)

Return a ctypes array allocated from shared memory.

*typecode_or_type* determines the type of the elements of the returned array:
it is either a ctypes type or a one character typecode of the kind used by
the [`array`](array.html#module-array) module.  If *size_or_initializer* is an integer then it
...
multiprocessing.sharedctypes.RawValue(typecode_or_type, *args)[Â¶](#multiprocessing.sharedctypes.RawValue)
...
documentation for [`ctypes`](ctypes.html#module-ctypes).

multiprocessing.sharedctypes.Array(typecode_or_type, size_or_initializer, *, lock=True, ctx=None)[Â¶](#multiprocessing.sharedctypes.Array)
...
multiprocessing.sharedctypes.Value(typecode_or_type, *args, lock=True, ctx=None)[Â¶](#multiprocessing.sharedctypes.Value)
...
multiprocessing.sharedctypes.copy(obj)[Â¶](#multiprocessing.sharedctypes.copy)

Return a ctypes object allocated from shared memory which is a copy of the
ctypes object *obj*.

multiprocessing.sharedctypes.synchronized(obj, lock=None, ctx=None)[Â¶](#multiprocessing.sharedctypes.synchronized)
...
subclass of [`ctypes.Structure`](ctypes.html#ctypes.Structure).)
...
multiprocessing.Manager()[Â¶](#multiprocessing.Manager)
...
class multiprocessing.managers.BaseManager(address=None, authkey=None, serializer='pickle', ctx=None, *, shutdown_timeout=1.0)[Â¶](#multiprocessing.managers.BaseManager)
...
*serializer* must be `'pickle'` (use [`pickle`](pickle.html#module-pickle) serialization) or
`'xmlrpclib'` (use [`xmlrpc.client`](xmlrpc.client.html#module-xmlrpc.client) serialization).
...
start([initializer[, initargs]])[Â¶](#multiprocessing.managers.BaseManager.start)

Start a subprocess to start the manager.  If *initializer* is not `None`
then the subprocess will call `initializer(*initargs)` when it starts.

get_server()[Â¶](#multiprocessing.managers.BaseManager.get_server)
...
connect()[Â¶](#multiprocessing.managers.BaseManager.connect)
...
shutdown()[Â¶](#multiprocessing.managers.BaseManager.shutdown)
...
register(typeid[, callable[, proxytype[, exposed[, method_to_typeid[, create_method]]]]])[Â¶](#multiprocessing.managers.BaseManager.register)

A classmethod which can be used for registering a type or callable with
the manager class.

*typeid* is a âtype identifierâ which is used to identify a particular
...
where no exposed list is specified, all âpublic methodsâ of the shared
object will be accessible.  (Here a âpublic methodâ means any attribute
which has a [`__call__()`](../reference/datamodel.html#object.__call__) method and whose name does not begin
with `'_'`.)
...
methodâs name is not a key of this mapping or if the mapping is `None`
...
address[Â¶](#multiprocessing.managers.BaseManager.address)

The address used by the manager.

Changed in version 3.3: Manager objects support the context management protocol â see
[Context Manager Types](stdtypes.html#typecontextmanager).  [`__enter__()`](stdtypes.html#contextmanager.__enter__) starts the
server process (if it has not already started) and then returns the
manager object.  [`__exit__()`](stdtypes.html#contextmanager.__exit__) calls [`shutdown()`](#multiprocessing.managers.BaseManager.shutdown).

In previous versions [`__enter__()`](stdtypes.html#contextmanager.__enter__) did not start the
managerâs server process if it was not already started.

class multiprocessing.managers.SyncManager[Â¶](#multiprocessing.managers.SyncManager)
...
Barrier(parties[, action[, timeout]])[Â¶](#multiprocessing.managers.SyncManager.Barrier)

Create a shared [`threading.Barrier`](threading.html#threading.Barrier) object and return a
proxy for it.

Added in version 3.3.

BoundedSemaphore([value])[Â¶](#multiprocessing.managers.SyncManager.BoundedSemaphore)

Create a shared [`threading.BoundedSemaphore`](threading.html#threading.BoundedSemaphore) object and return a
proxy for it.

Condition([lock])[Â¶](#multiprocessing.managers.SyncManager.Condition)

Create a shared [`threading.Condition`](threading.html#threading.Condition) object and return a proxy for
it.

If *lock* is supplied then it should be a proxy for a
[`threading.Lock`](threading.html#threading.Lock) or [`threading.RLock`](threading.html#threading.RLock) object.

Changed in version 3.3: The [`wait_for()`](threading.html#threading.Condition.wait_for) method was added.

Event()[Â¶](#multiprocessing.managers.SyncManager.Event)

Create a shared [`threading.Event`](threading.html#threading.Event) object and return a proxy for it.

Lock()[Â¶](#multiprocessing.managers.SyncManager.Lock)

Create a shared [`threading.Lock`](threading.html#threading.Lock) object and return a proxy for it.

Namespace()[Â¶](#multiprocessing.managers.SyncManager.Namespace)

Create a shared [`Namespace`](#multiprocessing.managers.Namespace) object and return a proxy for it.

Queue([maxsize])[Â¶](#multiprocessing.managers.SyncManager.Queue)

Create a shared [`queue.Queue`](queue.html#queue.Queue) object and return a proxy for it.

RLock()[Â¶](#multiprocessing.managers.SyncManager.RLock)

Create a shared [`threading.RLock`](threading.html#threading.RLock) object and return a proxy for it.

Semaphore([value])[Â¶](#multiprocessing.managers.SyncManager.Semaphore)

Create a shared [`threading.Semaphore`](threading.html#threading.Semaphore) object and return a proxy for
it.

Array(typecode, sequence)[Â¶](#multiprocessing.managers.SyncManager.Array)

Create an array and return a proxy for it.

Value(typecode, value)[Â¶](#multiprocessing.managers.SyncManager.Value)

Create an object with a writable `value` attribute and return a proxy
for it.

dict()[Â¶](#multiprocessing.managers.SyncManager.dict)

dict(mapping)

dict(sequence)

Create a shared [`dict`](stdtypes.html#dict) object and return a proxy for it.

list()[Â¶](#multiprocessing.managers.SyncManager.list)

list(sequence)

Create a shared [`list`](stdtypes.html#list) object and return a proxy for it.

set()[Â¶](#multiprocessing.managers.SyncManager.set)

set(sequence)

set(mapping)

Create a shared [`set`](stdtypes.html#set) object and return a proxy for it.

Added in version 3.14: [`set`](stdtypes.html#set) support was added.
...
class multiprocessing.managers.Namespace[Â¶](#multiprocessing.managers.Namespace)
...
#### Customized managers[Â¶](#customized-managers)

To create oneâs own manager, one creates a subclass of [`BaseManager`](#multiprocessing.managers.BaseManager) and
...
#### Using a remote manager[Â¶](#using-a-remote-manager)
...
Notice that applying [`str()`](stdtypes.html#str) to a proxy will return the representation of
the referent, whereas applying [`repr()`](functions.html#repr) will return the representation of
the proxy.
...
If standard (non-proxy) [`list`](stdtypes.html#list) or [`dict`](stdtypes.html#dict) objects are contained
...
class multiprocessing.managers.BaseProxy[Â¶](#multiprocessing.managers.BaseProxy)

Proxy objects are instances of subclasses of [`BaseProxy`](#multiprocessing.managers.BaseProxy).

_callmethod(methodname[, args[, kwds]])[Â¶](#multiprocessing.managers.BaseProxy._callmethod)

Call and return the result of a method of the proxyâs referent.
...
in the managerâs process.

The returned value will be a copy of the result of the call or a proxy to
a new shared object â see documentation for the *method_to_typeid*
argument of [`BaseManager.register()`](#multiprocessing.managers.BaseManager.register).

If an exception is raised by the call, then is re-raised by
[`_callmethod()`](#multiprocessing.managers.BaseProxy._callmethod).  If some other exception is raised in the managerâs
...
_getvalue()[Â¶](#multiprocessing.managers.BaseProxy._getvalue)

Return a copy of the referent.

If the referent is unpicklable then this will raise an exception.

__repr__()[Â¶](#multiprocessing.managers.BaseProxy.__repr__)

Return a representation of the proxy object.

__str__()[Â¶](#multiprocessing.managers.BaseProxy.__str__)
...
### Process Pools[Â¶](#module-multiprocessing.pool)

One can create a pool of processes which will carry out tasks submitted to it
with the [`Pool`](#multiprocessing.pool.Pool) class.

class multiprocessing.pool.Pool([processes[, initializer[, initargs[, maxtasksperchild[, context]]]]])[Â¶](#multiprocessing.pool.Pool)
...
`None` then the number returned by [`os.process_cpu_count()`](os.html#os.process_cpu_count) is used.
...
(see [`object.__del__()`](../reference/datamodel.html#object.__del__) for more information).

Changed in version 3.2: Added the *maxtasksperchild* parameter.

Changed in version 3.4: Added the *context* parameter.

Changed in version 3.13: *processes* uses [`os.process_cpu_count()`](os.html#os.process_cpu_count) by default, instead of
[`os.cpu_count()`](os.html#os.cpu_count).

Note

Worker processes within a [`Pool`](#multiprocessing.pool.Pool) typically live for the complete
duration of the Poolâs work queue. A frequent pattern found in other
...
apply(func[, args[, kwds]])[Â¶](#multiprocessing.pool.Pool.apply)
...
apply_async(func[, args[, kwds[, callback[, error_callback]]]])[Â¶](#multiprocessing.pool.Pool.apply_async)
...
map(func, iterable[, chunksize])[Â¶](#multiprocessing.pool.Pool.map)

A parallel equivalent of the [`map()`](functions.html#map) built-in function (it supports only
...
map_async(func, iterable[, chunksize[, callback[, error_callback]]])[Â¶](#multiprocessing.pool.Pool.map_async)
...
imap(func, iterable[, chunksize])[Â¶](#multiprocessing.pool.Pool.imap)
...
imap_unordered(func, iterable[, chunksize])[Â¶](#multiprocessing.pool.Pool.imap_unordered)

The same as [`imap()`](#multiprocessing.pool.Pool.imap) except that the ordering of the results from the
returned iterator should be considered arbitrary.  (Only when there is
only one worker process is the order guaranteed to be âcorrectâ.)

starmap(func, iterable[, chunksize])[Â¶](#multiprocessing.pool.Pool.starmap)
...
starmap_async(func, iterable[, chunksize[, callback[, error_callback]]])[Â¶](#multiprocessing.pool.Pool.starmap_async)
...
close()[Â¶](#multiprocessing.pool.Pool.close)

Prevents any more tasks from being submitted to the pool.  Once all the
tasks have been completed the worker processes will exit.

terminate()[Â¶](#multiprocessing.pool.Pool.terminate)
...
join()[Â¶](#multiprocessing.pool.Pool.join)
...
Changed in version 3.3: Pool objects now support the context management protocol â see
[Context Manager Types](stdtypes.html#typecontextmanager).  [`__enter__()`](stdtypes.html#contextmanager.__enter__) returns the
pool object, and [`__exit__()`](stdtypes.html#contextmanager.__exit__) calls [`terminate()`](#multiprocessing.pool.Pool.terminate).

class multiprocessing.pool.AsyncResult[Â¶](#multiprocessing.pool.AsyncResult)
...
get([timeout])[Â¶](#multiprocessing.pool.AsyncResult.get)
...
wait([timeout])[Â¶](#multiprocessing.pool.AsyncResult.wait)

Wait until the result is available or until *timeout* seconds pass.

ready()[Â¶](#multiprocessing.pool.AsyncResult.ready)

Return whether the call has completed.

successful()[Â¶](#multiprocessing.pool.AsyncResult.successful)

Return whether the call completed without raising an exception.  Will
raise [`ValueError`](exceptions.html#ValueError) if the result is not ready.

Changed in version 3.7: If the result is not ready, [`ValueError`](exceptions.html#ValueError) is raised instead of
[`AssertionError`](exceptions.html#AssertionError).
...
### Listeners and Clients[Â¶](#module-multiprocessing.connection)
...
authentication using the [`hmac`](hmac.html#module-hmac) module, and for polling
multiple connections at the same time.

multiprocessing.connection.deliver_challenge(connection, authkey)[Â¶](#multiprocessing.connection.deliver_challenge)
...
multiprocessing.connection.answer_challenge(connection, authkey)[Â¶](#multiprocessing.connection.answer_challenge)
...
multiprocessing.connection.Client(address[, family[, authkey]])[Â¶](#multiprocessing.connection.Client)
...
class multiprocessing.connection.Listener([address[, family[, backlog[, authkey]]]])[Â¶](#multiprocessing.connection.Listener)

A wrapper for a bound socket or Windows named pipe which is âlisteningâ for
connections.
...
If an address of â0.0.0.0â is used, the address will not be a connectable
end point on Windows. If you require a connectable end-point,
you should use â127.0.0.1â.
...
private temporary directory created using [`tempfile.mkstemp()`](tempfile.html#tempfile.mkstemp).

If the listener object uses a socket then *backlog* (1 by default) is passed
to the [`listen()`](socket.html#socket.socket.listen) method of the socket once it has been
bound.
...
accept()[Â¶](#multiprocessing.connection.Listener.accept)
...
close()[Â¶](#multiprocessing.connection.Listener.close)
...
address[Â¶](#multiprocessing.connection.Listener.address)

The address which is being used by the Listener object.

last_accepted[Â¶](#multiprocessing.connection.Listener.last_accepted)

The address from which the last accepted connection came.  If this is
unavailable then it is `None`.

Changed in version 3.3: Listener objects now support the context management protocol â see
[Context Manager Types](stdtypes.html#typecontextmanager).  [`__enter__()`](stdtypes.html#contextmanager.__enter__) returns the
listener object, and [`__exit__()`](stdtypes.html#contextmanager.__exit__) calls [`close()`](#multiprocessing.connection.Listener.close).

multiprocessing.connection.wait(object_list, timeout=None)[Â¶](#multiprocessing.connection.wait)
...
- 
a connected and readable [`socket.socket`](socket.html#socket.socket) object; or
...
that, if [`select.select()`](select.html#select.select) is interrupted by a signal, it can
raise [`OSError`](exceptions.html#OSError) with an error number of `EINTR`, whereas
...
or it can be an object with a [`fileno()`](io.html#io.IOBase.fileno) method which returns a
...
#### Address Formats[Â¶](#address-formats)
...
### Authentication keys[Â¶](#authentication-keys)

When one uses [`Connection.recv`](#multiprocessing.connection.Connection.recv), the
data received is automatically
unpickled. Unfortunately unpickling data from an untrusted source is a security
risk. Therefore [`Listener`](#multiprocessing.connection.Listener) and [`Client()`](#multiprocessing.connection.Client) use the [`hmac`](hmac.html#module-hmac) module
...
Suitable authentication keys can also be generated by using [`os.urandom()`](os.html#os.urandom).

### Logging[Â¶](#logging)

Some support for logging is available.  Note, however, that the [`logging`](logging.html#module-logging)
package does not use process shared locks so it is possible (depending on the
handler type) for messages from different processes to get mixed up.

multiprocessing.get_logger()[Â¶](#multiprocessing.get_logger)

Returns the logger used by `multiprocessing`.  If necessary, a new one
will be created.

When first created the logger has level [`logging.NOTSET`](logging.html#logging.NOTSET) and no
default handler. Messages sent to this logger will not by default propagate
to the root logger.

Note that on Windows child processes will only inherit the level of the
parent processâs logger â any other customization of the logger will not be
inherited.

multiprocessing.log_to_stderr(level=None)[Â¶](#multiprocessing.log_to_stderr)

This function performs a call to [`get_logger()`](#multiprocessing.get_logger) but in addition to
returning the logger created by get_logger, it adds a handler which sends
output to [`sys.stderr`](sys.html#sys.stderr) using format
...
For a full table of logging levels, see the [`logging`](logging.html#module-logging) module.

### The `multiprocessing.dummy` module[Â¶](#module-multiprocessing.dummy)

`multiprocessing.dummy` replicates the API of `multiprocessing` but is
no more than a wrapper around the [`threading`](threading.html#module-threading) module.
...
class multiprocessing.pool.ThreadPool([processes[, initializer[, initargs]]])[Â¶](#multiprocessing.pool.ThreadPool)
...
the [`concurrent.futures`](concurrent.futures.html#module-concurrent.futures) module.  As such, it inherits some
operations that donât make sense for a pool backed by threads, and it
...
[`concurrent.futures.ThreadPoolExecutor`](concurrent.futures.html#concurrent.futures.ThreadPoolExecutor), which has a simpler
interface that was designed around threads from the start, and which
returns [`concurrent.futures.Future`](concurrent.futures.html#concurrent.futures.Future) instances that are
compatible with many other libraries, including [`asyncio`](asyncio.html#module-asyncio).

## Programming guidelines[Â¶](#programming-guidelines)

There are certain guidelines and idioms which should be adhered to when using
`multiprocessing`.

### All start methods[Â¶](#all-start-methods)
...
processâs [`Process.is_alive`](#multiprocessing.Process.is_alive) will
...
terminating until all the buffered items are fed by the âfeederâ thread to
...
Beware of replacing [`sys.stdin`](sys.html#sys.stdin) with a âfile like objectâ

`multiprocessing` originally unconditionally called:

os.close(sys.stdin.fileno())

in the `multiprocessing.Process._bootstrap()` method â this resulted
...
to applications which replace [`sys.stdin()`](sys.html#sys.stdin) with a âfile-like objectâ
with output buffering.  This danger is that if multiple processes call
[`close()`](io.html#io.IOBase.close) on this file-like object, it could result in the same
...
### The *spawn* and *forkserver* start methods[Â¶](#the-spawn-and-forkserver-start-methods)

There are a few extra restrictions which donât apply to the *fork*
start method.
...
Instead one should protect the âentry pointâ of the program by using if
...
and then run the moduleâs `foo()` function.
...
[`threading` â Thread-based parallelism](threading.html)
...
[`multiprocessing.shared_memory` â Shared memory for direct access across processes](multiprocessing.shared_memory.html)
