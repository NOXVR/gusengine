...
unittest â Unit testing framework — Python 3.14.3 documentation
...
- 
          [next](unittest.mock.html) |
        
- 
          [previous](doctest.html) |
...
# `unittest` â Unit testing framework[Â¶](#module-unittest)
...
Module [`doctest`](doctest.html#module-doctest)
...
Kent Beckâs original paper on testing frameworks using the pattern shared
by `unittest`.
...
## Command-Line Interface[Â¶](#command-line-interface)
...
to a module name by removing the â.pyâ and converting path separators into â.â.
If you want to execute a test file that isnât importable as a module you should
...
-b, --buffer[Â¶](#cmdoption-unittest-b)

The standard output and standard error streams are buffered during the test
run. Output during a passing test is discarded. Output is echoed normally
on test fail or error and is added to the failure messages.

-c, --catch[Â¶](#cmdoption-unittest-c)

Control-C during the test run waits for the current test to end and then
reports all the results so far. A second Control-C raises the normal
[`KeyboardInterrupt`](exceptions.html#KeyboardInterrupt) exception.

See [Signal Handling](#signal-handling) for the functions that provide this functionality.

-f, --failfast[Â¶](#cmdoption-unittest-f)

Stop the test run on the first error or failure.

-k[Â¶](#cmdoption-unittest-k)
...
test name using [`fnmatch.fnmatchcase()`](fnmatch.html#fnmatch.fnmatchcase); otherwise simple case-sensitive
...
--locals[Â¶](#cmdoption-unittest-locals)

Show local variables in tracebacks.

--durations N[Â¶](#cmdoption-unittest-durations)
...
## Test Discovery[Â¶](#test-discovery)
...
-v, --verbose[Â¶](#cmdoption-unittest-discover-v)

Verbose output

-s, --start-directory directory[Â¶](#cmdoption-unittest-discover-s)

Directory to start discovery (`.` default)

-p, --pattern pattern[Â¶](#cmdoption-unittest-discover-p)

Pattern to match test files (`test*.py` default)

-t, --top-level-directory directory[Â¶](#cmdoption-unittest-discover-t)
...
## Organizing test code[Â¶](#organizing-test-code)

The basic building blocks of unit testing are test cases â single
...
this: the test suite, represented by `unittest`âs
[`TestSuite`](#unittest.TestSuite) class.  In most cases, calling [`unittest.main()`](#unittest.main) will do
the right thing and collect all the moduleâs test cases for you and execute
them.
...
## Re-using old test code[Â¶](#re-using-old-test-code)
...
In some cases, the existing tests may have been written using the [`doctest`](doctest.html#module-doctest)
module.  If so, [`doctest`](doctest.html#module-doctest) provides a [`DocTestSuite`](doctest.html#doctest.DocTestSuite) class that can
automatically build [`unittest.TestSuite`](#unittest.TestSuite) instances from the existing
[`doctest`](doctest.html#module-doctest)-based tests.

## Skipping tests and expected failures[Â¶](#skipping-tests-and-expected-failures)

Added in version 3.1.

Unittest supports skipping individual test methods and even whole classes of
tests.  In addition, it supports marking a test as an âexpected failure,â a test
that is broken and will fail, but shouldnât be counted as a failure on a
[`TestResult`](#unittest.TestResult).

Skipping a test is simply a matter of using the [`skip()`](#unittest.skip) [decorator](../glossary.html#term-decorator)
...
Itâs easy to roll your own skipping decorators by making a decorator that calls
...
@unittest.skip(reason)[Â¶](#unittest.skip)

Unconditionally skip the decorated test.  *reason* should describe why the
test is being skipped.

@unittest.skipIf(condition, reason)[Â¶](#unittest.skipIf)

Skip the decorated test if *condition* is true.

@unittest.skipUnless(condition, reason)[Â¶](#unittest.skipUnless)

Skip the decorated test unless *condition* is true.

@unittest.expectedFailure[Â¶](#unittest.expectedFailure)
...
exception unittest.SkipTest(reason)[Â¶](#unittest.SkipTest)
...
## Distinguishing test iterations using subtests[Â¶](#distinguishing-test-iterations-using-subtests)
...
## Classes and functions[Â¶](#classes-and-functions)

This section describes in depth the API of `unittest`.

### Test cases[Â¶](#test-cases)

class unittest.TestCase(methodName='runTest')[Â¶](#unittest.TestCase)
...
setUp()[Â¶](#unittest.TestCase.setUp)

Method called to prepare the test fixture.  This is called immediately
before calling the test method; other than [`AssertionError`](exceptions.html#AssertionError) or [`SkipTest`](#unittest.SkipTest),
any exception raised by this method will be considered an error rather than
a test failure. The default implementation does nothing.

tearDown()[Â¶](#unittest.TestCase.tearDown)

Method called immediately after the test method has been called and the
result recorded.  This is called even if the test method raised an
exception, so the implementation in subclasses may need to be particularly
careful about checking internal state.  Any exception, other than
[`AssertionError`](exceptions.html#AssertionError) or [`SkipTest`](#unittest.SkipTest), raised by this method will be
...
setUpClass()[Â¶](#unittest.TestCase.setUpClass)

A class method called before tests in an individual class are run.
`setUpClass` is called with the class as the only argument
and must be decorated as a [`classmethod()`](functions.html#classmethod):
...
tearDownClass()[Â¶](#unittest.TestCase.tearDownClass)
...
run(result=None)[Â¶](#unittest.TestCase.run)
...
method) and used. The result object is returned to [`run()`](#unittest.TestCase.run)âs
caller.
...
skipTest(reason)[Â¶](#unittest.TestCase.skipTest)
...
subTest(msg=None, **params)[Â¶](#unittest.TestCase.subTest)
...
debug()[Â¶](#unittest.TestCase.debug)
...
[`assertEqual(a, b)`](#unittest.TestCase.assertEqual)

`a == b`

[`assertNotEqual(a, b)`](#unittest.TestCase.assertNotEqual)

`a != b`

[`assertTrue(x)`](#unittest.TestCase.assertTrue)

`bool(x) is True`

[`assertFalse(x)`](#unittest.TestCase.assertFalse)

`bool(x) is False`

[`assertIs(a, b)`](#unittest.TestCase.assertIs)

`a is b`

3.1

[`assertIsNot(a, b)`](#unittest.TestCase.assertIsNot)

`a is not b`

3.1

[`assertIsNone(x)`](#unittest.TestCase.assertIsNone)

`x is None`

3.1

[`assertIsNotNone(x)`](#unittest.TestCase.assertIsNotNone)

`x is not None`

3.1

[`assertIn(a, b)`](#unittest.TestCase.assertIn)

`a in b`

3.1

[`assertNotIn(a, b)`](#unittest.TestCase.assertNotIn)

`a not in b`

3.1

[`assertIsInstance(a, b)`](#unittest.TestCase.assertIsInstance)

`isinstance(a, b)`

3.2

[`assertNotIsInstance(a, b)`](#unittest.TestCase.assertNotIsInstance)

`not isinstance(a, b)`

3.2

[`assertIsSubclass(a, b)`](#unittest.TestCase.assertIsSubclass)

`issubclass(a, b)`

3.14

[`assertNotIsSubclass(a, b)`](#unittest.TestCase.assertNotIsSubclass)
...
assertEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertEqual)
...
assertNotEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertNotEqual)

Test that *first* and *second* are not equal.  If the values do
compare equal, the test will fail.

assertTrue(expr, msg=None)[Â¶](#unittest.TestCase.assertTrue)

assertFalse(expr, msg=None)[Â¶](#unittest.TestCase.assertFalse)
...
assertIs(first, second, msg=None)[Â¶](#unittest.TestCase.assertIs)

assertIsNot(first, second, msg=None)[Â¶](#unittest.TestCase.assertIsNot)

Test that *first* and *second* are (or are not) the same object.

Added in version 3.1.

assertIsNone(expr, msg=None)[Â¶](#unittest.TestCase.assertIsNone)

assertIsNotNone(expr, msg=None)[Â¶](#unittest.TestCase.assertIsNotNone)

Test that *expr* is (or is not) `None`.

Added in version 3.1.

assertIn(member, container, msg=None)[Â¶](#unittest.TestCase.assertIn)

assertNotIn(member, container, msg=None)[Â¶](#unittest.TestCase.assertNotIn)

Test that *member* is (or is not) in *container*.

Added in version 3.1.

assertIsInstance(obj, cls, msg=None)[Â¶](#unittest.TestCase.assertIsInstance)

assertNotIsInstance(obj, cls, msg=None)[Â¶](#unittest.TestCase.assertNotIsInstance)

Test that *obj* is (or is not) an instance of *cls* (which can be a
class or a tuple of classes, as supported by [`isinstance()`](functions.html#isinstance)).
To check for the exact type, use [`assertIs(type(obj), cls)`](#unittest.TestCase.assertIs).

Added in version 3.2.

assertIsSubclass(cls, superclass, msg=None)[Â¶](#unittest.TestCase.assertIsSubclass)

assertNotIsSubclass(cls, superclass, msg=None)[Â¶](#unittest.TestCase.assertNotIsSubclass)

Test that *cls* is (or is not) a subclass of *superclass* (which can be a
class or a tuple of classes, as supported by [`issubclass()`](functions.html#issubclass)).
...
[`assertRaises(exc, fun, *args, **kwds)`](#unittest.TestCase.assertRaises)

`fun(*args, **kwds)` raises *exc*

[`assertRaisesRegex(exc, r, fun, *args, **kwds)`](#unittest.TestCase.assertRaisesRegex)
...
[`assertWarns(warn, fun, *args, **kwds)`](#unittest.TestCase.assertWarns)

`fun(*args, **kwds)` raises *warn*

3.2

[`assertWarnsRegex(warn, r, fun, *args, **kwds)`](#unittest.TestCase.assertWarnsRegex)
...
[`assertLogs(logger, level)`](#unittest.TestCase.assertLogs)
...
[`assertNoLogs(logger, level)`](#unittest.TestCase.assertNoLogs)
...
assertRaises(exception, callable, *args, **kwds)[Â¶](#unittest.TestCase.assertRaises)
...
assertRaisesRegex(exception, regex, callable, *args, **kwds)[Â¶](#unittest.TestCase.assertRaisesRegex)
...
suitable for use by [`re.search()`](re.html#re.search).  Examples:
...
assertWarns(warning, callable, *args, **kwds)[Â¶](#unittest.TestCase.assertWarns)
...
fails if it isnât.  Any exception is an error.
...
assertWarnsRegex(warning, regex, callable, *args, **kwds)[Â¶](#unittest.TestCase.assertWarnsRegex)
...
by [`re.search()`](re.html#re.search).  Example:
...
assertLogs(logger=None, level=None)[Â¶](#unittest.TestCase.assertLogs)

A context manager to test that at least one message is logged on
the *logger* or one of its children, with at least the given
*level*.

If given, *logger* should be a [`logging.Logger`](logging.html#logging.Logger) object or a
[`str`](stdtypes.html#str) giving the name of a logger.  The default is the root
logger, which will catch all messages that were not blocked by a
non-propagating descendent logger.

If given, *level* should be either a numeric logging level or
its string equivalent (for example either `"ERROR"` or
[`logging.ERROR`](logging.html#logging.ERROR)).  The default is [`logging.INFO`](logging.html#logging.INFO).
...
records[Â¶](#unittest.TestCase.records)

A list of [`logging.LogRecord`](logging.html#logging.LogRecord) objects of the matching
log messages.

output[Â¶](#unittest.TestCase.output)

A list of [`str`](stdtypes.html#str) objects with the formatted output of
matching messages.
...
assertNoLogs(logger=None, level=None)[Â¶](#unittest.TestCase.assertNoLogs)
...
[`assertAlmostEqual(a, b)`](#unittest.TestCase.assertAlmostEqual)

`round(a-b, 7) == 0`

[`assertNotAlmostEqual(a, b)`](#unittest.TestCase.assertNotAlmostEqual)

`round(a-b, 7) != 0`

[`assertGreater(a, b)`](#unittest.TestCase.assertGreater)

`a > b`

3.1

[`assertGreaterEqual(a, b)`](#unittest.TestCase.assertGreaterEqual)

`a >= b`

3.1

[`assertLess(a, b)`](#unittest.TestCase.assertLess)

`a < b`

3.1

[`assertLessEqual(a, b)`](#unittest.TestCase.assertLessEqual)

`a <= b`

3.1

[`assertRegex(s, r)`](#unittest.TestCase.assertRegex)

`r.search(s)`

3.1

[`assertNotRegex(s, r)`](#unittest.TestCase.assertNotRegex)

`not r.search(s)`

3.2

[`assertCountEqual(a, b)`](#unittest.TestCase.assertCountEqual)
...
[`assertStartsWith(a, b)`](#unittest.TestCase.assertStartsWith)

`a.startswith(b)`

3.14

[`assertNotStartsWith(a, b)`](#unittest.TestCase.assertNotStartsWith)

`not a.startswith(b)`

3.14

[`assertEndsWith(a, b)`](#unittest.TestCase.assertEndsWith)

`a.endswith(b)`

3.14

[`assertNotEndsWith(a, b)`](#unittest.TestCase.assertNotEndsWith)

`not a.endswith(b)`

3.14

[`assertHasAttr(a, b)`](#unittest.TestCase.assertHasAttr)

`hastattr(a, b)`

3.14

[`assertNotHasAttr(a, b)`](#unittest.TestCase.assertNotHasAttr)

`not hastattr(a, b)`

3.14

assertAlmostEqual(first, second, places=7, msg=None, delta=None)[Â¶](#unittest.TestCase.assertAlmostEqual)

assertNotAlmostEqual(first, second, places=7, msg=None, delta=None)[Â¶](#unittest.TestCase.assertNotAlmostEqual)

Test that *first* and *second* are approximately (or not approximately)
equal by computing the difference, rounding to the given number of
decimal *places* (default 7), and comparing to zero.  Note that these
methods round the values to the given number of *decimal places* (i.e.
like the [`round()`](functions.html#round) function) and not *significant digits*.

If *delta* is supplied instead of *places* then the difference
between *first* and *second* must be less or equal to (or greater than) *delta*.

Supplying both *delta* and *places* raises a [`TypeError`](exceptions.html#TypeError).
...
assertGreater(first, second, msg=None)[Â¶](#unittest.TestCase.assertGreater)

assertGreaterEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertGreaterEqual)

assertLess(first, second, msg=None)[Â¶](#unittest.TestCase.assertLess)

assertLessEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertLessEqual)
...
assertRegex(text, regex, msg=None)[Â¶](#unittest.TestCase.assertRegex)

assertNotRegex(text, regex, msg=None)[Â¶](#unittest.TestCase.assertNotRegex)
...
expression suitable for use by [`re.search()`](re.html#re.search).
...
assertCountEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertCountEqual)

Test that sequence *first* contains the same elements as *second*,
regardless of their order. When they donât, an error message listing the
...
assertStartsWith(s, prefix, msg=None)[Â¶](#unittest.TestCase.assertStartsWith)

assertNotStartsWith(s, prefix, msg=None)[Â¶](#unittest.TestCase.assertNotStartsWith)
...
assertEndsWith(s, suffix, msg=None)[Â¶](#unittest.TestCase.assertEndsWith)

assertNotEndsWith(s, suffix, msg=None)[Â¶](#unittest.TestCase.assertNotEndsWith)
...
assertHasAttr(obj, name, msg=None)[Â¶](#unittest.TestCase.assertHasAttr)

assertNotHasAttr(obj, name, msg=None)[Â¶](#unittest.TestCase.assertNotHasAttr)
...
implemented for most of the built-in types, but itâs also possible to
register new methods using [`addTypeEqualityFunc()`](#unittest.TestCase.addTypeEqualityFunc):

addTypeEqualityFunc(typeobj, function)[Â¶](#unittest.TestCase.addTypeEqualityFunc)
...
between the first two parameters is detected â possibly providing useful
...
that itâs usually not necessary to invoke these methods directly.
...
[`assertMultiLineEqual(a, b)`](#unittest.TestCase.assertMultiLineEqual)

strings

3.1

[`assertSequenceEqual(a, b)`](#unittest.TestCase.assertSequenceEqual)

sequences

3.1

[`assertListEqual(a, b)`](#unittest.TestCase.assertListEqual)

lists

3.1

[`assertTupleEqual(a, b)`](#unittest.TestCase.assertTupleEqual)

tuples

3.1

[`assertSetEqual(a, b)`](#unittest.TestCase.assertSetEqual)

sets or frozensets

3.1

[`assertDictEqual(a, b)`](#unittest.TestCase.assertDictEqual)

dicts

3.1

assertMultiLineEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertMultiLineEqual)
...
assertSequenceEqual(first, second, msg=None, seq_type=None)[Â¶](#unittest.TestCase.assertSequenceEqual)
...
itâs used to implement [`assertListEqual()`](#unittest.TestCase.assertListEqual) and
[`assertTupleEqual()`](#unittest.TestCase.assertTupleEqual).

Added in version 3.1.

assertListEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertListEqual)

assertTupleEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertTupleEqual)
...
assertSetEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertSetEqual)

Tests that two sets are equal.  If not, an error message is constructed
that lists the differences between the sets.  This method is used by
default when comparing sets or frozensets with [`assertEqual()`](#unittest.TestCase.assertEqual).

Fails if either of *first* or *second* does not have a [`difference()`](stdtypes.html#frozenset.difference)
method.

Added in version 3.1.

assertDictEqual(first, second, msg=None)[Â¶](#unittest.TestCase.assertDictEqual)
...
fail(msg=None)[Â¶](#unittest.TestCase.fail)

Signals a test failure unconditionally, with *msg* or `None` for
the error message.

failureException[Â¶](#unittest.TestCase.failureException)

This class attribute gives the exception raised by the test method.  If a
test framework needs to use a specialized exception, possibly to carry
additional information, it must subclass this exception in order to âplay
fairâ with the framework.  The initial value of this attribute is
[`AssertionError`](exceptions.html#AssertionError).

longMessage[Â¶](#unittest.TestCase.longMessage)
...
maxDiff[Â¶](#unittest.TestCase.maxDiff)
...
countTestCases()[Â¶](#unittest.TestCase.countTestCases)

Return the number of tests represented by this test object.  For
[`TestCase`](#unittest.TestCase) instances, this will always be `1`.

defaultTestResult()[Â¶](#unittest.TestCase.defaultTestResult)
...
id()[Â¶](#unittest.TestCase.id)

Return a string identifying the specific test case.  This is usually the
full name of the test method, including the module and class name.

shortDescription()[Â¶](#unittest.TestCase.shortDescription)

Returns a description of the test, or `None` if no description
has been provided.  The default implementation of this method
returns the first line of the test methodâs docstring, if available,
or `None`.
...
addCleanup(function, /, *args, **kwargs)[Â¶](#unittest.TestCase.addCleanup)
...
enterContext(cm)[Â¶](#unittest.TestCase.enterContext)

Enter the supplied [context manager](../glossary.html#term-context-manager).  If successful, also
add its [`__exit__()`](../reference/datamodel.html#object.__exit__) method as a cleanup function by
[`addCleanup()`](#unittest.TestCase.addCleanup) and return the result of the
[`__enter__()`](../reference/datamodel.html#object.__enter__) method.

Added in version 3.11.

doCleanups()[Â¶](#unittest.TestCase.doCleanups)
...
classmethod addClassCleanup(function, /, *args, **kwargs)[Â¶](#unittest.TestCase.addClassCleanup)
...
classmethod enterClassContext(cm)[Â¶](#unittest.TestCase.enterClassContext)
...
classmethod doClassCleanups()[Â¶](#unittest.TestCase.doClassCleanups)
...
class unittest.IsolatedAsyncioTestCase(methodName='runTest')[Â¶](#unittest.IsolatedAsyncioTestCase)
...
loop_factory[Â¶](#unittest.IsolatedAsyncioTestCase.loop_factory)

The *loop_factory* passed to [`asyncio.Runner`](asyncio-runner.html#asyncio.Runner). Override
in subclasses with [`asyncio.EventLoop`](asyncio-eventloop.html#asyncio.EventLoop) to avoid using the
asyncio policy system.

Added in version 3.13.

async asyncSetUp()[Â¶](#unittest.IsolatedAsyncioTestCase.asyncSetUp)
...
[`AssertionError`](exceptions.html#AssertionError) or [`SkipTest`](#unittest.SkipTest), any exception raised by this method
will be considered an error rather than a test failure. The default implementation
does nothing.

async asyncTearDown()[Â¶](#unittest.IsolatedAsyncioTestCase.asyncTearDown)
...
addAsyncCleanup(function, /, *args, **kwargs)[Â¶](#unittest.IsolatedAsyncioTestCase.addAsyncCleanup)

This method accepts a coroutine that can be used as a cleanup function.

async enterAsyncContext(cm)[Â¶](#unittest.IsolatedAsyncioTestCase.enterAsyncContext)

Enter the supplied [asynchronous context manager](../glossary.html#term-asynchronous-context-manager).  If successful,
also add its [`__aexit__()`](../reference/datamodel.html#object.__aexit__) method as a cleanup function by
[`addAsyncCleanup()`](#unittest.IsolatedAsyncioTestCase.addAsyncCleanup) and return the result of the
[`__aenter__()`](../reference/datamodel.html#object.__aenter__) method.

Added in version 3.11.

run(result=None)[Â¶](#unittest.IsolatedAsyncioTestCase.run)
...
returned to [`run()`](#unittest.IsolatedAsyncioTestCase.run)âs caller. At the end of the test all the tasks
...
class unittest.FunctionTestCase(testFunc, setUp=None, tearDown=None, description=None)[Â¶](#unittest.FunctionTestCase)
...
### Grouping tests[Â¶](#grouping-tests)

class unittest.TestSuite(tests=())[Â¶](#unittest.TestSuite)
...
addTest(test)[Â¶](#unittest.TestSuite.addTest)

Add a [`TestCase`](#unittest.TestCase) or [`TestSuite`](#unittest.TestSuite) to the suite.

addTests(tests)[Â¶](#unittest.TestSuite.addTests)
...
run(result)[Â¶](#unittest.TestSuite.run)
...
debug()[Â¶](#unittest.TestSuite.debug)

Run the tests associated with this suite without collecting the
result. This allows exceptions raised by the test to be propagated to the
caller and can be used to support running tests under a debugger.

countTestCases()[Â¶](#unittest.TestSuite.countTestCases)

Return the number of tests represented by this test object, including all
individual tests and sub-suites.

__iter__()[Â¶](#unittest.TestSuite.__iter__)
...
than through iteration, so overriding `__iter__()` wasnât sufficient
for providing tests.
...
### Loading and running tests[Â¶](#loading-and-running-tests)

class unittest.TestLoader[Â¶](#unittest.TestLoader)
...
errors[Â¶](#unittest.TestLoader.errors)
...
loadTestsFromTestCase(testCaseClass)[Â¶](#unittest.TestLoader.loadTestsFromTestCase)
...
loadTestsFromModule(module, *, pattern=None)[Â¶](#unittest.TestLoader.loadTestsFromModule)
...
This is the [load_tests protocol](#id1).  The *pattern* argument is passed as
...
loadTestsFromName(name, module=None)[Â¶](#unittest.TestLoader.loadTestsFromName)

Return a suite of all test cases given a string specifier.

The specifier *name* is a âdotted nameâ that may resolve either to a
...
case class will be picked up as âa test method within a test case classâ,
rather than âa callable objectâ.
...
Changed in version 3.5: If an [`ImportError`](exceptions.html#ImportError) or [`AttributeError`](exceptions.html#AttributeError) occurs while traversing
*name* then a synthetic test that raises that error when run will be
returned. These errors are included in the errors accumulated by
self.errors.

loadTestsFromNames(names, module=None)[Â¶](#unittest.TestLoader.loadTestsFromNames)
...
getTestCaseNames(testCaseClass)[Â¶](#unittest.TestLoader.getTestCaseNames)

Return a sorted sequence of method names found within *testCaseClass*;
this should be a subclass of [`TestCase`](#unittest.TestCase).

discover(start_dir, pattern='test*.py', top_level_dir=None)[Â¶](#unittest.TestLoader.discover)
...
nested calls to `discover()`. That is, if a packageâs `load_tests`
...
*start_dir* can be a [namespace packages](../glossary.html#term-namespace-package).

Paths are sorted before being imported so that execution order is the
same even if the underlying file systemâs ordering is not dependent
on file name.
...
Changed in version 3.11: *start_dir* can not be a [namespace packages](../glossary.html#term-namespace-package).
...
Changed in version 3.14: *start_dir* can once again be a [namespace package](../glossary.html#term-namespace-package).

The following attributes of a [`TestLoader`](#unittest.TestLoader) can be configured either by
subclassing or assignment on an instance:

testMethodPrefix[Â¶](#unittest.TestLoader.testMethodPrefix)
...
sortTestMethodsUsing[Â¶](#unittest.TestLoader.sortTestMethodsUsing)

Function to be used to compare method names when sorting them in
[`getTestCaseNames()`](#unittest.TestLoader.getTestCaseNames) and all the `loadTestsFrom*` methods.

suiteClass[Â¶](#unittest.TestLoader.suiteClass)
...
testNamePatterns[Â¶](#unittest.TestLoader.testNamePatterns)
...
Note that matches are always performed using [`fnmatch.fnmatchcase()`](fnmatch.html#fnmatch.fnmatchcase),
...
class unittest.TestResult[Â¶](#unittest.TestResult)
...
expectedFailures[Â¶](#unittest.TestResult.expectedFailures)

A list containing 2-tuples of [`TestCase`](#unittest.TestCase) instances and strings
holding formatted tracebacks.  Each tuple represents an expected failure
or error of the test case.

unexpectedSuccesses[Â¶](#unittest.TestResult.unexpectedSuccesses)

A list containing [`TestCase`](#unittest.TestCase) instances that were marked as expected
failures, but succeeded.

collectedDurations[Â¶](#unittest.TestResult.collectedDurations)
...
shouldStop[Â¶](#unittest.TestResult.shouldStop)

Set to `True` when the execution of tests should stop by [`stop()`](#unittest.TestResult.stop).

testsRun[Â¶](#unittest.TestResult.testsRun)

The total number of tests run so far.

buffer[Â¶](#unittest.TestResult.buffer)
...
failfast[Â¶](#unittest.TestResult.failfast)
...
tb_locals[Â¶](#unittest.TestResult.tb_locals)

If set to true then local variables will be shown in tracebacks.

Added in version 3.5.

wasSuccessful()[Â¶](#unittest.TestResult.wasSuccessful)
...
stop()[Â¶](#unittest.TestResult.stop)
...
startTest(test)[Â¶](#unittest.TestResult.startTest)

Called when the test case *test* is about to be run.

stopTest(test)[Â¶](#unittest.TestResult.stopTest)

Called after the test case *test* has been executed, regardless of the
outcome.

startTestRun()[Â¶](#unittest.TestResult.startTestRun)

Called once before any tests are executed.

Added in version 3.1.

stopTestRun()[Â¶](#unittest.TestResult.stopTestRun)

Called once after all tests are executed.

Added in version 3.1.

addError(test, err)[Â¶](#unittest.TestResult.addError)

Called when the test case *test* raises an unexpected exception. *err* is a
tuple of the form returned by [`sys.exc_info()`](sys.html#sys.exc_info): (type, value,
traceback).

The default implementation appends a tuple `(test, formatted_err)` to
the instanceâs [`errors`](#unittest.TestResult.errors) attribute, where *formatted_err* is a
formatted traceback derived from *err*.

addFailure(test, err)[Â¶](#unittest.TestResult.addFailure)

Called when the test case *test* signals a failure. *err* is a tuple of
the form returned by [`sys.exc_info()`](sys.html#sys.exc_info): `(type, value, traceback)`.

The default implementation appends a tuple `(test, formatted_err)` to
the instanceâs [`failures`](#unittest.TestResult.failures) attribute, where *formatted_err* is a
formatted traceback derived from *err*.

addSuccess(test)[Â¶](#unittest.TestResult.addSuccess)

Called when the test case *test* succeeds.

The default implementation does nothing.

addSkip(test, reason)[Â¶](#unittest.TestResult.addSkip)

Called when the test case *test* is skipped.  *reason* is the reason the
test gave for skipping.

The default implementation appends a tuple `(test, reason)` to the
instanceâs [`skipped`](#unittest.TestResult.skipped) attribute.

addExpectedFailure(test, err)[Â¶](#unittest.TestResult.addExpectedFailure)

Called when the test case *test* fails or errors, but was marked with
the [`expectedFailure()`](#unittest.expectedFailure) decorator.

The default implementation appends a tuple `(test, formatted_err)` to
the instanceâs [`expectedFailures`](#unittest.TestResult.expectedFailures) attribute, where *formatted_err*
is a formatted traceback derived from *err*.

addUnexpectedSuccess(test)[Â¶](#unittest.TestResult.addUnexpectedSuccess)

Called when the test case *test* was marked with the
[`expectedFailure()`](#unittest.expectedFailure) decorator, but succeeded.

The default implementation appends the test to the instanceâs
[`unexpectedSuccesses`](#unittest.TestResult.unexpectedSuccesses) attribute.

addSubTest(test, subtest, outcome)[Â¶](#unittest.TestResult.addSubTest)
...
If *outcome* is [`None`](constants.html#None), the subtest succeeded.  Otherwise,
it failed with an exception where *outcome* is a tuple of the form
returned by [`sys.exc_info()`](sys.html#sys.exc_info): `(type, value, traceback)`.
...
addDuration(test, elapsed)[Â¶](#unittest.TestResult.addDuration)
...
class unittest.TextTestResult(stream, descriptions, verbosity, *, durations=None)[Â¶](#unittest.TextTestResult)
...
unittest.defaultTestLoader[Â¶](#unittest.defaultTestLoader)
...
class unittest.TextTestRunner(stream=None, descriptions=True, verbosity=1, failfast=False, buffer=False, resultclass=None, warnings=None, *, tb_locals=False, durations=None)[Â¶](#unittest.TextTestRunner)

A basic test runner implementation that outputs results to a stream. If *stream*
is `None`, the default, [`sys.stderr`](sys.html#sys.stderr) is used as the output stream. This class
...
By default this runner shows [`DeprecationWarning`](exceptions.html#DeprecationWarning),
[`PendingDeprecationWarning`](exceptions.html#PendingDeprecationWarning), [`ResourceWarning`](exceptions.html#ResourceWarning) and
[`ImportWarning`](exceptions.html#ImportWarning) even if they are [ignored by default](warnings.html#warning-ignored).  This behavior can
be overridden using Pythonâs `-Wd` or `-Wa` options
(see [Warning control](../using/cmdline.html#using-on-warnings)) and leaving
*warnings* to `None`.

Changed in version 3.2: Added the *warnings* parameter.

Changed in version 3.2: The default stream is set to [`sys.stderr`](sys.html#sys.stderr) at instantiation time rather
than import time.
...
_makeResult()[Â¶](#unittest.TextTestRunner._makeResult)
...
run(test)[Â¶](#unittest.TextTestRunner.run)
...
unittest.main(module='__main__', defaultTest=None, argv=None, testRunner=None, testLoader=unittest.defaultTestLoader, exit=True, verbosity=1, failfast=None, catchbreak=None, buffer=None, warnings=None)[Â¶](#unittest.main)
...
the values of [`sys.argv`](sys.html#sys.argv) are used.

The *testRunner* argument can either be a test runner class or an already
created instance of it. By default `main` calls [`sys.exit()`](sys.html#sys.exit) with
...
calling [`sys.exit()`](sys.html#sys.exit):
...
that should be used while running the tests.  If itâs not specified, it will
...
#### load_tests Protocol[Â¶](#load-tests-protocol)
...
another directory.  Otherwise, discovery of the packageâs tests will be left up
...
continue (and potentially modify) test discovery. A âdo nothingâ
...
## Class and Module Fixtures[Â¶](#class-and-module-fixtures)
...
ordering. A `BaseTestSuite` still exists for frameworks that donât want to
...
the standard unittest test runner then this detail doesnât matter, but if you
are a framework author it may be relevant.

### setUpClass and tearDownClass[Â¶](#setupclass-and-teardownclass)
...
### setUpModule and tearDownModule[Â¶](#setupmodule-and-teardownmodule)
...
unittest.addModuleCleanup(function, /, *args, **kwargs)[Â¶](#unittest.addModuleCleanup)
...
unittest.enterModuleContext(cm)[Â¶](#unittest.enterModuleContext)
...
unittest.doModuleCleanups()[Â¶](#unittest.doModuleCleanups)
...
## Signal Handling[Â¶](#signal-handling)
...
control-c will raise a [`KeyboardInterrupt`](exceptions.html#KeyboardInterrupt) in the usual way.

The control-c handling signal handler attempts to remain compatible with code or
tests that install their own [`signal.SIGINT`](signal.html#signal.SIGINT) handler. If the `unittest`
handler is called but *isnât* the installed [`signal.SIGINT`](signal.html#signal.SIGINT) handler,
...
unittest.installHandler()[Â¶](#unittest.installHandler)

Install the control-c handler. When a [`signal.SIGINT`](signal.html#signal.SIGINT) is received
(usually in response to the user pressing control-c) all registered results
have [`stop()`](#unittest.TestResult.stop) called.

unittest.registerResult(result)[Â¶](#unittest.registerResult)

Register a [`TestResult`](#unittest.TestResult) object for control-c handling. Registering a
result stores a weak reference to it, so it doesnât prevent the result from
...
unittest.removeResult(result)[Â¶](#unittest.removeResult)

Remove a registered result. Once a result has been removed then
[`stop()`](#unittest.TestResult.stop) will no longer be called on that result object in
response to a control-c.

unittest.removeHandler(function=None)[Â¶](#unittest.removeHandler)
...
- [load_tests Protocol](#load-tests-protocol)
...
[`doctest` â Test interactive Python examples](doctest.html)
...
[`unittest.mock` â mock object library](unittest.mock.html)
