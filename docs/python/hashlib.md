...
# `hashlib` â Secure hashes and message digests[Â¶](#module-hashlib)
...
202 standard) as well as the legacy algorithms SHA1 ([formerly part of FIPS](https://csrc.nist.gov/news/2023/decision-to-revise-fips-180-4))
and the MD5 algorithm (defined in internet [**RFC 1321**](https://datatracker.ietf.org/doc/html/rfc1321.html)).

Note

If you want the adler32 or crc32 hash functions, they are available in
the [`zlib`](zlib.html#module-zlib) module.

## Hash algorithms[Â¶](#hash-algorithms)
...
[bytes-like objects](../glossary.html#term-bytes-like-object) (normally [`bytes`](stdtypes.html#bytes)) using
...
if you are using a rare âFIPS compliantâ build of Python.
These correspond to [`algorithms_guaranteed`](#hashlib.algorithms_guaranteed).

Additional algorithms may also be available if your Python distributionâs
...
## Constructors[Â¶](#constructors)

hashlib.new(name, [data, ]*, usedforsecurity=True)[Â¶](#hashlib.new)
...
hashlib.md5([data, ]*, usedforsecurity=True)[Â¶](#hashlib.md5)

hashlib.sha1([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha1)

hashlib.sha224([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha224)

hashlib.sha256([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha256)

hashlib.sha384([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha384)

hashlib.sha512([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha512)

hashlib.sha3_224([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha3_224)

hashlib.sha3_256([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha3_256)

hashlib.sha3_384([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha3_384)

hashlib.sha3_512([data, ]*, usedforsecurity=True)[Â¶](#hashlib.sha3_512)
...
hashlib.algorithms_guaranteed[Â¶](#hashlib.algorithms_guaranteed)

A set containing the names of the hash algorithms guaranteed to be supported
by this module on all platforms.  Note that âmd5â is in this list despite
some upstream vendors offering an odd âFIPS compliantâ Python build that
excludes it.

Added in version 3.2.

hashlib.algorithms_available[Â¶](#hashlib.algorithms_available)
...
## Hash Objects[Â¶](#hash-objects)

The following values are provided as constant attributes of the hash objects
returned by the constructors:

hash.digest_size[Â¶](#hashlib.hash.digest_size)

The size of the resulting hash in bytes.

hash.block_size[Â¶](#hashlib.hash.block_size)

The internal block size of the hash algorithm in bytes.

A hash object has the following attributes:

hash.name[Â¶](#hashlib.hash.name)
...
hash.update(data)[Â¶](#hashlib.hash.update)
...
hash.digest()[Â¶](#hashlib.hash.digest)
...
hash.hexdigest()[Â¶](#hashlib.hash.hexdigest)

Like [`digest()`](#hashlib.hash.digest) except the digest is returned as a string object of
double length, containing only hexadecimal digits.  This may be used to
exchange the value safely in email or other non-binary environments.

hash.copy()[Â¶](#hashlib.hash.copy)

Return a copy (âcloneâ) of the hash object.  This can be used to efficiently
compute the digests of data sharing a common initial substring.

## SHAKE variable length digests[Â¶](#shake-variable-length-digests)

hashlib.shake_128([data, ]*, usedforsecurity=True)[Â¶](#hashlib.shake_128)

hashlib.shake_256([data, ]*, usedforsecurity=True)[Â¶](#hashlib.shake_256)
...
shake.digest(length)[Â¶](#hashlib.shake.digest)

Return the digest of the data passed to the [`update()`](#hashlib.hash.update) method so far.
This is a bytes object of size *length* which may contain bytes in
the whole range from 0 to 255.

shake.hexdigest(length)[Â¶](#hashlib.shake.hexdigest)
...
## File hashing[Â¶](#file-hashing)

The hashlib module provides a helper function for efficient hashing of
a file or file-like object.

hashlib.file_digest(fileobj, digest, /)[Â¶](#hashlib.file_digest)

Return a digest object that has been updated with contents of file object.

*fileobj* must be a file-like object opened for reading in binary mode.
It accepts file objects from  builtin [`open()`](functions.html#open), [`BytesIO`](io.html#io.BytesIO)
instances, SocketIO objects from [`socket.socket.makefile()`](socket.html#socket.socket.makefile), and
similar. *fileobj* must be opened in blocking mode, otherwise a
[`BlockingIOError`](exceptions.html#BlockingIOError) may be raised.

The function may bypass Pythonâs I/O and use the file descriptor
from [`fileno()`](io.html#io.IOBase.fileno) directly. *fileobj* must be assumed to be
...
Changed in version 3.14: Now raises a [`BlockingIOError`](exceptions.html#BlockingIOError) if the file is opened in non-blocking
mode. Previously, spurious null bytes were added to the digest.

## Key derivation[Â¶](#key-derivation)
...
hashlib.pbkdf2_hmac(hash_name, password, salt, iterations, dklen=None)[Â¶](#hashlib.pbkdf2_hmac)

The function provides PKCS#5 password-based key derivation function 2. It
uses HMAC as pseudorandom function.

The string *hash_name* is the desired name of the hash digest algorithm for
HMAC, e.g. âsha1â or âsha256â. *password* and *salt* are interpreted as
buffers of bytes. Applications and libraries should limit *password* to
a sensible length (e.g. 1024). *salt* should be about 16 or more bytes from
a proper source, e.g. [`os.urandom()`](os.html#os.urandom).
...
hashlib.scrypt(password, *, salt, n, r, p, maxmem=0, dklen=64)[Â¶](#hashlib.scrypt)
...
bytes from a proper source, e.g. [`os.urandom()`](os.html#os.urandom).
...
Hash objects from this module follow the API of standard libraryâs
`hashlib` objects.

### Creating hash objects[Â¶](#creating-hash-objects)

New hash objects are created by calling constructor functions:

hashlib.blake2b(data=b'', *, digest_size=64, key=b'', salt=b'', person=b'', fanout=1, depth=1, leaf_size=0, node_offset=0, node_depth=0, inner_size=0, last_node=False, usedforsecurity=True)[Â¶](#hashlib.blake2b)

hashlib.blake2s(data=b'', *, digest_size=32, key=b'', salt=b'', person=b'', fanout=1, depth=1, leaf_size=0, node_offset=0, node_depth=0, inner_size=0, last_node=False, usedforsecurity=True)[Â¶](#hashlib.blake2s)
...
blake2b.SALT_SIZE[Â¶](#hashlib.blake2b.SALT_SIZE)

blake2s.SALT_SIZE[Â¶](#hashlib.blake2s.SALT_SIZE)

Salt length (maximum length accepted by constructors).

blake2b.PERSON_SIZE[Â¶](#hashlib.blake2b.PERSON_SIZE)

blake2s.PERSON_SIZE[Â¶](#hashlib.blake2s.PERSON_SIZE)

Personalization string length (maximum length accepted by constructors).

blake2b.MAX_KEY_SIZE[Â¶](#hashlib.blake2b.MAX_KEY_SIZE)

blake2s.MAX_KEY_SIZE[Â¶](#hashlib.blake2s.MAX_KEY_SIZE)

Maximum key size.

blake2b.MAX_DIGEST_SIZE[Â¶](#hashlib.blake2b.MAX_DIGEST_SIZE)

blake2s.MAX_DIGEST_SIZE[Â¶](#hashlib.blake2s.MAX_DIGEST_SIZE)

Maximum digest size that the hash function can output.

### Examples[Â¶](#examples)

#### Simple hashing[Â¶](#simple-hashing)
...
#### Using different digest sizes[Â¶](#using-different-digest-sizes)
...
to users and later verify them to make sure they werenât tampered with:
...
Even though thereâs a native keyed hashing mode, BLAKE2 can, of course, be used
in HMAC construction with [`hmac`](hmac.html#module-hmac) module:
...
#### Randomized hashing[Â¶](#randomized-hashing)
...
([NIST SP-800-106 âRandomized Hashing for Digital Signaturesâ](https://csrc.nist.gov/pubs/sp/800/106/final))
...
#### Personalization[Â¶](#personalization)
...
#### Tree mode[Â¶](#tree-mode)

Hereâs an example of hashing a minimal tree with two leaf nodes:
...
Wilcox-OâHearn, and *Christian Winnerlein* based on [SHA-3](https://en.wikipedia.org/wiki/Secure_Hash_Algorithms) finalist [BLAKE](https://web.archive.org/web/20200918190133/https://131002.net/blake/)
...
Module [`hmac`](hmac.html#module-hmac)
A module to generate message authentication codes using hashes.

Module [`base64`](base64.html#module-base64)
...
[`hmac` â Keyed-Hashing for Message Authentication](hmac.html)
