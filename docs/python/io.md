...
io â Core tools for working with streams — Python 3.14.3 documentation
...
# `io` â Core tools for working with streams[Â¶](#module-io)

**Source code:** [Lib/io.py](https://github.com/python/cpython/tree/3.14/Lib/io.py)

## Overview[Â¶](#overview)

The `io` module provides Pythonâs main facilities for dealing with various
...
giving a [`str`](stdtypes.html#str) object to the `write()` method of a binary stream
will raise a [`TypeError`](exceptions.html#TypeError).  So will giving a [`bytes`](stdtypes.html#bytes) object to the
`write()` method of a text stream.

Changed in version 3.3: Operations that used to raise [`IOError`](exceptions.html#IOError) now raise [`OSError`](exceptions.html#OSError), since
[`IOError`](exceptions.html#IOError) is now an alias of [`OSError`](exceptions.html#OSError).

### Text I/O[Â¶](#text-i-o)

Text I/O expects and produces [`str`](stdtypes.html#str) objects.  This means that whenever
the backing store is natively made of bytes (such as in the case of a file),
encoding and decoding of data is made transparently as well as optional
translation of platform-specific newline characters.

The easiest way to create a text stream is with [`open()`](functions.html#open), optionally
...
might raise a [`BlockingIOError`](exceptions.html#BlockingIOError) if the stream cannot perform the operation
immediately.

The text stream API is described in detail in the documentation of
[`TextIOBase`](#io.TextIOBase).

### Binary I/O[Â¶](#binary-i-o)

Binary I/O (also called *buffered I/O*) expects
[bytes-like objects](../glossary.html#term-bytes-like-object) and produces [`bytes`](stdtypes.html#bytes)
objects.  No encoding, decoding, or newline translation is performed.  This
category of streams can be used for all kinds of non-text data, and also when
manual control over the handling of text data is desired.

The easiest way to create a binary stream is with [`open()`](functions.html#open) with `'b'` in
the mode string:
...
streams.  See [`socket.socket.makefile()`](socket.html#socket.socket.makefile) for example.
...
## Text Encoding[Â¶](#text-encoding)

The default encoding of [`TextIOWrapper`](#io.TextIOWrapper) and [`open()`](functions.html#open) is
locale-specific ([`locale.getencoding()`](locale.html#locale.getencoding)).
...
### Opt-in EncodingWarning[Â¶](#opt-in-encodingwarning)
...
emit an [`EncodingWarning`](exceptions.html#EncodingWarning) when the default encoding is used.

If you are providing an API that uses [`open()`](functions.html#open) or
...
[`EncodingWarning`](exceptions.html#EncodingWarning) if they donât pass an `encoding`. However,
please consider using UTF-8 by default (i.e. `encoding="utf-8"`) for
new APIs.

## High-level Module Interface[Â¶](#high-level-module-interface)

io.DEFAULT_BUFFER_SIZE[Â¶](#io.DEFAULT_BUFFER_SIZE)

An int containing the default buffer size used by the moduleâs buffered I/O
classes.  [`open()`](functions.html#open) uses the fileâs blksize (as obtained by
[`os.stat()`](os.html#os.stat)) if possible.

io.open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None)[Â¶](#io.open)

This is an alias for the builtin [`open()`](functions.html#open) function.

This function raises an [auditing event](sys.html#auditing) `open` with
arguments *path*, *mode* and *flags*. The *mode* and *flags*
arguments may have been modified or inferred from the original call.

io.open_code(path)[Â¶](#io.open_code)

Opens the provided file with mode `'rb'`. This function should be used
when the intent is to treat the contents as executable code.

*path* should be a [`str`](stdtypes.html#str) and an absolute path.

The behavior of this function may be overridden by an earlier call to the
[`PyFile_SetOpenCodeHook()`](../c-api/file.html#c.PyFile_SetOpenCodeHook). However, assuming that *path* is a
[`str`](stdtypes.html#str) and an absolute path, `open_code(path)` should always behave
the same as `open(path, 'rb')`. Overriding the behavior is intended for
additional validation or preprocessing of the file.

Added in version 3.8.

io.text_encoding(encoding, stacklevel=2, /)[Â¶](#io.text_encoding)

This is a helper function for callables that use [`open()`](functions.html#open) or
...
This function emits an [`EncodingWarning`](exceptions.html#EncodingWarning) if
[`sys.flags.warn_default_encoding`](sys.html#sys.flags) is true and *encoding*
...
In this example, an [`EncodingWarning`](exceptions.html#EncodingWarning) is emitted for the caller of
`read_text()`.
...
Changed in version 3.11: [`text_encoding()`](#io.text_encoding) returns âutf-8â when UTF-8 mode is enabled and
*encoding* is `None`.

exception io.BlockingIOError[Â¶](#io.BlockingIOError)

This is a compatibility alias for the builtin [`BlockingIOError`](exceptions.html#BlockingIOError)
exception.

exception io.UnsupportedOperation[Â¶](#io.UnsupportedOperation)

An exception inheriting [`OSError`](exceptions.html#OSError) and [`ValueError`](exceptions.html#ValueError) that is raised
when an unsupported operation is called on a stream.

See also

[`sys`](sys.html#module-sys)
contains the standard IO streams: [`sys.stdin`](sys.html#sys.stdin), [`sys.stdout`](sys.html#sys.stdout),
and [`sys.stderr`](sys.html#sys.stderr).

## Class hierarchy[Â¶](#class-hierarchy)
...
[`open()`](functions.html#open) are intended to be used as keyword arguments.
...
Inherited [`IOBase`](#io.IOBase) methods, `read`,
and `readall`
...
Inherited [`IOBase`](#io.IOBase) methods, `readinto`,
and `readinto1`
...
Inherited [`IOBase`](#io.IOBase) methods, `encoding`,
`errors`, and `newlines`

### I/O Base Classes[Â¶](#i-o-base-classes)

class io.IOBase[Â¶](#io.IOBase)
...
implementations may raise a [`ValueError`](exceptions.html#ValueError) (or [`UnsupportedOperation`](#io.UnsupportedOperation))
when operations they do not support are called.

The basic type used for binary data read from or written to a file is
[`bytes`](stdtypes.html#bytes).  Other [bytes-like objects](../glossary.html#term-bytes-like-object) are
accepted as method arguments too.  Text I/O classes work with [`str`](stdtypes.html#str) data.

Note that calling any method (even inquiries) on a closed stream is
undefined.  Implementations may raise [`ValueError`](exceptions.html#ValueError) in this case.
...
`with` statementâs suite is finishedâeven if an exception occurs:
...
close()[Â¶](#io.IOBase.close)

Flush and close this stream. This method has no effect if the file is
already closed. Once the file is closed, any operation on the file
(e.g. reading or writing) will raise a [`ValueError`](exceptions.html#ValueError).

As a convenience, it is allowed to call this method more than once;
only the first call, however, will have an effect.

closed[Â¶](#io.IOBase.closed)

`True` if the stream is closed.

fileno()[Â¶](#io.IOBase.fileno)

Return the underlying file descriptor (an integer) of the stream if it
exists.  An [`OSError`](exceptions.html#OSError) is raised if the IO object does not use a file
descriptor.

flush()[Â¶](#io.IOBase.flush)

Flush the write buffers of the stream if applicable.  This does nothing
for read-only and non-blocking streams.

isatty()[Â¶](#io.IOBase.isatty)

Return `True` if the stream is interactive (i.e., connected to
a terminal/tty device).

readable()[Â¶](#io.IOBase.readable)

Return `True` if the stream can be read from.
If `False`, `read()` will raise [`OSError`](exceptions.html#OSError).

readline(size=-1, /)[Â¶](#io.IOBase.readline)

Read and return one line from the stream.  If *size* is specified, at
most *size* bytes will be read.

The line terminator is always `b'\n'` for binary files; for text files,
the *newline* argument to [`open()`](functions.html#open) can be used to select the line
terminator(s) recognized.

readlines(hint=-1, /)[Â¶](#io.IOBase.readlines)
...
Note that itâs already possible to iterate on file objects using for
line in file: ... without calling `file.readlines()`.

seek(offset, whence=os.SEEK_SET, /)[Â¶](#io.IOBase.seek)
...
- 
[`os.SEEK_SET`](os.html#os.SEEK_SET) or `0` â start of the stream (the default);
*offset* should be zero or positive

- 
[`os.SEEK_CUR`](os.html#os.SEEK_CUR) or `1` â current stream position;
*offset* may be negative

- 
[`os.SEEK_END`](os.html#os.SEEK_END) or `2` â end of the stream;
...
[`os.SEEK_HOLE`](os.html#os.SEEK_HOLE) or [`os.SEEK_DATA`](os.html#os.SEEK_DATA). The valid values
for a file could depend on it being open in text or binary mode.

seekable()[Â¶](#io.IOBase.seekable)

Return `True` if the stream supports random access.  If `False`,
[`seek()`](#io.IOBase.seek), [`tell()`](#io.IOBase.tell) and [`truncate()`](#io.IOBase.truncate) will raise [`OSError`](exceptions.html#OSError).

tell()[Â¶](#io.IOBase.tell)

Return the current stream position.

truncate(size=None, /)[Â¶](#io.IOBase.truncate)

Resize the stream to the given *size* in bytes (or the current position
if *size* is not specified).  The current stream position isnât changed.
...
writable()[Â¶](#io.IOBase.writable)

Return `True` if the stream supports writing.  If `False`,
`write()` and [`truncate()`](#io.IOBase.truncate) will raise [`OSError`](exceptions.html#OSError).

writelines(lines, /)[Â¶](#io.IOBase.writelines)

Write a list of lines to the stream.  Line separators are not added, so it
is usual for each of the lines provided to have a line separator at the
end.

__del__()[Â¶](#io.IOBase.__del__)
...
class io.RawIOBase[Â¶](#io.RawIOBase)
...
read(size=-1, /)[Â¶](#io.RawIOBase.read)
...
readall()[Â¶](#io.RawIOBase.readall)

Read and return all the bytes from the stream until EOF, using multiple
calls to the stream if necessary.

readinto(b, /)[Â¶](#io.RawIOBase.readinto)

Read bytes into a pre-allocated, writable
[bytes-like object](../glossary.html#term-bytes-like-object) *b*, and return the
number of bytes read.  For example, *b* might be a [`bytearray`](stdtypes.html#bytearray).
If the object is in non-blocking mode and no bytes
are available, `None` is returned.

write(b, /)[Â¶](#io.RawIOBase.write)
...
class io.BufferedIOBase[Â¶](#io.BufferedIOBase)
...
system returns would block [`write()`](#io.BufferedIOBase.write) will raise [`BlockingIOError`](exceptions.html#BlockingIOError)
with [`BlockingIOError.characters_written`](exceptions.html#BlockingIOError.characters_written) and [`read()`](#io.BufferedIOBase.read) will return
...
detach()[Â¶](#io.BufferedIOBase.detach)
...
read(size=-1, /)[Â¶](#io.BufferedIOBase.read)

Read and return up to *size* bytes. If the argument is omitted, `None`,
or negative read as much as possible.

Fewer bytes may be returned than requested. An empty [`bytes`](stdtypes.html#bytes) object
...
[`os.read()`](os.html#os.read) and [**PEP 475**](https://peps.python.org/pep-0475/) for more details. Less than size bytes
...
returns `None`, an empty [`bytes`](stdtypes.html#bytes), or a non-retryable error. For
...
either raise [`BlockingIOError`](exceptions.html#BlockingIOError) or return `None` if no data is
available. `io` implementations return `None`.

read1(size=-1, /)[Â¶](#io.BufferedIOBase.read1)

Read and return up to *size* bytes, calling [`readinto()`](#io.RawIOBase.readinto)
which may retry if [`EINTR`](errno.html#errno.EINTR) is encountered per
...
readinto(b, /)[Â¶](#io.BufferedIOBase.readinto)

Read bytes into a pre-allocated, writable
[bytes-like object](../glossary.html#term-bytes-like-object) *b* and return the number of bytes read.
For example, *b* might be a [`bytearray`](stdtypes.html#bytearray).

Like [`read()`](#io.BufferedIOBase.read), multiple reads may be issued to the underlying raw
stream, unless the latter is interactive.

A [`BlockingIOError`](exceptions.html#BlockingIOError) is raised if the underlying raw stream is in non
blocking-mode, and has no data available at the moment.

readinto1(b, /)[Â¶](#io.BufferedIOBase.readinto1)

Read bytes into a pre-allocated, writable
[bytes-like object](../glossary.html#term-bytes-like-object) *b*, using at most one call to
the underlying raw streamâs [`read()`](#io.RawIOBase.read) (or
...
write(b, /)[Â¶](#io.BufferedIOBase.write)
...
the write fails an [`OSError`](exceptions.html#OSError) will be raised).  Depending on the
...
When in non-blocking mode, a [`BlockingIOError`](exceptions.html#BlockingIOError) is raised if the
...
### Raw File I/O[Â¶](#raw-file-i-o)

class io.FileIO(name, mode='r', closefd=True, opener=None)[Â¶](#io.FileIO)
...
- 
a character string or [`bytes`](stdtypes.html#bytes) object representing the path to the
...
truncated when opened for writing. [`FileExistsError`](exceptions.html#FileExistsError) will be raised if
...
[`os.open`](os.html#os.open) as *opener* results in functionality similar to passing
`None`).

The newly created file is [non-inheritable](os.html#fd-inheritance).

See the [`open()`](functions.html#open) built-in function for examples on using the *opener*
parameter.
...
mode[Â¶](#io.FileIO.mode)

The mode as given in the constructor.

name[Â¶](#io.FileIO.name)

The file name.  This is the file descriptor of the file when no name is
given in the constructor.

### Buffered Streams[Â¶](#buffered-streams)

Buffered I/O streams provide a higher-level interface to an I/O device
than raw I/O does.

class io.BytesIO(initial_bytes=b'')[Â¶](#io.BytesIO)
...
The optional argument *initial_bytes* is a [bytes-like object](../glossary.html#term-bytes-like-object) that
...
getbuffer()[Â¶](#io.BytesIO.getbuffer)
...
getvalue()[Â¶](#io.BytesIO.getvalue)

Return [`bytes`](stdtypes.html#bytes) containing the entire contents of the buffer.

read1(size=-1, /)[Â¶](#io.BytesIO.read1)
...
readinto1(b, /)[Â¶](#io.BytesIO.readinto1)

In [`BytesIO`](#io.BytesIO), this is the same as [`readinto()`](#io.BufferedIOBase.readinto).

Added in version 3.5.

class io.BufferedReader(raw, buffer_size=DEFAULT_BUFFER_SIZE)[Â¶](#io.BufferedReader)
...
peek(size=0, /)[Â¶](#io.BufferedReader.peek)
...
flush()[Â¶](#io.BufferedWriter.flush)

Force bytes held in the buffer into the raw stream.  A
[`BlockingIOError`](exceptions.html#BlockingIOError) should be raised if the raw stream blocks.

write(b, /)[Â¶](#io.BufferedWriter.write)

Write the [bytes-like object](../glossary.html#term-bytes-like-object), *b*, and return the
number of bytes written.  When in non-blocking mode, a
[`BlockingIOError`](exceptions.html#BlockingIOError) with [`BlockingIOError.characters_written`](exceptions.html#BlockingIOError.characters_written) set
is raised if the buffer needs to be written out but the raw stream blocks.

class io.BufferedRandom(raw, buffer_size=DEFAULT_BUFFER_SIZE)[Â¶](#io.BufferedRandom)
...
class io.BufferedRWPair(reader, writer, buffer_size=DEFAULT_BUFFER_SIZE, /)[Â¶](#io.BufferedRWPair)

A buffered binary stream providing higher-level access to two non seekable
[`RawIOBase`](#io.RawIOBase) raw binary streamsâone readable, the other writeable.
...
class io.TextIOBase[Â¶](#io.TextIOBase)
...
encoding[Â¶](#io.TextIOBase.encoding)

The name of the encoding used to decode the streamâs bytes into
strings, and to encode strings into bytes.

errors[Â¶](#io.TextIOBase.errors)

The error setting of the decoder or encoder.

newlines[Â¶](#io.TextIOBase.newlines)

A string, a tuple of strings, or `None`, indicating the newlines
translated so far.  Depending on the implementation and the initial
constructor flags, this may not be available.

buffer[Â¶](#io.TextIOBase.buffer)
...
read(size=-1, /)[Â¶](#io.TextIOBase.read)

Read and return at most *size* characters from the stream as a single
[`str`](stdtypes.html#str).  If *size* is negative or `None`, reads until EOF.

readline(size=-1, /)[Â¶](#io.TextIOBase.readline)

Read until newline or EOF and return a single [`str`](stdtypes.html#str).  If the stream is
already at EOF, an empty string is returned.

If *size* is specified, at most *size* characters will be read.

seek(offset, whence=SEEK_SET, /)[Â¶](#io.TextIOBase.seek)
...
- 
`SEEK_CUR` or `1`: âseekâ to the current position;
...
tell()[Â¶](#io.TextIOBase.tell)

Return the current stream position as an opaque number.  The number
does not usually represent a number of bytes in the underlying
binary storage.

write(s, /)[Â¶](#io.TextIOBase.write)

Write the string *s* to the stream and return the number of characters
written.

class io.TextIOWrapper(buffer, encoding=None, errors=None, newline=None, line_buffering=False, write_through=False)[Â¶](#io.TextIOWrapper)
...
Otherwise, it defaults to [`locale.getencoding()`](locale.html#locale.getencoding).
`encoding="locale"` can be used to specify the current localeâs encoding
explicitly. See [Text Encoding](#io-text-encoding) for more information.

*errors* is an optional string that specifies how encoding and decoding
errors are to be handled.  Pass `'strict'` to raise a [`ValueError`](exceptions.html#ValueError)
...
[`codecs.register_error()`](codecs.html#codecs.register_error) is also valid.
...
[`os.linesep`](os.html#os.linesep).  If *newline* is `''` or `'\n'`, no translation
...
instead of `locale.getpreferredencoding()`. Donât change temporary the
locale encoding using [`locale.setlocale()`](locale.html#locale.setlocale), use the current locale
...
When the underlying raw stream is non-blocking, a [`BlockingIOError`](exceptions.html#BlockingIOError)
...
line_buffering[Â¶](#io.TextIOWrapper.line_buffering)

Whether line buffering is enabled.

write_through[Â¶](#io.TextIOWrapper.write_through)
...
reconfigure(*, encoding=None, errors=None, newline=None, line_buffering=None, write_through=None)[Â¶](#io.TextIOWrapper.reconfigure)
...
seek(cookie, whence=os.SEEK_SET, /)[Â¶](#io.TextIOWrapper.seek)

Set the stream position.
Return the new stream position as an [`int`](functions.html#int).
...
[`os.SEEK_SET`](os.html#os.SEEK_SET), [`os.SEEK_CUR`](os.html#os.SEEK_CUR), and [`os.SEEK_END`](os.html#os.SEEK_END).

tell()[Â¶](#io.TextIOWrapper.tell)
...
getvalue()[Â¶](#io.StringIO.getvalue)

Return a [`str`](stdtypes.html#str) containing the entire contents of the buffer.
...
class io.IncrementalNewlineDecoder[Â¶](#io.IncrementalNewlineDecoder)

A helper codec that decodes newlines for [universal newlines](../glossary.html#term-universal-newlines) mode.
It inherits from [`codecs.IncrementalDecoder`](codecs.html#codecs.IncrementalDecoder).

## Static Typing[Â¶](#static-typing)
...
with [`@typing.runtime_checkable`](typing.html#typing.runtime_checkable).

class io.Reader[T][Â¶](#io.Reader)

Generic protocol for reading from a file or other input stream. `T` will
usually be [`str`](stdtypes.html#str) or [`bytes`](stdtypes.html#bytes), but can be any type that is
read from the stream.

Added in version 3.14.

read()[Â¶](#io.Reader.read)
...
class io.Writer[T][Â¶](#io.Writer)

Generic protocol for writing to a file or other output stream. `T` will
usually be [`str`](stdtypes.html#str) or [`bytes`](stdtypes.html#bytes), but can be any type that can be
written to the stream.

Added in version 3.14.

write(data, /)[Â¶](#io.Writer.write)
...
### Multi-threading[Â¶](#multi-threading)
...
### Reentrancy[Â¶](#reentrancy)

Binary buffered objects (instances of [`BufferedReader`](#io.BufferedReader),
[`BufferedWriter`](#io.BufferedWriter), [`BufferedRandom`](#io.BufferedRandom) and [`BufferedRWPair`](#io.BufferedRWPair))
are not reentrant.  While reentrant calls will not happen in normal situations,
they can arise from doing I/O in a [`signal`](signal.html#module-signal) handler.  If a thread tries to
re-enter a buffered object which it is already accessing, a [`RuntimeError`](exceptions.html#RuntimeError)
is raised.  Note this doesnât prohibit a different thread from entering the
buffered object.

The above implicitly extends to text files, since the [`open()`](functions.html#open) function
will wrap a buffered object inside a [`TextIOWrapper`](#io.TextIOWrapper).  This includes
standard streams and therefore affects the built-in [`print()`](functions.html#print) function as
well.
...
[`os` â Miscellaneous operating system interfaces](os.html)
