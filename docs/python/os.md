...
os â Miscellaneous operating system interfaces — Python 3.14.3 documentation
...
# `os` â Miscellaneous operating system interfaces[Â¶](#module-os)

**Source code:** [Lib/os.py](https://github.com/python/cpython/tree/3.14/Lib/os.py)

This module provides a portable way of using operating system dependent
functionality.  If you just want to read or write a file see [`open()`](functions.html#open), if
you want to manipulate paths, see the [`os.path`](os.path.html#module-os.path) module, and if you want to
read all the lines in all the files on the command line see the [`fileinput`](fileinput.html#module-fileinput)
module.  For creating temporary files and directories see the [`tempfile`](tempfile.html#module-tempfile)
module, and for high-level file and directory handling see the [`shutil`](shutil.html#module-shutil)
module.
...
All functions in this module raise [`OSError`](exceptions.html#OSError) (or subclasses thereof) in
...
exception os.error[Â¶](#os.error)

An alias for the built-in [`OSError`](exceptions.html#OSError) exception.
...
[`sys.platform`](sys.html#sys.platform) has a finer granularity.  [`os.uname()`](#os.uname) gives
system-dependent version information.

The [`platform`](platform.html#module-platform) module provides detailed checks for the
systemâs identity.

## File Names, Command Line Arguments, and Environment Variables[Â¶](#file-names-command-line-arguments-and-environment-variables)
...
conversion (see [`sys.getfilesystemencoding()`](sys.html#sys.getfilesystemencoding)).
...
startup by the [`PyConfig_Read()`](../c-api/init_config.html#c.PyConfig_Read) function: see
[`filesystem_encoding`](../c-api/init_config.html#c.PyConfig.filesystem_encoding) and
[`filesystem_errors`](../c-api/init_config.html#c.PyConfig.filesystem_errors) members of [`PyConfig`](../c-api/init_config.html#c.PyConfig).
...
[`UnicodeError`](exceptions.html#UnicodeError).

See also the [locale encoding](../glossary.html#term-locale-encoding).

## Python UTF-8 Mode[Â¶](#python-utf-8-mode)
...
- 
[`sys.getfilesystemencoding()`](sys.html#sys.getfilesystemencoding) returns `'utf-8'`.

- 
[`locale.getpreferredencoding()`](locale.html#locale.getpreferredencoding) returns `'utf-8'` (the *do_setlocale*
argument has no effect).

- 
[`sys.stdin`](sys.html#sys.stdin), [`sys.stdout`](sys.html#sys.stdout), and [`sys.stderr`](sys.html#sys.stderr) all use
UTF-8 as their text encoding, with the `surrogateescape`
[error handler](codecs.html#error-handlers) being enabled for [`sys.stdin`](sys.html#sys.stdin)
and [`sys.stdout`](sys.html#sys.stdout) ([`sys.stderr`](sys.html#sys.stderr) continues to use
...
- 
[`open()`](functions.html#open), [`io.open()`](io.html#io.open), and [`codecs.open()`](codecs.html#codecs.open) use the UTF-8
encoding by default. However, they still use the strict error handler by
default so that attempting to open a binary file in text mode is likely
to raise an exception rather than producing nonsense data.

The [Python UTF-8 Mode](#utf8-mode) is enabled if the LC_CTYPE locale is
`C` or `POSIX` at Python startup (see the [`PyConfig_Read()`](../c-api/init_config.html#c.PyConfig_Read)
function).
...
can be read from [`sys.flags.utf8_mode`](sys.html#sys.flags).
...
## Process Parameters[Â¶](#process-parameters)

These functions and data items provide information and operate on the current
process and user.

os.ctermid()[Â¶](#os.ctermid)

Return the filename corresponding to the controlling terminal of the process.

[Availability](intro.html#availability): Unix, not WASI.

os.environ[Â¶](#os.environ)
...
On Unix, keys and values use [`sys.getfilesystemencoding()`](sys.html#sys.getfilesystemencoding) and
...
Calling [`putenv()`](#os.putenv) directly does not change [`os.environ`](#os.environ), so itâs better
...
[`os.environ`](#os.environ), and when one of the `pop()` or `clear()` methods is
called.
...
Changed in version 3.9: Updated to support [**PEP 584**](https://peps.python.org/pep-0584/)âs merge (`|`) and update (`|=`) operators.

os.environb[Â¶](#os.environb)

Bytes version of [`environ`](#os.environ): a [mapping](../glossary.html#term-mapping) object where both keys
and values are [`bytes`](stdtypes.html#bytes) objects representing the process environment.
...
os.reload_environ()[Â¶](#os.reload_environ)
...
os.fsencode(filename)[Â¶](#os.fsencode)

Encode [path-like](../glossary.html#term-path-like-object) *filename* to the
[filesystem encoding and error handler](../glossary.html#term-filesystem-encoding-and-error-handler); return [`bytes`](stdtypes.html#bytes)
unchanged.
...
os.fsdecode(filename)[Â¶](#os.fsdecode)

Decode the [path-like](../glossary.html#term-path-like-object) *filename* from the
[filesystem encoding and error handler](../glossary.html#term-filesystem-encoding-and-error-handler); return [`str`](stdtypes.html#str)
unchanged.
...
os.fspath(path)[Â¶](#os.fspath)

Return the file system representation of the path.

If [`str`](stdtypes.html#str) or [`bytes`](stdtypes.html#bytes) is passed in, it is returned unchanged.
Otherwise [`__fspath__()`](#os.PathLike.__fspath__) is called and its value is
returned as long as it is a [`str`](stdtypes.html#str) or [`bytes`](stdtypes.html#bytes) object.
In all other cases, [`TypeError`](exceptions.html#TypeError) is raised.

Added in version 3.6.

class os.PathLike[Â¶](#os.PathLike)

An [abstract base class](../glossary.html#term-abstract-base-class) for objects representing a file system path,
e.g. [`pathlib.PurePath`](pathlib.html#pathlib.PurePath).

Added in version 3.6.

abstractmethod __fspath__()[Â¶](#os.PathLike.__fspath__)

Return the file system path representation of the object.

The method should only return a [`str`](stdtypes.html#str) or [`bytes`](stdtypes.html#bytes) object,
with the preference being for [`str`](stdtypes.html#str).

os.getenv(key, default=None)[Â¶](#os.getenv)

Return the value of the environment variable *key* as a string if it exists, or
*default* if it doesnât. *key* is a string. Note that
...
On Unix, keys and values are decoded with [`sys.getfilesystemencoding()`](sys.html#sys.getfilesystemencoding)
...
os.getenvb(key, default=None)[Â¶](#os.getenvb)

Return the value of the environment variable *key* as bytes if it exists, or
*default* if it doesnât. *key* must be bytes. Note that
...
os.get_exec_path(env=None)[Â¶](#os.get_exec_path)
...
os.getegid()[Â¶](#os.getegid)

Return the effective group id of the current process.  This corresponds to the
âset idâ bit on the file being executed in the current process.

[Availability](intro.html#availability): Unix, not WASI.

os.geteuid()[Â¶](#os.geteuid)

Return the current processâs effective user id.

[Availability](intro.html#availability): Unix, not WASI.

os.getgid()[Â¶](#os.getgid)
...
os.getgrouplist(user, group, /)[Â¶](#os.getgrouplist)
...
obtained with [`sysconfig.get_config_var()`](sysconfig.html#sysconfig.get_config_var).

os.getlogin()[Â¶](#os.getlogin)

Return the name of the user logged in on the controlling terminal of the
process.  For most purposes, it is more useful to use
[`getpass.getuser()`](getpass.html#getpass.getuser) since the latter checks the environment variables
...
os.getpgrp()[Â¶](#os.getpgrp)

Return the id of the current process group.

[Availability](intro.html#availability): Unix, not WASI.

os.getpid()[Â¶](#os.getpid)
...
os.getppid()[Â¶](#os.getppid)

Return the parentâs process id.  When the parent process has exited, on Unix
...
os.getpriority(which, who)[Â¶](#os.getpriority)
...
os.PRIO_PROCESS[Â¶](#os.PRIO_PROCESS)

os.PRIO_PGRP[Â¶](#os.PRIO_PGRP)

os.PRIO_USER[Â¶](#os.PRIO_USER)
...
os.PRIO_DARWIN_THREAD[Â¶](#os.PRIO_DARWIN_THREAD)

os.PRIO_DARWIN_PROCESS[Â¶](#os.PRIO_DARWIN_PROCESS)

os.PRIO_DARWIN_BG[Â¶](#os.PRIO_DARWIN_BG)

os.PRIO_DARWIN_NONUI[Â¶](#os.PRIO_DARWIN_NONUI)
...
os.getresuid()[Â¶](#os.getresuid)

Return a tuple (ruid, euid, suid) denoting the current processâs
...
os.getresgid()[Â¶](#os.getresgid)

Return a tuple (rgid, egid, sgid) denoting the current processâs
...
os.getuid()[Â¶](#os.getuid)

Return the current processâs real user id.
...
os.initgroups(username, gid, /)[Â¶](#os.initgroups)
...
os.putenv(key, value, /)[Â¶](#os.putenv)
...
donât update [`os.environ`](#os.environ), so it is actually preferable to assign to items
...
os.setegid(egid, /)[Â¶](#os.setegid)

Set the current processâs effective group id.

[Availability](intro.html#availability): Unix, not WASI, not Android.

os.seteuid(euid, /)[Â¶](#os.seteuid)

Set the current processâs effective user id.

[Availability](intro.html#availability): Unix, not WASI, not Android.

os.setgid(gid, /)[Â¶](#os.setgid)

Set the current processâ group id.

[Availability](intro.html#availability): Unix, not WASI, not Android.

os.setgroups(groups, /)[Â¶](#os.setgroups)
...
os.setns(fd, nstype=0)[Â¶](#os.setns)
...
The callerâs memberships in unspecified namespaces are left unchanged.

*fd* can be any object with a [`fileno()`](io.html#io.IOBase.fileno) method, or a raw file descriptor.

This example reassociates the thread with the `init` processâs network namespace:
...
os.setpgid(pid, pgrp, /)[Â¶](#os.setpgid)
...
os.setpriority(which, who, priority)[Â¶](#os.setpriority)
...
os.setregid(rgid, egid, /)[Â¶](#os.setregid)

Set the current processâs real and effective group ids.

[Availability](intro.html#availability): Unix, not WASI, not Android.

os.setresgid(rgid, egid, sgid, /)[Â¶](#os.setresgid)

Set the current processâs real, effective, and saved group ids.

[Availability](intro.html#availability): Unix, not WASI, not Android.

Added in version 3.2.

os.setresuid(ruid, euid, suid, /)[Â¶](#os.setresuid)
...
os.setreuid(ruid, euid, /)[Â¶](#os.setreuid)

Set the current processâs real and effective user ids.

[Availability](intro.html#availability): Unix, not WASI, not Android.

os.getsid(pid, /)[Â¶](#os.getsid)

Call the system call `getsid()`.  See the Unix manual for the semantics.

[Availability](intro.html#availability): Unix, not WASI.

os.setsid()[Â¶](#os.setsid)

Call the system call `setsid()`.  See the Unix manual for the semantics.

[Availability](intro.html#availability): Unix, not WASI.

os.setuid(uid, /)[Â¶](#os.setuid)

Set the current processâs user id.

[Availability](intro.html#availability): Unix, not WASI, not Android.

os.strerror(code, /)[Â¶](#os.strerror)

Return the error message corresponding to the error code in *code*.
On platforms where `strerror()` returns `NULL` when given an unknown
error number, [`ValueError`](exceptions.html#ValueError) is raised.

os.supports_bytes_environ[Â¶](#os.supports_bytes_environ)
...
[`socket.gethostname()`](socket.html#socket.gethostname)  or even
`socket.gethostbyaddr(socket.gethostname())`.

On macOS, iOS and Android, this returns the *kernel* name and version (i.e.,
`'Darwin'` on macOS and iOS; `'Linux'` on Android). [`platform.uname()`](platform.html#platform.uname)
...
donât update [`os.environ`](#os.environ), so it is actually preferable to delete items of
...
os.CLONE_FILES[Â¶](#os.CLONE_FILES)

os.CLONE_FS[Â¶](#os.CLONE_FS)

os.CLONE_NEWCGROUP[Â¶](#os.CLONE_NEWCGROUP)

os.CLONE_NEWIPC[Â¶](#os.CLONE_NEWIPC)

os.CLONE_NEWNET[Â¶](#os.CLONE_NEWNET)

os.CLONE_NEWNS[Â¶](#os.CLONE_NEWNS)

os.CLONE_NEWPID[Â¶](#os.CLONE_NEWPID)

os.CLONE_NEWTIME[Â¶](#os.CLONE_NEWTIME)

os.CLONE_NEWUSER[Â¶](#os.CLONE_NEWUSER)

os.CLONE_NEWUTS[Â¶](#os.CLONE_NEWUTS)

os.CLONE_SIGHAND[Â¶](#os.CLONE_SIGHAND)

os.CLONE_SYSVSEM[Â¶](#os.CLONE_SYSVSEM)

os.CLONE_THREAD[Â¶](#os.CLONE_THREAD)

os.CLONE_VM[Â¶](#os.CLONE_VM)

## File Object Creation[Â¶](#file-object-creation)

These functions create new [file objects](../glossary.html#term-file-object).  (See also
[`open()`](#os.open) for opening file descriptors.)

os.fdopen(fd, *args, **kwargs)[Â¶](#os.fdopen)

Return an open file object connected to the file descriptor *fd*.  This is an
alias of the [`open()`](functions.html#open) built-in function and accepts the same arguments.
The only difference is that the first argument of [`fdopen()`](#os.fdopen) must always
be an integer.

## File Descriptor Operations[Â¶](#file-descriptor-operations)
...
descriptor as returned by [`os.open()`](#os.open) or [`pipe()`](#os.pipe).  To close a âfile
objectâ returned by the built-in function [`open()`](functions.html#open) or by [`popen()`](#os.popen) or
[`fdopen()`](#os.fdopen), use its [`close()`](io.html#io.IOBase.close) method.

os.closerange(fd_low, fd_high, /)[Â¶](#os.closerange)
...
os.copy_file_range(src, dst, count, offset_src=None, offset_dst=None)[Â¶](#os.copy_file_range)
...
must reside in the same filesystem, otherwise an [`OSError`](exceptions.html#OSError) is
raised with [`errno`](exceptions.html#OSError.errno) set to [`errno.EXDEV`](errno.html#errno.EXDEV).
...
os.device_encoding(fd)[Â¶](#os.device_encoding)

Return a string describing the encoding of the device associated with *fd*
if it is connected to a terminal; else return [`None`](constants.html#None).
...
os.dup2(fd, fd2, inheritable=True)[Â¶](#os.dup2)
...
os.fchown(fd, uid, gid)[Â¶](#os.fchown)
...
os.fdatasync(fd)[Â¶](#os.fdatasync)
...
os.fpathconf(fd, name, /)[Â¶](#os.fpathconf)
...
If *name* is a string and is not known, [`ValueError`](exceptions.html#ValueError) is raised.  If a
specific value for *name* is not supported by the host system, even if it is
included in `pathconf_names`, an [`OSError`](exceptions.html#OSError) is raised with
[`errno.EINVAL`](errno.html#errno.EINVAL) for the error number.

As of Python 3.3, this is equivalent to `os.pathconf(fd, name)`.

[Availability](intro.html#availability): Unix.

os.fstat(fd)[Â¶](#os.fstat)
...
os.fsync(fd)[Â¶](#os.fsync)

Force write of file with filedescriptor *fd* to disk.  On Unix, this calls the
native `fsync()` function; on Windows, the MS `_commit()` function.

If youâre starting with a buffered Python [file object](../glossary.html#term-file-object) *f*, first do
`f.flush()`, and then do `os.fsync(f.fileno())`, to ensure that all internal
buffers associated with *f* are written to disk.

[Availability](intro.html#availability): Unix, Windows.

os.ftruncate(fd, length, /)[Â¶](#os.ftruncate)
...
os.get_blocking(fd, /)[Â¶](#os.get_blocking)

Get the blocking mode of the file descriptor: `False` if the
[`O_NONBLOCK`](#os.O_NONBLOCK) flag is set, `True` if the flag is cleared.

See also [`set_blocking()`](#os.set_blocking) and [`socket.socket.setblocking()`](socket.html#socket.socket.setblocking).
...
os.isatty(fd, /)[Â¶](#os.isatty)

Return `True` if the file descriptor *fd* is open and connected to a
tty(-like) device, else `False`.

os.lockf(fd, cmd, len, /)[Â¶](#os.lockf)
...
os.F_LOCK[Â¶](#os.F_LOCK)

os.F_TLOCK[Â¶](#os.F_TLOCK)

os.F_ULOCK[Â¶](#os.F_ULOCK)

os.F_TEST[Â¶](#os.F_TEST)
...
os.login_tty(fd, /)[Â¶](#os.login_tty)
...
os.lseek(fd, pos, whence, /)[Â¶](#os.lseek)
...
- 
[`SEEK_SET`](#os.SEEK_SET) or `0` â set *pos* relative to the beginning of the file

- 
[`SEEK_CUR`](#os.SEEK_CUR) or `1` â set *pos* relative to the current file position

- 
[`SEEK_END`](#os.SEEK_END) or `2` â set *pos* relative to the end of the file

- 
[`SEEK_HOLE`](#os.SEEK_HOLE) â set *pos* to the next data location, relative to *pos*

- 
[`SEEK_DATA`](#os.SEEK_DATA) â set *pos* to the next data hole, relative to *pos*

Changed in version 3.3: Add support for `SEEK_HOLE` and `SEEK_DATA`.

os.SEEK_SET[Â¶](#os.SEEK_SET)

os.SEEK_CUR[Â¶](#os.SEEK_CUR)

os.SEEK_END[Â¶](#os.SEEK_END)

Parameters to the [`lseek()`](#os.lseek) function and the [`seek()`](io.html#io.IOBase.seek)
...
os.SEEK_HOLE[Â¶](#os.SEEK_HOLE)

os.SEEK_DATA[Â¶](#os.SEEK_DATA)
...
os.open(path, flags, mode=0o777, *, dir_fd=None)[Â¶](#os.open)
...
built-in function [`open()`](functions.html#open), which returns a [file object](../glossary.html#term-file-object) with
`read()` and `write()` methods (and many more).  To
wrap a file descriptor in a file object, use [`fdopen()`](#os.fdopen).
...
[`InterruptedError`](exceptions.html#InterruptedError) exception (see [**PEP 475**](https://peps.python.org/pep-0475/) for the rationale).
...
os.O_RDONLY[Â¶](#os.O_RDONLY)

os.O_WRONLY[Â¶](#os.O_WRONLY)

os.O_RDWR[Â¶](#os.O_RDWR)

os.O_APPEND[Â¶](#os.O_APPEND)

os.O_CREAT[Â¶](#os.O_CREAT)

os.O_EXCL[Â¶](#os.O_EXCL)

os.O_TRUNC[Â¶](#os.O_TRUNC)

The above constants are available on Unix and Windows.

os.O_DSYNC[Â¶](#os.O_DSYNC)

os.O_RSYNC[Â¶](#os.O_RSYNC)

os.O_SYNC[Â¶](#os.O_SYNC)

os.O_NDELAY[Â¶](#os.O_NDELAY)

os.O_NONBLOCK[Â¶](#os.O_NONBLOCK)

os.O_NOCTTY[Â¶](#os.O_NOCTTY)

os.O_CLOEXEC[Â¶](#os.O_CLOEXEC)

The above constants are only available on Unix.

Changed in version 3.3: Add [`O_CLOEXEC`](#os.O_CLOEXEC) constant.

os.O_BINARY[Â¶](#os.O_BINARY)

os.O_NOINHERIT[Â¶](#os.O_NOINHERIT)

os.O_SHORT_LIVED[Â¶](#os.O_SHORT_LIVED)

os.O_TEMPORARY[Â¶](#os.O_TEMPORARY)

os.O_RANDOM[Â¶](#os.O_RANDOM)

os.O_SEQUENTIAL[Â¶](#os.O_SEQUENTIAL)

os.O_TEXT[Â¶](#os.O_TEXT)

The above constants are only available on Windows.

os.O_EVTONLY[Â¶](#os.O_EVTONLY)

os.O_FSYNC[Â¶](#os.O_FSYNC)

os.O_SYMLINK[Â¶](#os.O_SYMLINK)

os.O_NOFOLLOW_ANY[Â¶](#os.O_NOFOLLOW_ANY)
...
os.O_ASYNC[Â¶](#os.O_ASYNC)

os.O_DIRECT[Â¶](#os.O_DIRECT)

os.O_DIRECTORY[Â¶](#os.O_DIRECTORY)

os.O_NOFOLLOW[Â¶](#os.O_NOFOLLOW)

os.O_NOATIME[Â¶](#os.O_NOATIME)

os.O_PATH[Â¶](#os.O_PATH)

os.O_TMPFILE[Â¶](#os.O_TMPFILE)

os.O_SHLOCK[Â¶](#os.O_SHLOCK)

os.O_EXLOCK[Â¶](#os.O_EXLOCK)
...
os.posix_fallocate(fd, offset, len, /)[Â¶](#os.posix_fallocate)
...
os.posix_fadvise(fd, offset, len, advice, /)[Â¶](#os.posix_fadvise)
...
os.POSIX_FADV_NORMAL[Â¶](#os.POSIX_FADV_NORMAL)

os.POSIX_FADV_SEQUENTIAL[Â¶](#os.POSIX_FADV_SEQUENTIAL)

os.POSIX_FADV_RANDOM[Â¶](#os.POSIX_FADV_RANDOM)

os.POSIX_FADV_NOREUSE[Â¶](#os.POSIX_FADV_NOREUSE)

os.POSIX_FADV_WILLNEED[Â¶](#os.POSIX_FADV_WILLNEED)

os.POSIX_FADV_DONTNEED[Â¶](#os.POSIX_FADV_DONTNEED)
...
os.pread(fd, n, offset, /)[Â¶](#os.pread)
...
os.posix_openpt(oflag, /)[Â¶](#os.posix_openpt)
...
os.preadv(fd, buffers, offset, flags=0, /)[Â¶](#os.preadv)
...
[`errno.EAGAIN`](errno.html#errno.EAGAIN).

[Availability](intro.html#availability): Linux >= 4.14.

Added in version 3.7.

os.RWF_HIPRI[Â¶](#os.RWF_HIPRI)
...
os.pwrite(fd, str, offset, /)[Â¶](#os.pwrite)
...
os.pwritev(fd, buffers, offset, flags=0, /)[Â¶](#os.pwritev)
...
âfile objectâ returned by the built-in function [`open()`](functions.html#open) or by
[`popen()`](#os.popen) or [`fdopen()`](#os.fdopen), or [`sys.stdin`](sys.html#sys.stdin), use its
...
os.readinto(fd, buffer, /)[Â¶](#os.readinto)
...
âfile objectâ returned by the built-in function [`open()`](functions.html#open), or
[`sys.stdin`](sys.html#sys.stdin), use its member functions, for example
[`io.BufferedIOBase.readinto()`](io.html#io.BufferedIOBase.readinto), [`io.BufferedIOBase.read()`](io.html#io.BufferedIOBase.read), or
[`io.TextIOBase.read()`](io.html#io.TextIOBase.read)

Added in version 3.14.

os.sendfile(out_fd, in_fd, offset, count)[Â¶](#os.sendfile)
...
[`socket.socket.sendfile()`](socket.html#socket.socket.sendfile).

Added in version 3.3.

Changed in version 3.9: Parameters *out* and *in* was renamed to *out_fd* and *in_fd*.

os.SF_NODISKIO[Â¶](#os.SF_NODISKIO)

os.SF_MNOWAIT[Â¶](#os.SF_MNOWAIT)

os.SF_SYNC[Â¶](#os.SF_SYNC)
...
os.SF_NOCACHE[Â¶](#os.SF_NOCACHE)

Parameter to the [`sendfile()`](#os.sendfile) function, if the implementation supports
it. The data wonât be cached in the virtual memory and will be freed afterwards.

[Availability](intro.html#availability): Unix, not WASI.

Added in version 3.11.

os.set_blocking(fd, blocking, /)[Â¶](#os.set_blocking)

Set the blocking mode of the specified file descriptor. Set the
[`O_NONBLOCK`](#os.O_NONBLOCK) flag if blocking is `False`, clear the flag otherwise.

See also [`get_blocking()`](#os.get_blocking) and [`socket.socket.setblocking()`](socket.html#socket.socket.setblocking).
...
os.splice(src, dst, count, offset_src=None, offset_dst=None, flags=0)[Â¶](#os.splice)
...
the same filesystem, otherwise an [`OSError`](exceptions.html#OSError) is raised with
[`errno`](exceptions.html#OSError.errno) set to [`errno.EXDEV`](errno.html#errno.EXDEV).
...
os.SPLICE_F_MOVE[Â¶](#os.SPLICE_F_MOVE)

os.SPLICE_F_NONBLOCK[Â¶](#os.SPLICE_F_NONBLOCK)

os.SPLICE_F_MORE[Â¶](#os.SPLICE_F_MORE)

Added in version 3.10.

os.readv(fd, buffers, /)[Â¶](#os.readv)
...
os.tcgetpgrp(fd, /)[Â¶](#os.tcgetpgrp)
...
os.tcsetpgrp(fd, pg, /)[Â¶](#os.tcsetpgrp)
...
This function is intended for low-level I/O and must be applied to a file
descriptor as returned by [`os.open()`](#os.open) or [`pipe()`](#os.pipe).  To write a âfile
objectâ returned by the built-in function [`open()`](functions.html#open) or by [`popen()`](#os.popen) or
[`fdopen()`](#os.fdopen), or [`sys.stdout`](sys.html#sys.stdout) or [`sys.stderr`](sys.html#sys.stderr), use its
`write()` method.
...
os.writev(fd, buffers, /)[Â¶](#os.writev)
...
### Querying the size of a terminal[Â¶](#querying-the-size-of-a-terminal)

Added in version 3.3.

os.get_terminal_size(fd=STDOUT_FILENO, /)[Â¶](#os.get_terminal_size)
...
If the file descriptor is not connected to a terminal, an [`OSError`](exceptions.html#OSError)
is raised.

[`shutil.get_terminal_size()`](shutil.html#shutil.get_terminal_size) is the high-level function which
should normally be used, `os.get_terminal_size` is the low-level
implementation.

[Availability](intro.html#availability): Unix, Windows.

class os.terminal_size[Â¶](#os.terminal_size)

A subclass of tuple, holding `(columns, lines)` of the terminal window size.

columns[Â¶](#os.terminal_size.columns)

Width of the terminal window in characters.

lines[Â¶](#os.terminal_size.lines)

Height of the terminal window in characters.

### Inheritance of File Descriptors[Â¶](#inheritance-of-file-descriptors)

Added in version 3.4.

A file descriptor has an âinheritableâ flag which indicates if the file descriptor
...
Using the [`subprocess`](subprocess.html#module-subprocess) module, all file descriptors except standard
streams are closed, and inheritable handles are only inherited if the
*close_fds* parameter is `False`.

On WebAssembly platforms, the file descriptor cannot be modified.

os.get_inheritable(fd, /)[Â¶](#os.get_inheritable)

Get the âinheritableâ flag of the specified file descriptor (a boolean).

os.set_inheritable(fd, inheritable, /)[Â¶](#os.set_inheritable)

Set the âinheritableâ flag of the specified file descriptor.

os.get_handle_inheritable(handle, /)[Â¶](#os.get_handle_inheritable)

Get the âinheritableâ flag of the specified handle (a boolean).

[Availability](intro.html#availability): Windows.

os.set_handle_inheritable(handle, inheritable, /)[Â¶](#os.set_handle_inheritable)

Set the âinheritableâ flag of the specified handle.

[Availability](intro.html#availability): Windows.

## Files and Directories[Â¶](#files-and-directories)
...
[`NotImplementedError`](exceptions.html#NotImplementedError).

If the function also supports *dir_fd* or *follow_symlinks* arguments, itâs
...
on your platform using [`os.supports_dir_fd`](#os.supports_dir_fd).  If itâs unavailable,
using it will raise a [`NotImplementedError`](exceptions.html#NotImplementedError).
...
If itâs unavailable, using it will raise a [`NotImplementedError`](exceptions.html#NotImplementedError).

os.access(path, mode, *, dir_fd=None, effective_ids=False, follow_symlinks=True)[Â¶](#os.access)
...
[`X_OK`](#os.X_OK) to test permissions.  Return [`True`](constants.html#True) if access is allowed,
[`False`](constants.html#False) if not. See the Unix man page [access(2)](https://manpages.debian.org/access(2)) for more
information.

This function can support specifying paths relative to directory
descriptors and [not following symlinks](#follow-symlinks).
...
unavailable, using it will raise a [`NotImplementedError`](exceptions.html#NotImplementedError).

Note

Using [`access()`](#os.access) to check if a user is authorized to e.g. open a file
before actually doing so using [`open()`](functions.html#open) creates a security hole,
because the user might exploit the short time interval between checking
and opening the file to manipulate it. Itâs preferable to use [EAFP](../glossary.html#term-EAFP)
...
This function can raise [`OSError`](exceptions.html#OSError) and subclasses such as
[`FileNotFoundError`](exceptions.html#FileNotFoundError), [`PermissionError`](exceptions.html#PermissionError), and [`NotADirectoryError`](exceptions.html#NotADirectoryError).
...
os.chflags(path, flags, *, follow_symlinks=True)[Â¶](#os.chflags)

Set the flags of *path* to the numeric *flags*. *flags* may take a combination
(bitwise OR) of the following values (as defined in the [`stat`](stat.html#module-stat) module):

- 
[`stat.UF_NODUMP`](stat.html#stat.UF_NODUMP)

- 
[`stat.UF_IMMUTABLE`](stat.html#stat.UF_IMMUTABLE)

- 
[`stat.UF_APPEND`](stat.html#stat.UF_APPEND)

- 
[`stat.UF_OPAQUE`](stat.html#stat.UF_OPAQUE)

- 
[`stat.UF_NOUNLINK`](stat.html#stat.UF_NOUNLINK)

- 
[`stat.UF_COMPRESSED`](stat.html#stat.UF_COMPRESSED)

- 
[`stat.UF_HIDDEN`](stat.html#stat.UF_HIDDEN)

- 
[`stat.SF_ARCHIVED`](stat.html#stat.SF_ARCHIVED)

- 
[`stat.SF_IMMUTABLE`](stat.html#stat.SF_IMMUTABLE)

- 
[`stat.SF_APPEND`](stat.html#stat.SF_APPEND)

- 
[`stat.SF_NOUNLINK`](stat.html#stat.SF_NOUNLINK)

- 
[`stat.SF_SNAPSHOT`](stat.html#stat.SF_SNAPSHOT)
...
os.chmod(path, mode, *, dir_fd=None, follow_symlinks=True)[Â¶](#os.chmod)

Change the mode of *path* to the numeric *mode*. *mode* may take one of the
following values (as defined in the [`stat`](stat.html#module-stat) module) or bitwise ORed
combinations of them:

- 
[`stat.S_ISUID`](stat.html#stat.S_ISUID)

- 
[`stat.S_ISGID`](stat.html#stat.S_ISGID)

- 
[`stat.S_ENFMT`](stat.html#stat.S_ENFMT)

- 
[`stat.S_ISVTX`](stat.html#stat.S_ISVTX)

- 
[`stat.S_IREAD`](stat.html#stat.S_IREAD)

- 
[`stat.S_IWRITE`](stat.html#stat.S_IWRITE)

- 
[`stat.S_IEXEC`](stat.html#stat.S_IEXEC)

- 
[`stat.S_IRWXU`](stat.html#stat.S_IRWXU)

- 
[`stat.S_IRUSR`](stat.html#stat.S_IRUSR)

- 
[`stat.S_IWUSR`](stat.html#stat.S_IWUSR)

- 
[`stat.S_IXUSR`](stat.html#stat.S_IXUSR)

- 
[`stat.S_IRWXG`](stat.html#stat.S_IRWXG)

- 
[`stat.S_IRGRP`](stat.html#stat.S_IRGRP)

- 
[`stat.S_IWGRP`](stat.html#stat.S_IWGRP)

- 
[`stat.S_IXGRP`](stat.html#stat.S_IXGRP)

- 
[`stat.S_IRWXO`](stat.html#stat.S_IRWXO)

- 
[`stat.S_IROTH`](stat.html#stat.S_IROTH)

- 
[`stat.S_IWOTH`](stat.html#stat.S_IWOTH)

- 
[`stat.S_IXOTH`](stat.html#stat.S_IXOTH)
...
Although Windows supports [`chmod()`](#os.chmod), you can only set the fileâs
...
os.chown(path, uid, gid, *, dir_fd=None, follow_symlinks=True)[Â¶](#os.chown)
...
See [`shutil.chown()`](shutil.html#shutil.chown) for a higher-level function that accepts names in
...
os.getcwd()[Â¶](#os.getcwd)

Return a string representing the current working directory.

os.getcwdb()[Â¶](#os.getcwdb)
...
os.lchflags(path, flags)[Â¶](#os.lchflags)
...
os.lchmod(path, mode)[Â¶](#os.lchmod)
...
os.lchown(path, uid, gid)[Â¶](#os.lchown)
...
os.link(src, dst, *, src_dir_fd=None, dst_dir_fd=None, follow_symlinks=True)[Â¶](#os.link)
...
May raise [`OSError`](exceptions.html#OSError) if an error occurs collecting the drive
names.
...
os.listmounts(volume)[Â¶](#os.listmounts)
...
Raises [`OSError`](exceptions.html#OSError) if the volume is not recognized or if an error
...
os.listvolumes()[Â¶](#os.listvolumes)
...
May raise [`OSError`](exceptions.html#OSError) if an error occurs collecting the volumes.
...
os.lstat(path, *, dir_fd=None)[Â¶](#os.lstat)
...
os.mkdir(path, mode=0o777, *, dir_fd=None)[Â¶](#os.mkdir)

Create a directory named *path* with numeric mode *mode*.

If the directory already exists, [`FileExistsError`](exceptions.html#FileExistsError) is raised. If a parent
directory in the path does not exist, [`FileNotFoundError`](exceptions.html#FileNotFoundError) is raised.
...
[`tempfile`](tempfile.html#module-tempfile) moduleâs [`tempfile.mkdtemp()`](tempfile.html#tempfile.mkdtemp) function.
...
os.makedirs(name, mode=0o777, exist_ok=False)[Â¶](#os.makedirs)
...
If *exist_ok* is `False` (the default), a [`FileExistsError`](exceptions.html#FileExistsError) is
raised if the target directory already exists.

Note

[`makedirs()`](#os.makedirs) will become confused if the path elements to create
include [`pardir`](#os.pardir) (eg. â..â on UNIX systems).
...
os.mkfifo(path, mode=0o666, *, dir_fd=None)[Â¶](#os.mkfifo)
...
doesnât open the FIFO â it just creates the rendezvous point.
...
os.mknod(path, mode=0o600, device=0, *, dir_fd=None)[Â¶](#os.mknod)

Create a filesystem node (file, device special file or named pipe) named
*path*. *mode* specifies both the permissions to use and the type of node
to be created, being combined (bitwise OR) with one of `stat.S_IFREG`,
`stat.S_IFCHR`, `stat.S_IFBLK`, and `stat.S_IFIFO` (those constants are
available in [`stat`](stat.html#module-stat)).  For `stat.S_IFCHR` and `stat.S_IFBLK`,
...
os.major(device, /)[Â¶](#os.major)

Extract the device major number from a raw device number (usually the
`st_dev` or `st_rdev` field from `stat`).

os.minor(device, /)[Â¶](#os.minor)

Extract the device minor number from a raw device number (usually the
`st_dev` or `st_rdev` field from `stat`).

os.makedev(major, minor, /)[Â¶](#os.makedev)

Compose a raw device number from the major and minor device numbers.

os.pathconf(path, name)[Â¶](#os.pathconf)
...
os.pathconf_names[Â¶](#os.pathconf_names)
...
os.readlink(path, *, dir_fd=None)[Â¶](#os.readlink)
...
[`realpath()`](os.path.html#os.path.realpath) to properly handle recursion and platform
differences.
...
than the optional âprint nameâ field that was previously returned.

os.remove(path, *, dir_fd=None)[Â¶](#os.remove)

Remove (delete) the file *path*.  If *path* is a directory, an
[`OSError`](exceptions.html#OSError) is raised.  Use [`rmdir()`](#os.rmdir) to remove directories.
If the file does not exist, a [`FileNotFoundError`](exceptions.html#FileNotFoundError) is raised.
...
os.removedirs(name)[Â¶](#os.removedirs)
...
they are empty. Raises [`OSError`](exceptions.html#OSError) if the leaf directory could not be
...
os.rename(src, dst, *, src_dir_fd=None, dst_dir_fd=None)[Â¶](#os.rename)

Rename the file or directory *src* to *dst*. If *dst* exists, the operation
will fail with an [`OSError`](exceptions.html#OSError) subclass in a number of cases:

On Windows, if *dst* exists a [`FileExistsError`](exceptions.html#FileExistsError) is always raised.
The operation may fail if *src* and *dst* are on different filesystems. Use
[`shutil.move()`](shutil.html#shutil.move) to support moves to a different filesystem.

On Unix, if *src* is a file and *dst* is a directory or vice-versa, an
[`IsADirectoryError`](exceptions.html#IsADirectoryError) or a [`NotADirectoryError`](exceptions.html#NotADirectoryError) will be raised
respectively.  If both are directories and *dst* is empty, *dst* will be
silently replaced.  If *dst* is a non-empty directory, an [`OSError`](exceptions.html#OSError)
...
os.renames(old, new)[Â¶](#os.renames)
...
os.replace(src, dst, *, src_dir_fd=None, dst_dir_fd=None)[Â¶](#os.replace)

Rename the file or directory *src* to *dst*.  If *dst* is a non-empty directory,
[`OSError`](exceptions.html#OSError) will be raised.  If *dst* exists and is a file, it will
...
os.rmdir(path, *, dir_fd=None)[Â¶](#os.rmdir)

Remove (delete) the directory *path*.  If the directory does not exist or is
not empty, a [`FileNotFoundError`](exceptions.html#FileNotFoundError) or an [`OSError`](exceptions.html#OSError) is raised
respectively.  In order to remove whole directory trees,
[`shutil.rmtree()`](shutil.html#shutil.rmtree) can be used.
...
The [`scandir()`](#os.scandir) iterator supports the [context manager](../glossary.html#term-context-manager) protocol
and has the following method:

scandir.close()[Â¶](#os.scandir.close)
...
the files (excluding directories) in the given *path* that donât start with
...
On Unix-based systems, [`scandir()`](#os.scandir) uses the systemâs
...
exhausted nor explicitly closed a [`ResourceWarning`](exceptions.html#ResourceWarning) will be emitted
in its destructor.
...
class os.DirEntry[Â¶](#os.DirEntry)
...
also raise [`OSError`](exceptions.html#OSError). If you need very fine-grained
control over errors, you can catch [`OSError`](exceptions.html#OSError) when calling one of the
...
name[Â¶](#os.DirEntry.name)

The entryâs base filename, relative to the [`scandir()`](#os.scandir) *path*
argument.
...
path[Â¶](#os.DirEntry.path)

The entryâs full path name: equivalent to os.path.join(scandir_path,
...
argument was a [file descriptor](#path-fd), the [`path`](os.path.html#module-os.path)
attribute is the same as the [`name`](#os.name) attribute.

The [`path`](os.path.html#module-os.path) attribute will be `bytes` if the [`scandir()`](#os.scandir)
*path* argument is of type `bytes` and `str` otherwise.  Use
[`fsdecode()`](#os.fsdecode) to decode byte filenames.

inode()[Â¶](#os.DirEntry.inode)
...
is_dir(*, follow_symlinks=True)[Â¶](#os.DirEntry.is_dir)

Return `True` if this entry is a directory or a symbolic link pointing
to a directory; return `False` if the entry is or points to any other
kind of file, or if it doesnât exist anymore.

If *follow_symlinks* is `False`, return `True` only if this entry
is a directory (without following symlinks); return `False` if the
entry is any other kind of file or if it doesnât exist anymore.
...
with [`stat.S_ISDIR()`](stat.html#stat.S_ISDIR) to fetch up-to-date information.
...
This method can raise [`OSError`](exceptions.html#OSError), such as [`PermissionError`](exceptions.html#PermissionError),
but [`FileNotFoundError`](exceptions.html#FileNotFoundError) is caught and not raised.

is_file(*, follow_symlinks=True)[Â¶](#os.DirEntry.is_file)

Return `True` if this entry is a file or a symbolic link pointing to a
file; return `False` if the entry is or points to a directory or other
non-file entry, or if it doesnât exist anymore.

If *follow_symlinks* is `False`, return `True` only if this entry
is a file (without following symlinks); return `False` if the entry is
a directory or other non-file entry, or if it doesnât exist anymore.
...
is_symlink()[Â¶](#os.DirEntry.is_symlink)

Return `True` if this entry is a symbolic link (even if broken);
return `False` if the entry points to a directory or any kind of file,
or if it doesnât exist anymore.

The result is cached on the `os.DirEntry` object. Call
[`os.path.islink()`](os.path.html#os.path.islink) to fetch up-to-date information.
...
is_junction()[Â¶](#os.DirEntry.is_junction)

Return `True` if this entry is a junction (even if broken);
return `False` if the entry points to a regular directory, any kind
of file, a symlink, or if it doesnât exist anymore.

The result is cached on the `os.DirEntry` object. Call
[`os.path.isjunction()`](os.path.html#os.path.isjunction) to fetch up-to-date information.

Added in version 3.12.

stat(*, follow_symlinks=True)[Â¶](#os.DirEntry.stat)
...
and methods of `os.DirEntry` and of [`pathlib.Path`](pathlib.html#pathlib.Path).  In
...
for [`bytes`](stdtypes.html#bytes) paths on Windows.
...
os.stat(path, *, dir_fd=None, follow_symlinks=True)[Â¶](#os.stat)

Get the status of a file or a file descriptor. Perform the equivalent of a
`stat()` system call on the given path. *path* may be specified as
either a string or bytes â directly or indirectly through the [`PathLike`](#os.PathLike)
interface â or as an open file descriptor. Return a [`stat_result`](#os.stat_result)
object.
...
[`os.path.realpath()`](os.path.html#os.path.realpath) function to resolve the path name as far as
...
class os.stat_result[Â¶](#os.stat_result)
...
st_mode[Â¶](#os.stat_result.st_mode)

File mode: file type and file mode bits (permissions).

st_ino[Â¶](#os.stat_result.st_ino)
...
st_dev[Â¶](#os.stat_result.st_dev)

Identifier of the device on which this file resides.

st_nlink[Â¶](#os.stat_result.st_nlink)

Number of hard links.

st_uid[Â¶](#os.stat_result.st_uid)

User identifier of the file owner.

st_gid[Â¶](#os.stat_result.st_gid)

Group identifier of the file owner.

st_size[Â¶](#os.stat_result.st_size)
...
st_atime[Â¶](#os.stat_result.st_atime)

Time of most recent access expressed in seconds.

st_mtime[Â¶](#os.stat_result.st_mtime)

Time of most recent content modification expressed in seconds.

st_ctime[Â¶](#os.stat_result.st_ctime)
...
st_atime_ns[Â¶](#os.stat_result.st_atime_ns)

Time of most recent access expressed in nanoseconds as an integer.

Added in version 3.3.

st_mtime_ns[Â¶](#os.stat_result.st_mtime_ns)
...
st_ctime_ns[Â¶](#os.stat_result.st_ctime_ns)
...
st_blocks[Â¶](#os.stat_result.st_blocks)

Number of 512-byte blocks allocated for file.
This may be smaller than [`st_size`](#os.stat_result.st_size)/512 when the file has holes.

st_blksize[Â¶](#os.stat_result.st_blksize)

âPreferredâ blocksize for efficient file system I/O. Writing to a file in
smaller chunks may cause an inefficient read-modify-rewrite.

st_rdev[Â¶](#os.stat_result.st_rdev)

Type of device if an inode device.

st_flags[Â¶](#os.stat_result.st_flags)
...
st_gen[Â¶](#os.stat_result.st_gen)
...
st_fstype[Â¶](#os.stat_result.st_fstype)
...
st_rsize[Â¶](#os.stat_result.st_rsize)

Real size of the file.

st_creator[Â¶](#os.stat_result.st_creator)

Creator of the file.

st_type[Â¶](#os.stat_result.st_type)

File type.

On Windows systems, the following attributes are also available:

st_file_attributes[Â¶](#os.stat_result.st_file_attributes)

Windows file attributes: `dwFileAttributes` member of the
`BY_HANDLE_FILE_INFORMATION` structure returned by
`GetFileInformationByHandle()`.
See the `FILE_ATTRIBUTE_* <stat.FILE_ATTRIBUTE_ARCHIVE>`
constants in the [`stat`](stat.html#module-stat) module.

Added in version 3.5.

st_reparse_tag[Â¶](#os.stat_result.st_reparse_tag)

When [`st_file_attributes`](#os.stat_result.st_file_attributes) has the [`FILE_ATTRIBUTE_REPARSE_POINT`](stat.html#stat.FILE_ATTRIBUTE_REPARSE_POINT)
set, this field contains the tag identifying the type of reparse point.
See the [`IO_REPARSE_TAG_*`](stat.html#stat.IO_REPARSE_TAG_SYMLINK)
constants in the [`stat`](stat.html#module-stat) module.

The standard module [`stat`](stat.html#module-stat) defines functions and constants that are
...
Two module-level constants are defined for the `f_flag` attributeâs
...
os.supports_dir_fd[Â¶](#os.supports_dir_fd)

A [`set`](stdtypes.html#set) object indicating which functions in the `os`
...
when itâs not locally available. (Specifying `None` for *dir_fd*
...
os.supports_effective_ids[Â¶](#os.supports_effective_ids)

A [`set`](stdtypes.html#set) object indicating whether [`os.access()`](#os.access) permits
...
os.supports_fd[Â¶](#os.supports_fd)

A [`set`](stdtypes.html#set) object indicating which functions in the
...
os.supports_follow_symlinks[Â¶](#os.supports_follow_symlinks)

A [`set`](stdtypes.html#set) object indicating which functions in the `os` module
...
os.symlink(src, dst, target_is_directory=False, *, dir_fd=None)[Â¶](#os.symlink)
...
[`OSError`](exceptions.html#OSError) is raised when the function is called by an unprivileged
user.
...
os.truncate(path, length)[Â¶](#os.truncate)
...
os.unlink(path, *, dir_fd=None)[Â¶](#os.unlink)
...
os.utime(path, times=None, *, [ns, ]dir_fd=None, follow_symlinks=True)[Â¶](#os.utime)
...
os.walk(top, topdown=True, onerror=None, followlinks=False)[Â¶](#os.walk)
...
(perhaps using [`del`](../reference/simple_stmts.html#del) or slice assignment), and [`walk()`](#os.walk) will only
...
one argument, an [`OSError`](exceptions.html#OSError) instance.  It can report the error to continue
...
If you pass a relative pathname, donât change the current working directory
between resumptions of [`walk()`](#os.walk).  [`walk()`](#os.walk) never changes the current
directory, and assumes that its caller doesnât either.

This example displays the number of bytes taken by non-directory files in each
directory under the starting directory, except that it doesnât look under any
...
In the next example (simple implementation of [`shutil.rmtree()`](shutil.html#shutil.rmtree)),
walking the tree bottom-up is essential, [`rmdir()`](#os.rmdir) doesnât allow
...
os.fwalk(top='.', topdown=True, onerror=None, *, follow_symlinks=False, dir_fd=None)[Â¶](#os.fwalk)
...
[`rmdir()`](#os.rmdir) doesnât allow deleting a directory before the directory is
empty:
...
Changed in version 3.7: Added support for [`bytes`](stdtypes.html#bytes) paths.

os.memfd_create(name[, flags=os.MFD_CLOEXEC])[Â¶](#os.memfd_create)
...
os.MFD_CLOEXEC[Â¶](#os.MFD_CLOEXEC)

os.MFD_ALLOW_SEALING[Â¶](#os.MFD_ALLOW_SEALING)

os.MFD_HUGETLB[Â¶](#os.MFD_HUGETLB)

os.MFD_HUGE_SHIFT[Â¶](#os.MFD_HUGE_SHIFT)

os.MFD_HUGE_MASK[Â¶](#os.MFD_HUGE_MASK)

os.MFD_HUGE_64KB[Â¶](#os.MFD_HUGE_64KB)

os.MFD_HUGE_512KB[Â¶](#os.MFD_HUGE_512KB)

os.MFD_HUGE_1MB[Â¶](#os.MFD_HUGE_1MB)

os.MFD_HUGE_2MB[Â¶](#os.MFD_HUGE_2MB)

os.MFD_HUGE_8MB[Â¶](#os.MFD_HUGE_8MB)

os.MFD_HUGE_16MB[Â¶](#os.MFD_HUGE_16MB)

os.MFD_HUGE_32MB[Â¶](#os.MFD_HUGE_32MB)

os.MFD_HUGE_256MB[Â¶](#os.MFD_HUGE_256MB)

os.MFD_HUGE_512MB[Â¶](#os.MFD_HUGE_512MB)

os.MFD_HUGE_1GB[Â¶](#os.MFD_HUGE_1GB)

os.MFD_HUGE_2GB[Â¶](#os.MFD_HUGE_2GB)

os.MFD_HUGE_16GB[Â¶](#os.MFD_HUGE_16GB)
...
os.eventfd(initval[, flags=os.EFD_CLOEXEC])[Â¶](#os.eventfd)
...
[`select()`](select.html#select.select), [`poll()`](select.html#select.poll) and similar. See man page
...
os.eventfd_read(fd)[Â¶](#os.eventfd_read)
...
os.eventfd_write(fd, value)[Â¶](#os.eventfd_write)
...
os.EFD_CLOEXEC[Â¶](#os.EFD_CLOEXEC)
...
os.EFD_NONBLOCK[Â¶](#os.EFD_NONBLOCK)
...
os.EFD_SEMAPHORE[Â¶](#os.EFD_SEMAPHORE)
...
### Timer File Descriptors[Â¶](#timer-file-descriptors)

Added in version 3.13.

These functions provide support for Linuxâs *timer file descriptor* API.
Naturally, they are all only available on Linux.

os.timerfd_create(clockid, /, *, flags=0)[Â¶](#os.timerfd_create)
...
- 
[`poll()`](select.html#select.poll)

The file descriptorâs [`read()`](#os.read) method can be called with a buffer size
of 8. If the timer has already expired one or more times, [`read()`](#os.read)
returns the number of expirations with the hostâs endianness, which may be
converted to an [`int`](functions.html#int) by `int.from_bytes(x, byteorder=sys.byteorder)`.

[`select()`](select.html#select.select) and [`poll()`](select.html#select.poll) can be used to wait until
timer expires and the file descriptor is readable.

*clockid* must be a valid [clock ID](time.html#time-clock-id-constants),
as defined in the [`time`](time.html#module-time) module:

- 
[`time.CLOCK_REALTIME`](time.html#time.CLOCK_REALTIME)

- 
[`time.CLOCK_MONOTONIC`](time.html#time.CLOCK_MONOTONIC)

- 
[`time.CLOCK_BOOTTIME`](time.html#time.CLOCK_BOOTTIME) (Since Linux 3.15 for timerfd_create)

If *clockid* is [`time.CLOCK_REALTIME`](time.html#time.CLOCK_REALTIME), a settable system-wide
...
If *clockid* is [`time.CLOCK_MONOTONIC`](time.html#time.CLOCK_MONOTONIC), a non-settable monotonically
increasing clock is used. Even if the system clock is changed, the timer
setting will not be affected.

If *clockid* is [`time.CLOCK_BOOTTIME`](time.html#time.CLOCK_BOOTTIME), same as [`time.CLOCK_MONOTONIC`](time.html#time.CLOCK_MONOTONIC)
except it includes any time that the system is suspended.

The file descriptorâs behaviour can be modified by specifying a *flags* value.
Any of the following variables may used, combined using bitwise OR
(the `|` operator):
...
the timer expires. If it is set as a flag, [`read()`](#os.read) doesnât block, but
If there hasnât been an expiration since the last call to read,
[`read()`](#os.read) raises [`OSError`](exceptions.html#OSError) with `errno` is set to
[`errno.EAGAIN`](errno.html#errno.EAGAIN).
...
The [timerfd_create(2)](https://manpages.debian.org/timerfd_create(2)) man page.

[Availability](intro.html#availability): Linux >= 2.6.27 with glibc >= 2.8

Added in version 3.13.

os.timerfd_settime(fd, /, *, flags=flags, initial=0.0, interval=0.0)[Â¶](#os.timerfd_settime)

Alter a timer file descriptorâs internal timer.
...
The timerâs behaviour can be modified by specifying a *flags* value.
Any of the following variables may used, combined using bitwise OR
(the `|` operator):
...
If *initial* is less than zero, it raises an [`OSError`](exceptions.html#OSError) exception
with `errno` set to [`errno.EINVAL`](errno.html#errno.EINVAL)
...
The timerâs interval is set by the *interval* [`float`](functions.html#float).
...
If *interval* is less than zero, it raises [`OSError`](exceptions.html#OSError) with `errno`
set to [`errno.EINVAL`](errno.html#errno.EINVAL)
...
[`time.CLOCK_REALTIME`](time.html#time.CLOCK_REALTIME), the timer is marked as cancelable if the
...
[timerfd_create(2)](https://manpages.debian.org/timerfd_create(2)), [timerfd_settime(2)](https://manpages.debian.org/timerfd_settime(2)),
[settimeofday(2)](https://manpages.debian.org/settimeofday(2)), [clock_settime(2)](https://manpages.debian.org/clock_settime(2)),
...
os.timerfd_settime_ns(fd, /, *, flags=0, initial=0, interval=0)[Â¶](#os.timerfd_settime_ns)
...
os.timerfd_gettime(fd, /)[Â¶](#os.timerfd_gettime)
...
`interval` denotes the timerâs interval.
...
os.timerfd_gettime_ns(fd, /)[Â¶](#os.timerfd_gettime_ns)
...
os.TFD_NONBLOCK[Â¶](#os.TFD_NONBLOCK)
...
os.TFD_CLOEXEC[Â¶](#os.TFD_CLOEXEC)
...
os.TFD_TIMER_ABSTIME[Â¶](#os.TFD_TIMER_ABSTIME)
...
timerâs clock (in UTC seconds or nanoseconds since the Unix Epoch).

[Availability](intro.html#availability): Linux >= 2.6.27 with glibc >= 2.8

Added in version 3.13.

os.TFD_TIMER_CANCEL_ON_SET[Â¶](#os.TFD_TIMER_CANCEL_ON_SET)
...
### Linux extended attributes[Â¶](#linux-extended-attributes)

Added in version 3.3.

These functions are all available on Linux only.

os.getxattr(path, attribute, *, follow_symlinks=True)[Â¶](#os.getxattr)
...
os.listxattr(path=None, *, follow_symlinks=True)[Â¶](#os.listxattr)
...
os.removexattr(path, attribute, *, follow_symlinks=True)[Â¶](#os.removexattr)
...
os.setxattr(path, attribute, value, flags=0, *, follow_symlinks=True)[Â¶](#os.setxattr)
...
os.XATTR_SIZE_MAX[Â¶](#os.XATTR_SIZE_MAX)

The maximum size the value of an extended attribute can be. Currently, this
is 64 KiB on Linux.

os.XATTR_CREATE[Â¶](#os.XATTR_CREATE)

This is a possible value for the flags argument in [`setxattr()`](#os.setxattr). It
indicates the operation must create an attribute.

os.XATTR_REPLACE[Â¶](#os.XATTR_REPLACE)

This is a possible value for the flags argument in [`setxattr()`](#os.setxattr). It
indicates the operation must replace an existing attribute.

## Process Management[Â¶](#process-management)
...
passed to a programâs `main()`.  For example, os.execv('/bin/echo',
...
[`signal.signal()`](signal.html#signal.signal).

os.add_dll_directory(path)[Â¶](#os.add_dll_directory)

Add a path to the DLL search path.

This search path is used when resolving dependencies for imported
extension modules (the module itself is resolved through
[`sys.path`](sys.html#sys.path)), and also by [`ctypes`](ctypes.html#module-ctypes).
...
os.execl(path, arg0, arg1, ...)[Â¶](#os.execl)

os.execle(path, arg0, arg1, ..., env)[Â¶](#os.execle)

os.execlp(file, arg0, arg1, ...)[Â¶](#os.execlp)

os.execlpe(file, arg0, arg1, ..., env)[Â¶](#os.execlpe)

os.execv(path, args)[Â¶](#os.execv)

os.execve(path, args, env)[Â¶](#os.execve)

os.execvp(file, args)[Â¶](#os.execvp)

os.execvpe(file, args, env)[Â¶](#os.execvpe)

These functions all execute a new program, replacing the current process; they
do not return.  On Unix, the new executable is loaded into the current process,
and will have the same process id as the caller.  Errors will be reported as
[`OSError`](exceptions.html#OSError) exceptions.

The current process is replaced immediately. Open file objects and
descriptors are not flushed, so if there may be data buffered
on these open files, you should flush them using
`sys.stdout.flush()` or [`os.fsync()`](#os.fsync) before calling an
[`exec*`](#os.execl) function.

The âlâ and âvâ variants of the [`exec*`](#os.execl) functions differ in how
...
functions.  The âvâ variants are good when the number of parameters is
variable, with the arguments being passed in a list or tuple as the *args*
parameter.  In either case, the arguments to the child process should start with
the name of the command being run, but this is not enforced.

The variants which include a âpâ near the end ([`execlp()`](#os.execlp),
...
instead of the current processâ environment); the functions [`execl()`](#os.execl),
...
If it is unavailable, using it will raise a [`NotImplementedError`](exceptions.html#NotImplementedError).
...
The standard way to exit is [`sys.exit(n)`](sys.html#sys.exit).  `_exit()` should
...
written in Python, such as a mail serverâs external command delivery program.
...
os.EX_DATAERR[Â¶](#os.EX_DATAERR)

Exit code that means the input data was incorrect.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_NOINPUT[Â¶](#os.EX_NOINPUT)

Exit code that means an input file did not exist or was not readable.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_NOUSER[Â¶](#os.EX_NOUSER)

Exit code that means a specified user did not exist.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_NOHOST[Â¶](#os.EX_NOHOST)

Exit code that means a specified host did not exist.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_UNAVAILABLE[Â¶](#os.EX_UNAVAILABLE)

Exit code that means that a required service is unavailable.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_SOFTWARE[Â¶](#os.EX_SOFTWARE)

Exit code that means an internal software error was detected.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_OSERR[Â¶](#os.EX_OSERR)
...
os.EX_CANTCREAT[Â¶](#os.EX_CANTCREAT)

Exit code that means a user specified output file could not be created.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_IOERR[Â¶](#os.EX_IOERR)

Exit code that means that an error occurred while doing I/O on some file.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_TEMPFAIL[Â¶](#os.EX_TEMPFAIL)
...
os.EX_PROTOCOL[Â¶](#os.EX_PROTOCOL)
...
os.EX_CONFIG[Â¶](#os.EX_CONFIG)

Exit code that means that some kind of configuration error occurred.

[Availability](intro.html#availability): Unix, not WASI.

os.EX_NOTFOUND[Â¶](#os.EX_NOTFOUND)

Exit code that means something like âan entry was not foundâ.
...
parent.  If an error occurs [`OSError`](exceptions.html#OSError) is raised.
...
the warning in the [`ssl`](ssl.html#module-ssl) documentation.

Warning

On macOS the use of this function is unsafe when mixed with using
higher-level system APIs, and that includes using [`urllib.request`](urllib.request.html#module-urllib.request).

Changed in version 3.8: Calling `fork()` in a subinterpreter is no longer supported
([`RuntimeError`](exceptions.html#RuntimeError) is raised).

Changed in version 3.12: If Python is able to detect that your process has multiple
threads, [`os.fork()`](#os.fork) now raises a [`DeprecationWarning`](exceptions.html#DeprecationWarning).
...
for technical details of why weâre surfacing this longstanding
platform compatibility problem to developers.

[Availability](intro.html#availability): POSIX, not WASI, not Android, not iOS.

os.forkpty()[Â¶](#os.forkpty)

Fork a child process, using a new pseudo-terminal as the childâs controlling
terminal. Return a pair of `(pid, fd)`, where *pid* is `0` in the child, the
new childâs process id in the parent, and *fd* is the file descriptor of the
master end of the pseudo-terminal.  For a more portable approach, use the
[`pty`](pty.html#module-pty) module.  If an error occurs [`OSError`](exceptions.html#OSError) is raised.
...
Changed in version 3.12: If Python is able to detect that your process has multiple
threads, this now raises a [`DeprecationWarning`](exceptions.html#DeprecationWarning). See the
longer explanation on [`os.fork()`](#os.fork).

[Availability](intro.html#availability): Unix, not WASI, not Android, not iOS.

os.kill(pid, sig, /)[Â¶](#os.kill)

Send signal *sig* to the process *pid*.  Constants for the specific signals
available on the host platform are defined in the [`signal`](signal.html#module-signal) module.

Windows: The [`signal.CTRL_C_EVENT`](signal.html#signal.CTRL_C_EVENT) and
[`signal.CTRL_BREAK_EVENT`](signal.html#signal.CTRL_BREAK_EVENT) signals are special signals which can
only be sent to console processes which share a common console window,
e.g., some subprocesses. Any other value for *sig* will cause the process
to be unconditionally killed by the TerminateProcess API, and the exit code
will be set to *sig*.

See also [`signal.pthread_kill()`](signal.html#signal.pthread_kill).
...
os.killpg(pgid, sig, /)[Â¶](#os.killpg)
...
os.nice(increment, /)[Â¶](#os.nice)

Add *increment* to the processâs ânicenessâ.  Return the new niceness.

[Availability](intro.html#availability): Unix, not WASI.

os.pidfd_open(pid, flags=0)[Â¶](#os.pidfd_open)
...
[Availability](intro.html#availability): Linux >= 5.3, Android >= [`build-time`](sys.html#sys.getandroidapilevel) API level 31

Added in version 3.9.

os.PIDFD_NONBLOCK[Â¶](#os.PIDFD_NONBLOCK)
...
will immediately return the error [`EAGAIN`](errno.html#errno.EAGAIN) rather than blocking.

[Availability](intro.html#availability): Linux >= 5.10

Added in version 3.12.

os.plock(op, /)[Â¶](#os.plock)
...
os.popen(cmd, mode='r', buffering=-1)[Â¶](#os.popen)
...
the corresponding argument to the built-in [`open()`](functions.html#open) function. The
returned file object reads or writes text strings rather than bytes.

The `close` method returns [`None`](constants.html#None) if the subprocess exited
...
This is implemented using [`subprocess.Popen`](subprocess.html#subprocess.Popen); see that classâs
...
[`popen()`](#os.popen) is a simple wrapper around [`subprocess.Popen`](subprocess.html#subprocess.Popen).
Use [`subprocess.Popen`](subprocess.html#subprocess.Popen) or [`subprocess.run()`](subprocess.html#subprocess.run) to
...
write new code. The [`subprocess`](subprocess.html#module-subprocess) module is recommended instead.

os.posix_spawn(path, argv, env, *, file_actions=None, setpgroup=None, resetids=False, setsid=False, setsigmask=(), setsigdef=(), scheduler=None)[Â¶](#os.posix_spawn)

Wraps the `posix_spawn()` C library API for use from Python.

Most users should use [`subprocess.run()`](subprocess.html#subprocess.run) instead of [`posix_spawn()`](#os.posix_spawn).

The positional-only arguments *path*, *args*, and *env* are similar to
[`execve()`](#os.execve). *env* is allowed to be `None`, in which case current
processâ environment is used.
...
os.POSIX_SPAWN_OPEN[Â¶](#os.POSIX_SPAWN_OPEN)

(`os.POSIX_SPAWN_OPEN`, *fd*, *path*, *flags*, *mode*)

Performs `os.dup2(os.open(path, flags, mode), fd)`.

os.POSIX_SPAWN_CLOSE[Â¶](#os.POSIX_SPAWN_CLOSE)

(`os.POSIX_SPAWN_CLOSE`, *fd*)

Performs `os.close(fd)`.

os.POSIX_SPAWN_DUP2[Â¶](#os.POSIX_SPAWN_DUP2)

(`os.POSIX_SPAWN_DUP2`, *fd*, *new_fd*)

Performs `os.dup2(fd, new_fd)`.

os.POSIX_SPAWN_CLOSEFROM[Â¶](#os.POSIX_SPAWN_CLOSEFROM)
...
or `POSIX_SPAWN_SETSID_NP` flag. Otherwise, [`NotImplementedError`](exceptions.html#NotImplementedError)
is raised.
...
parentâs signal mask. This argument corresponds to the C library
...
os.posix_spawnp(path, argv, env, *, file_actions=None, setpgroup=None, resetids=False, setsid=False, setsigmask=(), setsigdef=(), scheduler=None)[Â¶](#os.posix_spawnp)
...
os.register_at_fork(*, before=None, after_in_parent=None, after_in_child=None)[Â¶](#os.register_at_fork)
...
Python interpreter.  A typical [`subprocess`](subprocess.html#module-subprocess) launch will not
...
call those functions, unless it explicitly calls [`PyOS_BeforeFork()`](../c-api/sys.html#c.PyOS_BeforeFork),
[`PyOS_AfterFork_Parent()`](../c-api/sys.html#c.PyOS_AfterFork_Parent) and [`PyOS_AfterFork_Child()`](../c-api/sys.html#c.PyOS_AfterFork_Child).
...
os.spawnl(mode, path, ...)[Â¶](#os.spawnl)

os.spawnle(mode, path, ..., env)[Â¶](#os.spawnle)

os.spawnlp(mode, file, ...)[Â¶](#os.spawnlp)

os.spawnlpe(mode, file, ..., env)[Â¶](#os.spawnlpe)

os.spawnv(mode, path, args)[Â¶](#os.spawnv)

os.spawnve(mode, path, args, env)[Â¶](#os.spawnve)

os.spawnvp(mode, file, args)[Â¶](#os.spawnvp)

os.spawnvpe(mode, file, args, env)[Â¶](#os.spawnvpe)

Execute the program *path* in a new process.

(Note that the [`subprocess`](subprocess.html#module-subprocess) module provides more powerful facilities for
...
process; if *mode* is [`P_WAIT`](#os.P_WAIT), returns the processâs exit code if it
exits normally, or `-signal`, where *signal* is the signal that killed the
process.  On Windows, the process id will actually be the process handle, so can
be used with the [`waitpid()`](#os.waitpid) function.

Note on VxWorks, this function doesnât return `-signal` when the new process is
killed. Instead it raises OSError exception.

The âlâ and âvâ variants of the [`spawn*`](#os.spawnl) functions differ in how
command-line arguments are passed.  The âlâ variants are perhaps the easiest
to work with if the number of parameters is fixed when the code is written; the
individual parameters simply become additional parameters to the
`spawnl*()` functions.  The âvâ variants are good when the number of
parameters is variable, with the arguments being passed in a list or tuple as
the *args* parameter.  In either case, the arguments to the child process must
start with the name of the command being run.

The variants which include a second âpâ near the end ([`spawnlp()`](#os.spawnlp),
...
(note that these all end in âeâ), the *env* parameter must be a mapping
which is used to define the environment variables for the new process (they are
used instead of the current processâ environment); the functions
...
[`subprocess`](subprocess.html#module-subprocess) module instead.
...
to write new code. The [`subprocess`](subprocess.html#module-subprocess) module is recommended instead.

os.P_NOWAIT[Â¶](#os.P_NOWAIT)

os.P_NOWAITO[Â¶](#os.P_NOWAITO)
...
os.P_DETACH[Â¶](#os.P_DETACH)

os.P_OVERLAY[Â¶](#os.P_OVERLAY)
...
os.startfile(path[, operation][, arguments][, cwd][, show_cmd])[Â¶](#os.startfile)
...
When another *operation* is given, it must be a âcommand verbâ that specifies
...
character is not a slash (`'/'`)  Use [`pathlib`](pathlib.html#module-pathlib) or the
[`os.path.normpath()`](os.path.html#os.path.normpath) function to ensure that paths are properly encoded for
Win32.

To reduce interpreter startup overhead, the Win32 `ShellExecute()`
function is not resolved until this function is first called.  If the function
cannot be resolved, [`NotImplementedError`](exceptions.html#NotImplementedError) will be raised.
...
os.system(command)[Â¶](#os.system)

Execute the command (a string) in a subshell.  This is implemented by calling
the Standard C function `system()`, and has the same limitations.
Changes to [`sys.stdin`](sys.html#sys.stdin), etc. are not reflected in the environment of
...
The [`subprocess`](subprocess.html#module-subprocess) module provides more powerful facilities for spawning
...
the [`subprocess`](subprocess.html#module-subprocess) documentation for some helpful recipes.
...
If there are no children that could be waited for, [`ChildProcessError`](exceptions.html#ChildProcessError)
is raised.
...
os.waitid(idtype, id, options, /)[Â¶](#os.waitid)
...
- 
`si_signo` (always [`SIGCHLD`](signal.html#signal.SIGCHLD))
...
that could be waited for, [`ChildProcessError`](exceptions.html#ChildProcessError) is raised.
...
os.waitpid(pid, options, /)[Â¶](#os.waitpid)
...
for, [`ChildProcessError`](exceptions.html#ChildProcessError) is raised.  Other options that can be used are
...
[`resource.getrusage()`](resource.html#resource.getrusage) for details on resource usage information.  The
...
os.wait4(pid, options)[Â¶](#os.wait4)

Similar to [`waitpid()`](#os.waitpid), except a 3-element tuple, containing the childâs
process id, exit status indication, and resource usage information is
returned.  Refer to [`resource.getrusage()`](resource.html#resource.getrusage) for details on resource usage
...
os.P_PGID[Â¶](#os.P_PGID)

os.P_ALL[Â¶](#os.P_ALL)

os.P_PIDFD[Â¶](#os.P_PIDFD)
...
os.CLD_EXITED[Â¶](#os.CLD_EXITED)

os.CLD_KILLED[Â¶](#os.CLD_KILLED)

os.CLD_DUMPED[Â¶](#os.CLD_DUMPED)

os.CLD_TRAPPED[Â¶](#os.CLD_TRAPPED)

os.CLD_STOPPED[Â¶](#os.CLD_STOPPED)

os.CLD_CONTINUED[Â¶](#os.CLD_CONTINUED)
...
os.waitstatus_to_exitcode(status)[Â¶](#os.waitstatus_to_exitcode)
...
- 
Otherwise, raise a [`ValueError`](exceptions.html#ValueError).
...
os.WCOREDUMP(status, /)[Â¶](#os.WCOREDUMP)
...
os.WIFCONTINUED(status)[Â¶](#os.WIFCONTINUED)

Return `True` if a stopped child has been resumed by delivery of
[`SIGCONT`](signal.html#signal.SIGCONT) (if the process has been continued from a job
...
os.WIFSTOPPED(status)[Â¶](#os.WIFSTOPPED)
...
os.WIFSIGNALED(status)[Â¶](#os.WIFSIGNALED)
...
os.WIFEXITED(status)[Â¶](#os.WIFEXITED)
...
os.WEXITSTATUS(status)[Â¶](#os.WEXITSTATUS)
...
os.WSTOPSIG(status)[Â¶](#os.WSTOPSIG)
...
os.WTERMSIG(status)[Â¶](#os.WTERMSIG)
...
## Interface to the scheduler[Â¶](#interface-to-the-scheduler)
...
os.SCHED_OTHER[Â¶](#os.SCHED_OTHER)

The default scheduling policy.

os.SCHED_BATCH[Â¶](#os.SCHED_BATCH)

Scheduling policy for CPU-intensive processes that tries to preserve
interactivity on the rest of the computer.

os.SCHED_DEADLINE[Â¶](#os.SCHED_DEADLINE)

Scheduling policy for tasks with deadline constraints.

Added in version 3.14.

os.SCHED_IDLE[Â¶](#os.SCHED_IDLE)

Scheduling policy for extremely low priority background tasks.

os.SCHED_NORMAL[Â¶](#os.SCHED_NORMAL)

Alias for [`SCHED_OTHER`](#os.SCHED_OTHER).

Added in version 3.14.

os.SCHED_SPORADIC[Â¶](#os.SCHED_SPORADIC)

Scheduling policy for sporadic server programs.

os.SCHED_FIFO[Â¶](#os.SCHED_FIFO)

A First In First Out scheduling policy.

os.SCHED_RR[Â¶](#os.SCHED_RR)

A round-robin scheduling policy.

os.SCHED_RESET_ON_FORK[Â¶](#os.SCHED_RESET_ON_FORK)

This flag can be ORâed with any other scheduling policy. When a process with
this flag set forks, its childâs scheduling policy and priority are reset to
the default.

class os.sched_param(sched_priority)[Â¶](#os.sched_param)
...
sched_priority[Â¶](#os.sched_param.sched_priority)

The scheduling priority for a scheduling policy.

os.sched_get_priority_min(policy)[Â¶](#os.sched_get_priority_min)

Get the minimum priority value for *policy*. *policy* is one of the
scheduling policy constants above.

os.sched_get_priority_max(policy)[Â¶](#os.sched_get_priority_max)

Get the maximum priority value for *policy*. *policy* is one of the
scheduling policy constants above.

os.sched_setscheduler(pid, policy, param, /)[Â¶](#os.sched_setscheduler)
...
os.sched_getscheduler(pid, /)[Â¶](#os.sched_getscheduler)

Return the scheduling policy for the process with PID *pid*. A *pid* of 0
means the calling process. The result is one of the scheduling policy
constants above.

os.sched_setparam(pid, param, /)[Â¶](#os.sched_setparam)
...
os.sched_getaffinity(pid, /)[Â¶](#os.sched_getaffinity)
...
## Miscellaneous System Information[Â¶](#miscellaneous-system-information)

os.confstr(name, /)[Â¶](#os.confstr)
...
If the configuration value specified by *name* isnât defined, `None` is
returned.

If *name* is a string and is not known, [`ValueError`](exceptions.html#ValueError) is raised.  If a
specific value for *name* is not supported by the host system, even if it is
included in `confstr_names`, an [`OSError`](exceptions.html#OSError) is raised with
[`errno.EINVAL`](errno.html#errno.EINVAL) for the error number.

[Availability](intro.html#availability): Unix.

os.confstr_names[Â¶](#os.confstr_names)
...
os.getloadavg()[Â¶](#os.getloadavg)

Return the number of processes in the system run queue averaged over the last
1, 5, and 15 minutes or raises [`OSError`](exceptions.html#OSError) if the load average was
unobtainable.

[Availability](intro.html#availability): Unix.

os.process_cpu_count()[Â¶](#os.process_cpu_count)
...
os.sysconf_names[Â¶](#os.sysconf_names)
...
Higher-level operations on pathnames are defined in the [`os.path`](os.path.html#module-os.path) module.

os.curdir[Â¶](#os.curdir)

The constant string used by the operating system to refer to the current
directory. This is `'.'` for Windows and POSIX. Also available via
[`os.path`](os.path.html#module-os.path).

os.pardir[Â¶](#os.pardir)
...
is not sufficient to be able to parse or concatenate pathnames â use
[`os.path.split()`](os.path.html#os.path.split) and [`os.path.join()`](os.path.html#os.path.join) â but it is occasionally
useful. Also available via [`os.path`](os.path.html#module-os.path).

os.altsep[Â¶](#os.altsep)
...
os.extsep[Â¶](#os.extsep)

The character which separates the base filename from the extension; for example,
the `'.'` in `os.py`. Also available via [`os.path`](os.path.html#module-os.path).

os.pathsep[Â¶](#os.pathsep)

The character conventionally used by the operating system to separate search
path components (as in `PATH`), such as `':'` for POSIX or `';'` for
Windows. Also available via [`os.path`](os.path.html#module-os.path).

os.defpath[Â¶](#os.defpath)

The default search path used by [`exec*p*`](#os.execl) and
[`spawn*p*`](#os.spawnl) if the environment doesnât have a `'PATH'`
key. Also available via [`os.path`](os.path.html#module-os.path).

os.linesep[Â¶](#os.linesep)
...
os.devnull[Â¶](#os.devnull)

The file path of the null device. For example: `'/dev/null'` for
POSIX, `'nul'` for Windows.  Also available via [`os.path`](os.path.html#module-os.path).

os.RTLD_LAZY[Â¶](#os.RTLD_LAZY)

os.RTLD_NOW[Â¶](#os.RTLD_NOW)

os.RTLD_GLOBAL[Â¶](#os.RTLD_GLOBAL)

os.RTLD_LOCAL[Â¶](#os.RTLD_LOCAL)

os.RTLD_NODELETE[Â¶](#os.RTLD_NODELETE)

os.RTLD_NOLOAD[Â¶](#os.RTLD_NOLOAD)

os.RTLD_DEEPBIND[Â¶](#os.RTLD_DEEPBIND)

Flags for use with the [`setdlopenflags()`](sys.html#sys.setdlopenflags) and
[`getdlopenflags()`](sys.html#sys.getdlopenflags) functions.  See the Unix manual page
[dlopen(3)](https://manpages.debian.org/dlopen(3)) for what the different flags mean.

Added in version 3.3.

## Random numbers[Â¶](#random-numbers)

os.getrandom(size, flags=0)[Â¶](#os.getrandom)
...
[`NotImplementedError`](exceptions.html#NotImplementedError) exception is raised.

On Windows, it will use `BCryptGenRandom()`.

See also

The [`secrets`](secrets.html#module-secrets) module provides higher level functions. For an
easy-to-use interface to the random number generator provided by your
platform, please see [`random.SystemRandom`](random.html#random.SystemRandom).
...
os.GRND_NONBLOCK[Â¶](#os.GRND_NONBLOCK)
...
block in these cases, but instead immediately raises [`BlockingIOError`](exceptions.html#BlockingIOError).

Added in version 3.6.

os.GRND_RANDOM[Â¶](#os.GRND_RANDOM)
...
- [Timer File Descriptors](#timer-file-descriptors)
