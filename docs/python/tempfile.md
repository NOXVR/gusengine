...
tempfile â Generate temporary files and directories — Python 3.14.3 documentation
...
# `tempfile` â Generate temporary files and directories[Â¶](#module-tempfile)
...
[context managers](../glossary.html#term-context-manager). [`mkstemp()`](#tempfile.mkstemp) and
...
tempfile.TemporaryFile(mode='w+b', buffering=-1, encoding=None, newline=None, suffix=None, prefix=None, dir=None, *, errors=None)[Â¶](#tempfile.TemporaryFile)

Return a [file-like object](../glossary.html#term-file-like-object) that can be used as a temporary storage area.
...
The resulting object can be used as a [context manager](../glossary.html#term-context-manager) (see
...
The [`os.O_TMPFILE`](os.html#os.O_TMPFILE) flag is used if it is available and works
...
Changed in version 3.5: The [`os.O_TMPFILE`](os.html#os.O_TMPFILE) flag is now used if available.

Changed in version 3.8: Added *errors* parameter.

tempfile.NamedTemporaryFile(mode='w+b', buffering=-1, encoding=None, newline=None, suffix=None, prefix=None, dir=None, delete=True, *, errors=None, delete_on_close=True)[Â¶](#tempfile.NamedTemporaryFile)
...
The returned object is always a [file-like object](../glossary.html#term-file-like-object) whose `file`
attribute is the underlying true file object. This file-like object
can be used in a [`with`](../reference/compound_stmts.html#with) statement, just like a normal file.  The
...
only, or else when the [file-like object](../glossary.html#term-file-like-object) is finalized. Deletion is not
always guaranteed in this case (see [`object.__del__()`](../reference/datamodel.html#object.__del__)). If *delete* is
...
a [`with`](../reference/compound_stmts.html#with) statement, set the *delete_on_close* parameter to be false.
...
- 
additional open shares delete access (e.g. by calling [`os.open()`](os.html#os.open)
...
created via builtin [`open()`](functions.html#open)) must be closed before exiting the
context manager, else the [`os.unlink()`](os.html#os.unlink) call on context manager
exit will fail with a [`PermissionError`](exceptions.html#PermissionError).

On Windows, if *delete_on_close* is false, and the file is created in a
directory for which the user lacks delete access, then the [`os.unlink()`](os.html#os.unlink)
call on exit of the context manager will fail with a [`PermissionError`](exceptions.html#PermissionError).
...
class tempfile.SpooledTemporaryFile(max_size=0, mode='w+b', buffering=-1, encoding=None, newline=None, suffix=None, prefix=None, dir=None, *, errors=None)[Â¶](#tempfile.SpooledTemporaryFile)

This class operates exactly as [`TemporaryFile()`](#tempfile.TemporaryFile) does, except that
data is spooled in memory until the file size exceeds *max_size*, or
until the fileâs [`fileno()`](io.html#io.IOBase.fileno) method is called, at which point the
contents are written to disk and operation proceeds as with
[`TemporaryFile()`](#tempfile.TemporaryFile).

rollover()[Â¶](#tempfile.SpooledTemporaryFile.rollover)
...
is either an [`io.BytesIO`](io.html#io.BytesIO) or [`io.TextIOWrapper`](io.html#io.TextIOWrapper) object
...
file-like object can be used in a [`with`](../reference/compound_stmts.html#with) statement, just like
a normal file.
...
Changed in version 3.11: Fully implements the [`io.BufferedIOBase`](io.html#io.BufferedIOBase) and
[`io.TextIOBase`](io.html#io.TextIOBase) abstract base classes (depending on whether binary
or text *mode* was specified).

class tempfile.TemporaryDirectory(suffix=None, prefix=None, dir=None, ignore_cleanup_errors=False, *, delete=True)[Â¶](#tempfile.TemporaryDirectory)
...
name[Â¶](#tempfile.TemporaryDirectory.name)

The directory name can be retrieved from the `name` attribute of the
returned object.  When the returned object is used as a [context manager](../glossary.html#term-context-manager), the
`name` will be assigned to the target of the `as` clause in
the [`with`](../reference/compound_stmts.html#with) statement, if there is one.

cleanup()[Â¶](#tempfile.TemporaryDirectory.cleanup)
...
[`PermissionError`](exceptions.html#PermissionError) removing open files on Windows) will be ignored,
and the remaining removable items deleted on a âbest-effortâ basis.
...
tempfile.mkstemp(suffix=None, prefix=None, dir=None, text=False)[Â¶](#tempfile.mkstemp)

Creates a temporary file in the most secure manner possible.  There are
no race conditions in the fileâs creation, assuming that the platform
properly implements the [`os.O_EXCL`](os.html#os.O_EXCL) flag for [`os.open()`](os.html#os.open).  The
file is readable and writable only by the creating user ID.  If the
platform uses permission bits to indicate whether a file is executable,
the file is executable by no one.

The file descriptor is [not inherited by child processes](os.html#fd-inheritance).
...
file (as would be returned by [`os.open()`](os.html#os.open)) and the absolute pathname
...
Changed in version 3.6: The *dir* parameter now accepts a [path-like object](../glossary.html#term-path-like-object).

tempfile.mkdtemp(suffix=None, prefix=None, dir=None)[Â¶](#tempfile.mkdtemp)

Creates a temporary directory in the most secure manner possible. There
are no race conditions in the directoryâs creation.  The directory is
...
tempfile.gettempdir()[Â¶](#tempfile.gettempdir)
...
tempfile.gettempdirb()[Â¶](#tempfile.gettempdirb)

Same as [`gettempdir()`](#tempfile.gettempdir) but the return value is in bytes.

Added in version 3.5.

tempfile.gettempprefix()[Â¶](#tempfile.gettempprefix)

Return the filename prefix used to create temporary files.  This does not
contain the directory component.

tempfile.gettempprefixb()[Â¶](#tempfile.gettempprefixb)
...
tempfile.tempdir[Â¶](#tempfile.tempdir)
...
## Deprecated functions and variables[Â¶](#deprecated-functions-and-variables)
...
tempfile.mktemp(suffix='', prefix='tmp', dir=None)[Â¶](#tempfile.mktemp)
