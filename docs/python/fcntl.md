...
fcntl â The fcntl and ioctl system calls — Python 3.14.3 documentation
...
# `fcntl` â The `fcntl` and `ioctl` system calls[Â¶](#module-fcntl)
...
`sys.stdin.fileno()`, or an [`io.IOBase`](io.html#io.IOBase) object, such as `sys.stdin`
itself, which provides a [`fileno()`](io.html#io.IOBase.fileno) that returns a genuine file
descriptor.

Changed in version 3.3: Operations in this module used to raise an [`IOError`](exceptions.html#IOError) where they now
raise an [`OSError`](exceptions.html#OSError).

Changed in version 3.8: The `fcntl` module now contains `F_ADD_SEALS`, `F_GET_SEALS`, and
`F_SEAL_*` constants for sealing of [`os.memfd_create()`](os.html#os.memfd_create) file
descriptors.
...
`F_SETPIPE_SZ` constants, which allow to check and modify a pipeâs size
respectively.
...
XFS). This behavior is commonly referred to as âcopy-on-writeâ.
...
fcntl.fcntl(fd, cmd, arg=0, /)[Â¶](#fcntl.fcntl)

Perform the operation *cmd* on file descriptor *fd* (file objects providing
a [`fileno()`](io.html#io.IOBase.fileno) method are accepted as well).  The values used
...
[bytes-like object](../glossary.html#term-bytes-like-object), or a string.
...
for example, created by [`struct.pack()`](struct.html#struct.pack).
...
call is the contents of the buffer, converted to a [`bytes`](stdtypes.html#bytes) object.
The length of the returned object will be the same as the length of the
*arg* argument. This is limited to 1024 bytes.

If the `fcntl()` call fails, an [`OSError`](exceptions.html#OSError) is raised.
...
Changed in version 3.14: Add support of arbitrary [bytes-like objects](../glossary.html#term-bytes-like-object),
not only [`bytes`](stdtypes.html#bytes).

fcntl.ioctl(fd, request, arg=0, mutate_flag=True, /)[Â¶](#fcntl.ioctl)
...
found in the [`termios`](termios.html#module-termios) module, under the same names as used in
...
If *arg* supports the read-write buffer interface (like [`bytearray`](stdtypes.html#bytearray))
...
If the `ioctl()` call fails, an [`OSError`](exceptions.html#OSError) exception is raised.
...
fcntl.flock(fd, operation, /)[Â¶](#fcntl.flock)

Perform the lock operation *operation* on file descriptor *fd* (file objects providing
a [`fileno()`](io.html#io.IOBase.fileno) method are accepted as well). See the Unix manual
[flock(2)](https://manpages.debian.org/flock(2)) for details.  (On some systems, this function is emulated
using `fcntl()`.)

If the `flock()` call fails, an [`OSError`](exceptions.html#OSError) exception is raised.

Raises an [auditing event](sys.html#auditing) `fcntl.flock` with arguments `fd`, `operation`.

fcntl.lockf(fd, cmd, len=0, start=0, whence=0, /)[Â¶](#fcntl.lockf)

This is essentially a wrapper around the [`fcntl()`](#fcntl.fcntl) locking calls.
*fd* is the file descriptor (file objects providing a [`fileno()`](io.html#io.IOBase.fileno)
method are accepted as well) of the file to lock or unlock, and *cmd*
is one of the following values:

fcntl.LOCK_UN[Â¶](#fcntl.LOCK_UN)

Release an existing lock.

fcntl.LOCK_SH[Â¶](#fcntl.LOCK_SH)

Acquire a shared lock.

fcntl.LOCK_EX[Â¶](#fcntl.LOCK_EX)

Acquire an exclusive lock.

fcntl.LOCK_NB[Â¶](#fcntl.LOCK_NB)
...
[`OSError`](exceptions.html#OSError) will be raised and the exception will have an *errno*
attribute set to [`EACCES`](errno.html#errno.EACCES) or [`EAGAIN`](errno.html#errno.EAGAIN) (depending on the
...
[`io.IOBase.seek()`](io.html#io.IOBase.seek), specifically:

- 
`0` â relative to the start of the file ([`os.SEEK_SET`](os.html#os.SEEK_SET))

- 
`1` â relative to the current buffer position ([`os.SEEK_CUR`](os.html#os.SEEK_CUR))

- 
`2` â relative to the end of the file ([`os.SEEK_END`](os.html#os.SEEK_END))
...
integer value; in the second example it will hold a [`bytes`](stdtypes.html#bytes) object.  The
...
If the locking flags [`O_SHLOCK`](os.html#os.O_SHLOCK) and [`O_EXLOCK`](os.html#os.O_EXLOCK) are
present in the [`os`](os.html#module-os) module (on BSD only), the [`os.open()`](os.html#os.open)
...
[`pty` â Pseudo-terminal utilities](pty.html)
...
[`resource` â Resource usage information](resource.html)
