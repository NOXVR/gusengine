...
textwrap â Text wrapping and filling — Python 3.14.3 documentation
...
- 
          [next](unicodedata.html) |
        
- 
          [previous](difflib.html) |
...
# `textwrap` â Text wrapping and filling[Â¶](#module-textwrap)
...
If youâre just wrapping or filling one or two text strings, the convenience
functions should be good enough; otherwise, you should use an instance of
[`TextWrapper`](#textwrap.TextWrapper) for efficiency.

textwrap.wrap(text, width=70, *, initial_indent='', subsequent_indent='', expand_tabs=True, replace_whitespace=True, fix_sentence_endings=False, break_long_words=True, drop_whitespace=True, break_on_hyphens=True, tabsize=8, max_lines=None, placeholder=' [...]')[Â¶](#textwrap.wrap)
...
textwrap.fill(text, width=70, *, initial_indent='', subsequent_indent='', expand_tabs=True, replace_whitespace=True, fix_sentence_endings=False, break_long_words=True, drop_whitespace=True, break_on_hyphens=True, tabsize=8, max_lines=None, placeholder=' [...]')[Â¶](#textwrap.fill)
...
textwrap.shorten(text, width, *, fix_sentence_endings=False, break_long_words=True, break_on_hyphens=True, placeholder=' [...]')[Â¶](#textwrap.shorten)
...
textwrap.dedent(text)[Â¶](#textwrap.dedent)
...
textwrap.indent(text, prefix, predicate=None)[Â¶](#textwrap.indent)
...
class textwrap.TextWrapper(**kwargs)[Â¶](#textwrap.TextWrapper)
...
width[Â¶](#textwrap.TextWrapper.width)
...
expand_tabs[Â¶](#textwrap.TextWrapper.expand_tabs)

(default: `True`) If true, then all tab characters in *text* will be
expanded to spaces using the [`expandtabs()`](stdtypes.html#str.expandtabs) method of *text*.

tabsize[Â¶](#textwrap.TextWrapper.tabsize)
...
replace_whitespace[Â¶](#textwrap.TextWrapper.replace_whitespace)
...
be split into paragraphs (using [`str.splitlines()`](stdtypes.html#str.splitlines) or similar)
which are wrapped separately.

drop_whitespace[Â¶](#textwrap.TextWrapper.drop_whitespace)
...
initial_indent[Â¶](#textwrap.TextWrapper.initial_indent)

(default: `''`) String that will be prepended to the first line of
wrapped output.  Counts towards the length of the first line.  The empty
string is not indented.

subsequent_indent[Â¶](#textwrap.TextWrapper.subsequent_indent)

(default: `''`) String that will be prepended to all lines of wrapped
output except the first.  Counts towards the length of each line except
the first.

fix_sentence_endings[Â¶](#textwrap.TextWrapper.fix_sentence_endings)
...
unable to detect the difference between âDr.â in
...
the definition of âlowercase letterâ, and a convention of using two spaces
after a period to separate sentences on the same line, it is specific to
English-language texts.

break_long_words[Â¶](#textwrap.TextWrapper.break_long_words)
...
break_on_hyphens[Â¶](#textwrap.TextWrapper.break_on_hyphens)
...
max_lines[Â¶](#textwrap.TextWrapper.max_lines)
...
placeholder[Â¶](#textwrap.TextWrapper.placeholder)
...
wrap(text)[Â¶](#textwrap.TextWrapper.wrap)
...
fill(text)[Â¶](#textwrap.TextWrapper.fill)
...
[`unicodedata` â Unicode Database](unicodedata.html)
