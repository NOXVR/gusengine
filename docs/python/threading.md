...
threading â Thread-based parallelism — Python 3.14.3 documentation
...
- 
          [next](multiprocessing.html) |
        
- 
          [previous](concurrency.html) |
...
# `threading` â Thread-based parallelism[Â¶](#module-threading)

**Source code:** [Lib/threading.py](https://github.com/python/cpython/tree/3.14/Lib/threading.py)

This module constructs higher-level threading interfaces on top of the lower
level [`_thread`](_thread.html#module-_thread) module.
...
threads that can process multiple tasks concurrently.  Hereâs a basic example of
...
[`concurrent.futures.ThreadPoolExecutor`](concurrent.futures.html#concurrent.futures.ThreadPoolExecutor) offers a higher level interface
...
[`queue`](queue.html#module-queue) provides a thread-safe interface for exchanging data between
running threads.

[`asyncio`](asyncio.html#module-asyncio) offers an alternative approach to achieving task level
...
[`multiprocessing`](multiprocessing.html#module-multiprocessing) or [`concurrent.futures.ProcessPoolExecutor`](concurrent.futures.html#concurrent.futures.ProcessPoolExecutor).
However, threading is still an appropriate model if you want to run
multiple I/O-bound tasks simultaneously.

## GIL and performance considerations[Â¶](#gil-and-performance-considerations)

Unlike the [`multiprocessing`](multiprocessing.html#module-multiprocessing) module, which uses separate processes to
...
threading.active_count()[Â¶](#threading.active_count)
...
threading.current_thread()[Â¶](#threading.current_thread)

Return the current [`Thread`](#threading.Thread) object, corresponding to the callerâs thread
of control.  If the callerâs thread of control was not created through the
`threading` module, a dummy thread object with limited functionality is
returned.

The function `currentThread` is a deprecated alias for this function.

threading.excepthook(args, /)[Â¶](#threading.excepthook)
...
If *exc_type* is [`SystemExit`](exceptions.html#SystemExit), the exception is silently ignored.
Otherwise, the exception is printed out on [`sys.stderr`](sys.html#sys.stderr).

If  this function raises an exception, [`sys.excepthook()`](sys.html#sys.excepthook) is called to
handle it.
...
[`sys.excepthook()`](sys.html#sys.excepthook) handles uncaught exceptions.

Added in version 3.8.

threading.__excepthook__[Â¶](#threading.__excepthook__)
...
threading.get_ident()[Â¶](#threading.get_ident)

Return the âthread identifierâ of the current thread.  This is a nonzero
...
threading.get_native_id()[Â¶](#threading.get_native_id)
...
threading.enumerate()[Â¶](#threading.enumerate)
...
threading.main_thread()[Â¶](#threading.main_thread)
...
threading.settrace(func)[Â¶](#threading.settrace)

Set a trace function for all threads started from the `threading` module.
The *func* will be passed to  [`sys.settrace()`](sys.html#sys.settrace) for each thread, before its
[`run()`](#threading.Thread.run) method is called.

threading.settrace_all_threads(func)[Â¶](#threading.settrace_all_threads)
...
threading.gettrace()[Â¶](#threading.gettrace)

Get the trace function as set by [`settrace()`](#threading.settrace).

Added in version 3.10.

threading.setprofile(func)[Â¶](#threading.setprofile)

Set a profile function for all threads started from the `threading` module.
The *func* will be passed to  [`sys.setprofile()`](sys.html#sys.setprofile) for each thread, before its
[`run()`](#threading.Thread.run) method is called.

threading.setprofile_all_threads(func)[Â¶](#threading.setprofile_all_threads)
...
threading.getprofile()[Â¶](#threading.getprofile)

Get the profiler function as set by [`setprofile()`](#threading.setprofile).

Added in version 3.10.

threading.stack_size([size])[Â¶](#threading.stack_size)
...
unsupported, a [`RuntimeError`](exceptions.html#RuntimeError) is raised.  If the specified stack size is
invalid, a [`ValueError`](exceptions.html#ValueError) is raised and the stack size is unmodified.  32 KiB
...
threading.TIMEOUT_MAX[Â¶](#threading.TIMEOUT_MAX)
...
[`OverflowError`](exceptions.html#OverflowError).
...
The design of this module is loosely based on Javaâs threading model. However,
where Java makes locks and condition variables basic behavior of every object,
they are separate objects in Python.  Pythonâs [`Thread`](#threading.Thread) class supports a
subset of the behavior of Javaâs Thread class; currently, there are no
priorities, no thread groups, and threads cannot be destroyed, stopped,
suspended, resumed, or interrupted.  The static methods of Javaâs Thread class,
when implemented, are mapped to module-level functions.

All of the methods described below are executed atomically.

### Thread-local data[Â¶](#thread-local-data)
...
You can also access the [`local`](#threading.local)-objectâs dictionary:
...
donât affect data seen in this thread:

>>> mydata.number
42

Of course, values you get from a [`local`](#threading.local) object, including their
[`__dict__`](../reference/datamodel.html#object.__dict__) attribute, are for whatever thread was current
at the time the attribute was read.  For that reason, you generally
donât want to save these values across threads, as they apply only to
...
initialization.  Note that if you define an [`__init__()`](../reference/datamodel.html#object.__init__)
method, it will be called each time the [`local`](#threading.local) object is used
in a separate thread.  This is necessary to initialize each threadâs
dictionary.
...
without affecting this threadâs data:
...
Note that subclasses can define [__slots__](../glossary.html#term-__slots__), but they are not
...
class threading.local[Â¶](#threading.local)

A class that represents thread-local data.

### Thread objects[Â¶](#thread-objects)
...
threadâs [`start()`](#threading.Thread.start) method.  This invokes the [`run()`](#threading.Thread.run)
method in a separate thread of control.

Once the threadâs activity is started, the thread is considered âaliveâ. It
stops being alive when its [`run()`](#threading.Thread.run) method terminates â either
...
Other threads can call a threadâs [`join()`](#threading.Thread.join) method.  This blocks
...
[`threading.excepthook()`](#threading.excepthook) ignores silently [`SystemExit`](exceptions.html#SystemExit).

A thread can be flagged as a âdaemon threadâ.  The significance of this flag is
...
There is a âmain threadâ object; this corresponds to the initial thread of
control in the Python program.  It is not a daemon thread.

There is the possibility that âdummy thread objectsâ are created. These are
thread objects corresponding to âalien threadsâ, which are threads of control
started outside the threading module, such as directly from C code.  Dummy
thread objects have limited functionality; they are always considered alive and
daemonic, and cannot be [joined](#meth-thread-join).  They are never deleted,
since it is impossible to detect the termination of alien threads.

class threading.Thread(group=None, target=None, name=None, args=(), kwargs={}, *, daemon=None, context=None)[Â¶](#threading.Thread)
...
of the form âThread-*N*â where *N* is a small decimal number,
or âThread-*N* (target)â where âtargetâ is `target.__name__` if the
...
*context* is the [`Context`](contextvars.html#contextvars.Context) value to use when starting
the thread.  The default value is `None` which indicates that the
[`sys.flags.thread_inherit_context`](sys.html#sys.flags.thread_inherit_context) flag controls the behaviour.  If
...
[`Context()`](contextvars.html#contextvars.Context).  To explicitly start with a copy of the
current context, pass the value from [`copy_context()`](contextvars.html#contextvars.copy_context). The
...
start()[Â¶](#threading.Thread.start)

Start the threadâs activity.

It must be called at most once per thread object.  It arranges for the
objectâs [`run()`](#threading.Thread.run) method to be invoked in a separate thread
of control.

This method will raise a [`RuntimeError`](exceptions.html#RuntimeError) if called more than once
...
join(timeout=None)[Â¶](#threading.Thread.join)

Wait until the thread terminates. This blocks the calling thread until
the thread whose [`join()`](#threading.Thread.join) method is called terminates â either
normally or through an unhandled exception â or until the optional
timeout occurs.
...
decide whether a timeout happened â if the thread is still alive, the
...
[`join()`](#threading.Thread.join) raises a [`RuntimeError`](exceptions.html#RuntimeError) if an attempt is made
...
raises a [`PythonFinalizationError`](exceptions.html#PythonFinalizationError).

Changed in version 3.14: May raise [`PythonFinalizationError`](exceptions.html#PythonFinalizationError).

name[Â¶](#threading.Thread.name)
...
getName()[Â¶](#threading.Thread.getName)

setName()[Â¶](#threading.Thread.setName)
...
ident[Â¶](#threading.Thread.ident)

The âthread identifierâ of this thread or `None` if the thread has not
been started.  This is a nonzero integer.  See the [`get_ident()`](#threading.get_ident)
function.  Thread identifiers may be recycled when a thread exits and
another thread is created.  The identifier is available even after the
thread has exited.

native_id[Â¶](#threading.Thread.native_id)
...
is_alive()[Â¶](#threading.Thread.is_alive)
...
daemon[Â¶](#threading.Thread.daemon)

A boolean value indicating whether this thread is a daemon thread (`True`)
or not (`False`).  This must be set before [`start()`](#threading.Thread.start) is called,
otherwise [`RuntimeError`](exceptions.html#RuntimeError) is raised.  Its initial value is inherited
...
isDaemon()[Â¶](#threading.Thread.isDaemon)

setDaemon()[Â¶](#threading.Thread.setDaemon)
...
### Lock objects[Â¶](#lock-objects)

A primitive lock is a synchronization primitive that is not owned by a
particular thread when locked.  In Python, it is currently the lowest level
synchronization primitive available, implemented directly by the [`_thread`](_thread.html#module-_thread)
extension module.

A primitive lock is in one of two states, âlockedâ or âunlockedâ. It is created
...
[`RuntimeError`](exceptions.html#RuntimeError) will be raised.
...
class threading.Lock[Â¶](#threading.Lock)
...
acquire(blocking=True, timeout=-1)[Â¶](#threading.Lock.acquire)
...
release()[Â¶](#threading.Lock.release)
...
When invoked on an unlocked lock, a [`RuntimeError`](exceptions.html#RuntimeError) is raised.

There is no return value.

locked()[Â¶](#threading.Lock.locked)

Return `True` if the lock is acquired.

### RLock objects[Â¶](#rlock-objects)

A reentrant lock is a synchronization primitive that may be acquired multiple
times by the same thread.  Internally, it uses the concepts of âowning threadâ
and ârecursion levelâ in addition to the locked/unlocked state used by primitive
locks.  In the locked state, some thread owns the lock; in the unlocked state,
no thread owns it.

Threads call a lockâs [`acquire()`](#threading.RLock.acquire) method to lock it,
...
RLockâs [`acquire()`](#threading.RLock.acquire)/[`release()`](#threading.RLock.release) call pairs may be nested,
unlike Lockâs [`acquire()`](#threading.Lock.acquire)/[`release()`](#threading.Lock.release). Only the final
...
class threading.RLock[Â¶](#threading.RLock)
...
acquire(blocking=True, timeout=-1)[Â¶](#threading.RLock.acquire)
...
release()[Â¶](#threading.RLock.release)
...
[`RuntimeError`](exceptions.html#RuntimeError) is raised if this method is called when the lock is
not acquired.

There is no return value.

locked()[Â¶](#threading.RLock.locked)

Return a boolean indicating whether this object is locked right now.

Added in version 3.14.

### Condition objects[Â¶](#condition-objects)

A condition variable is always associated with some kind of lock; this can be
passed in or one will be created by default.  Passing one in is useful when
several condition variables must share the same lock.  The lock is part of
the condition object: you donât have to track it separately.
...
donât release the lock; this means that the thread or threads awakened will
...
class threading.Condition(lock=None)[Â¶](#threading.Condition)
...
acquire(*args)[Â¶](#threading.Condition.acquire)

Acquire the underlying lock. This method calls the corresponding method on
the underlying lock; the return value is whatever that method returns.

release()[Â¶](#threading.Condition.release)

Release the underlying lock. This method calls the corresponding method on
the underlying lock; there is no return value.

locked()[Â¶](#threading.Condition.locked)

Return a boolean indicating whether this object is locked right now.

Added in version 3.14.

wait(timeout=None)[Â¶](#threading.Condition.wait)

Wait until notified or until a timeout occurs. If the calling thread has
not acquired the lock when this method is called, a [`RuntimeError`](exceptions.html#RuntimeError) is
raised.
...
wait_for(predicate, timeout=None)[Â¶](#threading.Condition.wait_for)
...
notify(n=1)[Â¶](#threading.Condition.notify)
...
threads are waiting.  However, itâs not safe to rely on this behavior.
...
notify_all()[Â¶](#threading.Condition.notify_all)
...
### Semaphore objects[Â¶](#semaphore-objects)
...
class threading.Semaphore(value=1)[Â¶](#threading.Semaphore)
...
defaults to `1`. If the *value* given is less than 0, [`ValueError`](exceptions.html#ValueError) is
raised.

Changed in version 3.3: changed from a factory function to a class.

acquire(blocking=True, timeout=None)[Â¶](#threading.Semaphore.acquire)
...
release(n=1)[Â¶](#threading.Semaphore.release)
...
class threading.BoundedSemaphore(value=1)[Â¶](#threading.BoundedSemaphore)

Class implementing bounded semaphore objects.  A bounded semaphore checks to
make sure its current value doesnât exceed its initial value.  If it does,
[`ValueError`](exceptions.html#ValueError) is raised. In most situations semaphores are used to guard
resources with limited capacity.  If the semaphore is released too many times
itâs a sign of a bug.  If not given, *value* defaults to 1.

Changed in version 3.3: changed from a factory function to a class.

### [`Semaphore`](#threading.Semaphore) example[Â¶](#semaphore-example)
...
Once spawned, worker threads call the semaphoreâs acquire and release methods
...
causes the semaphore to be released more than itâs acquired will go undetected.

### Event objects[Â¶](#event-objects)
...
class threading.Event[Â¶](#threading.Event)
...
### Timer objects[Â¶](#timer-objects)

This class represents an action that should be run only after a certain amount
of time has passed â a timer.  [`Timer`](#threading.Timer) is a subclass of [`Thread`](#threading.Thread)
...
class threading.Timer(interval, function, args=None, kwargs=None)[Â¶](#threading.Timer)
...
cancel()[Â¶](#threading.Timer.cancel)

Stop the timer, and cancel the execution of the timerâs action.  This will
only work if the timer is still in its waiting stage.

### Barrier objects[Â¶](#barrier-objects)
...
class threading.Barrier(parties, action=None, timeout=None)[Â¶](#threading.Barrier)
...
wait(timeout=None)[Â¶](#threading.Barrier.wait)
...
The return value is an integer in the range 0 to *parties* â 1, different
...
reset()[Â¶](#threading.Barrier.reset)
...
abort()[Â¶](#threading.Barrier.abort)
...
parties[Â¶](#threading.Barrier.parties)

The number of threads required to pass the barrier.

n_waiting[Â¶](#threading.Barrier.n_waiting)

The number of threads currently waiting in the barrier.

broken[Â¶](#threading.Barrier.broken)

A boolean that is `True` if the barrier is in the broken state.

exception threading.BrokenBarrierError[Â¶](#threading.BrokenBarrierError)

This exception, a subclass of [`RuntimeError`](exceptions.html#RuntimeError), is raised when the
[`Barrier`](#threading.Barrier) object is reset or broken.

## Using locks, conditions, and semaphores in the `with` statement[Â¶](#using-locks-conditions-and-semaphores-in-the-with-statement)
...
- [GIL and performance considerations](#gil-and-performance-considerations)
...
[`multiprocessing` â Process-based parallelism](multiprocessing.html)
