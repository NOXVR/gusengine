...
logging.config â Logging configuration — Python 3.14.3 documentation
...
# `logging.config` â Logging configuration[Â¶](#module-logging.config)

**Source code:** [Lib/logging/config.py](https://github.com/python/cpython/tree/3.14/Lib/logging/config.py)
...
## Configuration functions[Â¶](#configuration-functions)

The following functions configure the logging module. They are located in the
`logging.config` module.  Their use is optional â you can configure the
logging module using these functions or by making calls to the main API (defined
in [`logging`](logging.html#module-logging) itself) and defining handlers which are declared either in
[`logging`](logging.html#module-logging) or [`logging.handlers`](logging.handlers.html#module-logging.handlers).

logging.config.dictConfig(config)[Â¶](#logging.config.dictConfig)
...
raise a [`ValueError`](exceptions.html#ValueError), [`TypeError`](exceptions.html#TypeError), [`AttributeError`](exceptions.html#AttributeError)
or [`ImportError`](exceptions.html#ImportError) with a suitably descriptive message.  The
...
logging.config.fileConfig(fname, defaults=None, disable_existing_loggers=True, encoding=None)[Â¶](#logging.config.fileConfig)

Reads the logging configuration from a [`configparser`](configparser.html#module-configparser)-format file. The
...
It will raise [`FileNotFoundError`](exceptions.html#FileNotFoundError) if the file
doesnât exist and [`RuntimeError`](exceptions.html#RuntimeError) if the file is invalid or
empty.

Parameters:

- 
**fname** â A filename, or a file-like object, or an instance derived
from [`RawConfigParser`](configparser.html#configparser.RawConfigParser). If a
`RawConfigParser`-derived instance is passed, it is used as
is. Otherwise, a [`ConfigParser`](configparser.html#configparser.ConfigParser) is
instantiated, and the configuration read by it from the
object passed in `fname`. If that has a [`readline()`](readline.html#module-readline)
method, it is assumed to be a file-like object and read using
[`read_file()`](configparser.html#configparser.ConfigParser.read_file); otherwise,
it is assumed to be a filename and passed to
[`read()`](configparser.html#configparser.ConfigParser.read).

- 
**defaults** â Defaults to be passed to the `ConfigParser` can be specified
in this argument.

- 
**disable_existing_loggers** â If specified as `False`, loggers which
...
- 
**encoding** â The encoding used to open file when *fname* is filename.

Changed in version 3.4: An instance of a subclass of [`RawConfigParser`](configparser.html#configparser.RawConfigParser) is
...
doesnât exist or is invalid or empty.

logging.config.listen(port=DEFAULT_LOGGING_CONFIG_PORT, verify=None)[Â¶](#logging.config.listen)

Starts up a socket server on the specified port, and listens for new
configurations. If no port is specified, the moduleâs default
...
[`fileConfig()`](#logging.config.fileConfig). Returns a [`Thread`](threading.html#threading.Thread) instance on which
you can call [`start()`](threading.html#threading.Thread.start) to start the server, and which
you can [`join()`](threading.html#threading.Thread.join) when appropriate. To stop the server,
...
[`eval()`](functions.html#eval), use of this function may open its users to a security risk.
...
victim userâs process, simply by connecting to the victimâs
[`listen()`](#logging.config.listen) socket and sending a configuration which runs whatever
code the attacker wants to have executed in the victimâs process. This is
...
If you want to send configurations to the listener which donât
...
logging.config.stopListening()[Â¶](#logging.config.stopListening)
...
## Configuration dictionary schema[Â¶](#configuration-dictionary-schema)

Describing a logging configuration requires listing the various
objects to create and the connections between them; for example, you
may create a handler named âconsoleâ and then say that the logger
named âstartupâ will send its messages to the âconsoleâ handler.
These objects arenât limited to those provided by the [`logging`](logging.html#module-logging)
...
### Dictionary Schema Details[Â¶](#dictionary-schema-details)
...
as described below.  In all cases below where a âconfiguring dictâ is
...
configure the corresponding [`Formatter`](logging.html#logging.Formatter) instance.

The configuring dict is searched for the following optional keys
which correspond to the arguments passed to create a
[`Formatter`](logging.html#logging.Formatter) object:
...
An optional `class` key indicates the name of the formatterâs
class (as a dotted module and class name).  The instantiation
arguments are as for [`Formatter`](logging.html#logging.Formatter), thus this key is
most useful for instantiating a customised subclass of
[`Formatter`](logging.html#logging.Formatter).  For example, the alternative class
...
empty string) and this is used to construct a [`logging.Filter`](logging.html#logging.Filter)
instance.
...
handlerâs constructor.  For example, given the snippet:
...
[`logging.StreamHandler`](logging.handlers.html#logging.StreamHandler), using `sys.stdout` as the underlying
stream.  The handler with id `file` is instantiated as a
[`logging.handlers.RotatingFileHandler`](logging.handlers.html#logging.handlers.RotatingFileHandler) with the keyword arguments
...
### Incremental Configuration[Â¶](#incremental-configuration)
...
### Object connections[Â¶](#object-connections)
...
it unambiguously, and then using the id in the source objectâs
...
(Note: YAML used here because itâs a little more readable than the
...
### User-defined objects[Â¶](#user-defined-objects)
...
to provide a âfactoryâ - a callable which is called with a
configuration dictionary and which returns the instantiated object.
This is signalled by an absolute import path to the factory being
made available under the special key `'()'`.  Hereâs a concrete
example:
...
`brief`, is a standard [`logging.Formatter`](logging.html#logging.Formatter) instance with the
specified format string.  The second, with id `default`, has a
longer format and also defines the time format explicitly, and will
result in a [`logging.Formatter`](logging.html#logging.Formatter) initialized with those two format
...
standard [`logging.Formatter`](logging.html#logging.Formatter) instances are created.  The
...
### Handler configuration order[Â¶](#handler-configuration-order)
...
### Access to external objects[Â¶](#access-to-external-objects)
...
### Access to internal objects[Â¶](#access-to-internal-objects)
...
objects which are not known to the [`logging`](logging.html#module-logging) module.  For
example, consider [`logging.handlers.MemoryHandler`](logging.handlers.html#logging.handlers.MemoryHandler), which takes
...
### Import resolution and custom importers[Â¶](#import-resolution-and-custom-importers)

Import resolution, by default, uses the builtin [`__import__()`](functions.html#import__) function
...
it with [`staticmethod()`](functions.html#staticmethod). For example:
...
You donât need to wrap with [`staticmethod()`](functions.html#staticmethod) if youâre setting the import
callable on a configurator *instance*.

### Configuring QueueHandler and QueueListener[Â¶](#configuring-queuehandler-and-queuelistener)

If you want to configure a [`QueueHandler`](logging.handlers.html#logging.handlers.QueueHandler), noting that this
is normally used in conjunction with a [`QueueListener`](logging.handlers.html#logging.handlers.QueueListener), you
can configure both together. After the configuration, the `QueueListener` instance
will be available as the [`listener`](logging.handlers.html#logging.handlers.QueueHandler.listener) attribute of
the created handler, and that in turn will be available to you using
[`getHandlerByName()`](logging.html#logging.getHandlerByName) and passing the name you have used for the
...
- 
An object implementing the [`Queue.put_nowait`](queue.html#queue.Queue.put_nowait)
and [`Queue.get`](queue.html#queue.Queue.get) public API. For instance, this may be
an actual instance of [`queue.Queue`](queue.html#queue.Queue) or a subclass thereof, or a proxy
obtained by [`multiprocessing.managers.SyncManager.Queue()`](multiprocessing.html#multiprocessing.managers.SyncManager.Queue).
...
the queue instance to use. That callable could be a [`queue.Queue`](queue.html#queue.Queue) subclass
...
[`queue.Queue`](queue.html#queue.Queue) instance.

If the  `queue` key is absent, a standard unbounded [`queue.Queue`](queue.html#queue.Queue) instance is
created and used.

If the `listener` key is present, the corresponding value can be one of the following:

- 
A subclass of [`logging.handlers.QueueListener`](logging.handlers.html#logging.handlers.QueueListener). This is of course only
...
If the `listener` key is absent, [`logging.handlers.QueueListener`](logging.handlers.html#logging.handlers.QueueListener) is used.
...
initialization signatures as [`QueueHandler`](logging.handlers.html#logging.handlers.QueueHandler) and
[`QueueListener`](logging.handlers.html#logging.handlers.QueueListener).

Added in version 3.12.

## Configuration file format[Â¶](#configuration-file-format)

The configuration file format understood by [`fileConfig()`](#logging.config.fileConfig) is based on
[`configparser`](configparser.html#module-configparser) functionality. The file must contain sections called
...
you cannot configure [`Filter`](logging.html#logging.Filter) objects, which provide for
filtering of messages beyond simple integer levels, using [`fileConfig()`](#logging.config.fileConfig).
If you need to have instances of [`Filter`](logging.html#logging.Filter) in your logging
...
[`dictConfig()`](#logging.config.dictConfig), so itâs worth considering transitioning to this newer
API when itâs convenient to do so.
...
except that if a non-root loggerâs level is specified as `NOTSET`, the system
...
The `class` entry indicates the handlerâs class (as determined by [`eval()`](functions.html#eval)
in the `logging` packageâs namespace). The `level` is interpreted as for
loggers, and `NOTSET` is taken to mean âlog everythingâ.
...
packageâs namespace, is the list of arguments to the constructor for the handler
...
`logging` packageâs namespace, is the keyword argument dict to the constructor
...
the `logging` packageâs namespace, is a dictionary of default values for
custom formatting fields. If not provided, it defaults to `None`.

Note

Due to the use of [`eval()`](functions.html#eval) as described above, there are
...
- [Dictionary Schema Details](#dictionary-schema-details)
...
- [Handler configuration order](#handler-configuration-order)

- [Access to external objects](#access-to-external-objects)

- [Access to internal objects](#access-to-internal-objects)

- [Import resolution and custom importers](#import-resolution-and-custom-importers)

- [Configuring QueueHandler and QueueListener](#configuring-queuehandler-and-queuelistener)
...
[`logging.handlers` â Logging handlers](logging.handlers.html)
