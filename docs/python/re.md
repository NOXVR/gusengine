...
re â Regular expression operations — Python 3.14.3 documentation
...
- 
          [previous](string.templatelib.html) |
...
# `re` â Regular expression operations[Â¶](#module-re)
...
Both patterns and strings to be searched can be Unicode strings ([`str`](stdtypes.html#str))
as well as 8-bit strings ([`bytes`](stdtypes.html#bytes)).
...
their special meaning.  This collides with Pythonâs usage of the same
character for the same purpose in string literals; for example, to match
a literal backslash, one might have to write `'\\\\'` as the pattern
string, because the regular expression must be `\\`, and each
backslash must be expressed as `\\` inside a regular Python string
literal. Also, please note that any invalid escape sequences in Pythonâs
usage of the backslash in string literals now generate a [`SyntaxWarning`](exceptions.html#SyntaxWarning)
and in the future this will become a [`SyntaxError`](exceptions.html#SyntaxError). This behaviour
...
that donât require you to compile a regex object first, but miss some
...
## Regular Expression Syntax[Â¶](#regular-expression-syntax)
...
section, weâll write REâs in `this special style`, usually without quotes, and
...
matches both âfooâ and âfoobarâ, while the regular expression `foo$` matches
only âfooâ.  More interestingly, searching for `foo.$` in `'foo1\nfoo2\n'`
matches âfoo2â normally, but âfoo1â in [`MULTILINE`](#re.MULTILINE) mode; searching for
a single `$` in `'foo\n'` will find two (empty) matches: one just before
the newline, and one at the end of the string.

`*`
Causes the resulting RE to match 0 or more repetitions of the preceding RE, as
many repetitions as are possible.  `ab*` will match âaâ, âabâ, or âaâ followed
by any number of âbâs.

`+`
Causes the resulting RE to match 1 or more repetitions of the preceding RE.
`ab+` will match âaâ followed by any non-zero number of âbâs; it will not
match just âaâ.

`?`
Causes the resulting RE to match 0 or 1 repetitions of the preceding RE.
`ab?` will match either âaâ or âabâ.

`*?`, `+?`, `??`
The `'*'`, `'+'`, and `'?'` quantifiers are all greedy; they match
as much text as possible.  Sometimes this behaviour isnât desired; if the RE
...
If youâre not using a raw string to express the pattern, remember that Python
also uses the backslash as an escape sequence in string literals; if the escape
sequence isnât recognized by Pythonâs parser, the backslash and subsequent
character are included in the resulting string.  However, if Python would
recognize the resulting sequence, the backslash should be repeated twice.  This
is complicated and hard to understand, so itâs highly recommended that you use
...
`[a\-z]`) or if itâs placed as the first or last character
...
(defined below).
Note that `\b` represents a single âbackspaceâ character,
...
`'^'`.  `^` has no special meaning if itâs not the first character in
the set.
...
syntax, so to facilitate this change a [`FutureWarning`](exceptions.html#FutureWarning) will be raised
in ambiguous cases for the time being.
That includes sets starting with a literal `'['` or containing literal
character sequences `'--'`, `'&&'`, `'~~'`, and `'||'`.  To
avoid a warning escape them with a backslash.

Changed in version 3.7: [`FutureWarning`](exceptions.html#FutureWarning) is raised if a character set contains constructs
...
as inline flags, so they canât be combined or follow `'-'`.  Instead,
...
Python identifiers, and in [`bytes`](stdtypes.html#bytes) patterns they can only contain
...
Context of reference to group âquoteâ
...
Changed in version 3.12: In [`bytes`](stdtypes.html#bytes) patterns, group *name* can only contain bytes
...
`(?=...)`
Matches if `...` matches next, but doesnât consume any of the string.  This is
called a lookahead assertion.  For example, `Isaac (?=Asimov)` will match
`'Isaac '` only if itâs followed by `'Asimov'`.

`(?!...)`
Matches if `...` doesnât match next.  This is a negative lookahead assertion.
For example, `Isaac (?!Asimov)` will match `'Isaac '` only if itâs *not*
followed by `'Asimov'`.
...
*name* exists, and with `no-pattern` if it doesnât. `no-pattern` is
...
In [`bytes`](stdtypes.html#bytes) patterns, group *name* can only contain bytes
in the ASCII range (`b'\x00'`-`b'\x7f'`).
...
`\s`
For Unicode (str) patterns:
Matches Unicode whitespace characters (as defined by [`str.isspace()`](stdtypes.html#str.isspace)).
...
(as defined by [`str.isalnum()`](stdtypes.html#str.isalnum)),
...
for which [`str.isalnum()`](stdtypes.html#str.isalnum) returns `False`.
...
(Note that `\b` is used to represent word boundaries, and means âbackspaceâ
...
[`enum.IntFlag`](enum.html#enum.IntFlag).

class re.RegexFlag[Â¶](#re.RegexFlag)

An [`enum.IntFlag`](enum.html#enum.IntFlag) class containing the regex options listed below.
...
and Unicode matching isnât allowed for bytes patterns.
...
re.IGNORECASE[Â¶](#re.IGNORECASE)

Perform case-insensitive matching;
expressions like `[A-Z]` will also  match lowercase letters.
Full Unicode matching (such as `Ã` matching `Ã¼`)
...
letters and 4 additional non-ASCII letters: âÄ°â (U+0130, Latin capital
letter I with dot above), âÄ±â (U+0131, Latin small letter dotless i),
âÅ¿â (U+017F, Latin small letter long s) and ââªâ (U+212A, Kelvin sign).
If the [`ASCII`](#re.ASCII) flag is used, only letters âaâ to âzâ
and âAâ to âZâ are matched.

re.L[Â¶](#re.L)

re.LOCALE[Â¶](#re.LOCALE)
...
as it only handles one âcultureâ at a time
...
re.compile(pattern, flags=0)[Â¶](#re.compile)
...
[`match()`](#re.Pattern.match), [`search()`](#re.Pattern.search) and other methods, described
below.
...
programs that use only a few regular expressions at a time neednât worry
about compiling regular expressions.

re.search(pattern, string, flags=0)[Â¶](#re.search)
...
re.match(pattern, string, flags=0)[Â¶](#re.match)
...
Note that even in [`MULTILINE`](#re.MULTILINE) mode, [`re.match()`](#re.match) will only match
...
re.fullmatch(pattern, string, flags=0)[Â¶](#re.fullmatch)
...
re.split(pattern, string, maxsplit=0, flags=0)[Â¶](#re.split)
...
re.findall(pattern, string, flags=0)[Â¶](#re.findall)
...
re.finditer(pattern, string, flags=0)[Â¶](#re.finditer)

Return an [iterator](../glossary.html#term-iterator) yielding [`Match`](#re.Match) objects over
...
re.sub(pattern, repl, string, count=0, flags=0)[Â¶](#re.sub)

Return the string obtained by replacing the leftmost non-overlapping occurrences
of *pattern* in *string* by the replacement *repl*.  If the pattern isnât found,
...
group number; `\g<2>` is therefore equivalent to `\2`, but isnât ambiguous
...
Changed in version 3.12: Group *id* can only contain ASCII digits.
In [`bytes`](stdtypes.html#bytes) replacement strings, group *name* can only contain bytes
in the ASCII range (`b'\x00'`-`b'\x7f'`).
...
re.subn(pattern, repl, string, count=0, flags=0)[Â¶](#re.subn)
...
exception re.PatternError(msg, pattern=None, pos=None)[Â¶](#re.PatternError)
...
msg[Â¶](#re.PatternError.msg)

The unformatted error message.

pattern[Â¶](#re.PatternError.pattern)

The regular expression pattern.

pos[Â¶](#re.PatternError.pos)

The index in *pattern* where compilation failed (may be `None`).

lineno[Â¶](#re.PatternError.lineno)

The line corresponding to *pos* (may be `None`).

colno[Â¶](#re.PatternError.colno)
...
## Regular Expression Objects[Â¶](#regular-expression-objects)

class re.Pattern[Â¶](#re.Pattern)
...
Pattern.search(string[, pos[, endpos]])[Â¶](#re.Pattern.search)
...
Pattern.match(string[, pos[, endpos]])[Â¶](#re.Pattern.match)
...
[`search()`](#re.Pattern.search) instead (see also [search() vs. match()](#search-vs-match)).

Pattern.fullmatch(string[, pos[, endpos]])[Â¶](#re.Pattern.fullmatch)
...
Pattern.groups[Â¶](#re.Pattern.groups)

The number of capturing groups in the pattern.

Pattern.groupindex[Â¶](#re.Pattern.groupindex)

A dictionary mapping any symbolic group names defined by `(?P<id>)` to group
numbers.  The dictionary is empty if no symbolic groups were used in the
pattern.

Pattern.pattern[Â¶](#re.Pattern.pattern)

The pattern string from which the pattern object was compiled.

Changed in version 3.7: Added support of [`copy.copy()`](copy.html#copy.copy) and [`copy.deepcopy()`](copy.html#copy.deepcopy).  Compiled
regular expression objects are considered atomic.

## Match Objects[Â¶](#match-objects)
...
Match.expand(template)[Â¶](#re.Match.expand)
...
Match.group([group1, ...])[Â¶](#re.Match.group)
...
pattern, an [`IndexError`](exceptions.html#IndexError) exception is raised. If a group is contained in a
...
string argument is not used as a group name in the pattern, an [`IndexError`](exceptions.html#IndexError)
exception is raised.
...
Match.__getitem__(g)[Â¶](#re.Match.__getitem__)
...
Match.groups(default=None)[Â¶](#re.Match.groups)
...
Match.groupdict(default=None)[Â¶](#re.Match.groupdict)
...
Match.start([group])[Â¶](#re.Match.start)

Match.end([group])[Â¶](#re.Match.end)
...
2, and `m.start(2)` raises an [`IndexError`](exceptions.html#IndexError) exception.
...
Match.span([group])[Â¶](#re.Match.span)

For a match *m*, return the 2-tuple `(m.start(group), m.end(group))`. Note
that if *group* did not contribute to the match, this is `(-1, -1)`.
*group* defaults to zero, the entire match.

Match.pos[Â¶](#re.Match.pos)
...
Match.lastgroup[Â¶](#re.Match.lastgroup)

The name of the last matched capturing group, or `None` if the group didnât
have a name, or if no group was matched at all.

Match.re[Â¶](#re.Match.re)
...
Match.string[Â¶](#re.Match.string)

The string passed to [`match()`](#re.Pattern.match) or [`search()`](#re.Pattern.search).

Changed in version 3.7: Added support of [`copy.copy()`](copy.html#copy.copy) and [`copy.deepcopy()`](copy.html#copy.deepcopy).  Match objects
are considered atomic.

## Regular Expression Examples[Â¶](#regular-expression-examples)

### Checking for a Pair[Â¶](#checking-for-a-pair)

In this example, weâll use the following helper function to display match
...
Suppose you are writing a poker program where a playerâs hand is represented as
a 5-character string with each character representing a card, âaâ for ace, âkâ
for king, âqâ for queen, âjâ for jack, âtâ for 10, and â2â through â9â
...
### Simulating scanf()[Â¶](#simulating-scanf)
...
### search() vs. match()[Â¶](#search-vs-match)

Python offers different primitive operations based on regular expressions:

- 
[`re.match()`](#re.match) checks for a match only at the beginning of the string
...
Note however that in [`MULTILINE`](#re.MULTILINE) mode [`match()`](#re.match) only matches at the
...
### Making a Phonebook[Â¶](#making-a-phonebook)
...
### Finding all Adverbs[Â¶](#finding-all-adverbs)
...
### Finding all Adverbs and their Positions[Â¶](#finding-all-adverbs-and-their-positions)
...
### Raw String Notation[Â¶](#raw-string-notation)
...
### Writing a Tokenizer[Â¶](#writing-a-tokenizer)
...
Friedl, Jeffrey. Mastering Regular Expressions. 3rd ed., OâReilly
...
[`difflib` â Helpers for computing deltas](difflib.html)
