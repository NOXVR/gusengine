...
typing â Support for type hints — Python 3.14.3 documentation
...
# `typing` â Support for type hints[Â¶](#typing-support-for-type-hints)
...
be an instance of [`float`](functions.html#float), as indicated by the [type hint](../glossary.html#term-type-hint)
`edge_length: float`. The function is expected to return an instance
of [`str`](stdtypes.html#str), as indicated by the `-> str` hint.

While type hints can be simple classes like [`float`](functions.html#float) or [`str`](stdtypes.html#str),
...
However, it is possible to create a [`NewType`](#typing.NewType) based on a âderivedâ `NewType`:
...
## Annotating callable objects[Â¶](#annotating-callable-objects)

Functions â or other [callable](../glossary.html#term-callable) objects â can be annotated using
[`collections.abc.Callable`](collections.abc.html#collections.abc.Callable) or deprecated [`typing.Callable`](#typing.Callable).
`Callable[[int], str]` signifies a function that takes a single parameter
of type [`int`](functions.html#int) and returns a [`str`](stdtypes.html#str).
...
[`__call__()`](../reference/datamodel.html#object.__call__) method:
...
## Annotating tuples[Â¶](#annotating-tuples)
...
[`list`](stdtypes.html#list) only accepts one type argument, so a type checker would emit an
error on the `y` assignment above. Similarly,
[`Mapping`](collections.abc.html#collections.abc.Mapping) only accepts two type arguments: the first
...
reason, tuples are special-cased in Pythonâs typing system. [`tuple`](stdtypes.html#tuple)
...
## The type of class objects[Â¶](#the-type-of-class-objects)
...
themselves â specifically, it will accept the *class object* of `C`. For
example:
...
The only legal parameters for [`type`](functions.html#type) are classes, [`Any`](#typing.Any),
...
`type[Any]` is equivalent to [`type`](functions.html#type), which is the root of Pythonâs
[metaclass hierarchy](../reference/datamodel.html#metaclasses).

## Annotating generators and coroutines[Â¶](#annotating-generators-and-coroutines)

A generator can be annotated using the generic type
[`Generator[YieldType, SendType, ReturnType]`](collections.abc.html#collections.abc.Generator).
For example:
...
the `SendType` of [`Generator`](collections.abc.html#collections.abc.Generator) behaves
...
[`Iterable[YieldType]`](collections.abc.html#collections.abc.Iterable)
or [`Iterator[YieldType]`](collections.abc.html#collections.abc.Iterator):
...
Async generators are handled in a similar fashion, but donât
expect a `ReturnType` type argument
([`AsyncGenerator[YieldType, SendType]`](collections.abc.html#collections.abc.AsyncGenerator)).
...
[`AsyncIterable[YieldType]`](collections.abc.html#collections.abc.AsyncIterable)
and [`AsyncIterator[YieldType]`](collections.abc.html#collections.abc.AsyncIterator) are
available as well:
...
[`Coroutine[YieldType, SendType, ReturnType]`](collections.abc.html#collections.abc.Coroutine).
Generic arguments correspond to those of [`Generator`](collections.abc.html#collections.abc.Generator),
for example:
...
## User-defined generic types[Â¶](#user-defined-generic-types)
...
Generic classes have [`__class_getitem__()`](../reference/datamodel.html#object.__class_getitem__) methods, meaning they
...
with type variablesâ described above as parameter specification variables are
...
## The [`Any`](#typing.Any) type[Â¶](#the-any-type)
...
declared to be of type [`str`](stdtypes.html#str) and receives an [`int`](functions.html#int) value at
runtime!
...
Contrast the behavior of [`Any`](#typing.Any) with the behavior of [`object`](functions.html#object).
Similar to [`Any`](#typing.Any), every type is a subtype of [`object`](functions.html#object). However,
unlike [`Any`](#typing.Any), the reverse is not true: [`object`](functions.html#object) is *not* a
subtype of every other type.

That means when the type of a value is [`object`](functions.html#object), a type checker will
...
Use [`object`](functions.html#object) to indicate that a value could be any type in a typesafe
manner. Use [`Any`](#typing.Any) to indicate that a value is dynamically typed.

## Nominal vs structural subtyping[Â¶](#nominal-vs-structural-subtyping)
...
[`Iterable`](collections.abc.html#collections.abc.Iterable). The problem with this approach is that a class had
...
### Special typing primitives[Â¶](#special-typing-primitives)

#### Special types[Â¶](#special-types)

These can be used as types in annotations. They do not support subscription
using `[]`.

typing.Any[Â¶](#typing.Any)
...
`AnyStr` is meant to be used for functions that may accept [`str`](stdtypes.html#str) or
[`bytes`](stdtypes.html#bytes) arguments but cannot allow the two to mix.
...
[`Any`](#typing.Any) type, nor does it mean âany stringâ. In particular, `AnyStr`
...
typing.LiteralString[Â¶](#typing.LiteralString)
...
typing.Never[Â¶](#typing.Never)

typing.NoReturn[Â¶](#typing.NoReturn)
...
such as [`sys.exit()`](sys.html#sys.exit):
...
- 
[`classmethod`](functions.html#classmethod)s that are used as alternative constructors and return instances
of the `cls` parameter.

- 
Annotating an [`__enter__()`](../reference/datamodel.html#object.__enter__) method which returns self.
...
typing.TypeAlias[Â¶](#typing.TypeAlias)
...
Deprecated since version 3.12: [`TypeAlias`](#typing.TypeAlias) is deprecated in favor of the [`type`](../reference/simple_stmts.html#type) statement,
...
Changed in version 3.7: Donât remove explicit subclasses from unions at runtime.

Changed in version 3.10: Unions can now be written as `X | Y`. See
[union type expressions](stdtypes.html#types-union).

Changed in version 3.14: [`types.UnionType`](types.html#types.UnionType) is now an alias for [`Union`](#typing.Union), and both
...
typing.Optional[Â¶](#typing.Optional)
...
typing.Concatenate[Â¶](#typing.Concatenate)
...
[`threading.Lock`](threading.html#threading.Lock) to the decorated function,  `Concatenate` can be
...
callableâs parameter types are dependent on the parameter types of the
...
- 
[**PEP 612**](https://peps.python.org/pep-0612/) â Parameter Specification Variables (the PEP which introduced
...
typing.Literal[Â¶](#typing.Literal)

Special typing form to define âliteral typesâ.
...
will now raise a [`TypeError`](exceptions.html#TypeError) exception during equality comparisons
if one of their parameters are not [hashable](../glossary.html#term-hashable).

typing.ClassVar[Â¶](#typing.ClassVar)
...
be used with [`isinstance()`](functions.html#isinstance) or [`issubclass()`](functions.html#issubclass).
...
typing.Required[Â¶](#typing.Required)
...
typing.NotRequired[Â¶](#typing.NotRequired)
...
typing.ReadOnly[Â¶](#typing.ReadOnly)
...
typing.Annotated[Â¶](#typing.Annotated)
...
that wants to use annotations for purposes outside Pythonâs static typing
system.
...
(e.g., using [`isinstance()`](functions.html#isinstance)).
...
where `T1`, `T2`, â¦ are [`TypeVars`](#typing.TypeVar). This is invalid as
...
`TypeIs` aims to benefit *type narrowing* â a technique used by static
type checkers to determine a more precise type of an expression within a
programâs code flow.  Usually type narrowing is done by analyzing
conditional code flow and applying the narrowing to a block of code.  The
conditional expression here is sometimes referred to as a âtype predicateâ:
...
is the intersection of the argumentâs original type and `NarrowedType`.

- 
If the return value is `False`, the type of its argument
...
functionâs argument; if it is not, static type checkers will raise
an error.  An incorrectly written `TypeIs` function can lead to
unsound behavior in the type system; it is the userâs responsibility
...
typing.TypeGuard[Â¶](#typing.TypeGuard)
...
[`builtins.tuple`](stdtypes.html#tuple) types. You might see `Unpack` being used
explicitly in older versions of Python, where `*` couldnât be used in
certain places:
...
#### Building generic types and type aliases[Â¶](#building-generic-types-and-type-aliases)
...
class typing.Generic[Â¶](#typing.Generic)
...
class typing.TypeVar(name, *constraints, bound=None, covariant=False, contravariant=False, infer_variance=False, default=typing.NoDefault)[Â¶](#typing.TypeVar)
...
__name__[Â¶](#typing.TypeVar.__name__)

The name of the type variable.

__covariant__[Â¶](#typing.TypeVar.__covariant__)

Whether the type var has been explicitly marked as covariant.

__contravariant__[Â¶](#typing.TypeVar.__contravariant__)

Whether the type var has been explicitly marked as contravariant.

__infer_variance__[Â¶](#typing.TypeVar.__infer_variance__)

Whether the type variableâs variance should be inferred by type checkers.

Added in version 3.12.

__bound__[Â¶](#typing.TypeVar.__bound__)
...
evaluate_bound()[Â¶](#typing.TypeVar.evaluate_bound)

An [evaluate function](../glossary.html#term-evaluate-function) corresponding to the [`__bound__`](#typing.TypeVar.__bound__) attribute.
When called directly, this method supports only the [`VALUE`](annotationlib.html#annotationlib.Format.VALUE)
format, which is equivalent to accessing the [`__bound__`](#typing.TypeVar.__bound__) attribute directly,
but the method object can be passed to [`annotationlib.call_evaluate_function()`](annotationlib.html#annotationlib.call_evaluate_function)
to evaluate the value in a different format.

Added in version 3.14.

__constraints__[Â¶](#typing.TypeVar.__constraints__)
...
evaluate_constraints()[Â¶](#typing.TypeVar.evaluate_constraints)

An [evaluate function](../glossary.html#term-evaluate-function) corresponding to the [`__constraints__`](#typing.TypeVar.__constraints__) attribute.
When called directly, this method supports only the [`VALUE`](annotationlib.html#annotationlib.Format.VALUE)
...
__default__[Â¶](#typing.TypeVar.__default__)
...
evaluate_default()[Â¶](#typing.TypeVar.evaluate_default)

An [evaluate function](../glossary.html#term-evaluate-function) corresponding to the [`__default__`](#typing.TypeVar.__default__) attribute.
When called directly, this method supports only the [`VALUE`](annotationlib.html#annotationlib.Format.VALUE)
...
has_default()[Â¶](#typing.TypeVar.has_default)
...
class typing.TypeVarTuple(name, *, default=typing.NoDefault)[Â¶](#typing.TypeVarTuple)
...
__name__[Â¶](#typing.TypeVarTuple.__name__)

The name of the type variable tuple.

__default__[Â¶](#typing.TypeVarTuple.__default__)
...
evaluate_default()[Â¶](#typing.TypeVarTuple.evaluate_default)

An [evaluate function](../glossary.html#term-evaluate-function) corresponding to the [`__default__`](#typing.TypeVarTuple.__default__) attribute.
When called directly, this method supports only the [`VALUE`](annotationlib.html#annotationlib.Format.VALUE)
...
has_default()[Â¶](#typing.TypeVarTuple.has_default)
...
class typing.ParamSpec(name, *, bound=None, covariant=False, contravariant=False, default=typing.NoDefault)[Â¶](#typing.ParamSpec)
...
callable to another callable â a pattern commonly found in higher order
...
- 
The type checker canât type check the `inner` function because
`*args` and `**kwargs` have to be typed [`Any`](#typing.Any).

- 
[`cast()`](#typing.cast) may be required in the body of the `add_logging`
decorator when returning the `inner` function, or the static type
checker must be told to ignore the `return inner`.

args[Â¶](#typing.ParamSpec.args)

kwargs[Â¶](#typing.ParamSpec.kwargs)
...
__name__[Â¶](#typing.ParamSpec.__name__)

The name of the parameter specification.

__default__[Â¶](#typing.ParamSpec.__default__)
...
evaluate_default()[Â¶](#typing.ParamSpec.evaluate_default)

An [evaluate function](../glossary.html#term-evaluate-function) corresponding to the [`__default__`](#typing.ParamSpec.__default__) attribute.
When called directly, this method supports only the [`VALUE`](annotationlib.html#annotationlib.Format.VALUE)
...
has_default()[Â¶](#typing.ParamSpec.has_default)
...
typing.ParamSpecArgs[Â¶](#typing.ParamSpecArgs)

typing.ParamSpecKwargs[Â¶](#typing.ParamSpecKwargs)
...
class typing.TypeAliasType(name, value, *, type_params=())[Â¶](#typing.TypeAliasType)
...
__name__[Â¶](#typing.TypeAliasType.__name__)
...
__module__[Â¶](#typing.TypeAliasType.__module__)
...
__type_params__[Â¶](#typing.TypeAliasType.__type_params__)
...
__value__[Â¶](#typing.TypeAliasType.__value__)

The type aliasâs value. This is [lazily evaluated](../reference/executionmodel.html#lazy-evaluation),
...
evaluate_value()[Â¶](#typing.TypeAliasType.evaluate_value)

An [evaluate function](../glossary.html#term-evaluate-function) corresponding to the [`__value__`](#typing.TypeAliasType.__value__) attribute.
When called directly, this method supports only the [`VALUE`](annotationlib.html#annotationlib.Format.VALUE)
...
#### Other special directives[Â¶](#other-special-directives)

These functions and classes should not be used directly as annotations.
Their intended purpose is to be building blocks for creating and declaring
types.

class typing.NamedTuple[Â¶](#typing.NamedTuple)

Typed version of [`collections.namedtuple()`](collections.html#collections.namedtuple).
...
`_field_defaults` attribute, both of which are part of the [`namedtuple()`](collections.html#collections.namedtuple)
API.)
...
Changed in version 3.14: Using [`super()`](functions.html#super) (and the `__class__` [closure variable](../glossary.html#term-closure-variable)) in methods of `NamedTuple` subclasses
is unsupported and causes a [`TypeError`](exceptions.html#TypeError).
...
pass a value to the âfieldsâ parameter (`NT = NamedTuple("NT")`) is
deprecated. Passing `None` to the âfieldsâ parameter
(`NT = NamedTuple("NT", None)`) is also deprecated. Both will be
disallowed in Python 3.15. To create a NamedTuple class with 0 fields,
use `class NT(NamedTuple): pass` or `NT = NamedTuple("NT", [])`.

class typing.NewType(name, tp)[Â¶](#typing.NewType)
...
__module__[Â¶](#typing.NewType.__module__)

The name of the module in which the new type is defined.

__name__[Â¶](#typing.NewType.__name__)

The name of the new type.

__supertype__[Â¶](#typing.NewType.__supertype__)
...
class typing.Protocol(Generic)[Â¶](#typing.Protocol)
...
as the second argument to [`isinstance()`](functions.html#isinstance) or [`issubclass()`](functions.html#issubclass).
...
@typing.runtime_checkable[Â¶](#typing.runtime_checkable)

Mark a protocol class as a runtime protocol.

Such a protocol can be used with [`isinstance()`](functions.html#isinstance) and [`issubclass()`](functions.html#issubclass).
This allows a simple-minded structural check, very similar to âone trick poniesâ
in [`collections.abc`](collections.abc.html#module-collections.abc) such as [`Iterable`](collections.abc.html#collections.abc.Iterable).  For example:
...
This decorator raises [`TypeError`](exceptions.html#TypeError) when applied to a non-protocol class.

Note

`runtime_checkable()` will check only the presence of the required
methods or attributes, not their type signatures or types.
For example, [`ssl.SSLObject`](ssl.html#ssl.SSLObject)
is a class, therefore it passes an [`issubclass()`](functions.html#issubclass)
check against [Callable](#annotating-callables). However, the
`ssl.SSLObject.__init__` method exists only to raise a
[`TypeError`](exceptions.html#TypeError) with a more informative message, therefore making
it impossible to call (instantiate) [`ssl.SSLObject`](ssl.html#ssl.SSLObject).

Note

An [`isinstance()`](functions.html#isinstance) check against a runtime-checkable protocol can be
surprisingly slow compared to an `isinstance()` check against
a non-protocol class. Consider using alternative idioms such as
[`hasattr()`](functions.html#hasattr) calls for structural checks in performance-sensitive
code.

Added in version 3.8.

Changed in version 3.12: The internal implementation of [`isinstance()`](functions.html#isinstance) checks against
runtime-checkable protocols now uses [`inspect.getattr_static()`](inspect.html#inspect.getattr_static)
to look up attributes (previously, [`hasattr()`](functions.html#hasattr) was used).
...
Changed in version 3.12: The members of a runtime-checkable protocol are now considered âfrozenâ
...
have no impact on [`isinstance()`](functions.html#isinstance) checks comparing objects to the
protocol. See [Whatâs new in Python 3.12](../whatsnew/3.12.html#whatsnew-typing-py312)
for more details.

class typing.TypedDict(dict)[Â¶](#typing.TypedDict)

Special construct to add type hints to a dictionary.
At runtime â`TypedDict` instancesâ are simply [`dicts`](stdtypes.html#dict).
...
function-call syntax. The second argument must be a literal [`dict`](stdtypes.html#dict):
...
__total__[Â¶](#typing.TypedDict.__total__)
...
__required_keys__[Â¶](#typing.TypedDict.__required_keys__)

Added in version 3.9.

__optional_keys__[Â¶](#typing.TypedDict.__optional_keys__)

`Point2D.__required_keys__` and `Point2D.__optional_keys__` return
[`frozenset`](stdtypes.html#frozenset) objects containing required and non-required keys, respectively.
...
__readonly_keys__[Â¶](#typing.TypedDict.__readonly_keys__)

A [`frozenset`](stdtypes.html#frozenset) containing the names of all read-only keys. Keys
are read-only if they carry the [`ReadOnly`](#typing.ReadOnly) qualifier.

Added in version 3.13.

__mutable_keys__[Â¶](#typing.TypedDict.__mutable_keys__)

A [`frozenset`](stdtypes.html#frozenset) containing the names of all mutable keys. Keys
...
pass a value to the âfieldsâ parameter (`TD = TypedDict("TD")`) is
deprecated. Passing `None` to the âfieldsâ parameter
...
class typing.SupportsAbs[Â¶](#typing.SupportsAbs)

An ABC with one abstract method `__abs__` that is covariant
in its return type.

class typing.SupportsBytes[Â¶](#typing.SupportsBytes)

An ABC with one abstract method `__bytes__`.

class typing.SupportsComplex[Â¶](#typing.SupportsComplex)

An ABC with one abstract method `__complex__`.

class typing.SupportsFloat[Â¶](#typing.SupportsFloat)

An ABC with one abstract method `__float__`.

class typing.SupportsIndex[Â¶](#typing.SupportsIndex)

An ABC with one abstract method `__index__`.

Added in version 3.8.

class typing.SupportsInt[Â¶](#typing.SupportsInt)

An ABC with one abstract method `__int__`.

class typing.SupportsRound[Â¶](#typing.SupportsRound)

An ABC with one abstract method `__round__`
that is covariant in its return type.

### ABCs and Protocols for working with I/O[Â¶](#abcs-and-protocols-for-working-with-i-o)

class typing.IO[AnyStr][Â¶](#typing.IO)

class typing.TextIO[Â¶](#typing.TextIO)

class typing.BinaryIO[Â¶](#typing.BinaryIO)

Generic class `IO[AnyStr]` and its subclasses `TextIO(IO[str])`
and `BinaryIO(IO[bytes])`
represent the types of I/O streams such as returned by
[`open()`](functions.html#open). Please note that these classes are not protocols, and
their interface is fairly broad.

The protocols [`io.Reader`](io.html#io.Reader) and [`io.Writer`](io.html#io.Writer) offer a simpler
...
Also consider using [`collections.abc.Iterable`](collections.abc.html#collections.abc.Iterable) for iterating over
...
### Functions and decorators[Â¶](#functions-and-decorators)

typing.cast(typ, val)[Â¶](#typing.cast)

Cast a value to a type.

This returns the value unchanged.  To the type checker this
signals that the return value has the designated type, but at
runtime we intentionally donât check anything (we want this
to be as fast as possible).

typing.assert_type(val, typ, /)[Â¶](#typing.assert_type)
...
This function is useful for ensuring the type checkerâs understanding of a
script is in line with the developerâs intentions:
...
typing.assert_never(arg, /)[Â¶](#typing.assert_never)
...
an [`int`](functions.html#int) or a [`str`](stdtypes.html#str), and both options are covered by
earlier cases.
...
emit an error pointing out that `unreachable` is of type [`float`](functions.html#float).
...
typing.reveal_type(obj, /)[Â¶](#typing.reveal_type)
...
[`sys.stderr`](sys.html#sys.stderr) and returns the argument unchanged (allowing the call to
...
@typing.dataclass_transform(*, eq_default=True, order_default=False, kw_only_default=False, frozen_default=False, field_specifiers=(), **kwargs)[Â¶](#typing.dataclass_transform)

Decorator to mark an object as providing
[`dataclass`](dataclasses.html#dataclasses.dataclass)-like behavior.

`dataclass_transform` may be used to
decorate a class, metaclass, or a function that is itself a decorator.
The presence of `@dataclass_transform()` tells a static type checker that the
decorated object performs runtime âmagicâ that
transforms a class in a similar way to
[`@dataclasses.dataclass`](dataclasses.html#dataclasses.dataclass).
...
would have on the
[`@dataclasses.dataclass`](dataclasses.html#dataclasses.dataclass) decorator: `init`,
...
- 
**eq_default** ([*bool*](functions.html#bool)) â Indicates whether the `eq` parameter is assumed to be
`True` or `False` if it is omitted by the caller.
Defaults to `True`.

- 
**order_default** ([*bool*](functions.html#bool)) â Indicates whether the `order` parameter is
assumed to be `True` or `False` if it is omitted by the caller.
Defaults to `False`.

- 
**kw_only_default** ([*bool*](functions.html#bool)) â Indicates whether the `kw_only` parameter is
assumed to be `True` or `False` if it is omitted by the caller.
Defaults to `False`.

- 
**frozen_default** ([*bool*](functions.html#bool)) â 
...
- 
**field_specifiers** ([*tuple*](stdtypes.html#tuple)*[*[*Callable*](collections.abc.html#collections.abc.Callable)*[**...**, **Any**]**, **...**]*) â Specifies a static list of supported classes
or functions that describe fields, similar to [`dataclasses.field()`](dataclasses.html#dataclasses.field).
Defaults to `()`.

- 
****kwargs** (*Any*) â Arbitrary other keyword arguments are accepted in order to allow for
possible future extensions.

Type checkers recognize the following optional parameters on field
specifiers:

**Recognised parameters for field specifiers**[Â¶](#id7)
...
@typing.overload[Â¶](#typing.overload)
...
typing.get_overloads(func)[Â¶](#typing.get_overloads)
...
typing.clear_overloads()[Â¶](#typing.clear_overloads)
...
@typing.no_type_check[Â¶](#typing.no_type_check)
...
@typing.no_type_check_decorator[Â¶](#typing.no_type_check_decorator)

Decorator to give another decorator the [`no_type_check()`](#typing.no_type_check) effect.

This wraps the decorator with something that wraps the decorated
function in [`no_type_check()`](#typing.no_type_check).
...
@typing.override[Â¶](#typing.override)
...
@typing.type_check_only[Â¶](#typing.type_check_only)
...
### Introspection helpers[Â¶](#introspection-helpers)

typing.get_type_hints(obj, globalns=None, localns=None, include_extras=False)[Â¶](#typing.get_type_hints)
...
(where applicable) *obj*âs [type parameter](../reference/compound_stmts.html#type-params) namespace.
If *globalns* or *localns* is not given, appropriate namespace
dictionaries are inferred from *obj*.

- 
`None` is replaced with [`types.NoneType`](types.html#types.NoneType).
...
annotations from `C`âs base classes with those on `C` directly. This
is done by traversing [`C.__mro__`](../reference/datamodel.html#type.__mro__) and iteratively
combining
...
See also [`annotationlib.get_annotations()`](annotationlib.html#annotationlib.get_annotations), a lower-level function that
...
such as [`NameError`](exceptions.html#NameError). For example, this can happen with imported
[type aliases](#type-aliases) that include forward references,
or with names imported under [`if TYPE_CHECKING`](#typing.TYPE_CHECKING).
...
typing.get_origin(tp)[Â¶](#typing.get_origin)

Get the unsubscripted version of a type: for a typing object of the form
`X[Y, Z, ...]` return `X`.

If `X` is a typing-module alias for a builtin or
[`collections`](collections.html#module-collections) class, it will be normalized to the original class.
...
typing.get_args(tp)[Â¶](#typing.get_args)
...
typing.get_protocol_members(tp)[Â¶](#typing.get_protocol_members)
...
Raise [`TypeError`](exceptions.html#TypeError) for arguments that are not Protocols.

Added in version 3.13.

typing.is_protocol(tp)[Â¶](#typing.is_protocol)
...
typing.is_typeddict(tp)[Â¶](#typing.is_typeddict)
...
class typing.ForwardRef[Â¶](#typing.ForwardRef)
...
Changed in version 3.14: This is now an alias for [`annotationlib.ForwardRef`](annotationlib.html#annotationlib.ForwardRef). Several undocumented
...
typing.evaluate_forward_ref(forward_ref, *, owner=None, globals=None, locals=None, type_params=None, format=annotationlib.Format.VALUE)[Â¶](#typing.evaluate_forward_ref)

Evaluate an [`annotationlib.ForwardRef`](annotationlib.html#annotationlib.ForwardRef) as a [type hint](../glossary.html#term-type-hint).

This is similar to calling [`annotationlib.ForwardRef.evaluate()`](annotationlib.html#annotationlib.ForwardRef.evaluate),
but unlike that method, `evaluate_forward_ref()` also
recursively evaluates forward references nested within the type hint.

See the documentation for [`annotationlib.ForwardRef.evaluate()`](annotationlib.html#annotationlib.ForwardRef.evaluate) for
...
typing.NoDefault[Â¶](#typing.NoDefault)
...
typing.TYPE_CHECKING[Â¶](#typing.TYPE_CHECKING)

A special constant that is assumed to be `True` by 3rd party static
type checkers. Itâs `False` at runtime.
...
being imported at runtime; annotations arenât eagerly evaluated
(see [**PEP 649**](https://peps.python.org/pep-0649/)) so using undefined symbols in annotations is
harmlessâas long as you donât later examine them.
...
[`annotationlib.get_annotations()`](annotationlib.html#annotationlib.get_annotations) with a `format` parameter
of [`annotationlib.Format.STRING`](annotationlib.html#annotationlib.Format.STRING) or
[`annotationlib.Format.FORWARDREF`](annotationlib.html#annotationlib.Format.FORWARDREF) to safely retrieve the
annotations without raising [`NameError`](exceptions.html#NameError).

Added in version 3.5.2.

### Deprecated aliases[Â¶](#deprecated-aliases)
...
#### Aliases to built-in types[Â¶](#aliases-to-built-in-types)

class typing.Dict(dict, MutableMapping[KT, VT])[Â¶](#typing.Dict)

Deprecated alias to [`dict`](stdtypes.html#dict).

Note that to annotate arguments, it is preferred
to use an abstract collection type such as [`Mapping`](collections.abc.html#collections.abc.Mapping)
rather than to use [`dict`](stdtypes.html#dict) or `typing.Dict`.

Deprecated since version 3.9: [`builtins.dict`](stdtypes.html#dict) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.List(list, MutableSequence[T])[Â¶](#typing.List)

Deprecated alias to [`list`](stdtypes.html#list).

Note that to annotate arguments, it is preferred
to use an abstract collection type such as
[`Sequence`](collections.abc.html#collections.abc.Sequence) or [`Iterable`](collections.abc.html#collections.abc.Iterable)
rather than to use [`list`](stdtypes.html#list) or `typing.List`.

Deprecated since version 3.9: [`builtins.list`](stdtypes.html#list) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Set(set, MutableSet[T])[Â¶](#typing.Set)

Deprecated alias to [`builtins.set`](stdtypes.html#set).

Note that to annotate arguments, it is preferred
to use an abstract collection type such as [`collections.abc.Set`](collections.abc.html#collections.abc.Set)
rather than to use [`set`](stdtypes.html#set) or [`typing.Set`](#typing.Set).

Deprecated since version 3.9: [`builtins.set`](stdtypes.html#set) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.FrozenSet(frozenset, AbstractSet[T_co])[Â¶](#typing.FrozenSet)

Deprecated alias to [`builtins.frozenset`](stdtypes.html#frozenset).

Deprecated since version 3.9: [`builtins.frozenset`](stdtypes.html#frozenset)
now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

typing.Tuple[Â¶](#typing.Tuple)

Deprecated alias for [`tuple`](stdtypes.html#tuple).

[`tuple`](stdtypes.html#tuple) and `Tuple` are special-cased in the type system; see
[Annotating tuples](#annotating-tuples) for more details.

Deprecated since version 3.9: [`builtins.tuple`](stdtypes.html#tuple) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Type(Generic[CT_co])[Â¶](#typing.Type)

Deprecated alias to [`type`](functions.html#type).

See [The type of class objects](#type-of-class-objects) for details on using [`type`](functions.html#type) or
`typing.Type` in type annotations.

Added in version 3.5.2.

Deprecated since version 3.9: [`builtins.type`](functions.html#type) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

#### Aliases to types in [`collections`](collections.html#module-collections)[Â¶](#aliases-to-types-in-collections)

class typing.DefaultDict(collections.defaultdict, MutableMapping[KT, VT])[Â¶](#typing.DefaultDict)

Deprecated alias to [`collections.defaultdict`](collections.html#collections.defaultdict).

Added in version 3.5.2.

Deprecated since version 3.9: [`collections.defaultdict`](collections.html#collections.defaultdict) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.OrderedDict(collections.OrderedDict, MutableMapping[KT, VT])[Â¶](#typing.OrderedDict)

Deprecated alias to [`collections.OrderedDict`](collections.html#collections.OrderedDict).

Added in version 3.7.2.

Deprecated since version 3.9: [`collections.OrderedDict`](collections.html#collections.OrderedDict) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.ChainMap(collections.ChainMap, MutableMapping[KT, VT])[Â¶](#typing.ChainMap)

Deprecated alias to [`collections.ChainMap`](collections.html#collections.ChainMap).

Added in version 3.6.1.

Deprecated since version 3.9: [`collections.ChainMap`](collections.html#collections.ChainMap) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Counter(collections.Counter, Dict[T, int])[Â¶](#typing.Counter)

Deprecated alias to [`collections.Counter`](collections.html#collections.Counter).

Added in version 3.6.1.

Deprecated since version 3.9: [`collections.Counter`](collections.html#collections.Counter) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Deque(deque, MutableSequence[T])[Â¶](#typing.Deque)

Deprecated alias to [`collections.deque`](collections.html#collections.deque).

Added in version 3.6.1.

Deprecated since version 3.9: [`collections.deque`](collections.html#collections.deque) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

#### Aliases to other concrete types[Â¶](#aliases-to-other-concrete-types)

class typing.Pattern[Â¶](#typing.Pattern)

class typing.Match[Â¶](#typing.Match)

Deprecated aliases corresponding to the return types from
[`re.compile()`](re.html#re.compile) and [`re.match()`](re.html#re.match).
...
Deprecated since version 3.9: Classes `Pattern` and `Match` from [`re`](re.html#module-re) now support `[]`.
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Text[Â¶](#typing.Text)

Deprecated alias for [`str`](stdtypes.html#str).
...
[`str`](stdtypes.html#str) instead of `Text`.

#### Aliases to container ABCs in [`collections.abc`](collections.abc.html#module-collections.abc)[Â¶](#aliases-to-container-abcs-in-collections-abc)

class typing.AbstractSet(Collection[T_co])[Â¶](#typing.AbstractSet)

Deprecated alias to [`collections.abc.Set`](collections.abc.html#collections.abc.Set).

Deprecated since version 3.9: [`collections.abc.Set`](collections.abc.html#collections.abc.Set) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.ByteString(Sequence[int])[Â¶](#typing.ByteString)

Deprecated alias to [`collections.abc.ByteString`](collections.abc.html#collections.abc.ByteString).

Use `isinstance(obj, collections.abc.Buffer)` to test if `obj`
implements the [buffer protocol](../c-api/buffer.html#bufferobjects) at runtime. For use in
type annotations, either use [`Buffer`](collections.abc.html#collections.abc.Buffer) or a union
...
would serve as a supertype of both [`bytes`](stdtypes.html#bytes) and [`bytearray`](stdtypes.html#bytearray).
However, since the ABC never had any methods, knowing that an object was an
instance of `ByteString` never actually told you anything useful
about the object. Other common buffer types such as [`memoryview`](stdtypes.html#memoryview) were
...
class typing.Collection(Sized, Iterable[T_co], Container[T_co])[Â¶](#typing.Collection)

Deprecated alias to [`collections.abc.Collection`](collections.abc.html#collections.abc.Collection).

Added in version 3.6.

Deprecated since version 3.9: [`collections.abc.Collection`](collections.abc.html#collections.abc.Collection) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Container(Generic[T_co])[Â¶](#typing.Container)

Deprecated alias to [`collections.abc.Container`](collections.abc.html#collections.abc.Container).

Deprecated since version 3.9: [`collections.abc.Container`](collections.abc.html#collections.abc.Container) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.ItemsView(MappingView, AbstractSet[tuple[KT_co, VT_co]])[Â¶](#typing.ItemsView)

Deprecated alias to [`collections.abc.ItemsView`](collections.abc.html#collections.abc.ItemsView).

Deprecated since version 3.9: [`collections.abc.ItemsView`](collections.abc.html#collections.abc.ItemsView) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.KeysView(MappingView, AbstractSet[KT_co])[Â¶](#typing.KeysView)

Deprecated alias to [`collections.abc.KeysView`](collections.abc.html#collections.abc.KeysView).

Deprecated since version 3.9: [`collections.abc.KeysView`](collections.abc.html#collections.abc.KeysView) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Mapping(Collection[KT], Generic[KT, VT_co])[Â¶](#typing.Mapping)

Deprecated alias to [`collections.abc.Mapping`](collections.abc.html#collections.abc.Mapping).

Deprecated since version 3.9: [`collections.abc.Mapping`](collections.abc.html#collections.abc.Mapping) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.MappingView(Sized)[Â¶](#typing.MappingView)

Deprecated alias to [`collections.abc.MappingView`](collections.abc.html#collections.abc.MappingView).

Deprecated since version 3.9: [`collections.abc.MappingView`](collections.abc.html#collections.abc.MappingView) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.MutableMapping(Mapping[KT, VT])[Â¶](#typing.MutableMapping)

Deprecated alias to [`collections.abc.MutableMapping`](collections.abc.html#collections.abc.MutableMapping).

Deprecated since version 3.9: [`collections.abc.MutableMapping`](collections.abc.html#collections.abc.MutableMapping)
now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.MutableSequence(Sequence[T])[Â¶](#typing.MutableSequence)

Deprecated alias to [`collections.abc.MutableSequence`](collections.abc.html#collections.abc.MutableSequence).

Deprecated since version 3.9: [`collections.abc.MutableSequence`](collections.abc.html#collections.abc.MutableSequence)
now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.MutableSet(AbstractSet[T])[Â¶](#typing.MutableSet)

Deprecated alias to [`collections.abc.MutableSet`](collections.abc.html#collections.abc.MutableSet).

Deprecated since version 3.9: [`collections.abc.MutableSet`](collections.abc.html#collections.abc.MutableSet) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Sequence(Reversible[T_co], Collection[T_co])[Â¶](#typing.Sequence)

Deprecated alias to [`collections.abc.Sequence`](collections.abc.html#collections.abc.Sequence).

Deprecated since version 3.9: [`collections.abc.Sequence`](collections.abc.html#collections.abc.Sequence) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.ValuesView(MappingView, Collection[_VT_co])[Â¶](#typing.ValuesView)

Deprecated alias to [`collections.abc.ValuesView`](collections.abc.html#collections.abc.ValuesView).

Deprecated since version 3.9: [`collections.abc.ValuesView`](collections.abc.html#collections.abc.ValuesView) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

#### Aliases to asynchronous ABCs in [`collections.abc`](collections.abc.html#module-collections.abc)[Â¶](#aliases-to-asynchronous-abcs-in-collections-abc)

class typing.Coroutine(Awaitable[ReturnType], Generic[YieldType, SendType, ReturnType])[Â¶](#typing.Coroutine)

Deprecated alias to [`collections.abc.Coroutine`](collections.abc.html#collections.abc.Coroutine).

See [Annotating generators and coroutines](#annotating-generators-and-coroutines)
for details on using [`collections.abc.Coroutine`](collections.abc.html#collections.abc.Coroutine)
and `typing.Coroutine` in type annotations.

Added in version 3.5.3.

Deprecated since version 3.9: [`collections.abc.Coroutine`](collections.abc.html#collections.abc.Coroutine) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.AsyncGenerator(AsyncIterator[YieldType], Generic[YieldType, SendType])[Â¶](#typing.AsyncGenerator)

Deprecated alias to [`collections.abc.AsyncGenerator`](collections.abc.html#collections.abc.AsyncGenerator).

See [Annotating generators and coroutines](#annotating-generators-and-coroutines)
for details on using [`collections.abc.AsyncGenerator`](collections.abc.html#collections.abc.AsyncGenerator)
and `typing.AsyncGenerator` in type annotations.

Added in version 3.6.1.

Deprecated since version 3.9: [`collections.abc.AsyncGenerator`](collections.abc.html#collections.abc.AsyncGenerator)
now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

Changed in version 3.13: The `SendType` parameter now has a default.

class typing.AsyncIterable(Generic[T_co])[Â¶](#typing.AsyncIterable)

Deprecated alias to [`collections.abc.AsyncIterable`](collections.abc.html#collections.abc.AsyncIterable).

Added in version 3.5.2.

Deprecated since version 3.9: [`collections.abc.AsyncIterable`](collections.abc.html#collections.abc.AsyncIterable) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.AsyncIterator(AsyncIterable[T_co])[Â¶](#typing.AsyncIterator)

Deprecated alias to [`collections.abc.AsyncIterator`](collections.abc.html#collections.abc.AsyncIterator).

Added in version 3.5.2.

Deprecated since version 3.9: [`collections.abc.AsyncIterator`](collections.abc.html#collections.abc.AsyncIterator) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Awaitable(Generic[T_co])[Â¶](#typing.Awaitable)

Deprecated alias to [`collections.abc.Awaitable`](collections.abc.html#collections.abc.Awaitable).

Added in version 3.5.2.

Deprecated since version 3.9: [`collections.abc.Awaitable`](collections.abc.html#collections.abc.Awaitable) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

#### Aliases to other ABCs in [`collections.abc`](collections.abc.html#module-collections.abc)[Â¶](#aliases-to-other-abcs-in-collections-abc)

class typing.Iterable(Generic[T_co])[Â¶](#typing.Iterable)

Deprecated alias to [`collections.abc.Iterable`](collections.abc.html#collections.abc.Iterable).

Deprecated since version 3.9: [`collections.abc.Iterable`](collections.abc.html#collections.abc.Iterable) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Iterator(Iterable[T_co])[Â¶](#typing.Iterator)

Deprecated alias to [`collections.abc.Iterator`](collections.abc.html#collections.abc.Iterator).

Deprecated since version 3.9: [`collections.abc.Iterator`](collections.abc.html#collections.abc.Iterator) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

typing.Callable[Â¶](#typing.Callable)

Deprecated alias to [`collections.abc.Callable`](collections.abc.html#collections.abc.Callable).

See [Annotating callable objects](#annotating-callables) for details on how to use
[`collections.abc.Callable`](collections.abc.html#collections.abc.Callable) and `typing.Callable` in type annotations.

Deprecated since version 3.9: [`collections.abc.Callable`](collections.abc.html#collections.abc.Callable) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

Changed in version 3.10: `Callable` now supports [`ParamSpec`](#typing.ParamSpec) and [`Concatenate`](#typing.Concatenate).
See [**PEP 612**](https://peps.python.org/pep-0612/) for more details.

class typing.Generator(Iterator[YieldType], Generic[YieldType, SendType, ReturnType])[Â¶](#typing.Generator)

Deprecated alias to [`collections.abc.Generator`](collections.abc.html#collections.abc.Generator).

See [Annotating generators and coroutines](#annotating-generators-and-coroutines)
for details on using [`collections.abc.Generator`](collections.abc.html#collections.abc.Generator)
and `typing.Generator` in type annotations.

Deprecated since version 3.9: [`collections.abc.Generator`](collections.abc.html#collections.abc.Generator) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

Changed in version 3.13: Default values for the send and return types were added.

class typing.Hashable[Â¶](#typing.Hashable)

Deprecated alias to [`collections.abc.Hashable`](collections.abc.html#collections.abc.Hashable).

Deprecated since version 3.12: Use [`collections.abc.Hashable`](collections.abc.html#collections.abc.Hashable) directly instead.

class typing.Reversible(Iterable[T_co])[Â¶](#typing.Reversible)

Deprecated alias to [`collections.abc.Reversible`](collections.abc.html#collections.abc.Reversible).

Deprecated since version 3.9: [`collections.abc.Reversible`](collections.abc.html#collections.abc.Reversible) now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

class typing.Sized[Â¶](#typing.Sized)

Deprecated alias to [`collections.abc.Sized`](collections.abc.html#collections.abc.Sized).

Deprecated since version 3.12: Use [`collections.abc.Sized`](collections.abc.html#collections.abc.Sized) directly instead.

#### Aliases to [`contextlib`](contextlib.html#module-contextlib) ABCs[Â¶](#aliases-to-contextlib-abcs)

class typing.ContextManager(Generic[T_co, ExitT_co])[Â¶](#typing.ContextManager)

Deprecated alias to [`contextlib.AbstractContextManager`](contextlib.html#contextlib.AbstractContextManager).

The first type parameter, `T_co`, represents the type returned by
the [`__enter__()`](../reference/datamodel.html#object.__enter__) method. The optional second type parameter, `ExitT_co`,
which defaults to `bool | None`, represents the type returned by the
[`__exit__()`](../reference/datamodel.html#object.__exit__) method.

Added in version 3.5.4.

Deprecated since version 3.9: [`contextlib.AbstractContextManager`](contextlib.html#contextlib.AbstractContextManager)
now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

Changed in version 3.13: Added the optional second type parameter, `ExitT_co`.

class typing.AsyncContextManager(Generic[T_co, AExitT_co])[Â¶](#typing.AsyncContextManager)

Deprecated alias to [`contextlib.AbstractAsyncContextManager`](contextlib.html#contextlib.AbstractAsyncContextManager).

The first type parameter, `T_co`, represents the type returned by
the [`__aenter__()`](../reference/datamodel.html#object.__aenter__) method. The optional second type parameter, `AExitT_co`,
which defaults to `bool | None`, represents the type returned by the
[`__aexit__()`](../reference/datamodel.html#object.__aexit__) method.

Added in version 3.6.2.

Deprecated since version 3.9: [`contextlib.AbstractAsyncContextManager`](contextlib.html#contextlib.AbstractAsyncContextManager)
now supports subscripting (`[]`).
See [**PEP 585**](https://peps.python.org/pep-0585/) and [Generic Alias Type](stdtypes.html#types-genericalias).

Changed in version 3.13: Added the optional second type parameter, `AExitT_co`.

## Deprecation Timeline of Major Features[Â¶](#deprecation-timeline-of-major-features)
...
Undecided (see [Deprecated aliases](#deprecated-aliases) for more information)
...
[`typing.Hashable`](#typing.Hashable) and [`typing.Sized`](#typing.Sized)

3.12

Undecided

[gh-94309](https://github.com/python/cpython/issues/94309)
...
[gh-105578](https://github.com/python/cpython/issues/105578)
...
- [ABCs and Protocols for working with I/O](#abcs-and-protocols-for-working-with-i-o)
...
[`pydoc` â Documentation generator and online help system](pydoc.html)
