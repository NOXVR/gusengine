...
os.path â Common pathname manipulations — Python 3.14.3 documentation
...
# `os.path` â Common pathname manipulations[Â¶](#module-os.path)
...
files see [`open()`](functions.html#open), and for accessing the filesystem see the [`os`](os.html#module-os)
module. The path parameters can be passed as strings, or bytes, or any object
implementing the [`os.PathLike`](os.html#os.PathLike) protocol.
...
the [`glob`](glob.html#module-glob) module.)
...
os.path.abspath(path)[Â¶](#os.path.abspath)
...
os.path.basename(path, /)[Â¶](#os.path.basename)
...
os.path.commonpath(paths)[Â¶](#os.path.commonpath)

Return the longest common sub-path of each pathname in the iterable
*paths*.  Raise [`ValueError`](exceptions.html#ValueError) if *paths* contain both absolute
...
Changed in version 3.6: Accepts a sequence of [path-like objects](../glossary.html#term-path-like-object).

Changed in version 3.13: Any iterable can now be passed, rather than just sequences.

os.path.commonprefix(list, /)[Â¶](#os.path.commonprefix)
...
os.path.dirname(path, /)[Â¶](#os.path.dirname)
...
os.path.exists(path)[Â¶](#os.path.exists)
...
not granted to execute [`os.stat()`](os.html#os.stat) on the requested file, even
...
os.path.lexists(path)[Â¶](#os.path.lexists)
...
os.path.expanduser(path)[Â¶](#os.path.expanduser)

On Unix and Windows, return the argument with an initial component of `~` or
`~user` replaced by that *user*âs home directory.

On Unix, an initial `~` is replaced by the environment variable `HOME`
if it is set; otherwise the current userâs home directory is looked up in the
password directory through the built-in module [`pwd`](pwd.html#module-pwd). An initial `~user`
...
userâs home directory matches `USERNAME`, and replacing it if so.
...
os.path.expandvars(path)[Â¶](#os.path.expandvars)
...
os.path.getatime(path, /)[Â¶](#os.path.getatime)

Return the time of last access of *path*.  The return value is a floating-point number giving
the number of seconds since the epoch (see the  [`time`](time.html#module-time) module).  Raise
[`OSError`](exceptions.html#OSError) if the file does not exist or is inaccessible.

os.path.getmtime(path, /)[Â¶](#os.path.getmtime)

Return the time of last modification of *path*.  The return value is a floating-point number
giving the number of seconds since the epoch (see the  [`time`](time.html#module-time) module).
Raise [`OSError`](exceptions.html#OSError) if the file does not exist or is inaccessible.

Changed in version 3.6: Accepts a [path-like object](../glossary.html#term-path-like-object).

os.path.getctime(path, /)[Â¶](#os.path.getctime)

Return the systemâs ctime which, on some systems (like Unix) is the time of the
last metadata change, and, on others (like Windows), is the creation time for *path*.
The return value is a number giving the number of seconds since the epoch (see
the  [`time`](time.html#module-time) module).  Raise [`OSError`](exceptions.html#OSError) if the file does not exist or
is inaccessible.

Changed in version 3.6: Accepts a [path-like object](../glossary.html#term-path-like-object).

os.path.getsize(path, /)[Â¶](#os.path.getsize)

Return the size, in bytes, of *path*.  Raise [`OSError`](exceptions.html#OSError) if the file does
not exist or is inaccessible.

Changed in version 3.6: Accepts a [path-like object](../glossary.html#term-path-like-object).

os.path.isabs(path, /)[Â¶](#os.path.isabs)
...
os.path.isfile(path)[Â¶](#os.path.isfile)
...
os.path.isdir(path, /)[Â¶](#os.path.isdir)
...
os.path.isjunction(path)[Â¶](#os.path.isjunction)
...
os.path.islink(path)[Â¶](#os.path.islink)
...
os.path.ismount(path)[Â¶](#os.path.ismount)
...
subvolumes, even if they arenât mount points. On Windows, a drive letter root
...
os.path.isdevdrive(path)[Â¶](#os.path.isdevdrive)
...
os.path.isreserved(path)[Â¶](#os.path.isreserved)
...
those that contain colons (i.e. file streams such as âname:streamâ),
wildcard characters (i.e. `'*?"<>'`), pipe, or ASCII control characters;
as well as DOS device names such as âNULâ, âCONâ, âCONIN$â, âCONOUT$â,
âAUXâ, âPRNâ, âCOM1â, and âLPT1â.
...
os.path.join(path, /, *paths)[Â¶](#os.path.join)
...
Changed in version 3.6: Accepts a [path-like object](../glossary.html#term-path-like-object) for *path* and *paths*.

os.path.normcase(path, /)[Â¶](#os.path.normcase)
...
os.path.normpath(path)[Â¶](#os.path.normpath)
...
os.path.realpath(path, /, *, strict=False)[Â¶](#os.path.realpath)
...
exist, is a symlink loop, or whose evaluation raises [`OSError`](exceptions.html#OSError).
All such components are appended unchanged to the existing part of the path.

Some errors that are handled this way include âaccess deniedâ, ânot a
directoryâ, or âbad argument to internal functionâ. Thus, the
...
re-raised.
In particular, [`FileNotFoundError`](exceptions.html#FileNotFoundError) is raised if *path* does not exist,
or another [`OSError`](exceptions.html#OSError) if it is otherwise inaccessible.

If *strict* is [`os.path.ALLOW_MISSING`](#os.path.ALLOW_MISSING), errors other than
[`FileNotFoundError`](exceptions.html#FileNotFoundError) are re-raised (as with `strict=True`).
...
This function emulates the operating systemâs procedure for making a path
...
Operating system APIs make paths canonical as needed, so itâs not
...
Changed in version 3.14: The [`ALLOW_MISSING`](#os.path.ALLOW_MISSING) value for the *strict* parameter
was added.

os.path.ALLOW_MISSING[Â¶](#os.path.ALLOW_MISSING)

Special value used for the *strict* argument in [`realpath()`](#os.path.realpath).

Added in version 3.14.

os.path.relpath(path, start=os.curdir)[Â¶](#os.path.relpath)

Return a relative filepath to *path* either from the current directory or
from an optional *start* directory.  This is a path computation:  the
filesystem is not accessed to confirm the existence or nature of *path* or
*start*.  On Windows, [`ValueError`](exceptions.html#ValueError) is raised when *path* and *start*
are on different drives.

*start* defaults to [`os.curdir`](os.html#os.curdir).

Changed in version 3.6: Accepts a [path-like object](../glossary.html#term-path-like-object).

os.path.samefile(path1, path2, /)[Â¶](#os.path.samefile)

Return `True` if both pathname arguments refer to the same file or directory.
This is determined by the device number and i-node number and raises an
exception if an [`os.stat()`](os.html#os.stat) call on either pathname fails.
...
os.path.sameopenfile(fp1, fp2)[Â¶](#os.path.sameopenfile)
...
os.path.samestat(stat1, stat2, /)[Â¶](#os.path.samestat)

Return `True` if the stat tuples *stat1* and *stat2* refer to the same file.
These structures may have been returned by [`os.fstat()`](os.html#os.fstat),
[`os.lstat()`](os.html#os.lstat), or [`os.stat()`](os.html#os.stat).  This function implements the
...
os.path.split(path, /)[Â¶](#os.path.split)
...
os.path.splitdrive(path, /)[Â¶](#os.path.splitdrive)
...
os.path.splitroot(path, /)[Â¶](#os.path.splitroot)
...
os.path.splitext(path, /)[Â¶](#os.path.splitext)
...
os.path.supports_unicode_filenames[Â¶](#os.path.supports_unicode_filenames)
