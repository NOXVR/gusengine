...
shutil â High-level file operations — Python 3.14.3 documentation
...
- 
          [next](persistence.html) |
        
- 
          [previous](linecache.html) |
...
# `shutil` â High-level file operations[Â¶](#module-shutil)
...
## Directory and files operations[Â¶](#directory-and-files-operations)

shutil.copyfileobj(fsrc, fdst[, length])[Â¶](#shutil.copyfileobj)
...
you must ensure that you have called [`flush()`](io.html#io.IOBase.flush) or
[`close()`](io.html#io.IOBase.close) on the file-like object before attempting to read
the destination file.

shutil.copyfile(src, dst, *, follow_symlinks=True)[Â¶](#shutil.copyfile)
...
The destination location must be writable; otherwise, an [`OSError`](exceptions.html#OSError)
...
Changed in version 3.3: [`IOError`](exceptions.html#IOError) used to be raised instead of [`OSError`](exceptions.html#OSError).
...
exception shutil.SpecialFileError[Â¶](#shutil.SpecialFileError)
...
exception shutil.SameFileError[Â¶](#shutil.SameFileError)
...
shutil.copymode(src, dst, *, follow_symlinks=True)[Â¶](#shutil.copymode)
...
shutil.copystat(src, dst, *, follow_symlinks=True)[Â¶](#shutil.copystat)

Copy the permission bits, last access time, last modification time, and
flags from *src* to *dst*.  On Linux, [`copystat()`](#shutil.copystat) also copies the
âextended attributesâ where possible.  The file contents, owner, and
...
Please see [`os.supports_follow_symlinks`](os.html#os.supports_follow_symlinks)
...
shutil.copy(src, dst, *, follow_symlinks=True)[Â¶](#shutil.copy)
...
[`copy()`](#shutil.copy) copies the file data and the fileâs permission
mode (see [`os.chmod()`](os.html#os.chmod)).  Other metadata, like the
fileâs creation and modification times, is not preserved.
...
shutil.copy2(src, dst, *, follow_symlinks=True)[Â¶](#shutil.copy2)
...
shutil.ignore_patterns(*patterns)[Â¶](#shutil.ignore_patterns)

This factory function creates a function that can be used as a callable for
[`copytree()`](#shutil.copytree)'s *ignore* argument, ignoring files and directories that
match one of the glob-style *patterns* provided.  See the example below.

shutil.copytree(src, dst, symlinks=False, ignore=None, copy_function=copy2, ignore_dangling_symlinks=False, dirs_exist_ok=False)[Â¶](#shutil.copytree)
...
When *symlinks* is false, if the file pointed to by the symlink doesnât
...
on platforms that donât support [`os.symlink()`](os.html#os.symlink).

If *ignore* is given, it must be a callable that will receive as its
arguments the directory being visited by [`copytree()`](#shutil.copytree), and a list of its
contents, as returned by [`os.listdir()`](os.html#os.listdir).  Since [`copytree()`](#shutil.copytree) is
...
[`FileExistsError`](exceptions.html#FileExistsError) is raised. If *dirs_exist_ok* is true, the copying
...
shutil.rmtree(path, ignore_errors=False, onerror=None, *, onexc=None, dir_fd=None)[Â¶](#shutil.rmtree)
...
symlinks on the filesystem to delete files they wouldnât be able to access
...
parameter it receives is the tuple returned from [`sys.exc_info()`](sys.html#sys.exc_info).
...
Changed in version 3.13: `rmtree()` now ignores [`FileNotFoundError`](exceptions.html#FileNotFoundError) exceptions for all
but the top-level path.
Exceptions other than [`OSError`](exceptions.html#OSError) and subclasses of `OSError`
are now always propagated to the caller.

rmtree.avoids_symlink_attacks[Â¶](#shutil.rmtree.avoids_symlink_attacks)
...
shutil.move(src, dst, copy_function=copy2)[Â¶](#shutil.move)
...
depending on [`os.rename()`](os.html#os.rename) semantics.

If the destination is on the current filesystem, then [`os.rename()`](os.html#os.rename) is
...
if [`os.rename()`](os.html#os.rename) cannot be used.  If the source is a directory,
...
it to the behavior of GNUâs mv.
Now returns *dst*.
...
shutil.disk_usage(path)[Â¶](#shutil.disk_usage)
...
filesystem partition. On those platforms, CPython doesnât attempt to
...
shutil.chown(path, user=None, group=None, *, dir_fd=None, follow_symlinks=True)[Â¶](#shutil.chown)
...
See also [`os.chown()`](os.html#os.chown), the underlying function.
...
shutil.which(cmd, mode=os.F_OK | os.X_OK, path=None)[Â¶](#shutil.which)

Return the path to an executable which would be run if the given *cmd* was
called.  If no *cmd* would be called, return `None`.

*mode* is a permission mask passed to [`os.access()`](os.html#os.access), by default
determining if the file exists and is executable.

*path* is a â`PATH` stringâ specifying the directories to look in,
delimited by [`os.pathsep`](os.html#os.pathsep). When no *path* is specified, the
`PATH` environment variable is read from [`os.environ`](os.html#os.environ),
falling back to [`os.defpath`](os.html#os.defpath) if it is not set.
...
*path* or in the systemâs `PATH` environment variable.
...
Changed in version 3.8: The [`bytes`](stdtypes.html#bytes) type is now accepted.  If *cmd* type is
[`bytes`](stdtypes.html#bytes), the result type is also [`bytes`](stdtypes.html#bytes).
...
exception shutil.Error[Â¶](#shutil.Error)

This exception collects exceptions that are raised during a multi-file
operation. For [`copytree()`](#shutil.copytree), the exception argument is a list of 3-tuples
(*srcname*, *dstname*, *exception*).

### Platform-dependent efficient copy operations[Â¶](#platform-dependent-efficient-copy-operations)
...
âfast-copyâ means that the copying operation occurs within the kernel, avoiding
the use of userspace buffers in Python as in â`outfd.write(infd.read())`â.

On macOS [fcopyfile](http://www.manpagez.com/man/3/copyfile/) is used to copy the file content (not metadata).

On Linux [`os.copy_file_range()`](os.html#os.copy_file_range) or [`os.sendfile()`](os.html#os.sendfile) is used.

On Solaris [`os.sendfile()`](os.html#os.sendfile) is used.

On Windows [`shutil.copyfile()`](#shutil.copyfile) uses a bigger default buffer size (1 MiB
instead of 64 KiB) and a [`memoryview()`](stdtypes.html#memoryview)-based variant of
[`shutil.copyfileobj()`](#shutil.copyfileobj) is used.
...
Changed in version 3.14: Solaris now uses [`os.sendfile()`](os.html#os.sendfile).

Changed in version 3.14: Copy-on-write or server-side copy may be used internally via
[`os.copy_file_range()`](os.html#os.copy_file_range) on supported Linux filesystems.

### copytree example[Â¶](#copytree-example)
...
### rmtree example[Â¶](#rmtree-example)
...
## Archiving operations[Â¶](#archiving-operations)
...
provided.  They rely on the [`zipfile`](zipfile.html#module-zipfile) and [`tarfile`](tarfile.html#module-tarfile) modules.

shutil.make_archive(base_name, format[, root_dir[, base_dir[, verbose[, dry_run[, owner[, group[, logger]]]]]]])[Â¶](#shutil.make_archive)
...
âzipâ (if the [`zlib`](zlib.html#module-zlib) module is available), âtarâ, âgztarâ (if the
[`zlib`](zlib.html#module-zlib) module is available), âbztarâ (if the [`bz2`](bz2.html#module-bz2) module is
available), âxztarâ (if the [`lzma`](lzma.html#module-lzma) module is available), or âzstdtarâ
(if the [`compression.zstd`](compression.zstd.html#module-compression.zstd) module is available).
...
[`logging.Logger`](logging.html#logging.Logger).
...
shutil.get_archive_formats()[Â¶](#shutil.get_archive_formats)
...
- 
*zip*: ZIP file (if the [`zlib`](zlib.html#module-zlib) module is available).

- 
*tar*: Uncompressed tar file. Uses POSIX.1-2001 pax format for new archives.

- 
*gztar*: gzipâed tar-file (if the [`zlib`](zlib.html#module-zlib) module is available).

- 
*bztar*: bzip2âed tar-file (if the [`bz2`](bz2.html#module-bz2) module is available).

- 
*xztar*: xzâed tar-file (if the [`lzma`](lzma.html#module-lzma) module is available).

- 
*zstdtar*: Zstandard compressed tar-file (if the [`compression.zstd`](compression.zstd.html#module-compression.zstd)
module is available).

You can register new formats or provide your own archiver for any existing
formats, by using [`register_archive_format()`](#shutil.register_archive_format).

shutil.register_archive_format(name, function[, extra_args[, description]])[Â¶](#shutil.register_archive_format)
...
*base_dir* (which defaults to [`os.curdir`](os.html#os.curdir)) to start archiving from.
...
shutil.unregister_archive_format(name)[Â¶](#shutil.unregister_archive_format)

Remove the archive format *name* from the list of supported formats.

shutil.unpack_archive(filename[, extract_dir[, format[, filter]]])[Â¶](#shutil.unpack_archive)
...
*format* is the archive format: one of âzipâ, âtarâ, âgztarâ, âbztarâ,
âxztarâ, or âzstdtarâ.  Or any other format registered with
...
a [`ValueError`](exceptions.html#ValueError) is raised.
...
starting with â/â or filenames with two dots â..â.
...
Changed in version 3.7: Accepts a [path-like object](../glossary.html#term-path-like-object) for *filename* and *extract_dir*.

Changed in version 3.12: Added the *filter* argument.

shutil.register_unpack_format(name, extensions, function[, extra_args[, description]])[Â¶](#shutil.register_unpack_format)
...
shutil.unregister_unpack_format(name)[Â¶](#shutil.unregister_unpack_format)

Unregister an unpack format. *name* is the name of the format.

shutil.get_unpack_formats()[Â¶](#shutil.get_unpack_formats)
...
### Archiving example[Â¶](#archiving-example)

In this example, we create a gzipâed tar-file archive containing all files
...
### Archiving example with *base_dir*[Â¶](#archiving-example-with-base-dir)
...
## Querying the size of the output terminal[Â¶](#querying-the-size-of-the-output-terminal)

shutil.get_terminal_size(fallback=(columns, lines))[Â¶](#shutil.get_terminal_size)
...
the terminal connected to [`sys.__stdout__`](sys.html#sys.__stdout__) is queried
by invoking [`os.get_terminal_size()`](os.html#os.get_terminal_size).

If the terminal size cannot be successfully queried, either because
the system doesnât support querying, or because we are not
connected to a terminal, the value given in `fallback` parameter
is used. `fallback` defaults to `(80, 24)` which is the default
size used by many terminal emulators.

The value returned is a named tuple of type [`os.terminal_size`](os.html#os.terminal_size).
...
Changed in version 3.11: The `fallback` values are also used if [`os.get_terminal_size()`](os.html#os.get_terminal_size)
returns zeroes.
...
- [Archiving example with *base_dir*](#archiving-example-with-base-dir)
