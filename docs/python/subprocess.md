...
subprocess â Subprocess management — Python 3.14.3 documentation
...
# `subprocess` â Subprocess management[Â¶](#module-subprocess)
...
## Using the `subprocess` Module[Â¶](#using-the-subprocess-module)
...
subprocess.run(args, *, stdin=None, input=None, stdout=None, stderr=None, capture_output=False, shell=False, cwd=None, timeout=None, check=False, encoding=None, errors=None, text=None, env=None, universal_newlines=None, **other_popen_kwargs)[Â¶](#subprocess.run)
...
class subprocess.CompletedProcess[Â¶](#subprocess.CompletedProcess)

The return value from [`run()`](#subprocess.run), representing a process that has finished.

args[Â¶](#subprocess.CompletedProcess.args)

The arguments used to launch the process. This may be a list or a string.

returncode[Â¶](#subprocess.CompletedProcess.returncode)
...
stdout[Â¶](#subprocess.CompletedProcess.stdout)
...
stderr[Â¶](#subprocess.CompletedProcess.stderr)

Captured stderr from the child process. A bytes sequence, or a string if
[`run()`](#subprocess.run) was called with an encoding, errors, or text=True.
`None` if stderr was not captured.

check_returncode()[Â¶](#subprocess.CompletedProcess.check_returncode)
...
subprocess.DEVNULL[Â¶](#subprocess.DEVNULL)
...
subprocess.PIPE[Â¶](#subprocess.PIPE)
...
subprocess.STDOUT[Â¶](#subprocess.STDOUT)

Special value that can be used as the *stderr* argument to [`Popen`](#subprocess.Popen) and
indicates that standard error should go into the same handle as standard
output.

exception subprocess.SubprocessError[Â¶](#subprocess.SubprocessError)

Base class for all other exceptions from this module.

Added in version 3.3.

exception subprocess.TimeoutExpired[Â¶](#subprocess.TimeoutExpired)

Subclass of [`SubprocessError`](#subprocess.SubprocessError), raised when a timeout expires
while waiting for a child process.

cmd[Â¶](#subprocess.TimeoutExpired.cmd)

Command that was used to spawn the child process.

timeout[Â¶](#subprocess.TimeoutExpired.timeout)

Timeout in seconds.

output[Â¶](#subprocess.TimeoutExpired.output)
...
stdout[Â¶](#subprocess.TimeoutExpired.stdout)

Alias for output, for symmetry with [`stderr`](#subprocess.TimeoutExpired.stderr).

stderr[Â¶](#subprocess.TimeoutExpired.stderr)
...
exception subprocess.CalledProcessError[Â¶](#subprocess.CalledProcessError)
...
### Frequently Used Arguments[Â¶](#frequently-used-arguments)
...
created.  [`DEVNULL`](#subprocess.DEVNULL) indicates that the special file [`os.devnull`](os.html#os.devnull)
...
to the default line separator [`os.linesep`](os.html#os.linesep). For *stdout* and *stderr*,
all line endings in the output will be converted to `'\n'`.  For more
information see the documentation of the [`io.TextIOWrapper`](io.html#io.TextIOWrapper) class
...
implementations of many shell-like features (in particular, [`glob`](glob.html#module-glob),
[`fnmatch`](fnmatch.html#module-fnmatch), [`os.walk()`](os.html#os.walk), [`os.path.expandvars()`](os.path.html#os.path.expandvars),
[`os.path.expanduser()`](os.path.html#os.path.expanduser), and [`shutil`](shutil.html#module-shutil)).

Changed in version 3.3: When *universal_newlines* is `True`, the class uses the encoding
[`locale.getpreferredencoding(False)`](locale.html#locale.getpreferredencoding)
instead of `locale.getpreferredencoding()`.  See the
[`io.TextIOWrapper`](io.html#io.TextIOWrapper) class for more information on this change.
...
### Popen Constructor[Â¶](#popen-constructor)
...
class subprocess.Popen(args, bufsize=-1, executable=None, stdin=None, stdout=None, stderr=None, preexec_fn=None, close_fds=True, shell=False, cwd=None, env=None, universal_newlines=None, startupinfo=None, creationflags=0, restore_signals=True, start_new_session=False, pass_fds=(), *, group=None, extra_groups=None, user=None, umask=-1, encoding=None, errors=None, text=None, pipesize=-1, process_group=None)[Â¶](#subprocess.Popen)

Execute a child program in a new process.  On POSIX, the class uses
[`os.execvpe()`](os.html#os.execvpe)-like behavior to execute the child program.  On Windows,
...
[`shutil.which()`](shutil.html#shutil.which). On all platforms, passing [`sys.executable`](sys.html#sys.executable)
...
platform dependent. For POSIX, see [`os.execvpe()`](os.html#os.execvpe), and note that
...
especially in complex cases. [`shlex.split()`](shlex.html#shlex.split) can illustrate how to
...
[`open()`](functions.html#open) function when creating the stdin/stdout/stderr pipe
file objects:
...
code using *preexec_fn* to call [`os.setsid()`](os.html#os.setsid) or [`os.setpgid()`](os.html#os.setpgid) in the child.
...
[`RuntimeError`](exceptions.html#RuntimeError). The new restriction may affect applications that
...
Changed in version 3.2: The default for *close_fds* was changed from [`False`](constants.html#False) to
what is described above.

Changed in version 3.7: On Windows the default for *close_fds* was changed from [`False`](constants.html#False) to
[`True`](constants.html#True) when redirecting the standard handles. Itâs now possible to
set *close_fds* to [`True`](constants.html#True) when redirecting the standard handles.

*pass_fds* is an optional sequence of file descriptors to keep open
between the parent and child.  Providing any *pass_fds* forces
*close_fds* to be [`True`](constants.html#True).  (POSIX only)
...
value is a string, it will be looked up via [`grp.getgrnam()`](grp.html#grp.getgrnam) and
...
[`grp.getgrnam()`](grp.html#grp.getgrnam) and the values in `gr_gid` will be used.
...
child process prior to the execution of the subprocess. If the provided
value is a string, it will be looked up via [`pwd.getpwnam()`](pwd.html#pwd.getpwnam) and
...
[`os.environ`](os.html#os.environ) or [`os.environb`](os.html#os.environb).
...
Changed in version 3.6: Popen destructor now emits a [`ResourceWarning`](exceptions.html#ResourceWarning) warning if the child
process is still running.

Changed in version 3.8: Popen can use [`os.posix_spawn()`](os.html#os.posix_spawn) in some cases for better
performance. On Windows Subsystem for Linux and QEMU User Emulation,
Popen constructor using [`os.posix_spawn()`](os.html#os.posix_spawn) no longer raise an
...
The most common exception raised is [`OSError`](exceptions.html#OSError).  This occurs, for example,
when trying to execute a non-existent file.  Applications should prepare for
[`OSError`](exceptions.html#OSError) exceptions. Note that, when `shell=True`, [`OSError`](exceptions.html#OSError)
...
A [`ValueError`](exceptions.html#ValueError) will be raised if [`Popen`](#subprocess.Popen) is called with invalid
arguments.
...
to use [`shlex.quote()`](shlex.html#shlex.quote) for this escaping.
...
sleeps). Use the [`asyncio`](asyncio.html#module-asyncio) module for an asynchronous wait: see
[`asyncio.create_subprocess_exec`](asyncio-subprocess.html#asyncio.create_subprocess_exec).

Changed in version 3.3: *timeout* was added.

Popen.communicate(input=None, timeout=None)[Â¶](#subprocess.Popen.communicate)
...
Popen.send_signal(signal)[Â¶](#subprocess.Popen.send_signal)
...
Popen.terminate()[Â¶](#subprocess.Popen.terminate)

Stop the child. On POSIX OSs the method sends [`SIGTERM`](signal.html#signal.SIGTERM) to the
child. On Windows the Win32 API function `TerminateProcess()` is called
to stop the child.

Popen.kill()[Â¶](#subprocess.Popen.kill)
...
Popen.args[Â¶](#subprocess.Popen.args)

The *args* argument as it was passed to [`Popen`](#subprocess.Popen) â a
sequence of program arguments or else a single string.

Added in version 3.3.

Popen.stdin[Â¶](#subprocess.Popen.stdin)

If the *stdin* argument was [`PIPE`](#subprocess.PIPE), this attribute is a writeable
stream object as returned by [`open()`](functions.html#open). If the *encoding* or *errors*
...
Popen.stdout[Â¶](#subprocess.Popen.stdout)

If the *stdout* argument was [`PIPE`](#subprocess.PIPE), this attribute is a readable
stream object as returned by [`open()`](functions.html#open). Reading from the stream provides
...
Popen.stderr[Â¶](#subprocess.Popen.stderr)
...
Popen.pid[Â¶](#subprocess.Popen.pid)
...
Popen.returncode[Â¶](#subprocess.Popen.returncode)
...
## Windows Popen Helpers[Â¶](#windows-popen-helpers)

The [`STARTUPINFO`](#subprocess.STARTUPINFO) class and following constants are only available
on Windows.

class subprocess.STARTUPINFO(*, dwFlags=0, hStdInput=None, hStdOutput=None, hStdError=None, wShowWindow=0, lpAttributeList=None)[Â¶](#subprocess.STARTUPINFO)
...
dwFlags[Â¶](#subprocess.STARTUPINFO.dwFlags)
...
hStdInput[Â¶](#subprocess.STARTUPINFO.hStdInput)
...
hStdOutput[Â¶](#subprocess.STARTUPINFO.hStdOutput)
...
hStdError[Â¶](#subprocess.STARTUPINFO.hStdError)

If [`dwFlags`](#subprocess.STARTUPINFO.dwFlags) specifies [`STARTF_USESTDHANDLES`](#subprocess.STARTF_USESTDHANDLES), this attribute
is the standard error handle for the process. Otherwise, this attribute is
ignored and the default for standard error is the console windowâs buffer.

wShowWindow[Â¶](#subprocess.STARTUPINFO.wShowWindow)
...
lpAttributeList[Â¶](#subprocess.STARTUPINFO.lpAttributeList)
...
[`os.set_handle_inheritable()`](os.html#os.set_handle_inheritable) when passed to the [`Popen`](#subprocess.Popen)
constructor, else [`OSError`](exceptions.html#OSError) will be raised with Windows error
...
all handles such as [`os.system()`](os.html#os.system).  This also applies to
standard handle redirection, which temporarily creates inheritable
handles.

Added in version 3.7.

### Windows Constants[Â¶](#windows-constants)

The `subprocess` module exposes the following constants.

subprocess.STD_INPUT_HANDLE[Â¶](#subprocess.STD_INPUT_HANDLE)

The standard input device. Initially, this is the console input buffer,
`CONIN$`.

subprocess.STD_OUTPUT_HANDLE[Â¶](#subprocess.STD_OUTPUT_HANDLE)

The standard output device. Initially, this is the active console screen
buffer, `CONOUT$`.

subprocess.STD_ERROR_HANDLE[Â¶](#subprocess.STD_ERROR_HANDLE)

The standard error device. Initially, this is the active console screen
buffer, `CONOUT$`.

subprocess.SW_HIDE[Â¶](#subprocess.SW_HIDE)

Hides the window. Another window will be activated.

subprocess.STARTF_USESTDHANDLES[Â¶](#subprocess.STARTF_USESTDHANDLES)
...
subprocess.STARTF_USESHOWWINDOW[Â¶](#subprocess.STARTF_USESHOWWINDOW)

Specifies that the [`STARTUPINFO.wShowWindow`](#subprocess.STARTUPINFO.wShowWindow) attribute contains
additional information.

subprocess.STARTF_FORCEONFEEDBACK[Â¶](#subprocess.STARTF_FORCEONFEEDBACK)
...
subprocess.STARTF_FORCEOFFFEEDBACK[Â¶](#subprocess.STARTF_FORCEOFFFEEDBACK)
...
subprocess.CREATE_NEW_CONSOLE[Â¶](#subprocess.CREATE_NEW_CONSOLE)

The new process has a new console, instead of inheriting its parentâs
console (the default).

subprocess.CREATE_NEW_PROCESS_GROUP[Â¶](#subprocess.CREATE_NEW_PROCESS_GROUP)

A [`Popen`](#subprocess.Popen) `creationflags` parameter to specify that a new process
group will be created. This flag is necessary for using [`os.kill()`](os.html#os.kill)
on the subprocess.

This flag is ignored if [`CREATE_NEW_CONSOLE`](#subprocess.CREATE_NEW_CONSOLE) is specified.

subprocess.ABOVE_NORMAL_PRIORITY_CLASS[Â¶](#subprocess.ABOVE_NORMAL_PRIORITY_CLASS)
...
subprocess.BELOW_NORMAL_PRIORITY_CLASS[Â¶](#subprocess.BELOW_NORMAL_PRIORITY_CLASS)
...
subprocess.HIGH_PRIORITY_CLASS[Â¶](#subprocess.HIGH_PRIORITY_CLASS)
...
subprocess.IDLE_PRIORITY_CLASS[Â¶](#subprocess.IDLE_PRIORITY_CLASS)
...
subprocess.NORMAL_PRIORITY_CLASS[Â¶](#subprocess.NORMAL_PRIORITY_CLASS)
...
subprocess.REALTIME_PRIORITY_CLASS[Â¶](#subprocess.REALTIME_PRIORITY_CLASS)
...
subprocess.CREATE_NO_WINDOW[Â¶](#subprocess.CREATE_NO_WINDOW)
...
subprocess.DETACHED_PROCESS[Â¶](#subprocess.DETACHED_PROCESS)
...
subprocess.CREATE_DEFAULT_ERROR_MODE[Â¶](#subprocess.CREATE_DEFAULT_ERROR_MODE)
...
subprocess.CREATE_BREAKAWAY_FROM_JOB[Â¶](#subprocess.CREATE_BREAKAWAY_FROM_JOB)
...
## Older high-level API[Â¶](#older-high-level-api)
...
subprocess.call(args, *, stdin=None, stdout=None, stderr=None, shell=False, cwd=None, timeout=None, **other_popen_kwargs)[Â¶](#subprocess.call)
...
subprocess.check_call(args, *, stdin=None, stdout=None, stderr=None, shell=False, cwd=None, timeout=None, **other_popen_kwargs)[Â¶](#subprocess.check_call)
...
subprocess.check_output(args, *, stdin=None, stderr=None, shell=False, cwd=None, encoding=None, errors=None, universal_newlines=None, timeout=None, text=None, **other_popen_kwargs)[Â¶](#subprocess.check_output)
...
## Replacing Older Functions with the `subprocess` Module[Â¶](#replacing-older-functions-with-the-subprocess-module)
...
executed program cannot be found; the âbâ replacements raise [`OSError`](exceptions.html#OSError)
instead.
...
### Replacing /bin/sh shell command substitution[Â¶](#replacing-bin-sh-shell-command-substitution)
...
### Replacing shell pipeline[Â¶](#replacing-shell-pipeline)
...
### Replacing [`os.system()`](os.html#os.system)[Â¶](#replacing-os-system)
...
[`os.system()`](os.html#os.system).

- 
The [`os.system()`](os.html#os.system) function ignores SIGINT and SIGQUIT signals while
...
### Replacing the [`os.spawn`](os.html#os.spawnl) family[Â¶](#replacing-the-os-spawn-family)
...
### Replacing [`os.popen()`](os.html#os.popen)[Â¶](#replacing-os-popen)
...
## Legacy Shell Invocation Functions[Â¶](#legacy-shell-invocation-functions)
...
subprocess.getstatusoutput(cmd, *, encoding=None, errors=None)[Â¶](#subprocess.getstatusoutput)
...
subprocess.getoutput(cmd, *, encoding=None, errors=None)[Â¶](#subprocess.getoutput)
...
### Timeout Behavior[Â¶](#timeout-behavior)
...
- 
**Process Creation Delay**: The initial process creation itself cannot be interrupted
...
### Converting an argument sequence to a string on Windows[Â¶](#converting-an-argument-sequence-to-a-string-on-windows)
...
[`shlex`](shlex.html#module-shlex)
Module which provides function to parse and escape command lines.

### Disable use of `posix_spawn()`[Â¶](#disable-use-of-posix-spawn)
...
[`concurrent.interpreters` â Multiple interpreters in the same process](concurrent.interpreters.html)
...
[`sched` â Event scheduler](sched.html)
