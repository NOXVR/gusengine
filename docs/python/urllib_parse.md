...
urllib.parse â Parse URLs into components — Python 3.14.3 documentation
...
- 
          [next](urllib.error.html) |
        
- 
          [previous](urllib.request.html) |
...
# `urllib.parse` â Parse URLs into components[Â¶](#module-urllib.parse)
...
combine the components back into a URL string, and to convert a ârelative URLâ
to an absolute URL given a âbase URL.â
...
passing Appleâs App Store review process for the macOS and iOS App Stores.
...
This moduleâs functions use the deprecated term `netloc` (or `net_loc`),
...
## URL Parsing[Â¶](#url-parsing)

The URL parsing functions focus on splitting a URL string into its components,
or on combining URL components into a URL string.

urllib.parse.urlsplit(urlstring, scheme=None, allow_fragments=True)[Â¶](#urllib.parse.urlsplit)
...
a netloc only if it is properly introduced by â//â.  Otherwise the
...
Reading the `port` attribute will raise a [`ValueError`](exceptions.html#ValueError) if
...
`#`, `@`, or `:` will raise a [`ValueError`](exceptions.html#ValueError). If the URL is
...
Changed in version 3.6: Out-of-range port numbers now raise [`ValueError`](exceptions.html#ValueError), instead of
returning [`None`](constants.html#None).

Changed in version 3.8: Characters that affect netloc parsing under NFKC normalization will
now raise [`ValueError`](exceptions.html#ValueError).
...
urllib.parse.parse_qs(qs, keep_blank_values=False, strict_parsing=False, encoding='utf-8', errors='replace', max_num_fields=None, separator='&')[Â¶](#urllib.parse.parse_qs)
...
errors raise a [`ValueError`](exceptions.html#ValueError) exception.

The optional *encoding* and *errors* parameters specify how to decode
percent-encoded sequences into Unicode characters, as accepted by the
[`bytes.decode()`](stdtypes.html#bytes.decode) method.

The optional argument *max_num_fields* is the maximum number of fields to
read. If set, then throws a [`ValueError`](exceptions.html#ValueError) if there are more than
...
urllib.parse.parse_qsl(qs, keep_blank_values=False, strict_parsing=False, encoding='utf-8', errors='replace', max_num_fields=None, separator='&')[Â¶](#urllib.parse.parse_qsl)
...
urllib.parse.urlunsplit(parts)[Â¶](#urllib.parse.urlunsplit)
...
urllib.parse.urlparse(urlstring, scheme=None, allow_fragments=True)[Â¶](#urllib.parse.urlparse)

This is similar to [`urlsplit()`](#urllib.parse.urlsplit), but additionally splits the *path*
component on *path* and *params*.
This function returns a 6-item [named tuple](../glossary.html#term-named-tuple) [`ParseResult`](#urllib.parse.ParseResult)
...
urllib.parse.urlunparse(parts)[Â¶](#urllib.parse.urlunparse)
...
urllib.parse.urljoin(base, url, allow_fragments=True)[Â¶](#urllib.parse.urljoin)

Construct a full (âabsoluteâ) URL by combining a âbase URLâ (*base*) with
...
the *url*âs hostname and/or scheme will be present in the result.  For example:
...
urllib.parse.urldefrag(url)[Â¶](#urllib.parse.urldefrag)
...
urllib.parse.unwrap(url)[Â¶](#urllib.parse.unwrap)
...
## URL parsing security[Â¶](#url-parsing-security)
...
## Parsing ASCII Encoded Bytes[Â¶](#parsing-ascii-encoded-bytes)

The URL parsing functions were originally designed to operate on character
strings only. In practice, it is useful to be able to manipulate properly
quoted and encoded URLs as sequences of ASCII bytes. Accordingly, the
URL parsing functions in this module all operate on [`bytes`](stdtypes.html#bytes) and
[`bytearray`](stdtypes.html#bytearray) objects in addition to [`str`](stdtypes.html#str) objects.

If [`str`](stdtypes.html#str) data is passed in, the result will also contain only
[`str`](stdtypes.html#str) data. If [`bytes`](stdtypes.html#bytes) or [`bytearray`](stdtypes.html#bytearray) data is
passed in, the result will contain only [`bytes`](stdtypes.html#bytes) data.

Attempting to mix [`str`](stdtypes.html#str) data with [`bytes`](stdtypes.html#bytes) or
[`bytearray`](stdtypes.html#bytearray) in a single function call will result in a
[`TypeError`](exceptions.html#TypeError) being raised, while attempting to pass in non-ASCII
byte values will trigger [`UnicodeDecodeError`](exceptions.html#UnicodeDecodeError).

To support easier conversion of result objects between [`str`](stdtypes.html#str) and
[`bytes`](stdtypes.html#bytes), all return values from URL parsing functions provide
either an `encode()` method (when the result contains [`str`](stdtypes.html#str)
data) or a `decode()` method (when the result contains [`bytes`](stdtypes.html#bytes)
data). The signatures of these methods match those of the corresponding
[`str`](stdtypes.html#str) and [`bytes`](stdtypes.html#bytes) methods (except that the default encoding
is `'ascii'` rather than `'utf-8'`). Each produces a value of a
corresponding type that contains either [`bytes`](stdtypes.html#bytes) data (for
`encode()` methods) or [`str`](stdtypes.html#str) data (for
`decode()` methods).
...
## Structured Parse Results[Â¶](#structured-parse-results)

The result objects from the [`urlsplit()`](#urllib.parse.urlsplit), [`urlparse()`](#urllib.parse.urlparse)  and
[`urldefrag()`](#urllib.parse.urldefrag) functions are subclasses of the [`tuple`](stdtypes.html#tuple) type.
These subclasses add the attributes listed in the documentation for
those functions, the encoding and decoding support described in the
previous section, as well as an additional method:

urllib.parse.SplitResult.geturl()[Â¶](#urllib.parse.urllib.parse.SplitResult.geturl)
...
results when operating on [`str`](stdtypes.html#str) objects:

class urllib.parse.DefragResult(url, fragment)[Â¶](#urllib.parse.DefragResult)

Concrete class for [`urldefrag()`](#urllib.parse.urldefrag) results containing [`str`](stdtypes.html#str)
data. The `encode()` method returns a [`DefragResultBytes`](#urllib.parse.DefragResultBytes)
instance.

Added in version 3.2.

class urllib.parse.ParseResult(scheme, netloc, path, params, query, fragment)[Â¶](#urllib.parse.ParseResult)

Concrete class for [`urlparse()`](#urllib.parse.urlparse) results containing [`str`](stdtypes.html#str)
data. The `encode()` method returns a [`ParseResultBytes`](#urllib.parse.ParseResultBytes)
instance.

class urllib.parse.SplitResult(scheme, netloc, path, query, fragment)[Â¶](#urllib.parse.SplitResult)

Concrete class for [`urlsplit()`](#urllib.parse.urlsplit) results containing [`str`](stdtypes.html#str)
data. The `encode()` method returns a [`SplitResultBytes`](#urllib.parse.SplitResultBytes)
instance.

The following classes provide the implementations of the parse results when
operating on [`bytes`](stdtypes.html#bytes) or [`bytearray`](stdtypes.html#bytearray) objects:

class urllib.parse.DefragResultBytes(url, fragment)[Â¶](#urllib.parse.DefragResultBytes)

Concrete class for [`urldefrag()`](#urllib.parse.urldefrag) results containing [`bytes`](stdtypes.html#bytes)
data. The `decode()` method returns a [`DefragResult`](#urllib.parse.DefragResult)
instance.

Added in version 3.2.

class urllib.parse.ParseResultBytes(scheme, netloc, path, params, query, fragment)[Â¶](#urllib.parse.ParseResultBytes)

Concrete class for [`urlparse()`](#urllib.parse.urlparse) results containing [`bytes`](stdtypes.html#bytes)
data. The `decode()` method returns a [`ParseResult`](#urllib.parse.ParseResult)
instance.

Added in version 3.2.

class urllib.parse.SplitResultBytes(scheme, netloc, path, query, fragment)[Â¶](#urllib.parse.SplitResultBytes)

Concrete class for [`urlsplit()`](#urllib.parse.urlsplit) results containing [`bytes`](stdtypes.html#bytes)
data. The `decode()` method returns a [`SplitResult`](#urllib.parse.SplitResult)
instance.

Added in version 3.2.

## URL Quoting[Â¶](#url-quoting)

The URL quoting functions focus on taking program data and making it safe
for use as URL components by quoting special characters and appropriately
encoding non-ASCII text. They also support reversing these operations to
recreate the original data from the contents of a URL component if that
task isnât already covered by the URL parsing functions above.

urllib.parse.quote(string, safe='/', encoding=None, errors=None)[Â¶](#urllib.parse.quote)

Replace special characters in *string* using the `%*xx*` escape. Letters,
digits, and the characters `'_.-~'` are never quoted. By default, this
function is intended for quoting the path section of a URL. The optional
*safe* parameter specifies additional ASCII characters that should not be
quoted â its default value is `'/'`.

*string* may be either a [`str`](stdtypes.html#str) or a [`bytes`](stdtypes.html#bytes) object.

Changed in version 3.7: Moved from [**RFC 2396**](https://datatracker.ietf.org/doc/html/rfc2396.html) to [**RFC 3986**](https://datatracker.ietf.org/doc/html/rfc3986.html) for quoting URL strings. â~â is now
included in the set of unreserved characters.

The optional *encoding* and *errors* parameters specify how to deal with
non-ASCII characters, as accepted by the [`str.encode()`](stdtypes.html#str.encode) method.
*encoding* defaults to `'utf-8'`.
*errors* defaults to `'strict'`, meaning unsupported characters raise a
[`UnicodeEncodeError`](exceptions.html#UnicodeEncodeError).
*encoding* and *errors* must not be supplied if *string* is a
[`bytes`](stdtypes.html#bytes), or a [`TypeError`](exceptions.html#TypeError) is raised.

Note that `quote(string, safe, encoding, errors)` is equivalent to
`quote_from_bytes(string.encode(encoding, errors), safe)`.

Example: `quote('/El NiÃ±o/')` yields `'/El%20Ni%C3%B1o/'`.

urllib.parse.quote_plus(string, safe='', encoding=None, errors=None)[Â¶](#urllib.parse.quote_plus)
...
Example: `quote_plus('/El NiÃ±o/')` yields `'%2FEl+Ni%C3%B1o%2F'`.

urllib.parse.quote_from_bytes(bytes, safe='/')[Â¶](#urllib.parse.quote_from_bytes)

Like [`quote()`](#urllib.parse.quote), but accepts a [`bytes`](stdtypes.html#bytes) object rather than a
[`str`](stdtypes.html#str), and does not perform string-to-bytes encoding.

Example: `quote_from_bytes(b'a&\xef')` yields
`'a%26%EF'`.

urllib.parse.unquote(string, encoding='utf-8', errors='replace')[Â¶](#urllib.parse.unquote)
...
urllib.parse.unquote_plus(string, encoding='utf-8', errors='replace')[Â¶](#urllib.parse.unquote_plus)

Like [`unquote()`](#urllib.parse.unquote), but also replace plus signs with spaces, as required
for unquoting HTML form values.

*string* must be a [`str`](stdtypes.html#str).

Example: `unquote_plus('/El+Ni%C3%B1o/')` yields `'/El NiÃ±o/'`.

urllib.parse.unquote_to_bytes(string)[Â¶](#urllib.parse.unquote_to_bytes)

Replace `%*xx*` escapes with their single-octet equivalent, and return a
[`bytes`](stdtypes.html#bytes) object.

*string* may be either a [`str`](stdtypes.html#str) or a [`bytes`](stdtypes.html#bytes) object.

If it is a [`str`](stdtypes.html#str), unescaped non-ASCII characters in *string*
are encoded into UTF-8 bytes.

Example: `unquote_to_bytes('a%26%EF')` yields `b'a&\xef'`.

urllib.parse.urlencode(query, doseq=False, safe='', encoding=None, errors=None, quote_via=quote_plus)[Â¶](#urllib.parse.urlencode)

Convert a mapping object or a sequence of two-element tuples, which may
contain [`str`](stdtypes.html#str) or [`bytes`](stdtypes.html#bytes) objects, to a percent-encoded ASCII
text string.  If the resultant string is to be used as a *data* for POST
operation with the [`urlopen()`](urllib.request.html#urllib.request.urlopen) function, then
...
means spaces are quoted as a `'+'` character and â/â characters are
encoded as `%2F`, which follows the standard for GET requests
(`application/x-www-form-urlencoded`).  An alternate function that can be
passed as *quote_via* is [`quote()`](#urllib.parse.quote), which will encode spaces as `%20`
and not encode â/â characters.  For maximum control of what is quoted, use
...
when a query element is a [`str`](stdtypes.html#str)).
...
[**RFC 2732**](https://datatracker.ietf.org/doc/html/rfc2732.html) - Format for Literal IPv6 Addresses in URLâs.
...
relative URL, including a fair number of âAbnormal Examplesâ which govern the
...
[`urllib.request` â Extensible library for opening URLs](urllib.request.html)
...
[`urllib.error` â Exception classes raised by urllib.request](urllib.error.html)
