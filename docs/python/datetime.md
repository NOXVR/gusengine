...
datetime â Basic date and time types — Python 3.14.3 documentation
...
# `datetime` â Basic date and time types[Â¶](#module-datetime)
...
Module [`time`](time.html#module-time)
Time access and conversions.

Module [`zoneinfo`](zoneinfo.html#module-zoneinfo)
...
## Aware and Naive Objects[Â¶](#aware-and-naive-objects)
...
can be set to an instance of a subclass of the abstract [`tzinfo`](#datetime.tzinfo) class.
...
datetime.MINYEAR[Â¶](#datetime.MINYEAR)
...
datetime.MAXYEAR[Â¶](#datetime.MAXYEAR)
...
### Common Properties[Â¶](#common-properties)
...
- 
Objects of these types support efficient pickling via the [`pickle`](pickle.html#module-pickle) module.

### Determining if an Object is Aware or Naive[Â¶](#determining-if-an-object-is-aware-or-naive)
...
The distinction between aware and naive doesnât apply to [`timedelta`](#datetime.timedelta)
objects.

## [`timedelta`](#datetime.timedelta) Objects[Â¶](#timedelta-objects)
...
class datetime.timedelta(days=0, seconds=0, microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0)[Â¶](#datetime.timedelta)
...
*days*, *seconds* and *microseconds* are âmergedâ and normalized into those
...
[`OverflowError`](exceptions.html#OverflowError) is raised.
...
timedelta.days[Â¶](#datetime.timedelta.days)

Between -999,999,999 and 999,999,999 inclusive.

timedelta.seconds[Â¶](#datetime.timedelta.seconds)
...
when it is actually intended to get a [`total_seconds()`](#datetime.timedelta.total_seconds)
value instead:
...
timedelta.microseconds[Â¶](#datetime.timedelta.microseconds)
...
interval unit `t3`. Returns a [`float`](functions.html#float)
object.
...
[`timedelta`](#datetime.timedelta) object. (3)
...
Returns a [`timedelta`](#datetime.timedelta) object with the
same value. (2)
...
[`timedelta`](#datetime.timedelta) object as a constructor
...
- 
Division by zero raises [`ZeroDivisionError`](exceptions.html#ZeroDivisionError).

- 
`-timedelta.max` is not representable as a [`timedelta`](#datetime.timedelta) object.

- 
String representations of [`timedelta`](#datetime.timedelta) objects are normalized
...
[`timedelta`](#datetime.timedelta) object are now supported, as are remainder operations and
the [`divmod()`](functions.html#divmod) function. True division and multiplication of a
[`timedelta`](#datetime.timedelta) object by a [`float`](functions.html#float) object are now supported.
...
considered to be true if and only if it isnât equal to `timedelta(0)`.

Instance methods:

timedelta.total_seconds()[Â¶](#datetime.timedelta.total_seconds)
...
### Examples of usage: [`timedelta`](#datetime.timedelta)[Â¶](#examples-of-usage-timedelta)
...
## [`date`](#datetime.date) Objects[Â¶](#date-objects)
...
class datetime.date(year, month, day)[Â¶](#datetime.date)
...
If an argument outside those ranges is given, [`ValueError`](exceptions.html#ValueError) is raised.

Other constructors, all class methods:

classmethod date.today()[Â¶](#datetime.date.today)

Return the current local date.

This is equivalent to `date.fromtimestamp(time.time())`.

classmethod date.fromtimestamp(timestamp)[Â¶](#datetime.date.fromtimestamp)

Return the local date corresponding to the POSIX timestamp, such as is
returned by [`time.time()`](time.html#time.time).

This may raise [`OverflowError`](exceptions.html#OverflowError), if the timestamp is out
of the range of values supported by the platform C `localtime()`
function, and [`OSError`](exceptions.html#OSError) on `localtime()` failure.
...
Changed in version 3.3: Raise [`OverflowError`](exceptions.html#OverflowError) instead of [`ValueError`](exceptions.html#ValueError) if the timestamp
is out of the range of values supported by the platform C
`localtime()` function. Raise [`OSError`](exceptions.html#OSError) instead of
[`ValueError`](exceptions.html#ValueError) on `localtime()` failure.

classmethod date.fromordinal(ordinal)[Â¶](#datetime.date.fromordinal)

Return the date corresponding to the proleptic Gregorian ordinal, where
January 1 of year 1 has ordinal 1.

[`ValueError`](exceptions.html#ValueError) is raised unless 1 <= ordinal <=
date.max.toordinal(). For any date `d`,
`date.fromordinal(d.toordinal()) == d`.

classmethod date.fromisoformat(date_string)[Â¶](#datetime.date.fromisoformat)
...
classmethod date.fromisocalendar(year, week, day)[Â¶](#datetime.date.fromisocalendar)
...
classmethod date.strptime(date_string, format)[Â¶](#datetime.date.strptime)
...
[`ValueError`](exceptions.html#ValueError) is raised if the date_string and format
canât be parsed by [`time.strptime()`](time.html#time.strptime) or if it returns a value which isnât a
...
[`DeprecationWarning`](exceptions.html#DeprecationWarning) is emitted.  This is to avoid a quadrennial
...
date.min[Â¶](#datetime.date.min)

The earliest representable date, `date(MINYEAR, 1, 1)`.

date.max[Â¶](#datetime.date.max)

The latest representable date, `date(MAXYEAR, 12, 31)`.

date.resolution[Â¶](#datetime.date.resolution)
...
date.year[Â¶](#datetime.date.year)

Between [`MINYEAR`](#datetime.MINYEAR) and [`MAXYEAR`](#datetime.MAXYEAR) inclusive.

date.month[Â¶](#datetime.date.month)

Between 1 and 12 inclusive.

date.day[Â¶](#datetime.date.day)
...
[`OverflowError`](exceptions.html#OverflowError) is raised if `date2.year` would be smaller than
...
- 
[`date`](#datetime.date) objects are equal if they represent the same date.
...
date.replace(year=self.year, month=self.month, day=self.day)[Â¶](#datetime.date.replace)
...
The generic function [`copy.replace()`](copy.html#copy.replace) also supports [`date`](#datetime.date)
objects.

date.timetuple()[Â¶](#datetime.date.timetuple)

Return a [`time.struct_time`](time.html#time.struct_time) such as returned by [`time.localtime()`](time.html#time.localtime).
...
date.isoformat()[Â¶](#datetime.date.isoformat)
...
date.__str__()[Â¶](#datetime.date.__str__)

For a date `d`, `str(d)` is equivalent to `d.isoformat()`.

date.ctime()[Â¶](#datetime.date.ctime)
...
`ctime()` function (which [`time.ctime()`](time.html#time.ctime) invokes, but which
[`date.ctime()`](#datetime.date.ctime) does not invoke) conforms to the C standard.

date.strftime(format)[Â¶](#datetime.date.strftime)
...
date.__format__(format)[Â¶](#datetime.date.__format__)
...
literals and when using [`str.format()`](stdtypes.html#str.format).
See also [strftime() and strptime() Behavior](#strftime-strptime-behavior) and [`date.isoformat()`](#datetime.date.isoformat).

### Examples of Usage: [`date`](#datetime.date)[Â¶](#examples-of-usage-date)
...
## [`datetime`](#datetime.datetime) Objects[Â¶](#datetime-objects)
...
[`datetime`](#datetime.datetime) assumes there are exactly 3600*24 seconds in every day.

Constructor:

class datetime.datetime(year, month, day, hour=0, minute=0, second=0, microsecond=0, tzinfo=None, *, fold=0)[Â¶](#datetime.datetime)
...
classmethod datetime.today()[Â¶](#datetime.datetime.today)
...
classmethod datetime.now(tz=None)[Â¶](#datetime.datetime.now)
...
precision than can be gotten from going through a [`time.time()`](time.html#time.time) timestamp
(for example, this may be possible on platforms supplying the C
`gettimeofday()` function).

If *tz* is not `None`, it must be an instance of a [`tzinfo`](#datetime.tzinfo) subclass,
and the current date and time are converted to *tz*âs time zone.
...
classmethod datetime.utcnow()[Â¶](#datetime.datetime.utcnow)
...
classmethod datetime.fromtimestamp(timestamp, tz=None)[Â¶](#datetime.datetime.fromtimestamp)

Return the local date and time corresponding to the POSIX timestamp, such as is
returned by [`time.time()`](time.html#time.time). If optional argument *tz* is `None` or not
...
timestamp is converted to *tz*âs time zone.

[`fromtimestamp()`](#datetime.datetime.fromtimestamp) may raise [`OverflowError`](exceptions.html#OverflowError), if the timestamp is out of
the range of values supported by the platform C `localtime()` or
`gmtime()` functions, and [`OSError`](exceptions.html#OSError) on `localtime()` or
`gmtime()` failure.
...
Changed in version 3.3: Raise [`OverflowError`](exceptions.html#OverflowError) instead of [`ValueError`](exceptions.html#ValueError) if the timestamp
is out of the range of values supported by the platform C
`localtime()` or `gmtime()` functions. Raise [`OSError`](exceptions.html#OSError)
instead of [`ValueError`](exceptions.html#ValueError) on `localtime()` or `gmtime()`
failure.
...
classmethod datetime.utcfromtimestamp(timestamp)[Â¶](#datetime.datetime.utcfromtimestamp)
...
This may raise [`OverflowError`](exceptions.html#OverflowError), if the timestamp is
out of the range of values supported by the platform C `gmtime()` function,
and [`OSError`](exceptions.html#OSError) on `gmtime()` failure.
...
Changed in version 3.3: Raise [`OverflowError`](exceptions.html#OverflowError) instead of [`ValueError`](exceptions.html#ValueError) if the timestamp
is out of the range of values supported by the platform C
`gmtime()` function. Raise [`OSError`](exceptions.html#OSError) instead of
[`ValueError`](exceptions.html#ValueError) on `gmtime()` failure.

Deprecated since version 3.12: Use [`datetime.fromtimestamp()`](#datetime.datetime.fromtimestamp) with [`UTC`](#datetime.UTC) instead.

classmethod datetime.fromordinal(ordinal)[Â¶](#datetime.datetime.fromordinal)

Return the [`datetime`](#datetime.datetime) corresponding to the proleptic Gregorian ordinal,
where January 1 of year 1 has ordinal 1. [`ValueError`](exceptions.html#ValueError) is raised unless
...
classmethod datetime.combine(date, time, tzinfo=time.tzinfo)[Â¶](#datetime.datetime.combine)

Return a new [`datetime`](#datetime.datetime) object whose date components are equal to the
given [`date`](#datetime.date) objectâs, and whose time components
are equal to the given [`time`](#datetime.time) objectâs. If the *tzinfo*
...
is used.  If the *date* argument is a [`datetime`](#datetime.datetime) object, its time components
...
classmethod datetime.fromisoformat(date_string)[Â¶](#datetime.datetime.fromisoformat)
...
classmethod datetime.fromisocalendar(year, week, day)[Â¶](#datetime.datetime.fromisocalendar)
...
classmethod datetime.strptime(date_string, format)[Â¶](#datetime.datetime.strptime)
...
[`DeprecationWarning`](exceptions.html#DeprecationWarning) is now emitted.  This is to avoid a quadrennial
leap year bug in code seeking to parse only a month and day as the
default year used in absence of one in the format is not a leap year.
Such *format* values may raise an error as of Python 3.15.  The
workaround is to always include a year in your *format*.  If parsing
...
datetime.year[Â¶](#datetime.datetime.year)

Between [`MINYEAR`](#datetime.MINYEAR) and [`MAXYEAR`](#datetime.MAXYEAR) inclusive.

datetime.month[Â¶](#datetime.datetime.month)

Between 1 and 12 inclusive.

datetime.day[Â¶](#datetime.datetime.day)

Between 1 and the number of days in the given month of the given year.

datetime.hour[Â¶](#datetime.datetime.hour)

In `range(24)`.

datetime.minute[Â¶](#datetime.datetime.minute)

In `range(60)`.

datetime.second[Â¶](#datetime.datetime.second)

In `range(60)`.

datetime.microsecond[Â¶](#datetime.datetime.microsecond)

In `range(1000000)`.

datetime.tzinfo[Â¶](#datetime.datetime.tzinfo)

The object passed as the *tzinfo* argument to the [`datetime`](#datetime.datetime) constructor,
or `None` if none was passed.

datetime.fold[Â¶](#datetime.datetime.fold)
...
`datetime2 - datetime1 == timedelta` after. [`OverflowError`](exceptions.html#OverflowError) is raised if
...
- 
Subtraction of a [`datetime`](#datetime.datetime) from a [`datetime`](#datetime.datetime) is defined only if
both operands are naive, or if both are aware. If one is aware and the other is
naive, [`TypeError`](exceptions.html#TypeError) is raised.
...
- 
[`datetime`](#datetime.datetime) objects are equal if they represent the same date
...
raises [`TypeError`](exceptions.html#TypeError).
...
instances donât raise [`TypeError`](exceptions.html#TypeError).
...
datetime.date()[Â¶](#datetime.datetime.date)

Return [`date`](#datetime.date) object with same year, month and day.

datetime.time()[Â¶](#datetime.datetime.time)
...
datetime.timetz()[Â¶](#datetime.datetime.timetz)

Return [`time`](#datetime.time) object with same hour, minute, second, microsecond, fold, and
tzinfo attributes. See also method [`time()`](time.html#module-time).

Changed in version 3.6: The fold value is copied to the returned [`time`](#datetime.time) object.

datetime.replace(year=self.year, month=self.month, day=self.day, hour=self.hour, minute=self.minute, second=self.second, microsecond=self.microsecond, tzinfo=self.tzinfo, *, fold=0)[Â¶](#datetime.datetime.replace)
...
[`copy.replace()`](copy.html#copy.replace).

Changed in version 3.6: Added the *fold* parameter.

datetime.astimezone(tz=None)[Â¶](#datetime.datetime.astimezone)

Return a [`datetime`](#datetime.datetime) object with new [`tzinfo`](#datetime.datetime.tzinfo) attribute *tz*,
adjusting the date and time data so the result is the same UTC time as
*self*, but in *tz*âs local time.
...
datetime.utcoffset()[Â¶](#datetime.datetime.utcoffset)

If [`tzinfo`](#datetime.datetime.tzinfo) is `None`, returns `None`, else returns
`self.tzinfo.utcoffset(self)`, and raises an exception if the latter doesnât
return `None` or a [`timedelta`](#datetime.timedelta) object with magnitude less than one day.

Changed in version 3.7: The UTC offset is not restricted to a whole number of minutes.

datetime.dst()[Â¶](#datetime.datetime.dst)

If [`tzinfo`](#datetime.datetime.tzinfo) is `None`, returns `None`, else returns
`self.tzinfo.dst(self)`, and raises an exception if the latter doesnât return
...
datetime.tzname()[Â¶](#datetime.datetime.tzname)

If [`tzinfo`](#datetime.datetime.tzinfo) is `None`, returns `None`, else returns
`self.tzinfo.tzname(self)`, raises an exception if the latter doesnât return
`None` or a string object,

datetime.timetuple()[Â¶](#datetime.datetime.timetuple)
...
1st. The [`tm_isdst`](time.html#time.struct_time.tm_isdst) flag of the result is set according to the
...
datetime.utctimetuple()[Â¶](#datetime.datetime.utctimetuple)

If [`datetime`](#datetime.datetime) instance `d` is naive, this is the same as
`d.timetuple()` except that [`tm_isdst`](time.html#time.struct_time.tm_isdst) is forced to 0 regardless of what
`d.dst()` returns. DST is never in effect for a UTC time.

If `d` is aware, `d` is normalized to UTC time, by subtracting
`d.utcoffset()`, and a [`time.struct_time`](time.html#time.struct_time) for the
normalized time is returned. `tm_isdst` is forced to 0. Note
that an [`OverflowError`](exceptions.html#OverflowError) may be raised if `d.year` was
...
datetime.toordinal()[Â¶](#datetime.datetime.toordinal)

Return the proleptic Gregorian ordinal of the date. The same as
`self.date().toordinal()`.

datetime.timestamp()[Â¶](#datetime.datetime.timestamp)

Return POSIX timestamp corresponding to the [`datetime`](#datetime.datetime)
instance. The return value is a [`float`](functions.html#float) similar to that
returned by [`time.time()`](time.html#time.time).
...
function to perform the conversion. Since [`datetime`](#datetime.datetime)
supports wider range of values than `mktime()` on many
platforms, this method may raise [`OverflowError`](exceptions.html#OverflowError) or [`OSError`](exceptions.html#OSError)
...
datetime.isoformat(sep='T', timespec='auto')[Â¶](#datetime.datetime.isoformat)
...
[`ValueError`](exceptions.html#ValueError) will be raised on an invalid *timespec* argument:
...
datetime.__str__()[Â¶](#datetime.datetime.__str__)

For a [`datetime`](#datetime.datetime) instance `d`, `str(d)` is equivalent to
`d.isoformat(' ')`.

datetime.ctime()[Â¶](#datetime.datetime.ctime)
...
(which [`time.ctime()`](time.html#time.ctime) invokes, but which
[`datetime.ctime()`](#datetime.datetime.ctime) does not invoke) conforms to the C standard.

datetime.strftime(format)[Â¶](#datetime.datetime.strftime)
...
datetime.__format__(format)[Â¶](#datetime.datetime.__format__)
...
### Examples of Usage: [`datetime`](#datetime.datetime)[Â¶](#examples-of-usage-datetime)
...
## [`time`](#datetime.time) Objects[Â¶](#time-objects)
...
class datetime.time(hour=0, minute=0, second=0, microsecond=0, tzinfo=None, *, fold=0)[Â¶](#datetime.time)
...
If an argument outside those ranges is given, [`ValueError`](exceptions.html#ValueError) is raised. All
default to 0 except *tzinfo*, which defaults to `None`.

Class attributes:

time.min[Â¶](#datetime.time.min)

The earliest representable [`time`](#datetime.time), `time(0, 0, 0, 0)`.

time.max[Â¶](#datetime.time.max)

The latest representable [`time`](#datetime.time), `time(23, 59, 59, 999999)`.

time.resolution[Â¶](#datetime.time.resolution)
...
time.hour[Â¶](#datetime.time.hour)

In `range(24)`.

time.minute[Â¶](#datetime.time.minute)

In `range(60)`.

time.second[Â¶](#datetime.time.second)

In `range(60)`.

time.microsecond[Â¶](#datetime.time.microsecond)

In `range(1000000)`.

time.tzinfo[Â¶](#datetime.time.tzinfo)

The object passed as the tzinfo argument to the [`time`](#datetime.time) constructor, or
`None` if none was passed.

time.fold[Â¶](#datetime.time.fold)
...
donât raise [`TypeError`](exceptions.html#TypeError).
...
classmethod time.fromisoformat(time_string)[Â¶](#datetime.time.fromisoformat)
...
classmethod time.strptime(date_string, format)[Â¶](#datetime.time.strptime)
...
[`ValueError`](exceptions.html#ValueError) is raised if the *date_string* and *format*
cannot be parsed by [`time.strptime()`](time.html#time.strptime) or if it returns a value which is not a
time tuple.  See also [strftime() and strptime() Behavior](#strftime-strptime-behavior) and
...
time.replace(hour=self.hour, minute=self.minute, second=self.second, microsecond=self.microsecond, tzinfo=self.tzinfo, *, fold=0)[Â¶](#datetime.time.replace)
...
naive [`time`](#datetime.time) from an aware [`time`](#datetime.time), without conversion of the
time data.
...
time.isoformat(timespec='auto')[Â¶](#datetime.time.isoformat)
...
time.__str__()[Â¶](#datetime.time.__str__)

For a time `t`, `str(t)` is equivalent to `t.isoformat()`.

time.strftime(format)[Â¶](#datetime.time.strftime)
...
time.__format__(format)[Â¶](#datetime.time.__format__)
...
time.utcoffset()[Â¶](#datetime.time.utcoffset)

If [`tzinfo`](#datetime.time.tzinfo) is `None`, returns `None`, else returns
`self.tzinfo.utcoffset(None)`, and raises an exception if the latter doesnât
return `None` or a [`timedelta`](#datetime.timedelta) object with magnitude less than one day.

Changed in version 3.7: The UTC offset is not restricted to a whole number of minutes.

time.dst()[Â¶](#datetime.time.dst)

If [`tzinfo`](#datetime.time.tzinfo) is `None`, returns `None`, else returns
`self.tzinfo.dst(None)`, and raises an exception if the latter doesnât return
`None`, or a [`timedelta`](#datetime.timedelta) object with magnitude less than one day.

Changed in version 3.7: The DST offset is not restricted to a whole number of minutes.

time.tzname()[Â¶](#datetime.time.tzname)

If [`tzinfo`](#datetime.time.tzinfo) is `None`, returns `None`, else returns
`self.tzinfo.tzname(None)`, or raises an exception if the latter doesnât
return `None` or a string object.

### Examples of Usage: [`time`](#datetime.time)[Â¶](#examples-of-usage-time)
...
## [`tzinfo`](#datetime.tzinfo) Objects[Â¶](#tzinfo-objects)

class datetime.tzinfo[Â¶](#datetime.tzinfo)
...
view their attributes as being in local time, and the [`tzinfo`](#datetime.tzinfo) object
...
[`timezone`](#datetime.timezone), a simple concrete subclass of [`tzinfo`](#datetime.tzinfo) which can
...
[`__init__()`](../reference/datamodel.html#object.__init__) method that can be called with no arguments,
otherwise it can be
...
tzinfo.utcoffset(dt)[Â¶](#datetime.tzinfo.utcoffset)
...
[`utcoffset()`](#datetime.tzinfo.utcoffset) should return their sum. If the UTC offset isnât known,
...
tzinfo.dst(dt)[Â¶](#datetime.tzinfo.dst)
...
already been added to the UTC offset returned by [`utcoffset()`](#datetime.tzinfo.utcoffset), so thereâs
no need to consult [`dst()`](#datetime.tzinfo.dst) unless youâre interested in obtaining DST info
separately. For example, [`datetime.timetuple()`](#datetime.datetime.timetuple) calls its [`tzinfo`](#datetime.datetime.tzinfo)
attributeâs [`dst()`](#datetime.tzinfo.dst) method to determine how the [`tm_isdst`](time.html#time.struct_time.tm_isdst) flag
...
tz. For sane [`tzinfo`](#datetime.tzinfo) subclasses, this expression yields the time
...
responsibility to ensure it. If a [`tzinfo`](#datetime.tzinfo) subclass cannot guarantee
...
The default implementation of [`dst()`](#datetime.tzinfo.dst) raises [`NotImplementedError`](exceptions.html#NotImplementedError).

Changed in version 3.7: The DST offset is not restricted to a whole number of minutes.

tzinfo.tzname(dt)[Â¶](#datetime.tzinfo.tzname)
...
valid replies. Return `None` if a string name isnât known. Note that this is
a method rather than a fixed string primarily because some [`tzinfo`](#datetime.tzinfo)
subclasses will wish to return different names depending on the specific value
of *dt* passed, especially if the [`tzinfo`](#datetime.tzinfo) class is accounting for
daylight time.

The default implementation of [`tzname()`](#datetime.tzinfo.tzname) raises [`NotImplementedError`](exceptions.html#NotImplementedError).

These methods are called by a [`datetime`](#datetime.datetime) or [`time`](#datetime.time) object, in
response to their methods of the same names. A [`datetime`](#datetime.datetime) object passes
itself as the argument, and a [`time`](#datetime.time) object passes `None` as the
argument. A [`tzinfo`](#datetime.tzinfo) subclassâs methods should therefore be prepared to
accept a *dt* argument of `None`, or of class [`datetime`](#datetime.datetime).

When `None` is passed, itâs up to the class designer to decide the best
response. For example, returning `None` is appropriate if the class wishes to
say that time objects donât participate in the [`tzinfo`](#datetime.tzinfo) protocols. It
may be more useful for `utcoffset(None)` to return the standard UTC offset, as
there is no other convention for discovering the standard offset.

When a [`datetime`](#datetime.datetime) object is passed in response to a [`datetime`](#datetime.datetime)
method, `dt.tzinfo` is the same object as *self*. [`tzinfo`](#datetime.tzinfo) methods can
rely on this, unless user code calls [`tzinfo`](#datetime.tzinfo) methods directly. The
intent is that the [`tzinfo`](#datetime.tzinfo) methods interpret *dt* as being in local
...
`astimezone(Eastern)` wonât deliver a result with `hour == 2` on the day DST
...
[`astimezone()`](#datetime.datetime.astimezone) mimics the local clockâs behavior by mapping two adjacent UTC
...
or any other fixed-offset [`tzinfo`](#datetime.tzinfo) subclass (such as a class representing
only EST (fixed offset -5 hours), or only EDT (fixed offset -4 hours)).

See also

[`zoneinfo`](zoneinfo.html#module-zoneinfo)
...
## [`timezone`](#datetime.timezone) Objects[Â¶](#timezone-objects)
...
class datetime.timezone(offset, name=None)[Â¶](#datetime.timezone)
...
`timedelta(hours=24)`, otherwise [`ValueError`](exceptions.html#ValueError) is raised.
...
timezone.utcoffset(dt)[Â¶](#datetime.timezone.utcoffset)
...
timezone.tzname(dt)[Â¶](#datetime.timezone.tzname)
...
*offset* is `timedelta(0)`, the name is âUTCâ, otherwise it is a string in
the format `UTCÂ±HH:MM`, where Â± is the sign of `offset`, HH and MM are
...
timezone.dst(dt)[Â¶](#datetime.timezone.dst)

Always returns `None`.

timezone.fromutc(dt)[Â¶](#datetime.timezone.fromutc)
...
timezone.utc[Â¶](#datetime.timezone.utc)

The UTC time zone, `timezone(timedelta(0))`.

## [`strftime()`](#datetime.datetime.strftime) and [`strptime()`](#datetime.datetime.strptime) Behavior[Â¶](#strftime-and-strptime-behavior)
...
[`time.strptime()`](time.html#time.strptime) class methods create an object from a string
...
### [`strftime()`](#datetime.datetime.strftime) and [`strptime()`](#datetime.datetime.strptime) Format Codes[Â¶](#strftime-and-strptime-format-codes)
...
These may not be available on all platforms when used with the [`strftime()`](#datetime.datetime.strftime)
method. The ISO 8601 year and ISO 8601 week directives are not interchangeable
with the year and week number directives above. Calling [`strptime()`](#datetime.datetime.strptime) with
incomplete or ambiguous ISO 8601 directives will raise a [`ValueError`](exceptions.html#ValueError).

The full set of format codes supported varies across platforms, because Python
calls the platform C libraryâs `strftime()` function, and platform
...
### Technical Detail[Â¶](#technical-detail)

Broadly speaking, `d.strftime(fmt)` acts like the [`time`](time.html#module-time) moduleâs
...
be used, as `time` objects have no such values. If theyâre used anyway,
...
the output, while on others `strftime` may raise [`UnicodeError`](exceptions.html#UnicodeError) or return
...
- 
The [`strptime()`](#datetime.datetime.strptime) method can parse years in the full [1, 9999] range, but
...
- 
When used with the [`strptime()`](#datetime.datetime.strptime) method, the `%p` directive only affects
the output hour field if the `%I` directive is used to parse the hour.

- 
Unlike the [`time`](time.html#module-time) module, the `datetime` module does not support
leap seconds.

- 
When used with the [`strptime()`](#datetime.datetime.strptime) method, the `%f` directive
...
`Â±HHMM[SS[.ffffff]]`, where `HH` is a 2-digit string giving the number
...
- 
any value in `time.tzname` for your machineâs locale
...
- 
When used with the [`strptime()`](#datetime.datetime.strptime) method, `%U` and `%W` are only used
...
- 
When used with the [`strptime()`](#datetime.datetime.strptime) method, the leading zero is optional
for  formats `%d`, `%m`, `%H`, `%I`, `%M`, `%S`, `%j`, `%U`,
`%W`, and `%V`. Format `%y` does require a leading zero.

- 
When parsing a month and day using [`strptime()`](#datetime.datetime.strptime), always
...
Deprecated since version 3.13, will be removed in version 3.15: [`strptime()`](#datetime.datetime.strptime) calls using a format string containing
a day of month without a year now emit a
[`DeprecationWarning`](exceptions.html#DeprecationWarning). In 3.15 or later we may change this into
...
Dershowitz and Reingoldâs book *Calendrical Calculations*,
...
See R. H. van Gentâs [guide to the mathematics of the ISO 8601 calendar](https://web.archive.org/web/20220531051136/https://webspace.science.uu.nl/~gent0113/calendar/isocalendar.htm)
...
[`zoneinfo` â IANA time zone support](zoneinfo.html)
