...
logging â Logging facility for Python — Python 3.14.3 documentation
...
# `logging` â Logging facility for Python[Â¶](#module-logging)
...
## Logger Objects[Â¶](#logger-objects)
...
the recommended construction `logging.getLogger(__name__)`.  Thatâs because
in a module, `__name__` is the moduleâs name in the Python package namespace.

class logging.Logger[Â¶](#logging.Logger)

name[Â¶](#logging.Logger.name)

This is the loggerâs name, and is the value that was passed to [`getLogger()`](#logging.getLogger)
...
parent[Â¶](#logging.Logger.parent)
...
propagate[Â¶](#logging.Logger.propagate)
...
`logging.getLogger('A.B.C').error(...)` will [subject to passing that loggerâs
...
handlers[Â¶](#logging.Logger.handlers)
...
disabled[Â¶](#logging.Logger.disabled)
...
setLevel(level)[Â¶](#logging.Logger.setLevel)
...
unless a handlerâs level has been set to a higher severity level than *level*.
...
isEnabledFor(level)[Â¶](#logging.Logger.isEnabledFor)

Indicates if a message of severity *level* would be processed by this logger.
This method checks first the module-level level set by
`logging.disable(level)` and then the loggerâs effective level as determined
by [`getEffectiveLevel()`](#logging.Logger.getEffectiveLevel).

getEffectiveLevel()[Â¶](#logging.Logger.getEffectiveLevel)
...
getChild(suffix)[Â¶](#logging.Logger.getChild)
...
getChildren()[Â¶](#logging.Logger.getChildren)
...
loggers named `foo` and `bar`, but a logger named `foo.bar` wouldnât be
included in the set. Likewise, `logging.getLogger('foo').getChildren()` might
return a set including a logger named `foo.bar`, but it wouldnât include one
named `foo.bar.baz`.

Added in version 3.12.

debug(msg, *args, **kwargs)[Â¶](#logging.Logger.debug)
...
[`sys.exc_info()`](sys.html#sys.exc_info)) or an exception instance is provided, it is used;
otherwise, [`sys.exc_info()`](sys.html#sys.exc_info) is called to get the exception information.
...
parameter mirrors the equivalent one in the [`warnings`](warnings.html#module-warnings) module.

The fourth keyword argument is *extra* which can be used to pass a
dictionary which is used to populate the [`__dict__`](../reference/datamodel.html#object.__dict__) of the
...
error(msg, *args, **kwargs)[Â¶](#logging.Logger.error)

Logs a message with level [`ERROR`](#logging.ERROR) on this logger. The arguments are
interpreted as for [`debug()`](#logging.debug).

critical(msg, *args, **kwargs)[Â¶](#logging.Logger.critical)

Logs a message with level [`CRITICAL`](#logging.CRITICAL) on this logger. The arguments are
interpreted as for [`debug()`](#logging.debug).

log(level, msg, *args, **kwargs)[Â¶](#logging.Logger.log)

Logs a message with integer level *level* on this logger. The other arguments are
interpreted as for [`debug()`](#logging.debug).

exception(msg, *args, **kwargs)[Â¶](#logging.Logger.exception)

Logs a message with level [`ERROR`](#logging.ERROR) on this logger. The arguments are
interpreted as for [`debug()`](#logging.debug). Exception info is added to the logging
message. This method should only be called from an exception handler.

addFilter(filter)[Â¶](#logging.Logger.addFilter)

Adds the specified filter *filter* to this logger.

removeFilter(filter)[Â¶](#logging.Logger.removeFilter)

Removes the specified filter *filter* from this logger.

filter(record)[Â¶](#logging.Logger.filter)

Apply this loggerâs filters to the record and return `True` if the
record is to be processed. The filters are consulted in turn, until one of
them returns a false value. If none of them return a false value, the record
will be processed (passed to handlers). If one returns a false value, no
further processing of the record occurs.

addHandler(hdlr)[Â¶](#logging.Logger.addHandler)

Adds the specified handler *hdlr* to this logger.

removeHandler(hdlr)[Â¶](#logging.Logger.removeHandler)

Removes the specified handler *hdlr* from this logger.

findCaller(stack_info=False, stacklevel=1)[Â¶](#logging.Logger.findCaller)
...
handle(record)[Â¶](#logging.Logger.handle)
...
makeRecord(name, level, fn, lno, msg, args, exc_info, func=None, extra=None, sinfo=None)[Â¶](#logging.Logger.makeRecord)

This is a factory method which can be overridden in subclasses to create
specialized [`LogRecord`](#logging.LogRecord) instances.

hasHandlers()[Â¶](#logging.Logger.hasHandlers)
...
## Logging Levels[Â¶](#logging-levels)
...
logging.NOTSET[Â¶](#logging.NOTSET)
...
logging.WARNING[Â¶](#logging.WARNING)
...
logging.CRITICAL[Â¶](#logging.CRITICAL)
...
## Handler Objects[Â¶](#handler-objects)
...
class logging.Handler[Â¶](#logging.Handler)

__init__(level=NOTSET)[Â¶](#logging.Handler.__init__)
...
setFormatter(fmt)[Â¶](#logging.Handler.setFormatter)

Sets the formatter for this handler to *fmt*.
The *fmt* argument must be a [`Formatter`](#logging.Formatter) instance or `None`.

addFilter(filter)[Â¶](#logging.Handler.addFilter)

Adds the specified filter *filter* to this handler.

removeFilter(filter)[Â¶](#logging.Handler.removeFilter)

Removes the specified filter *filter* from this handler.

filter(record)[Â¶](#logging.Handler.filter)

Apply this handlerâs filters to the record and return `True` if the
record is to be processed. The filters are consulted in turn, until one of
them returns a false value. If none of them return a false value, the record
will be emitted. If one returns a false value, the handler will not emit the
record.

flush()[Â¶](#logging.Handler.flush)

Ensure all logging output has been flushed. This version does nothing and is
intended to be implemented by subclasses.

close()[Â¶](#logging.Handler.close)
...
handle(record)[Â¶](#logging.Handler.handle)

Conditionally emits the specified logging record, depending on filters which may
have been added to the handler. Wraps the actual emission of the record with
acquisition/release of the I/O thread lock.

handleError(record)[Â¶](#logging.Handler.handleError)
...
format(record)[Â¶](#logging.Handler.format)

Do formatting for a record - if a formatter is set, use it. Otherwise, use the
default formatter for the module.

emit(record)[Â¶](#logging.Handler.emit)
...
For a list of handlers included as standard, see [`logging.handlers`](logging.handlers.html#module-logging.handlers).

## Formatter Objects[Â¶](#formatter-objects)

class logging.Formatter(fmt=None, datefmt=None, style='%', validate=True, *, defaults=None)[Â¶](#logging.Formatter)
...
- 
**fmt** ([*str*](stdtypes.html#str)) â A format string in the given *style* for
the logged output as a whole.
The possible mapping keys are drawn from the [`LogRecord`](#logging.LogRecord) objectâs
[LogRecord attributes](#logrecord-attributes).
If not specified, `'%(message)s'` is used,
which is just the logged message.

- 
**datefmt** ([*str*](stdtypes.html#str)) â A format string for the date/time portion of the logged output.
If not specified, the default described in [`formatTime()`](#logging.Formatter.formatTime) is used.

- 
**style** ([*str*](stdtypes.html#str)) â Can be one of `'%'`, `'{'` or `'$'` and determines
how the format string will be merged with its data: using one of
[printf-style String Formatting](stdtypes.html#old-string-formatting) (`%`), [`str.format()`](stdtypes.html#str.format) (`{`)
or [`string.Template`](string.html#string.Template) (`$`). This only applies to
...
- 
**validate** ([*bool*](functions.html#bool)) â If `True` (the default), incorrect or mismatched
*fmt* and *style* will raise a [`ValueError`](exceptions.html#ValueError); for example,
`logging.Formatter('%(asctime)s - %(message)s', style='{')`.

- 
**defaults** ([*dict*](stdtypes.html#dict)*[*[*str*](stdtypes.html#str)*, **Any**]*) â A dictionary with default values to use in custom fields.
For example,
...
format(record)[Â¶](#logging.Formatter.format)
...
formatTime(record, datefmt=None)[Â¶](#logging.Formatter.formatTime)

This method should be called from [`format()`](functions.html#format) by a formatter which
wants to make use of a formatted time. This method can be overridden in
formatters to provide for any specific requirement, but the basic behavior
is as follows: if *datefmt* (a string) is specified, it is used with
[`time.strftime()`](time.html#time.strftime) to format the creation time of the
...
[`time.strftime()`](time.html#time.strftime) documentation.  An example time in this format is
`2003-01-23 00:29:50,411`.  The resulting string is returned.

This function uses a user-configurable function to convert the creation
time to a tuple. By default, [`time.localtime()`](time.html#time.localtime) is used; to change
this for a particular formatter instance, set the `converter` attribute
to a function with the same signature as [`time.localtime()`](time.html#time.localtime) or
[`time.gmtime()`](time.html#time.gmtime). To change it for all formatters, for example if you
...
appended using another format string, `'%s,%03d'` â and both of these
...
formatException(exc_info)[Â¶](#logging.Formatter.formatException)

Formats the specified exception information (a standard exception tuple as
returned by [`sys.exc_info()`](sys.html#sys.exc_info)) as a string. This default implementation
just uses [`traceback.print_exception()`](traceback.html#traceback.print_exception). The resulting string is
returned.

formatStack(stack_info)[Â¶](#logging.Formatter.formatStack)

Formats the specified stack information (a string as returned by
[`traceback.print_stack()`](traceback.html#traceback.print_stack), but with the last newline removed) as a
string. This default implementation just returns the input value.

class logging.BufferingFormatter(linefmt=None)[Â¶](#logging.BufferingFormatter)
...
formatHeader(records)[Â¶](#logging.BufferingFormatter.formatHeader)
...
formatFooter(records)[Â¶](#logging.BufferingFormatter.formatFooter)
...
format(records)[Â¶](#logging.BufferingFormatter.format)
...
## Filter Objects[Â¶](#filter-objects)
...
class logging.Filter(name='')[Â¶](#logging.Filter)
...
filter(record)[Â¶](#logging.Filter.filter)
...
You donât actually need to subclass `Filter`: you can pass any instance
which has a `filter` method with the same semantics.

Changed in version 3.2: You donât need to create specialized `Filter` classes, or use other
...
object has a `filter` attribute: if it does, itâs assumed to be a
`Filter` and its [`filter()`](#logging.Filter.filter) method is called. Otherwise, itâs
...
## LogRecord Objects[Â¶](#logrecord-objects)
...
class logging.LogRecord(name, level, pathname, lineno, msg, args, exc_info, func=None, sinfo=None)[Â¶](#logging.LogRecord)
...
- 
**name** ([*str*](stdtypes.html#str)) â The name of the logger used to log the event
...
- 
**level** ([*int*](functions.html#int)) â The [numeric level](#levels) of the logging event
...
- 
**pathname** ([*str*](stdtypes.html#str)) â The full string path of the source file
where the logging call was made.

- 
**lineno** ([*int*](functions.html#int)) â The line number in the source file
where the logging call was made.

- 
**msg** ([*Any*](typing.html#typing.Any)) â The event description message,
...
- 
**args** ([*tuple*](stdtypes.html#tuple)* | *[*dict*](stdtypes.html#dict)*[*[*str*](stdtypes.html#str)*, *[*Any*](typing.html#typing.Any)*]*) â Variable data to merge into the *msg* argument
to obtain the event description.

- 
**exc_info** ([*tuple*](stdtypes.html#tuple)*[*[*type*](functions.html#type)*[*[*BaseException*](exceptions.html#BaseException)*]**, *[*BaseException*](exceptions.html#BaseException)*, *[*types.TracebackType*](types.html#types.TracebackType)*] **| **None*) â An exception tuple with the current exception information,
as returned by [`sys.exc_info()`](sys.html#sys.exc_info),
or `None` if no exception information is available.

- 
**func** ([*str*](stdtypes.html#str)* | **None*) â The name of the function or method
from which the logging call was invoked.

- 
**sinfo** ([*str*](stdtypes.html#str)* | **None*) â A text string representing stack information
from the base of the stack in the current thread,
up to the logging call.

getMessage()[Â¶](#logging.LogRecord.getMessage)

Returns the message for this [`LogRecord`](#logging.LogRecord) instance after merging any
user-supplied arguments with the message. If the user-supplied message
argument to the logging call is not a string, [`str()`](stdtypes.html#str) is called on it to
...
## LogRecord attributes[Â¶](#logrecord-attributes)
...
If you are using {}-formatting ([`str.format()`](stdtypes.html#str.format)), you can use
`{attrname}` as the placeholder in the format string. If you are using
$-formatting ([`string.Template`](string.html#string.Template)), use the form `${attrname}`. In
...
`004`. Refer to the [`str.format()`](stdtypes.html#str.format) documentation for full details on
...
[`LogRecord`](#logging.LogRecord) was created.  By default
...
Time when the [`LogRecord`](#logging.LogRecord) was created
(as returned by [`time.time_ns()`](time.html#time.time_ns) / 1e9).
...
Exception tuple (Ã  la `sys.exc_info`) or,
...
This is set when [`Formatter.format()`](#logging.Formatter.format) is
...
[`Formatter.format()`](#logging.Formatter.format) is invoked.
...
[`LogRecord`](#logging.LogRecord) was created.
...
(see [Using arbitrary objects as messages](../howto/logging.html#arbitrary-object-messages)).
...
[`asyncio.Task`](asyncio-task.html#asyncio.Task) name (if available).

Changed in version 3.1: *processName* was added.

Changed in version 3.12: *taskName* was added.

## LoggerAdapter Objects[Â¶](#loggeradapter-objects)
...
class logging.LoggerAdapter(logger, extra=None, merge_extra=False)[Â¶](#logging.LoggerAdapter)
...
process(msg, kwargs)[Â¶](#logging.LoggerAdapter.process)

Modifies the message and/or keyword arguments passed to a logging call in
order to insert contextual information. This implementation takes the object
passed as *extra* to the constructor and adds it to *kwargs* using key
âextraâ. The return value is a (*msg*, *kwargs*) tuple which has the
(possibly modified) versions of the arguments passed in.

manager[Â¶](#logging.LoggerAdapter.manager)

Delegates to the underlying `manager` on *logger*.

_log[Â¶](#logging.LoggerAdapter._log)
...
If you are implementing asynchronous signal handlers using the [`signal`](signal.html#module-signal)
module, you may not be able to use logging from within such handlers. This is
because lock implementations in the [`threading`](threading.html#module-threading) module are not always
re-entrant, and so cannot be invoked from such signal handlers.

## Module-Level Functions[Â¶](#module-level-functions)

In addition to the classes described above, there are a number of module-level
functions.

logging.getLogger(name=None)[Â¶](#logging.getLogger)
...
dot-separated hierarchical name like *âaâ*, *âa.bâ* or *âa.b.c.dâ*. Choice
...
logging.getLoggerClass()[Â¶](#logging.getLoggerClass)
...
logging.getLogRecordFactory()[Â¶](#logging.getLogRecordFactory)
...
logging.debug(msg, *args, **kwargs)[Â¶](#logging.debug)
...
logging.info(msg, *args, **kwargs)[Â¶](#logging.info)
...
logging.warning(msg, *args, **kwargs)[Â¶](#logging.warning)
...
`CRITICAL` (this is not recommended), you wonât be able to rely on the
...
logging.addLevelName(level, levelName)[Â¶](#logging.addLevelName)
...
logging.getLevelNamesMapping()[Â¶](#logging.getLevelNamesMapping)

Returns a mapping from level names to their corresponding logging levels. For example, the
string âCRITICALâ maps to [`CRITICAL`](#logging.CRITICAL). The returned mapping is copied from an internal
mapping on each call to this function.

Added in version 3.11.

logging.getLevelName(level)[Â¶](#logging.getLevelName)
...
logging.getHandlerByName(name)[Â¶](#logging.getHandlerByName)
...
logging.getHandlerNames()[Â¶](#logging.getHandlerNames)

Returns an immutable set of all known handler names.

Added in version 3.12.

logging.makeLogRecord(attrdict)[Â¶](#logging.makeLogRecord)
...
logging.basicConfig(**kwargs)[Â¶](#logging.basicConfig)

Does basic configuration for the logging system by creating a
[`StreamHandler`](logging.handlers.html#logging.StreamHandler) with a default [`Formatter`](#logging.Formatter) and adding it to the
...
Specifies that a [`FileHandler`](logging.handlers.html#logging.FileHandler) be
created, using the specified filename,
rather than a [`StreamHandler`](logging.handlers.html#logging.StreamHandler).

*filemode*

If *filename* is specified, open the file
in this [mode](functions.html#filemodes). Defaults
to `'a'`.
...
accepted by [`time.strftime()`](time.html#time.strftime).

*style*

If *format* is specified, use this style
for the format string. One of `'%'`,
`'{'` or `'$'` for [printf-style](stdtypes.html#old-string-formatting),
[`str.format()`](stdtypes.html#str.format) or
[`string.Template`](string.html#string.Template) respectively.
Defaults to `'%'`.
...
[`StreamHandler`](logging.handlers.html#logging.StreamHandler). Note that this
...
[`FileHandler`](logging.handlers.html#logging.FileHandler) is created, and thus
...
it will be passed as such to [`open()`](functions.html#open),
...
logging.shutdown()[Â¶](#logging.shutdown)
...
handler (see [`atexit`](atexit.html#module-atexit)), so normally thereâs no need to do that
manually.

logging.setLoggerClass(klass)[Â¶](#logging.setLoggerClass)
...
logging.setLogRecordFactory(factory)[Â¶](#logging.setLogRecordFactory)
...
[`traceback.print_stack()`](traceback.html#traceback.print_stack), showing the call hierarchy.

kwargs:

Additional keyword arguments.

## Module-Level Attributes[Â¶](#module-level-attributes)

logging.lastResort[Â¶](#logging.lastResort)

A âhandler of last resortâ is available through this attribute. This
is a [`StreamHandler`](logging.handlers.html#logging.StreamHandler) writing to `sys.stderr` with a level of
...
logging.raiseExceptions[Â¶](#logging.raiseExceptions)
...
## Integration with the warnings module[Â¶](#integration-with-the-warnings-module)

The [`captureWarnings()`](#logging.captureWarnings) function can be used to integrate `logging`
with the [`warnings`](warnings.html#module-warnings) module.

logging.captureWarnings(capture)[Â¶](#logging.captureWarnings)

This function is used to turn the capture of warnings by logging on and
off.

If *capture* is `True`, warnings issued by the [`warnings`](warnings.html#module-warnings) module will
be redirected to the logging system. Specifically, a warning will be
formatted using [`warnings.formatwarning()`](warnings.html#warnings.formatwarning) and the resulting string
...
Module [`logging.handlers`](logging.handlers.html#module-logging.handlers)
...
[`time` â Time access and conversions](time.html)
