...
logging.handlers â Logging handlers — Python 3.14.3 documentation
...
# `logging.handlers` â Logging handlers[Â¶](#module-logging.handlers)

**Source code:** [Lib/logging/handlers.py](https://github.com/python/cpython/tree/3.14/Lib/logging/handlers.py)
...
[`NullHandler`](#logging.NullHandler)) are actually defined in the [`logging`](logging.html#module-logging) module itself,
but have been documented here along with the other handlers.

## StreamHandler[Â¶](#streamhandler)

The [`StreamHandler`](#logging.StreamHandler) class, located in the core [`logging`](logging.html#module-logging) package,
sends logging output to streams such as *sys.stdout*, *sys.stderr* or any
file-like object (or, more precisely, any object which supports `write()`
and `flush()` methods).

class logging.StreamHandler(stream=None)[Â¶](#logging.StreamHandler)

Returns a new instance of the [`StreamHandler`](#logging.StreamHandler) class. If *stream* is
specified, the instance will use it for logging output; otherwise, *sys.stderr*
will be used.

emit(record)[Â¶](#logging.StreamHandler.emit)
...
is present, it is formatted using [`traceback.print_exception()`](traceback.html#traceback.print_exception) and
appended to the stream.

flush()[Â¶](#logging.StreamHandler.flush)

Flushes the stream by calling its [`flush()`](#logging.StreamHandler.flush) method. Note that the
`close()` method is inherited from [`Handler`](logging.html#logging.Handler) and so
does no output, so an explicit [`flush()`](#logging.StreamHandler.flush) call may be needed at times.

setStream(stream)[Â¶](#logging.StreamHandler.setStream)

Sets the instanceâs stream to the specified value, if it is different.
...
the old stream, if the stream was changed, or `None` if it wasnât.

Added in version 3.7.

terminator[Â¶](#logging.StreamHandler.terminator)

String used as the terminator when writing a formatted record to a stream.
Default value is `'\n'`.

If you donât want a newline termination, you can set the handler instanceâs
...
## FileHandler[Â¶](#filehandler)

The [`FileHandler`](#logging.FileHandler) class, located in the core [`logging`](logging.html#module-logging) package,
sends logging output to a disk file.  It inherits the output functionality from
[`StreamHandler`](#logging.StreamHandler).

class logging.FileHandler(filename, mode='a', encoding=None, delay=False, errors=None)[Â¶](#logging.FileHandler)
...
*errors* is specified, itâs used to determine how encoding errors are handled.

Changed in version 3.6: As well as string values, [`Path`](pathlib.html#pathlib.Path) objects are also accepted
for the *filename* argument.

Changed in version 3.9: The *errors* parameter was added.

close()[Â¶](#logging.FileHandler.close)

Closes the file.

emit(record)[Â¶](#logging.FileHandler.emit)

Outputs the record to the file.

Note that if the file was closed due to logging shutdown at exit and the file
mode is âwâ, the record will not be emitted (see [bpo-42378](https://bugs.python.org/issue?@action=redirect&bpo=42378)).

## NullHandler[Â¶](#nullhandler)

Added in version 3.1.

The [`NullHandler`](#logging.NullHandler) class, located in the core [`logging`](logging.html#module-logging) package,
does not do any formatting or output. It is essentially a âno-opâ handler
for use by library developers.

class logging.NullHandler[Â¶](#logging.NullHandler)

Returns a new instance of the [`NullHandler`](#logging.NullHandler) class.

emit(record)[Â¶](#logging.NullHandler.emit)

This method does nothing.

handle(record)[Â¶](#logging.NullHandler.handle)

This method does nothing.

createLock()[Â¶](#logging.NullHandler.createLock)
...
## WatchedFileHandler[Â¶](#watchedfilehandler)
...
*ST_INO* is not supported under Windows; [`stat()`](os.html#os.stat) always returns zero
for this value.

class logging.handlers.WatchedFileHandler(filename, mode='a', encoding=None, delay=False, errors=None)[Â¶](#logging.handlers.WatchedFileHandler)
...
reopenIfNeeded()[Â¶](#logging.handlers.WatchedFileHandler.reopenIfNeeded)
...
emit(record)[Â¶](#logging.handlers.WatchedFileHandler.emit)
...
## BaseRotatingHandler[Â¶](#baserotatinghandler)
...
class logging.handlers.BaseRotatingHandler(filename, mode, encoding=None, delay=False, errors=None)[Â¶](#logging.handlers.BaseRotatingHandler)

The parameters are as for `FileHandler`. The attributes are:

namer[Â¶](#logging.handlers.BaseRotatingHandler.namer)
...
Itâs also worth noting that care should be taken when using a namer to
...
`backupCount` parameter passed to the handlerâs initializer) by determining
...
respect this. (If a namer is wanted that doesnât respect this scheme, it will
...
rotator[Â¶](#logging.handlers.BaseRotatingHandler.rotator)
...
rotation_filename(default_name)[Â¶](#logging.handlers.BaseRotatingHandler.rotation_filename)

Modify the filename of a log file when rotating.

This is provided so that a custom filename can be provided.

The default implementation calls the ânamerâ attribute of the handler,
if itâs callable, passing the default name to it. If the attribute isnât
callable (the default is `None`), the name is returned unchanged.

Parameters:

**default_name** â The default name for the log file.

Added in version 3.3.

rotate(source, dest)[Â¶](#logging.handlers.BaseRotatingHandler.rotate)

When rotating, rotate the current log.

The default implementation calls the ârotatorâ attribute of the handler,
if itâs callable, passing the source and dest arguments to it. If the
attribute isnât callable (the default is `None`), the source is simply
renamed to the destination.

Parameters:

- 
**source** â The source filename. This is normally the base
filename, e.g. âtest.logâ.

- 
**dest** â The destination filename. This is normally
what the source is rotated to, e.g. âtest.log.1â.
...
## RotatingFileHandler[Â¶](#rotatingfilehandler)

The [`RotatingFileHandler`](#logging.handlers.RotatingFileHandler) class, located in the `logging.handlers`
module, supports rotation of disk log files.

class logging.handlers.RotatingFileHandler(filename, mode='a', maxBytes=0, backupCount=0, encoding=None, delay=False, errors=None)[Â¶](#logging.handlers.RotatingFileHandler)
...
the extensions â.1â, â.2â etc., to the filename. For example, with a *backupCount*
...
doRollover()[Â¶](#logging.handlers.RotatingFileHandler.doRollover)

Does a rollover, as described above.

emit(record)[Â¶](#logging.handlers.RotatingFileHandler.emit)

Outputs the record to the file, catering for rollover as described
previously.

shouldRollover(record)[Â¶](#logging.handlers.RotatingFileHandler.shouldRollover)

See if the supplied record would cause the file to exceed the configured size limit.

## TimedRotatingFileHandler[Â¶](#timedrotatingfilehandler)
...
class logging.handlers.TimedRotatingFileHandler(filename, when='h', interval=1, backupCount=0, encoding=None, delay=False, utc=False, atTime=None, errors=None)[Â¶](#logging.handlers.TimedRotatingFileHandler)
...
When using weekday-based rotation, specify âW0â for Monday, âW1â for
Tuesday, and so on up to âW6â for Sunday. In this case, the value passed for
*interval* isnât used.
...
is set to happen âat midnightâ or âon a particular weekdayâ. Note that in
...
If *errors* is specified, itâs used to determine how encoding errors are
handled.
...
if an interval of âevery minuteâ is set, that does not mean you will
...
doRollover()[Â¶](#logging.handlers.TimedRotatingFileHandler.doRollover)

Does a rollover, as described above.

emit(record)[Â¶](#logging.handlers.TimedRotatingFileHandler.emit)

Outputs the record to the file, catering for rollover as described above.

getFilesToDelete()[Â¶](#logging.handlers.TimedRotatingFileHandler.getFilesToDelete)
...
close()[Â¶](#logging.handlers.SocketHandler.close)

Closes the socket.

emit()[Â¶](#logging.handlers.SocketHandler.emit)

Pickles the recordâs attribute dictionary and writes it to the socket in
binary format. If there is an error with the socket, silently drops the
packet. If the connection was previously lost, re-establishes the
connection. To unpickle the record at the receiving end into a
[`LogRecord`](logging.html#logging.LogRecord), use the [`makeLogRecord()`](logging.html#logging.makeLogRecord)
function.

handleError()[Â¶](#logging.handlers.SocketHandler.handleError)

Handles an error which has occurred during [`emit()`](#logging.handlers.SocketHandler.emit). The most likely
cause is a lost connection. Closes the socket so that we can retry on the
next event.

makeSocket()[Â¶](#logging.handlers.SocketHandler.makeSocket)

This is a factory method which allows subclasses to define the precise
type of socket they want. The default implementation creates a TCP socket
([`socket.SOCK_STREAM`](socket.html#socket.SOCK_STREAM)).

makePickle(record)[Â¶](#logging.handlers.SocketHandler.makePickle)

Pickles the recordâs attribute dictionary in binary format with a length
...
Note that pickles arenât completely secure. If you are concerned about
security, you may want to override this method to implement a more secure
mechanism. For example, you can sign pickles using HMAC and then verify
them on the receiving end, or alternatively you can disable unpickling of
global objects on the receiving end.

send(packet)[Â¶](#logging.handlers.SocketHandler.send)
...
createSocket()[Â¶](#logging.handlers.SocketHandler.createSocket)
...
after that delay the connection still canât be made, the handler will
...
been used, you could lose messages (since the handler wonât even attempt
a connection until the delay has elapsed, but just silently drop messages
during the delay period).

## DatagramHandler[Â¶](#datagramhandler)
...
class logging.handlers.DatagramHandler(host, port)[Â¶](#logging.handlers.DatagramHandler)
...
emit()[Â¶](#logging.handlers.DatagramHandler.emit)
...
makeSocket()[Â¶](#logging.handlers.DatagramHandler.makeSocket)

The factory method of [`SocketHandler`](#logging.handlers.SocketHandler) is here overridden to create
a UDP socket ([`socket.SOCK_DGRAM`](socket.html#socket.SOCK_DGRAM)).

send(s)[Â¶](#logging.handlers.DatagramHandler.send)
...
## SysLogHandler[Â¶](#sysloghandler)

The [`SysLogHandler`](#logging.handlers.SysLogHandler) class, located in the `logging.handlers` module,
supports sending logging messages to a remote or local Unix syslog.

class logging.handlers.SysLogHandler(address=('localhost', SYSLOG_UDP_PORT), facility=LOG_USER, socktype=socket.SOCK_DGRAM, timeout=None)[Â¶](#logging.handlers.SysLogHandler)
...
string, for example â/dev/logâ. In this case, a Unix domain socket is used to
send the message to the syslog. If *facility* is not specified,
`LOG_USER` is used. The type of socket opened depends on the
*socktype* argument, which defaults to [`socket.SOCK_DGRAM`](socket.html#socket.SOCK_DGRAM) and thus
opens a UDP socket. To open a TCP socket (for use with the newer syslog
daemons such as rsyslog), specify a value of [`socket.SOCK_STREAM`](socket.html#socket.SOCK_STREAM).
...
address you should be using for a domain socket - itâs system dependent.
For example, on Linux itâs usually â/dev/logâ but on OS/X itâs
â/var/run/syslogâ. Youâll need to check your platform and use the
...
close()[Â¶](#logging.handlers.SysLogHandler.close)

Closes the socket to the remote host.

createSocket()[Â¶](#logging.handlers.SysLogHandler.createSocket)

Tries to create a socket and, if itâs not a datagram socket, connect it
to the other end. This method is called during handler initialization,
but itâs not regarded as an error if the other end isnât listening at
this point - the method will be called again when emitting an event, if
there is no socket at that point.

Added in version 3.11.

emit(record)[Â¶](#logging.handlers.SysLogHandler.emit)
...
though itâs not in the relevant specification ([**RFC 5424**](https://datatracker.ietf.org/doc/html/rfc5424.html)). More recent
versions of these daemons donât expect the NUL byte but strip it off
if itâs there, and even more recent daemons (which adhere more closely
...
an âidentâ or âtagâ prefix to identify the source of the message. This
...
encodePriority(facility, priority)[Â¶](#logging.handlers.SysLogHandler.encodePriority)
...
mapPriority(levelname)[Â¶](#logging.handlers.SysLogHandler.mapPriority)
...
## NTEventLogHandler[Â¶](#nteventloghandler)

The [`NTEventLogHandler`](#logging.handlers.NTEventLogHandler) class, located in the `logging.handlers`
module, supports sending logging messages to a local Windows NT, Windows 2000 or
Windows XP event log. Before you can use it, you need Mark Hammondâs Win32
extensions for Python installed.

class logging.handlers.NTEventLogHandler(appname, dllname=None, logtype='Application')[Â¶](#logging.handlers.NTEventLogHandler)
...
close()[Â¶](#logging.handlers.NTEventLogHandler.close)
...
emit(record)[Â¶](#logging.handlers.NTEventLogHandler.emit)

Determines the message ID, event category and event type, and then logs
the message in the NT event log.

getEventCategory(record)[Â¶](#logging.handlers.NTEventLogHandler.getEventCategory)

Returns the event category for the record. Override this if you want to
specify your own categories. This version returns 0.

getEventType(record)[Â¶](#logging.handlers.NTEventLogHandler.getEventType)

Returns the event type for the record. Override this if you want to
specify your own types. This version does a mapping using the handlerâs
...
suitable dictionary in the handlerâs *typemap* attribute.

getMessageID(record)[Â¶](#logging.handlers.NTEventLogHandler.getMessageID)
...
## SMTPHandler[Â¶](#smtphandler)

The [`SMTPHandler`](#logging.handlers.SMTPHandler) class, located in the `logging.handlers` module,
supports sending logging messages to an email address via SMTP.

class logging.handlers.SMTPHandler(mailhost, fromaddr, toaddrs, subject, credentials=None, secure=None, timeout=1.0)[Â¶](#logging.handlers.SMTPHandler)
...
[`smtplib.SMTP.starttls()`](smtplib.html#smtplib.SMTP.starttls) method.)
...
emit(record)[Â¶](#logging.handlers.SMTPHandler.emit)

Formats the record and sends it to the specified addressees.

getSubject(record)[Â¶](#logging.handlers.SMTPHandler.getSubject)

If you want to specify a subject line which is record-dependent, override
this method.

## MemoryHandler[Â¶](#memoryhandler)
...
class logging.handlers.BufferingHandler(capacity)[Â¶](#logging.handlers.BufferingHandler)
...
emit(record)[Â¶](#logging.handlers.BufferingHandler.emit)
...
flush()[Â¶](#logging.handlers.BufferingHandler.flush)
...
shouldFlush(record)[Â¶](#logging.handlers.BufferingHandler.shouldFlush)

Return `True` if the buffer is up to capacity. This method can be
overridden to implement custom flushing strategies.

class logging.handlers.MemoryHandler(capacity, flushLevel=ERROR, target=None, flushOnClose=True)[Â¶](#logging.handlers.MemoryHandler)
...
close()[Â¶](#logging.handlers.MemoryHandler.close)

Calls [`flush()`](#logging.handlers.MemoryHandler.flush), sets the target to `None` and clears the
buffer.

flush()[Â¶](#logging.handlers.MemoryHandler.flush)
...
setTarget(target)[Â¶](#logging.handlers.MemoryHandler.setTarget)

Sets the target handler for this handler.

shouldFlush(record)[Â¶](#logging.handlers.MemoryHandler.shouldFlush)

Checks for buffer full or a record at the *flushLevel* or higher.

## HTTPHandler[Â¶](#httphandler)

The [`HTTPHandler`](#logging.handlers.HTTPHandler) class, located in the `logging.handlers` module,
supports sending logging messages to a web server, using either `GET` or
`POST` semantics.

class logging.handlers.HTTPHandler(host, url, method='GET', secure=False, credentials=None, context=None)[Â¶](#logging.handlers.HTTPHandler)
...
[`ssl.SSLContext`](ssl.html#ssl.SSLContext) instance to configure the SSL settings used for the
...
âAuthorizationâ header using Basic authentication. If you specify
credentials, you should also specify secure=True so that your userid and
password are not passed in cleartext across the wire.

Changed in version 3.5: The *context* parameter was added.

mapLogRecord(record)[Â¶](#logging.handlers.HTTPHandler.mapLogRecord)
...
subset of [`LogRecord`](logging.html#logging.LogRecord) is to be sent to the web server, or
if more specific customization of whatâs sent to the server is required.

emit(record)[Â¶](#logging.handlers.HTTPHandler.emit)
...
[`setFormatter()`](logging.html#logging.Handler.setFormatter) to specify a
[`Formatter`](logging.html#logging.Formatter) for a [`HTTPHandler`](#logging.handlers.HTTPHandler) has no effect.
Instead of calling [`format()`](logging.html#logging.Handler.format), this handler calls
[`mapLogRecord()`](#logging.handlers.HTTPHandler.mapLogRecord) and then [`urllib.parse.urlencode()`](urllib.parse.html#urllib.parse.urlencode) to encode the
dictionary in a form suitable for sending to a web server.

## QueueHandler[Â¶](#queuehandler)
...
[`queue`](queue.html#module-queue) or [`multiprocessing`](multiprocessing.html#module-multiprocessing) modules.
...
class logging.handlers.QueueHandler(queue)[Â¶](#logging.handlers.QueueHandler)
...
queue-like object; itâs used as-is by the [`enqueue()`](#logging.handlers.QueueHandler.enqueue) method, which
...
[`SimpleQueue`](queue.html#queue.SimpleQueue) instances for *queue*.

Note

If you are using [`multiprocessing`](multiprocessing.html#module-multiprocessing), you should avoid using
[`SimpleQueue`](queue.html#queue.SimpleQueue) and instead use [`multiprocessing.Queue`](multiprocessing.html#multiprocessing.Queue).

Warning

The [`multiprocessing`](multiprocessing.html#module-multiprocessing) module uses an internal logger created and
accessed via [`get_logger()`](multiprocessing.html#multiprocessing.get_logger).
[`multiprocessing.Queue`](multiprocessing.html#multiprocessing.Queue) will log `DEBUG` level messages upon
items being queued. If those log messages are processed by a
[`QueueHandler`](#logging.handlers.QueueHandler) using the same [`multiprocessing.Queue`](multiprocessing.html#multiprocessing.Queue) instance,
it will cause a deadlock or infinite recursion.

emit(record)[Â¶](#logging.handlers.QueueHandler.emit)

Enqueues the result of preparing the LogRecord. Should an exception
occur (e.g. because a bounded queue has filled up), the
[`handleError()`](logging.html#logging.Handler.handleError) method is called to handle the
error. This can result in the record silently being dropped (if
[`logging.raiseExceptions`](logging.html#logging.raiseExceptions) is `False`) or a message printed to
`sys.stderr` (if [`logging.raiseExceptions`](logging.html#logging.raiseExceptions) is `True`).

prepare(record)[Â¶](#logging.handlers.QueueHandler.prepare)
...
the recordâs `msg` and `message` attributes with the merged
message (obtained by calling the handlerâs [`format()`](functions.html#format) method), and
...
that a handler on the [`QueueListener`](#logging.handlers.QueueListener) side wonât have the
...
enqueue(record)[Â¶](#logging.handlers.QueueHandler.enqueue)

Enqueues the record on the queue using `put_nowait()`; you may
want to override this if you want to use blocking behaviour, or a
timeout, or a customized queue implementation.

listener[Â¶](#logging.handlers.QueueHandler.listener)

When created via configuration using [`dictConfig()`](logging.config.html#logging.config.dictConfig), this
...
## QueueListener[Â¶](#queuelistener)

Added in version 3.2.

The [`QueueListener`](#logging.handlers.QueueListener) class, located in the `logging.handlers`
module, supports receiving logging messages from a queue, such as those
implemented in the [`queue`](queue.html#module-queue) or [`multiprocessing`](multiprocessing.html#module-multiprocessing) modules. The
...
class logging.handlers.QueueListener(queue, *handlers, respect_handler_level=False)[Â¶](#logging.handlers.QueueListener)
...
object; itâs passed as-is to the [`dequeue()`](#logging.handlers.QueueListener.dequeue) method, which needs
to know how to get messages from it. The queue is not *required* to have the
task tracking API (though itâs used if available), which means that you can
use [`SimpleQueue`](queue.html#queue.SimpleQueue) instances for *queue*.
...
If `respect_handler_level` is `True`, a handlerâs level is respected
...
[`with`](../reference/compound_stmts.html#with). When entering the context, the listener is started. When
exiting the context, the listener is stopped.
[`__enter__()`](stdtypes.html#contextmanager.__enter__) returns the
[`QueueListener`](#logging.handlers.QueueListener) object.

dequeue(block)[Â¶](#logging.handlers.QueueListener.dequeue)
...
prepare(record)[Â¶](#logging.handlers.QueueListener.prepare)
...
handle(record)[Â¶](#logging.handlers.QueueListener.handle)
...
start()[Â¶](#logging.handlers.QueueListener.start)
...
Changed in version 3.14: Raises [`RuntimeError`](exceptions.html#RuntimeError) if called and the listener is already
running.

stop()[Â¶](#logging.handlers.QueueListener.stop)

Stops the listener.

This asks the thread to terminate, and then waits for it to do so.
Note that if you donât call this before your application exits, there
may be some records still left on the queue, which wonât be processed.

enqueue_sentinel()[Â¶](#logging.handlers.QueueListener.enqueue_sentinel)
...
Module [`logging`](logging.html#module-logging)
API reference for the logging module.

Module [`logging.config`](logging.config.html#module-logging.config)
...
[`logging.config` â Logging configuration](logging.config.html)
...
[`platform` â  Access to underlying platformâs identifying data](platform.html)
