...
struct â Interpret bytes as packed binary data — Python 3.14.3 documentation
...
# `struct` â Interpret bytes as packed binary data[Â¶](#struct-interpret-bytes-as-packed-binary-data)
...
as Python [`bytes`](stdtypes.html#bytes) objects.  Compact [format strings](#struct-format-strings)
describe the intended conversions to/from Python values.
The moduleâs functions and objects can be used for two largely
...
argument.  This refers to objects that implement the [Buffer Protocol](../c-api/buffer.html#bufferobjects) and
provide either a readable or read-writable buffer.  The most common types used
for that purpose are [`bytes`](stdtypes.html#bytes) and [`bytearray`](stdtypes.html#bytearray), but many other types
that can be viewed as an array of bytes implement the buffer protocol, so that
they can be read/filled without additional copying from a [`bytes`](stdtypes.html#bytes) object.

## Functions and Exceptions[Â¶](#functions-and-exceptions)

The module defines the following exception and functions:

exception struct.error[Â¶](#struct.error)

Exception raised on various occasions; argument is a string describing what
is wrong.

struct.pack(format, v1, v2, ...)[Â¶](#struct.pack)
...
struct.pack_into(format, buffer, offset, v1, v2, ...)[Â¶](#struct.pack_into)

Pack the values *v1*, *v2*, â¦ according to the format string *format* and
write the packed bytes into the writable buffer *buffer* starting at
position *offset*.  Note that *offset* is a required argument.

struct.unpack(format, buffer)[Â¶](#struct.unpack)

Unpack from the buffer *buffer* (presumably packed by `pack(format, ...)`)
according to the format string *format*.  The result is a tuple even if it
contains exactly one item.  The bufferâs size in bytes must match the
size required by the format, as reflected by [`calcsize()`](#struct.calcsize).

struct.unpack_from(format, /, buffer, offset=0)[Â¶](#struct.unpack_from)

Unpack from *buffer* starting at position *offset*, according to the format
string *format*.  The result is a tuple even if it contains exactly one
item.  The bufferâs size in bytes, starting at position *offset*, must be at
least the size required by the format, as reflected by [`calcsize()`](#struct.calcsize).

struct.iter_unpack(format, buffer)[Â¶](#struct.iter_unpack)

Iteratively unpack from the buffer *buffer* according to the format
string *format*.  This function returns an iterator which will read
equally sized chunks from the buffer until all its contents have been
consumed.  The bufferâs size in bytes must be a multiple of the size
...
struct.calcsize(format)[Â¶](#struct.calcsize)

Return the size of the struct (and hence of the bytes object produced by
`pack(format, ...)`) corresponding to the format string *format*.

## Format Strings[Â¶](#format-strings)
...
### Byte Order, Size, and Alignment[Â¶](#byte-order-size-and-alignment)
...
Use [`sys.byteorder`](sys.html#sys.byteorder) to check the endianness of your system.

Native size and alignment are determined using the C compilerâs
...
### Format Characters[Â¶](#format-characters)

Format characters have the following meaning; the conversion between C and
Python values should be obvious given their types.  The âStandard sizeâ column
...
- 
The `'?'` conversion code corresponds to the _Bool type
defined by C standards since C99.  In standard mode, it is
represented by one byte.

- 
When attempting to pack a non-integer using any of the integer conversion
codes, if the non-integer has a [`__index__()`](../reference/datamodel.html#object.__index__) method then that method is
called to convert the argument to an integer before packing.

Changed in version 3.2: Added use of the [`__index__()`](../reference/datamodel.html#object.__index__) method for non-integers.
...
- 
The IEEE 754 binary16 âhalf precisionâ type was introduced in the 2008
...
- 
The `'p'` format character encodes a âPascal stringâ, meaning a short
...
raised [`DeprecationWarning`](exceptions.html#DeprecationWarning) instead of [`struct.error`](#struct.error).

For the `'?'` format character, the return value is either [`True`](constants.html#True) or
[`False`](constants.html#False). When packing, the truth value of the argument object is used.
...
lack of any prefix character) may not match what the readerâs
machine produces as
...
at the end, assuming the platformâs longs are aligned on 4-byte boundaries:
...
Module [`array`](array.html#module-array)
...
### Native Formats[Â¶](#native-formats)
...
### Standard Formats[Â¶](#standard-formats)
...
zero-repeat format character wonât work.  Instead, the user must
...
The above results (executed on a 64-bit machine) arenât guaranteed to
...
class struct.Struct(format)[Â¶](#struct.Struct)
...
format strings neednât worry about reusing a single [`Struct`](#struct.Struct)
instance.

Compiled Struct objects support the following methods and attributes:

pack(v1, v2, ...)[Â¶](#struct.Struct.pack)

Identical to the [`pack()`](#struct.pack) function, using the compiled format.
(`len(result)` will equal [`size`](#struct.Struct.size).)

pack_into(buffer, offset, v1, v2, ...)[Â¶](#struct.Struct.pack_into)

Identical to the [`pack_into()`](#struct.pack_into) function, using the compiled format.

unpack(buffer)[Â¶](#struct.Struct.unpack)

Identical to the [`unpack()`](#struct.unpack) function, using the compiled format.
The bufferâs size in bytes must equal [`size`](#struct.Struct.size).

unpack_from(buffer, offset=0)[Â¶](#struct.Struct.unpack_from)

Identical to the [`unpack_from()`](#struct.unpack_from) function, using the compiled format.
The bufferâs size in bytes, starting at position *offset*, must be at least
[`size`](#struct.Struct.size).

iter_unpack(buffer)[Â¶](#struct.Struct.iter_unpack)

Identical to the [`iter_unpack()`](#struct.iter_unpack) function, using the compiled format.
The bufferâs size in bytes must be a multiple of [`size`](#struct.Struct.size).

Added in version 3.4.

format[Â¶](#struct.Struct.format)

The format string used to construct this Struct object.

Changed in version 3.7: The format string type is now [`str`](stdtypes.html#str) instead of [`bytes`](stdtypes.html#bytes).

size[Â¶](#struct.Struct.size)

The calculated size of the struct (and hence of the bytes object produced
by the [`pack()`](#struct.pack) method) corresponding to [`format`](functions.html#format).
...
[`codecs` â Codec registry and base classes](codecs.html)
