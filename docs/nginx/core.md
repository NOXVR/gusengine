...
     [ssl_object_cache_inheritable](#ssl_object_cache_inheritable)
...
[EPOLLEXCLUSIVE](events.html#epoll) flag (1.11.3) or
when using [reuseport](http/ngx_http_core_module.html#reuseport).
...
see “[A debugging log](debugging_log.html)”.
...
- 
inherited during a [live upgrade](control.html#upgrade)
of an executable file;

- 
used by the
[ngx_http_perl_module](http/ngx_http_perl_module.html) module;
...
[live upgrade](control.html#upgrade)
of an executable file.

 

The TZ variable is always inherited and available to the
[ngx_http_perl_module](http/ngx_http_perl_module.html)
...
Logging to [syslog](syslog.html) can be configured by specifying
the “`syslog:`” prefix.
Logging to a
[cyclic memory buffer](debugging_log.html#memory)
...
The directive is ignored if [kqueue](events.html#kqueue)
...
[without blocking](http/ngx_http_core_module.html#aio)
worker processes.
...
Specifies the [connection processing](events.html)
`method` to use.
...
When using [aio](http/ngx_http_core_module.html#aio)
with the [epoll](../docs/events.html#epoll)
