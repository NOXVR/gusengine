...
6. **Cognitive Query Expansion for Mechanical System Diagnostics** — A pre-retrieval reasoning step in which the LLM, prior to any vector database search, generates multiple subsystem-specific search queries by reasoning about the physical systems that could produce the reported symptoms — mimicking a master mechanic's causal reasoning ("what changes between idle and highway?") rather than relying on raw semantic text similarity. Each expanded query independently searches the vector database, and results are deduplicated and merged to produce a broader retrieval context that covers subsystems a single symptom-based search would miss. The system includes a multi-stage parse architecture (direct JSON, balanced-bracket extraction, regex fallback) that adapts to arbitrary LLM output formatting, and a per-request observability layer (`expansion_info`) that self-documents the parsing method, query count, and generated queries in every API response — providing deterministic proof of expansion utilization without reliance on server logs. *(Added 2026-03-13, deployed and proven 2026-03-13)*
...
8. **Multi-Diagnosis Queue Protocol** — A session management system in which the AI diagnostic engine detects when a user presents multiple unrelated symptoms spanning different subsystems, automatically decomposes them into a numbered issue queue, and works through each issue sequentially via a guided UX flow. Upon detecting a multi-diagnosis prompt, the system acknowledges all issues, assigns each to a diagnostic track, and begins the full diagnostic state machine (Triage → Funnel → Testing → Conclusion) for the first issue only. Upon reaching conclusion for each issue, the system presents a resolution gate ("Fixed — move to next issue" / "Still having the problem — continue debugging") before advancing to the next queued issue. The system maintains a `pending_issues` manifest in every response, providing session-wide visibility into resolved vs. unresolved issues. This prevents the cognitive overload of parallel multi-fault diagnosis while ensuring no reported issue is forgotten or dropped. *(Added 2026-03-14, deployed and proven 2026-03-14)*

9. **Diagnostic Session Persistence** — A stateful diagnostic job management system that captures byte-for-byte snapshots of the complete diagnostic context — including full chat history, pending issue queue state, resolved diagnostics with fixes, active issue position, and approximate token utilization — and persists them as auditable job records. Sessions auto-save on every interaction (zero-friction, no confirmation modals) and can be restored at any time, enabling a mechanic to close the browser, return hours or days later, and resume the exact diagnostic state. Each saved session is a complete forensic record of the diagnostic conversation. *(Added 2026-03-14, deployment ready)*

10. **Interactive Issue Queue Tiles with Zero-Friction Context Switching** — A visual diagnostic workflow interface in which each issue in the multi-diagnosis queue is represented as a tappable tile. Tiles dynamically appear only when an issue exists in the active queue (no pre-allocated slots). Tapping a tile triggers an automatic save of the current diagnostic state (no confirmation dialog — always saves) and immediately switches the diagnostic context to the tapped issue, resuming that issue's diagnostic state machine at its last position. Tile states visually indicate ACTIVE (highlighted), PENDING (outlined), and RESOLVED (checkmarked), providing at-a-glance session progress. This eliminates the need for users to explicitly manage issue switching and enables non-linear diagnostic workflows where mechanics can jump between issues based on shop floor priorities. *(Added 2026-03-14, deployment ready)*

11. **Token-Aware Context Checkpointing for Long-Running Diagnostic Sessions** — A proactive context window management system that monitors approximate token utilization across the system prompt, RAG context, and cumulative chat history during a diagnostic session. When utilization exceeds a configurable threshold (e.g., 80% of the model's context window), the system injects an advisory into the diagnostic flow, summarizing all critical findings to date and offering two options: continue in the current context (with degradation risk) or checkpoint and migrate to a fresh workspace. Migration automatically saves the full session, generates a condensed knowledge summary of all resolved and active diagnostics, and initializes a new session pre-loaded with the condensed context — preserving diagnostic continuity while resetting token utilization. The original session remains accessible indefinitely. This prevents silent context degradation in complex multi-issue diagnostic jobs. *(Added 2026-03-14, deployment ready)*
...
| 2026-03-14 | #8 | Multi-Diagnosis Queue Protocol | **Deployed and proven.** 3-issue Mustang test: Gus decomposed 3 unrelated faults into numbered queue with `pending_issues` tracking (ACTIVE/PENDING/RESOLVED status per issue). Post-deployment test confirmed answer paths focused on single issue only, no cross-issue mixing. |
| 2026-03-14 | #9 | Diagnostic Session Persistence | **Deployment ready.** `/api/session/save` and `/api/session/load` endpoints with full job state snapshots (chat history, pending issues, resolved diagnostics, token count). Auto-save on every interaction. |
| 2026-03-14 | #10 | Interactive Issue Queue Tiles | **Deployment ready.** Dynamic tappable tiles driven by `pending_issues` array. Tap = auto-save current + switch to tapped issue. No confirmation modals. Visual states: ACTIVE (filled), PENDING (outlined), RESOLVED (checkmarked). |
| 2026-03-14 | #11 | Token-Aware Context Checkpointing | **Deployment ready.** Context window monitoring with threshold advisories. Graceful migration: save full session → generate condensed summary → start fresh session with critical knowledge preserved. Old session retained indefinitely. |
