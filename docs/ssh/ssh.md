...
## SYNOPSIS          top
       ...
       mac_spec] [-O ctl_cmd] [-o option] [-P tag] [-p port] [-R address]
...
## DESCRIPTION          top
       ...
       -D [bind_address:]port
               ...
               to a specific address.  The bind_address of âlocalhostâ
       ...
       -F configfile
               ...
               file is ~/.ssh/config.  If set to ânoneâ, no configuration
               files will be read.
       ...
       -f      Requests ssh to go to background just before command
               ...
               âyesâ, then a client started with -f will wait for all
       ...
       -i identity_file
               ...
               ~/.ssh/id_ed25519_sk.  Identity files may also be
       ...
       -L local_socket:remote_socket
               ...
               The bind_address of âlocalhostâ indicates that the
       ...
       -M      Places the ssh client into âmasterâ mode for connection
               sharing.  Multiple -M options places ssh into âmasterâ
       ...
       -n      Redirects stdin from /dev/null (actually, prevents reading
               ...
               ssh program will be put in the background.  (This does not
               work if ssh needs to ask for a password or passphrase; see
               also the -f option.)  Refer to the description of
               StdinNull in ssh_config(5) for details.
       ...
       -Q query_option
               ...
               key-ca-sign (valid CA signature algorithms for
               certificates), key-cert (certificate key types), key-plain
               (non-certificate key types), key-sig (all key types and
       ...
       -R [bind_address:]port
               ...
               bind_address, or the address â*â, indicates that the
               ...
               If the port argument is â0â, the listen port will be
       ...
       -W host:port
               ...
               ClearAllForwardings, though these can be overridden in the
               configuration file or using -o command line options.
       ...
       -w local_tun[:remote_tun]
               ...
               âanyâ.  See also the Tunnel and TunnelDevice directives in
               ssh_config(5).
...
## AUTHENTICATION          top
       ...
       is considered for login.  Additionally, the server must be able to
       verify the client's host key (see the description of
       /etc/ssh/ssh_known_hosts and ~/.ssh/known_hosts, below) for login
       to be permitted.  This authentication method closes security holes
       due to IP spoofing, DNS spoofing, and routing spoofing.  [Note to
       the administrator: /etc/hosts.equiv, ~/.rhosts, and the rlogin/rsh
       ...
       stores the private key in ~/.ssh/id_ecdsa (ECDSA),
       ...
       ~/.ssh/id_ecdsa.pub (ECDSA), ~/.ssh/id_ecdsa_sk.pub
       (authenticator-hosted ECDSA), ~/.ssh/id_ed25519.pub (Ed25519),
       ~/.ssh/id_ed25519_sk.pub (authenticator-hosted Ed25519), or
       ~/.ssh/id_rsa.pub (RSA) in the user's home directory.  The user
       should then copy the public key to ~/.ssh/authorized_keys in their
       ...
       most systems, setting the escape character to ânoneâ will also
...
## TCP FORWARDING          top
       ...
       IRC server at âserver.example.comâ, joining channel â#usersâ,
       nickname âpinkyâ, using the standard IRC port, 6667:

           $ ssh -f -L 6667:localhost:6667 server.example.com sleep 10
           $ irc -c '#users' pinky IRC/127.0.0.1

       The -f option backgrounds ssh and the remote command âsleep 10â is
...
## X11 FORWARDING          top
       ...
       If the ForwardX11 variable is set to âyesâ (or see the description
       of the -X, -x, and -Y options above) and the user is using X11
       (the DISPLAY environment variable is set), the connection to the
       X11 display is automatically forwarded to the remote side in such
       a way that any X11 programs started from the shell (or command)
       will go through the encrypted channel, and the connection to the
       real X server will be made from the local machine.  The user
       should not manually set DISPLAY.  Forwarding of X11 connections
       ...
       happens because ssh creates a âproxyâ X server on the server
       ...
       If the ForwardAgent variable is set to âyesâ (or see the
...
## VERIFYING HOST KEYS          top
       ...
       option StrictHostKeyChecking has been disabled).  Fingerprints can
       be determined using ssh-keygen(1):

             $ ssh-keygen -l -f /etc/ssh/ssh_host_rsa_key

       If the fingerprint is already known, it can be matched and the key
       can be accepted or rejected.  If only legacy (MD5) fingerprints
       for the server are available, the ssh-keygen(1) -E option may be
       used to downgrade the fingerprint algorithm to match.

       Because of the difficulty of comparing host keys just by looking
       at fingerprint strings, there is also support to compare host keys
       visually, using random art.  By setting the VisualHostKey option
       to âyesâ, a small ASCII graphic gets displayed on every login to a
       ...
       the connecting client is able to match the fingerprint with that
       of the key presented.

       In this example, we are connecting a client to a server,
       âhost.example.comâ.  The SSHFP resource records should first be
...
## SSH-BASED VIRTUAL PRIVATE NETWORKS          top
       ...
       connections on tun(4) device 1 from user âjaneâ and on tun device
       2 from user âjohnâ, if PermitRootLogin is set to
       âforced-commands-onlyâ:
...
## ENVIRONMENT          top
       ...
       DISPLAY               The DISPLAY variable indicates the location
                             ...
                             âhostname:nâ, where âhostnameâ indicates the
                             host where the shell runs, and ânâ is an
                             integer â¥ 1.  ssh uses this special value to
                             ...
                             X11 connection insecure (and will require
                             the user to manually copy any required
       ...
       SSH_ASKPASS_REQUIRE   Allows further control over the use of an
                             ...
                             âneverâ then ssh will never attempt to use
                             one.  If it is set to âpreferâ, then ssh
                             will prefer to use the askpass program
                             instead of the TTY when requesting
                             passwords.  Finally, if the variable is set
                             to âforceâ, then the askpass program will be
                             used for all passphrase input regardless of
       ...
       SSH_USER_AUTH         Optionally set by sshd(8), this variable may
                             ...
                             authentication methods successfully used
                             when the session was established, including
       ...
       format âVARNAME=valueâ to the environment if the file exists and
...
## FILES          top
       ...
       ~/.ssh/authorized_keys
               Lists the public keys (ECDSA, Ed25519, RSA) that can be
