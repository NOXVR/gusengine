...
NAME | DESCRIPTION | TIME FORMATS | TOKENS | FILES | SEE ALSO | AUTHORS | COLOPHON
...
## NAME          top
       ...
       sshd_config â OpenSSH daemon configuration file
...
## DESCRIPTION          top
       ...
       starting with â#â and empty lines are interpreted as comments.
       ...
       AcceptEnv
               ...
               required by the protocol.  Variables are specified by
               name, which may contain the wildcard characters â*â and
               â?â.  Multiple environment variables may be separated by
               ...
               care should be taken in the use of this directive.  The
       ...
       AllowStreamLocalForwarding
               ...
               disabling StreamLocal forwarding does not improve security
       ...
       AllowTcpForwarding
               ...
               TCP forwarding, no to prevent all TCP forwarding, local to
       ...
       AuthenticationMethods
               ...
               to attempt password or keyboard-interactive authentication
               before public key.
       ...
       AuthorizedKeysCommand
               ...
               tokens described in the âTOKENSâ section.  If no arguments
               are specified then the username of the target user is
               used.

               The program should produce on standard output zero or more
               lines of authorized_keys output (see âAUTHORIZED_KEYSâ in
       ...
       AuthorizedKeysFile
               ...
               âTOKENSâ section.  After expansion, AuthorizedKeysFile is
               taken to be an absolute path or one relative to the user's
               home directory.  Multiple files may be listed, separated
               by whitespace.  Alternately this option may be set to none
       ...
       AuthorizedPrincipalsCommand
               ...
               accept the tokens described in the âTOKENSâ section.  If
               no arguments are specified then the username of the target
               user is used.
       ...
       AuthorizedPrincipalsFile
               ...
               described in âAUTHORIZED_KEYS FILE FORMATâ in sshd(8)).
               Empty lines and comments starting with â#â are ignored.

               Arguments to AuthorizedPrincipalsFile accept the tokens
               described in the âTOKENSâ section.  After expansion,
               AuthorizedPrincipalsFile is taken to be an absolute path
               or one relative to the user's home directory.  The default
               is none, i.e. not to use a principals file â in this case,
               the username of the user must appear in a certificate's
       ...
       Banner  The contents of the specified file are sent to the remote
               ...
               none then no banner is displayed.  By default, no banner
               is displayed.
       ...
       CASignatureAlgorithms
               ...
               If the specified list begins with a â+â character, then
               the specified algorithms will be appended to the default
               set instead of replacing them.  If the specified list
               begins with a â-â character, then the specified algorithms
       ...
       ChannelTimeout
               ...
               âtype=intervalâ pairs separated by whitespace, where the
               âtypeâ must be the special keyword âglobalâ or a channel
               type name from the list below, optionally containing
               wildcard characters.

               The timeout value âintervalâ is specified in seconds or
               may use any of the units documented in the âTIME FORMATSâ
               section.  For example, âsession=5mâ would cause
               interactive sessions to terminate after five minutes of
               inactivity.  Specifying a zero value disables the
               inactivity timeout.

               The special timeout âglobalâ applies to all active
       ...
       ChrootDirectory
               ...
               After the chroot, sshd(8) changes the working directory to
               the user's home directory.  Arguments to ChrootDirectory
               accept the tokens described in the âTOKENSâ section.
       ...
       Ciphers
               ...
               comma-separated.  If the specified list begins with a â+â
               character, then the specified ciphers will be appended to
               the default set instead of replacing them.  If the
               specified list begins with a â-â character, then the
       ...
       ClientAliveCountMax
               ...
               spoofable.  The client alive mechanism is valuable when
               the client or server depend on knowing when a connection
       ...
       Compression
               ...
               has authenticated successfully.  The argument must be yes,
       ...
       DenyGroups
               ...
               a numerical group ID is not recognized.  By default, login
               is allowed for all groups.  The allow/deny groups
       ...
       GSSAPIStrictAcceptorCheck
               ...
               assist with operation on multi homed machines.  The
               default is yes.
       ...
       HostCertificate
               ...
               The certificate's public key must match a private host key
       ...
       IgnoreUserKnownHosts
               ...
               /etc/ssh/ssh_known_hosts.  The default is ânoâ.
       ...
       Include
               ...
               to be in /etc/ssh.  An Include directive may appear inside
               a Match block to perform conditional inclusion.
       ...
       KexAlgorithms
               ...
               the server will offer to clients.  The ordering of this
               list is not important, as the client specifies the
               preference order.  Multiple algorithms must be comma-
               separated.
               ...
               with a â^â character, then the specified algorithms will
       ...
       ListenAddress
               ...
               addresses on the current default routing domain.  Multiple
               ListenAddress options are permitted.  For more information
       ...
       Match   Introduces a conditional block.  If all of the criteria on
               ...
               pairs or one of the single token criteria: All, which
               matches all criteria, or Invalid-User, which matches when
               the requested user-name does not match any known account.
               ...
               operators described in the âPATTERNSâ section of
               ssh_config(5).

               The patterns in an Address criteria may additionally
               contain addresses to match in CIDR address/masklen format,
               such as 192.0.2.0/24 or 2001:db8::/32.  Note that the mask
               length provided must be consistent with the address - it
               is an error to specify a mask length that is too long for
               the address or one with bits set in this host portion of
               the address.  For example, 192.0.2.0/33 and 192.0.2.0/8,
               respectively.
       ...
       ModuliFile
               ...
               âdiffie-hellman-group-exchange-sha1â and
               âdiffie-hellman-group-exchange-sha256â key exchange
               methods.  The default is /etc/moduli.

       PAMServiceName
               Specifies the service name used for Pluggable
               Authentication Modules (PAM) authentication, authorisation
               and session controls when UsePAM is enabled.  The default
               is sshd.
       ...
       PermitListen
               ...
               wildcard â*â can also be used in place of a port number to
               allow all ports.  By default all port forwarding listen
               requests are permitted.  Note that the GatewayPorts option
               may further restrict which addresses may be listened on.
               Note also that ssh(1) will request a listen host of
               âlocalhostâ if no listen host was specifically requested,
               and this name is treated differently to explicit localhost
               addresses of â127.0.0.1â and â::1â.

       PermitOpen
               Specifies the destinations to which TCP port forwarding is
               permitted.  The forwarding specification must be one of
               the following forms:
               ...
               requests.  The wildcard â*â can be used for host or port
       ...
       PermitUserEnvironment
               ...
               example "LANG,LC_*").  The default is no.  Enabling
       ...
       PerSourceMaxStartups
               ...
               allowed from a given source address, or ânoneâ if there is
               no limit.  This limit is applied in addition to
       ...
       PerSourcePenalties
               Controls penalties for various conditions that may
               represent attacks on sshd(8).  If a penalty is enforced
               against a client then its source address and any others in
               the same network, as defined by PerSourceNetBlockSize,
               will be refused connection for a period.

               A penalty doesn't affect concurrent connections in
               progress, but multiple penalties from the same source from
               concurrent connections will accumulate up to a maximum.
               Conversely, penalties are not applied until a minimum
               threshold time has been accumulated.

               Penalties are enabled by default with the default settings
               listed below but may disabled using the no keyword.  The
               defaults may be overridden by specifying one or more of
               the keywords below, separated by whitespace.  All keywords
               accept arguments, e.g. "crash:2m".

               crash:duration
                       Specifies how long to refuse clients that cause a
                       crash of sshd(8) (default: 90s).

               authfail:duration
                       Specifies how long to refuse clients that
                       disconnect after making one or more unsuccessful
                       authentication attempts (default: 5s).
               ...
               noauth:duration
                       Specifies how long to refuse clients that
                       disconnect without attempting authentication
                       (default: 1s).  This timeout should be used
                       cautiously otherwise it may penalise legitimate
                       scanning tools such as ssh-keyscan(1).
               ...
               max:duration
                       Specifies the maximum time a particular source
                       address range will be refused access for (default:
                       10m).  Repeated penalties will accumulate up to
                       this maximum.

               min:duration
                       Specifies the minimum penalty that must accrue
                       before enforcement begins (default: 15s).

               max-sources4:number, max-sources6:number
                       Specifies the maximum number of client IPv4 and
                       IPv6 address ranges to track for penalties
                       (default: 65536 for both).

               overflow:mode
                       Controls how the server behaves when max-sources4
                       or max-sources6 is exceeded.  There are two
                       ...
                       (default: permissive).  Note that client penalties
                       below the min threshold count against the total
                       number of tracked penalties.  IPv4 and IPv6
                       addresses are tracked separately, so an overflow
                       in one will not affect the other.

               overflow6:mode
                       Allows specifying a different overflow mode for
                       IPv6 addresses.  The default it to use the same
                       overflow mode as was specified for IPv4.

       PerSourcePenaltyExemptList
               Specifies a comma-separated list of addresses to exempt
               from penalties.  This list may contain wildcards and CIDR
               address/masklen ranges.  Note that the mask length
       ...
       PubkeyAcceptedAlgorithms
               ...
               patterns.  Alternately if the specified list begins with a
               â+â character, then the specified algorithms will be
               appended to the default set instead of replacing them.  If
               the specified list begins with a â-â character, then the
       ...
       PubkeyAuthOptions
               ...
               authentication (usually by touching the authenticator).
       ...
       RefuseConnection
               ...
               the connection.  Additionally, a refuseconnection penalty
               may be recorded against the source of the connection if
               PerSourcePenalties are enabled.  This option is only
               really useful in a Match block.
       ...
       RekeyLimit
               ...
               suffix of âKâ, âMâ, or âGâ to indicate Kilobytes,
               Megabytes, or Gigabytes, respectively.  The default is
               between â1Gâ and â4Gâ, depending on the cipher.  The
               optional second value is specified in seconds and may use
               any of the units documented in the âTIME FORMATSâ section.
       ...
       RDomain
               ...
               authentication has completed.  The user session, as well
               as any forwarded or listening IP sockets, will be bound to
       ...
       SetEnv  Specifies one or more environment variables to set in
               child sessions started by sshd(8) as âNAME=VALUEâ.  The
       ...
       SshdSessionPath
               Overrides the default path to the sshd-session binary that
               is invoked to handle each connection.  The default is
               /usr/libexec/sshd-session.  This option is intended for
               use by tests.
       ...
       Subsystem
               ...
               on clients.  It accepts the same command line arguments as
               sftp-server and even though it is in-process, settings
               such as LogLevel or SyslogFacility do not apply to it and
               must be set explicitly via command line arguments.

               By default no subsystems are defined.

       SyslogFacility
               Gives the facility code that is used when logging messages
               from sshd(8).  The possible values are: DAEMON, USER,
       ...
       TCPKeepAlive
               ...
               find it annoying.  On the other hand, if TCP keepalives
       ...
       TrustedUserCAKeys
               ...
               one per line; empty lines and comments starting with â#â
       ...
       UnusedConnectionTimeout
               ...
               not prevent the timeout.  The timeout value is specified
               in seconds or may use any of the units documented in the
               âTIME FORMATSâ section.
               ...
               no open channels.  This option may be useful in
       ...
       X11Forwarding
               ...
               forwarding is that the client's X11 display server may be
               exposed to attack when the SSH client requests forwarding
               (see the warnings for ForwardX11 in ssh_config(5)).  A
               system administrator may have a stance in which they want
               to protect clients that may expose themselves to attack by
       ...
       X11UseLocalhost
               ...
               older X11 clients may not function with this
...
## TOKENS          top
       ...
       AuthorizedPrincipalsCommand accepts the tokens %%, %C, %D, %F, %f,
...
## COLOPHON          top
       ...
       for this manual page, see â¨http://www.openssh.com/report.htmlâ©.
       This page was obtained from the tarball openssh-9.9p2.tar.gz
       fetched from
       â¨http://ftp.eu.openbsd.org/pub/OpenBSD/OpenSSH/portable/â© on
...
GNU                         September 15, 2024             SSHD_CONFIG(5)
