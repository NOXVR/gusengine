...
[`Annot.delete_responses()`](#Annot.delete_responses)

delete all responding annotations

[`Annot.get_file()`](#Annot.get_file)

get attached file content

[`Annot.get_oc()`](#Annot.get_oc)

get [`xref`](glossary.html#xref) of an [`OCG`](glossary.html#OCG) / [`OCMD`](glossary.html#OCMD)

[`Annot.get_pixmap()`](#Annot.get_pixmap)

image of the annotation as a pixmap

[`Annot.get_sound()`](#Annot.get_sound)

get the sound of an audio annotation

[`Annot.get_text()`](#Annot.get_text)

extract annotation text

[`Annot.get_textbox()`](#Annot.get_textbox)

extract annotation text

[`Annot.get_textpage()`](#Annot.get_textpage)

create a TextPage for the annotation

[`Annot.set_border()`](#Annot.set_border)

set annotation’s border properties

[`Annot.set_blendmode()`](#Annot.set_blendmode)

set annotation’s blend mode

[`Annot.set_colors()`](#Annot.set_colors)

set annotation’s colors

[`Annot.set_flags()`](#Annot.set_flags)

set annotation’s flags field

[`Annot.set_irt_xref()`](#Annot.set_irt_xref)

define the annotation to being “In Response To”

[`Annot.set_name()`](#Annot.set_name)

set annotation’s name field

[`Annot.set_oc()`](#Annot.set_oc)

set [`xref`](glossary.html#xref) to an [`OCG`](glossary.html#OCG) / [`OCMD`](glossary.html#OCMD)

[`Annot.set_opacity()`](#Annot.set_opacity)

change transparency

[`Annot.set_open()`](#Annot.set_open)

open / close annotation or its Popup

[`Annot.set_popup()`](#Annot.set_popup)

create a Popup for the annotation

[`Annot.set_rect()`](#Annot.set_rect)

change annotation rectangle

[`Annot.set_rotation()`](#Annot.set_rotation)

change rotation

[`Annot.update_file()`](#Annot.update_file)
...
[`Annot.blendmode`](#Annot.blendmode)

annotation BlendMode

[`Annot.border`](#Annot.border)
...
[`Annot.file_info`](#Annot.file_info)

get attached file information

[`Annot.flags`](#Annot.flags)

annotation flags

[`Annot.has_popup`](#Annot.has_popup)

whether annotation has a Popup

[`Annot.irt_xref`](#Annot.irt_xref)
...
[`Annot.is_open`](#Annot.is_open)

whether annotation or its Popup is open

[`Annot.line_ends`](#Annot.line_ends)

start / end appearance of line-type annotations

[`Annot.next`](#Annot.next)

link to the next annotation

[`Annot.opacity`](#Annot.opacity)

the annot’s transparency

[`Annot.parent`](#Annot.parent)

page object of the annotation

[`Annot.popup_rect`](#Annot.popup_rect)

rectangle of the annotation’s Popup

[`Annot.popup_xref`](#Annot.popup_xref)

the PDF [`xref`](glossary.html#xref) number of the annotation’s Popup

[`Annot.rect`](#Annot.rect)
...
[`Annot.vertices`](#Annot.vertices)

point coordinates of Polygons, PolyLines, etc.

[`Annot.xref`](#Annot.xref)

the PDF [`xref`](glossary.html#xref) number

**Class API**

class Annot[¶](#Annot)

get_pixmap(matrix=pymupdf.Identity, dpi=None, colorspace=pymupdf.csRGB, alpha=False)[¶](#Annot.get_pixmap)
...
- 
**matrix** (*matrix_like*) – a matrix to be used for image creation. Default is [Identity](identity.html#identity).
...
- 
If the annotation has just been created or modified, you should [`Document.reload_page()`](document.html#Document.reload_page) the page first via `page = doc.reload_page(page)`.
...
get_text(opt, clip=None, flags=None)[¶](#Annot.get_text)
...
- 
”text” – [`TextPage.extractTEXT()`](textpage.html#TextPage.extractTEXT), default

- 
”blocks” – [`TextPage.extractBLOCKS()`](textpage.html#TextPage.extractBLOCKS)

- 
”words” – [`TextPage.extractWORDS()`](textpage.html#TextPage.extractWORDS)

- 
”html” – [`TextPage.extractHTML()`](textpage.html#TextPage.extractHTML)

- 
”xhtml” – [`TextPage.extractXHTML()`](textpage.html#TextPage.extractXHTML)

- 
”xml” – [`TextPage.extractXML()`](textpage.html#TextPage.extractXML)

- 
”dict” – [`TextPage.extractDICT()`](textpage.html#TextPage.extractDICT)

- 
”json” – [`TextPage.extractJSON()`](textpage.html#TextPage.extractJSON)

- 
”rawdict” – [`TextPage.extractRAWDICT()`](textpage.html#TextPage.extractRAWDICT)

- 
**clip** (*rect-like*) – (keyword only) restrict the extraction to this area. Should hardly ever be required, defaults to [`Annot.rect`](#Annot.rect).

- 
**flags** (*int*) – (keyword only) control the amount of data returned. Defaults to simple text extraction.

get_textbox(rect)[¶](#Annot.get_textbox)

- 
New in 1.18.0

Return the annotation text. Mostly (except line breaks) equal to [`Annot.get_text()`](#Annot.get_text) with the “text” option.

Parameters:

**rect** (*rect-like*) – the area to consider, defaults to [`Annot.rect`](#Annot.rect).

get_textpage(clip=None, flags=3)[¶](#Annot.get_textpage)
...
- 
**flags** (*int*) – indicator bits controlling the content available for subsequent text extractions and searches – see the parameter of [`Annot.get_text()`](#Annot.get_text).
...
set_info(info=None, content=None, title=None, creationDate=None, modDate=None, subject=None)[¶](#Annot.set_info)
...
- 
**content** (*str*) – *(new in v1.16.10)* see description in [`info`](#Annot.info).

- 
**title** (*str*) – *(new in v1.16.10)* see description in [`info`](#Annot.info).
...
- 
**subject** (*str*) – *(new in v1.16.10)* see description in [`info`](#Annot.info).

set_line_ends(start, end)[¶](#Annot.set_line_ends)
...
set_oc(xref)[¶](#Annot.set_oc)

Set the annotation’s visibility using PDF optional content mechanisms. This visibility is controlled by the user interface of supporting PDF viewers. It is independent from other attributes like [`Annot.flags`](#Annot.flags).

Parameters:

**xref** (*int*) – the [`xref`](glossary.html#xref) of an optional contents group (OCG or OCMD). Any previous xref will be overwritten. If zero, a previous entry will be removed. An exception occurs if the xref is not zero and does not point to a valid PDF object.

Note

This does **not require executing** [`Annot.update()`](#Annot.update) to take effect.

get_oc()[¶](#Annot.get_oc)

Return the [`xref`](glossary.html#xref) of an optional content object, or zero if there is none.

Returns:

zero or the xref of an OCG (or OCMD).

set_irt_xref(xref)[¶](#Annot.set_irt_xref)
...
**xref** (*int*) – 
The [`xref`](glossary.html#xref) of another annotation.

Note

Must refer to an existing annotation on this page. Setting this property requires no subsequent [`update()`](#Annot.update).

set_open(value)[¶](#Annot.set_open)
...
set_popup(rect)[¶](#Annot.set_popup)
...
set_opacity(value)[¶](#Annot.set_opacity)

Set the annotation’s transparency. Opacity can also be set in [`Annot.update()`](#Annot.update).
...
set_blendmode(blendmode)[¶](#Annot.set_blendmode)

- 
New in v1.16.14

Set the annotation’s blend mode. See [Adobe PDF References](app3.html#adobemanual), page 324 for explanations. The blend mode can also be set in [`Annot.update()`](#Annot.update).

Parameters:

**blendmode** (*str*) – set the blend mode. Use [`Annot.update()`](#Annot.update) to reflect this in the visual appearance. For predefined values see [PDF Standard Blend Modes](vars.html#blendmodes). Use `PDF_BM_Normal` to **remove** a blend mode.

set_name(name)[¶](#Annot.set_name)
...
You **need not** invoke [`Annot.update()`](#Annot.update) for activation of the effect.

set_rotation(angle)[¶](#Annot.set_rotation)
...
- 
You **must invoke** [`Annot.update()`](#Annot.update) to activate the effect.
...
set_border(border=None, width=None, style=None, dashes=None, clouds=None)[¶](#Annot.set_border)
...
PDF only: Change border width, dashing, style and cloud effect. See the [`Annot.border`](#Annot.border) attribute for more details.

Parameters:

- 
**border** (*dict*) – a dictionary as returned by the [`border`](#Annot.border) property, with keys *“width”* (*float*), *“style”* (*str*),  *“dashes”* (*sequence*) and *clouds* (*int*). Omitted keys will leave the resp. property unchanged. Set the border argument to `None` (the default) to use the other arguments.
...
set_flags(flags)[¶](#Annot.set_flags)
...
set_colors(colors=None, stroke=None, fill=None)[¶](#Annot.set_colors)
...
delete_responses()[¶](#Annot.delete_responses)
...
update(opacity=None, blend_mode=None, fontsize=0, text_color=None, border_color=None, fill_color=None, cross_out=True, rotate=-1)[¶](#Annot.update)
...
- 
[`Annot.set_info()`](#Annot.set_info) (except any changes to *“content”*)
...
- 
**blend_mode** (*str*) – *(new in v1.16.14)* **valid for all annotation types:** change or set the annotation’s blend mode. For valid values see [PDF Standard Blend Modes](vars.html#blendmodes).

- 
**fontsize** (*float*) – change [`fontsize`](glossary.html#fontsize) of the text. ‘FreeText’ annotations only.

- 
**text_color** (*sequence**,**float*) – change the text color. ‘FreeText’ annotations only. This has the same effect as `border_color`. Note that the text color of rich-text annotations cannot be changed at all because it is set by HTML / CSS syntax and part of the text itself.

- 
**border_color** (*sequence**,**float*) – change the border color. ‘FreeText’ annotations only. This has the same effect as `text_color`.
...
- 
**rotate** (*int*) – new rotation value. Default (-1) means no change. Supports ‘FreeText’ and several other annotation types (see [`Annot.set_rotation()`](#Annot.set_rotation)), [[1]](#f1). Only choose 0, 90, 180, or 270 degrees for ‘FreeText’. Otherwise any integer is acceptable.

Return type:

bool

This method is the only way to change the colors of a FreeText annotation. You cannot use [`Annot.set_colors()`](#Annot.set_colors) for this purpose. But be aware that for rich-text annotations, the text color is never changed. The text color is set by the `text_color` entry of the `info` dictionary. This is a limitation of MuPDF and not a bug.

Caution

Using this method inside a [`Page.annots()`](page.html#Page.annots) loop is **not recommended!** This is because most annotation updates require the owning page to be reloaded – which cannot be done inside this loop. Please use the example coding pattern given in the documentation of this generator.
...
update_file(buffer=None, filename=None, ufilename=None, desc=None)[¶](#Annot.update_file)
...
The PDF [`xref`](glossary.html#xref).

Return type:

int

irt_xref[¶](#Annot.irt_xref)

The PDF [`xref`](glossary.html#xref) of an annotation to which this one responds. Return zero if this is no response annotation.
...
The PDF [`xref`](glossary.html#xref) of the associated Popup annotation. Zero if non-existent.
...
## Annotation Icons in MuPDF[¶](#annotation-icons-in-mupdf)

This is a list of icons referenceable by name for annotation types ‘Text’ and ‘FileAttachment’. You can use them via the *icon* parameter when adding an annotation, or use the as argument in [`Annot.set_name()`](#Annot.set_name). It is left to your discretion which item to choose when – no mechanism will keep you from using e.g. the “Speaker” icon for a ‘FileAttachment’.
...
Rotating an annotation also changes its rectangle. Depending on how the annotation was defined, the original rectangle is **cannot be reconstructed** by setting the rotation value to zero again and will be lost.

[[2](#id2)]

Only the following annotation types support method [`Annot.set_rect()`](#Annot.set_rect): Text, FreeText, Square, Circle, Redact, Stamp, Caret, FileAttachment, Sound, and Movie.
...
- [`Annot.set_info()`](#Annot.set_info)

- [`Annot.set_line_ends()`](#Annot.set_line_ends)
...
- [`Annot.rect_delta`](#Annot.rect_delta)
