...
To maintain a consistent API, PyMuPDF supports the page *location* syntax for **all file types** – documents without this feature simply have just one chapter. [`Document.load_page()`](#Document.load_page) and the equivalent index access now also support a *location* argument.
...
[`Document.add_layer()`](#Document.add_layer)

PDF only: make new optional content configuration

[`Document.add_ocg()`](#Document.add_ocg)

PDF only: add new optional content group

[`Document.authenticate()`](#Document.authenticate)

gain access to an encrypted document

[`Document.bake()`](#Document.bake)

PDF only: make annotations / fields permanent content

[`Document.can_save_incrementally()`](#Document.can_save_incrementally)

check if incremental save is possible

[`Document.chapter_page_count()`](#Document.chapter_page_count)
...
[`Document.convert_to_pdf()`](#Document.convert_to_pdf)

write a PDF version to memory

[`Document.copy_page()`](#Document.copy_page)

PDF only: copy a page reference

[`Document.del_toc_item()`](#Document.del_toc_item)

PDF only: remove a single TOC item

[`Document.delete_page()`](#Document.delete_page)

PDF only: delete a page

[`Document.delete_pages()`](#Document.delete_pages)

PDF only: delete multiple pages

[`Document.embfile_add()`](#Document.embfile_add)

PDF only: add a new embedded file from buffer

[`Document.embfile_count()`](#Document.embfile_count)

PDF only: number of embedded files

[`Document.embfile_del()`](#Document.embfile_del)

PDF only: delete an embedded file entry

[`Document.embfile_get()`](#Document.embfile_get)

PDF only: extract an embedded file buffer

[`Document.embfile_info()`](#Document.embfile_info)

PDF only: metadata of an embedded file

[`Document.embfile_names()`](#Document.embfile_names)

PDF only: list of embedded files

[`Document.embfile_upd()`](#Document.embfile_upd)

PDF only: change an embedded file

[`Document.extract_font()`](#Document.extract_font)

PDF only: extract a font by [`xref`](glossary.html#xref)

[`Document.extract_image()`](#Document.extract_image)

PDF only: extract an embedded image by [`xref`](glossary.html#xref)

[`Document.ez_save()`](#Document.ez_save)

PDF only: [`Document.save()`](#Document.save) with different defaults

[`Document.find_bookmark()`](#Document.find_bookmark)

retrieve page location after laid out document

[`Document.fullcopy_page()`](#Document.fullcopy_page)

PDF only: duplicate a page

[`Document.get_layer()`](#Document.get_layer)

PDF only: lists of OCGs in ON, OFF, RBGroups

[`Document.get_layers()`](#Document.get_layers)

PDF only: list of optional content configurations

[`Document.get_oc()`](#Document.get_oc)

PDF only: get OCG /OCMD xref of image / form xobject

[`Document.get_ocgs()`](#Document.get_ocgs)

PDF only: info on all optional content groups

[`Document.get_ocmd()`](#Document.get_ocmd)

PDF only: retrieve definition of an [`OCMD`](glossary.html#OCMD)

[`Document.get_page_fonts()`](#Document.get_page_fonts)

PDF only: list of fonts referenced by a page

[`Document.get_page_images()`](#Document.get_page_images)

PDF only: list of images referenced by a page

[`Document.get_page_labels()`](#Document.get_page_labels)

PDF only: list of page label definitions

[`Document.get_page_numbers()`](#Document.get_page_numbers)

PDF only: get page numbers having a given label

[`Document.get_page_pixmap()`](#Document.get_page_pixmap)

create a pixmap of a page by page number

[`Document.get_page_text()`](#Document.get_page_text)

extract the text of a page by page number

[`Document.get_page_xobjects()`](#Document.get_page_xobjects)

PDF only: list of XObjects referenced by a page

[`Document.get_sigflags()`](#Document.get_sigflags)

PDF only: determine signature state

[`Document.get_toc()`](#Document.get_toc)

extract the table of contents

[`Document.get_xml_metadata()`](#Document.get_xml_metadata)

PDF only: read the XML metadata

[`Document.has_annots()`](#Document.has_annots)

PDF only: check if PDF contains any annots

[`Document.has_links()`](#Document.has_links)

PDF only: check if PDF contains any links

[`Document.insert_page()`](#Document.insert_page)
...
[`Document.insert_file()`](#Document.insert_file)

PDF only: insert pages from arbitrary document

[`Document.journal_can_do()`](#Document.journal_can_do)

PDF only: which journal actions are possible

[`Document.journal_enable()`](#Document.journal_enable)

PDF only: enables journalling for the document

[`Document.journal_load()`](#Document.journal_load)

PDF only: load journal from a file

[`Document.journal_op_name()`](#Document.journal_op_name)

PDF only: return name of a journalling step

[`Document.journal_position()`](#Document.journal_position)

PDF only: return journalling status

[`Document.journal_redo()`](#Document.journal_redo)

PDF only: redo current operation

[`Document.journal_save()`](#Document.journal_save)

PDF only: save journal to a file

[`Document.journal_start_op()`](#Document.journal_start_op)

PDF only: start an “operation” giving it a name

[`Document.journal_stop_op()`](#Document.journal_stop_op)

PDF only: end current operation

[`Document.journal_undo()`](#Document.journal_undo)

PDF only: undo current operation

[`Document.layer_ui_configs()`](#Document.layer_ui_configs)
...
[`Document.load_page()`](#Document.load_page)

read a page

[`Document.make_bookmark()`](#Document.make_bookmark)

create a page pointer in reflowable documents

[`Document.move_page()`](#Document.move_page)

PDF only: move a page to different location in doc

[`Document.need_appearances()`](#Document.need_appearances)
...
[`Document.next_location()`](#Document.next_location)

return (chapter, pno) of following page

[`Document.outline_xref()`](#Document.outline_xref)

PDF only: [`xref`](glossary.html#xref) a TOC item

[`Document.page_cropbox()`](#Document.page_cropbox)

PDF only: the unrotated page rectangle

[`Document.page_xref()`](#Document.page_xref)

PDF only: [`xref`](glossary.html#xref) of a page number

[`Document.pages()`](#Document.pages)

iterator over a page range

[`Document.pdf_catalog()`](#Document.pdf_catalog)

PDF only: [`xref`](glossary.html#xref) of catalog (root)

[`Document.pdf_trailer()`](#Document.pdf_trailer)

PDF only: trailer source

[`Document.prev_location()`](#Document.prev_location)

return (chapter, pno) of preceding page

[`Document.rewrite_images()`](#Document.rewrite_images)

PDF only: rewrite / extra compression for images

[`Document.recolor()`](#Document.recolor)

PDF only: execute [`Page.recolor()`](page.html#Page.recolor) for all pages

[`Document.reload_page()`](#Document.reload_page)

PDF only: provide a new copy of a page

[`Document.resolve_names()`](#Document.resolve_names)
...
[`Document.saveIncr()`](#Document.saveIncr)
...
[`Document.search_page_for()`](#Document.search_page_for)
...
[`Document.set_layer_ui_config()`](#Document.set_layer_ui_config)

PDF only: set OCG visibility temporarily

[`Document.set_layer()`](#Document.set_layer)

PDF only: mass changing OCG states

[`Document.set_markinfo()`](#Document.set_markinfo)
...
[`Document.set_oc()`](#Document.set_oc)

PDF only: attach OCG/OCMD to image / form xobject

[`Document.set_ocmd()`](#Document.set_ocmd)

PDF only: create or update an [`OCMD`](glossary.html#OCMD)

[`Document.set_page_labels()`](#Document.set_page_labels)

PDF only: add/update page label definitions

[`Document.set_pagemode()`](#Document.set_pagemode)

PDF only: set the PageMode

[`Document.set_pagelayout()`](#Document.set_pagelayout)

PDF only: set the PageLayout

[`Document.set_toc_item()`](#Document.set_toc_item)

PDF only: change a single TOC item

[`Document.set_toc()`](#Document.set_toc)

PDF only: set the table of contents (TOC)

[`Document.set_xml_metadata()`](#Document.set_xml_metadata)

PDF only: create or update document XML metadata

[`Document.subset_fonts()`](#Document.subset_fonts)

PDF only: create font subsets

[`Document.switch_layer()`](#Document.switch_layer)

PDF only: activate OC configuration

[`Document.tobytes()`](#Document.tobytes)

PDF only: writes document to memory

[`Document.xref_copy()`](#Document.xref_copy)

PDF only: copy a PDF dictionary to another [`xref`](glossary.html#xref)

[`Document.xref_get_key()`](#Document.xref_get_key)

PDF only: get the value of a dictionary key

[`Document.xref_get_keys()`](#Document.xref_get_keys)

PDF only: list the keys of object at [`xref`](glossary.html#xref)

[`Document.xref_object()`](#Document.xref_object)

PDF only: get the definition source of [`xref`](glossary.html#xref)

[`Document.xref_set_key()`](#Document.xref_set_key)

PDF only: set the value of a dictionary key

[`Document.xref_stream_raw()`](#Document.xref_stream_raw)

PDF only: raw stream source at [`xref`](glossary.html#xref)

[`Document.xref_xml_metadata()`](#Document.xref_xml_metadata)

PDF only: [`xref`](glossary.html#xref) of XML metadata

[`Document.chapter_count`](#Document.chapter_count)

number of chapters

[`Document.FormFonts`](#Document.FormFonts)

PDF only: list of global widget fonts

[`Document.is_closed`](#Document.is_closed)

has document been closed?

[`Document.is_dirty`](#Document.is_dirty)

PDF only: has document been changed yet?

[`Document.is_encrypted`](#Document.is_encrypted)

document (still) encrypted?

[`Document.is_fast_webaccess`](#Document.is_fast_webaccess)

is PDF linearized?

[`Document.is_form_pdf`](#Document.is_form_pdf)

is this a Form PDF?

[`Document.is_pdf`](#Document.is_pdf)

is this a PDF?

[`Document.is_reflowable`](#Document.is_reflowable)

is this a reflowable document?

[`Document.is_repaired`](#Document.is_repaired)

PDF only: has this PDF been repaired during open?

[`Document.last_location`](#Document.last_location)
...
[`Document.markinfo`](#Document.markinfo)
...
[`Document.needs_pass`](#Document.needs_pass)

require password to access data?

[`Document.outline`](#Document.outline)

first [Outline](outline.html#outline) item
...
[`Document.pagemode`](#Document.pagemode)

PDF PageMode value

[`Document.pagelayout`](#Document.pagelayout)

PDF PageLayout value

[`Document.version_count`](#Document.version_count)
...
__init__(self, filename=None, stream=None, *, filetype=None, rect=None, width=0, height=0, fontsize=11)[¶](#Document.__init__)
...
- 
**fontsize** (*float*) – the default [`fontsize`](glossary.html#fontsize) for reflowable document types. This parameter is ignored if none of the parameters `rect` or `width` and `height` are specified. Will be used to calculate the page layout.
...
In case of problems you can see more detail in the internal messages store: `print(pymupdf.TOOLS.mupdf_warnings())` (which will be emptied by this call, but you can also prevent this – consult [`Tools.mupdf_warnings()`](tools.html#Tools.mupdf_warnings)).
...
get_oc(xref)[¶](#Document.get_oc)

- 
New in v1.18.4

Return the cross reference number of an [`OCG`](glossary.html#OCG) or [`OCMD`](glossary.html#OCMD) attached to an image or form xobject.

Parameters:

**xref** (*int*) – the [`xref`](glossary.html#xref) of an image or form xobject. Valid such cross reference numbers are returned by [`Document.get_page_images()`](#Document.get_page_images), resp. [`Document.get_page_xobjects()`](#Document.get_page_xobjects). For invalid numbers, an exception is raised.
...
set_oc(xref, ocxref)[¶](#Document.set_oc)

- 
New in v1.18.4

If [`xref`](glossary.html#xref) represents an image or form xobject, set or remove the cross reference number *ocxref* of an optional contents object.

Parameters:

- 
**xref** (*int*) – the [`xref`](glossary.html#xref) of an image or form xobject [[5]](#f5). Valid such cross reference numbers are returned by [`Document.get_page_images()`](#Document.get_page_images), resp. [`Document.get_page_xobjects()`](#Document.get_page_xobjects). For invalid numbers, an exception is raised.

- 
**ocxref** (*int*) – the [`xref`](glossary.html#xref) number of an [`OCG`](glossary.html#OCG) / [`OCMD`](glossary.html#OCMD). If not zero, an invalid reference raises an exception. If zero, any OC reference is removed.

get_layers()[¶](#Document.get_layers)
...
add_layer(name, creator=None, on=None)[¶](#Document.add_layer)
...
- 
**on** (*sequ*) – a sequence of OCG [`xref`](glossary.html#xref) numbers which should be set to ON when this layer gets activated. All OCGs not listed here will be set to OFF.

switch_layer(number, as_default=False)[¶](#Document.switch_layer)
...
add_ocg(name, config=-1, on=True, intent='View', usage='Artwork')[¶](#Document.add_ocg)
...
[`xref`](glossary.html#xref) of the created OCG. Use as entry for `oc` parameter in supporting objects.

Note

Multiple OCGs with identical parameters may be created. This will not cause problems. Garbage option 3 of [`Document.save()`](#Document.save) will get rid of any duplicates.

set_ocmd(xref=0, ocgs=None, policy='AnyOn', ve=None)[¶](#Document.set_ocmd)

- 
New in v1.18.4

Create or update an [`OCMD`](glossary.html#OCMD), **Optional Content Membership Dictionary.**

Parameters:

- 
**xref** (*int*) – [`xref`](glossary.html#xref) of the OCMD to be updated, or 0 for a new OCMD.

- 
**ocgs** (*list*) – a sequence of [`xref`](glossary.html#xref) numbers of existing [`OCG`](glossary.html#OCG) PDF objects.
...
[`xref`](glossary.html#xref) of the OCMD. Use as `oc=xref` parameter in supporting objects, and respectively in [`Document.set_oc()`](#Document.set_oc) or [`Annot.set_oc()`](annot.html#Annot.set_oc).
...
- 
Use the combination of *ocgs* and *policy*: The *policy* value is interpreted as follows:
...
Solution: use an **OCG** for object 1 and an **OCMD** for object 2. Create the OCMD via `set_ocmd(ocgs=[xref], policy="AllOff")`, with the [`xref`](glossary.html#xref) of the OCG.

- 
Use the **visibility expression** *ve*: This is a list of two or more items. The **first item** is a logical keyword: one of the strings **“and”**, **“or”**, or **“not”**. The **second** and all subsequent items must either be an integer or another list. An integer must be the [`xref`](glossary.html#xref) number of an OCG. A list must again have at least two items starting with one of the boolean keywords. This syntax is a bit awkward, but quite powerful:
...
get_ocmd(xref)[¶](#Document.get_ocmd)

- 
New in v1.18.4

Retrieve the definition of an [`OCMD`](glossary.html#OCMD).

Parameters:

**xref** (*int*) – the [`xref`](glossary.html#xref) of the OCMD.
...
a dictionary with the keys [`xref`](glossary.html#xref), *ocgs*, *policy* and *ve*.

get_layer(config=-1)[¶](#Document.get_layer)
...
set_layer(config, *, on=None, off=None, basestate=None, rbgroups=None, locked=None)[¶](#Document.set_layer)
...
- 
**on** (*list*) – list of [`xref`](glossary.html#xref) of OCGs to set ON. Replaces previous values. An empty list will cause no OCG being set to ON anymore. Should be specified if `basestate="ON"` is used.

- 
**off** (*list*) – list of [`xref`](glossary.html#xref) of OCGs to set OFF. Replaces previous values. An empty list will cause no OCG being set to OFF anymore. Should be specified if `basestate="OFF"` is used.
...
get_ocgs()[¶](#Document.get_ocgs)

- 
New in v1.18.3

Details of all optional content groups. This is a dictionary of dictionaries like this (key is the OCG’s [`xref`](glossary.html#xref)):
...
layer_ui_configs()[¶](#Document.layer_ui_configs)
...
set_layer_ui_config(number, action=0)[¶](#Document.set_layer_ui_config)
...
To make **permanent** changes, use [`Document.set_layer()`](#Document.set_layer).
...
authenticate(password)[¶](#Document.authenticate)
...
a positive value if successful, zero otherwise (the string does not match either password). If positive, the indicator [`Document.is_encrypted`](#Document.is_encrypted) is set to `False`. **Positive** return codes carry the following information detail:
...
The document may be protected by an owner, but **not** by a user password. Detect this situation via `doc.authenticate("") == 2`. This allows opening and reading the document without authentication, but, depending on the [`Document.permissions`](#Document.permissions) value, other actions may be prohibited. PyMuPDF (like MuPDF) in this case **ignores those restrictions**. So, – in contrast to any PDF viewers – you can for example extract text and add or modify content, even if the respective permission flags `PDF_PERM_COPY`, `PDF_PERM_MODIFY`, `PDF_PERM_ANNOTATE`, etc. are set off! It is your responsibility building a legally compliant application where applicable.

get_page_numbers(label, only_one=False)[¶](#Document.get_page_numbers)
...
get_page_labels()[¶](#Document.get_page_labels)

- 
New in v1.18.7

PDF only: Extract the list of page label definitions. Typically used for modifications before feeding it into [`Document.set_page_labels()`](#Document.set_page_labels).

Returns:

a list of dictionaries as defined in [`Document.set_page_labels()`](#Document.set_page_labels).

set_page_labels(labels)[¶](#Document.set_page_labels)
...
make_bookmark(loc)[¶](#Document.make_bookmark)
...
find_bookmark(bookmark)[¶](#Document.find_bookmark)
...
**bookmark** (*pointer*) – created by [`Document.make_bookmark()`](#Document.make_bookmark).
...
chapter_page_count(chapter)[¶](#Document.chapter_page_count)
...
next_location(page_id)[¶](#Document.next_location)
...
prev_location(page_id)[¶](#Document.prev_location)
...
load_page(page_id=0)[¶](#Document.load_page)
...
Either a 0-based page number, or a tuple *(chapter, pno)*. For an **integer**, any `-∞ < page_id < page_count` is acceptable. While page_id is negative, [`page_count`](#Document.page_count) will be added to it. For example: to load the last page, you can use *doc.load_page(-1)*. After this you have page.number = doc.page_count - 1.

For a tuple, *chapter* must be in range [`Document.chapter_count`](#Document.chapter_count), and *pno* must be in range [`Document.chapter_page_count()`](#Document.chapter_page_count) of that chapter. Both values are 0-based. Using this notation, [`Page.number`](page.html#Page.number) will equal the given tuple. Relevant only for document types with chapter support (EPUB currently).
...
For **absolute page numbers** only, expressions like *“for page in doc: …”* and *“for page in reversed(doc): …”* will successively yield the document’s pages. Refer to [`Document.pages()`](#Document.pages) which allows processing pages as with slicing.
...
To maintain a consistent API, for document types not supporting a chapter structure (like PDFs), [`Document.chapter_count`](#Document.chapter_count) is 1, and pages can also be loaded via tuples *(0, pno)*. See this [[3]](#f3) footnote for comments on performance improvements.

rewrite_images(dpi_threshold=None, dpi_target=0, quality=0, lossy=True, lossless=True, bitonal=True, color=True, gray=True, set_to_gray=False, options=None)[¶](#Document.rewrite_images)
...
- 
**dpi_target** (*int*) – target DPI value for the resampled images. Ignored if `dpi_threshold` is `None`, otherwise must be less than `dpi_threshold` and positive.

- 
**dpi_threshold** (*int*) – If None (the default) no resampling takes place. Otherwise images with a DPI value larger than this will be resampled to `dpi_target` (which must be less than `dpi_threshold`).
...
- 
**set_to_gray** (*bool*) – if True, the PDF will be converted to grayscale by executing [`Document.recolor()`](#Document.recolor) before all image processing. Please note that this will also change text and vector graphics to grayscale – not just the images.
...
recolor(components=1)[¶](#Document.recolor)
...
reload_page(page)[¶](#Document.reload_page)
...
resolve_names()[¶](#Document.resolve_names)
...
page_cropbox(pno)[¶](#Document.page_cropbox)

- 
New in v1.17.7

PDF only: Return the unrotated page rectangle – **without loading the page** (via [`Document.load_page()`](#Document.load_page)). This is meant for internal purpose requiring best possible performance.
...
[Rect](rect.html#rect) of the page like [`Page.rect()`](page.html#Page.rect), but ignoring any rotation.

page_xref(pno)[¶](#Document.page_xref)

- 
New in v1.17.7

PDF only: Return the [`xref`](glossary.html#xref) of the page – **without loading the page** (via [`Document.load_page()`](#Document.load_page)). This is meant for internal purpose requiring best possible performance.
...
[`xref`](glossary.html#xref) of the page like [`Page.xref`](page.html#Page.xref).

pages(start=None[, stop=None[, step=None]])[¶](#Document.pages)
...
- 
**start** (*int*) – start iteration with this page number. Default is zero, allowed values are `-∞ < start < page_count`. While this is negative, [`page_count`](#Document.page_count) is added **before** starting the iteration.

- 
**stop** (*int*) – stop iteration at this page number. Default is [`page_count`](#Document.page_count), possible are `-∞ < stop <= page_count`. Larger values are **silently replaced** by the default. Negative values will cyclically emit the pages in reversed order. As with the built-in *range()*, this is the first page **not** returned.
...
convert_to_pdf(from_page=-1, to_page=-1, rotate=0)[¶](#Document.convert_to_pdf)

Create a PDF version of the current document and write it to memory. **All document types** are supported. The parameters have the same meaning as in [`insert_pdf()`](#Document.insert_pdf). In essence, you can restrict the conversion to a page subset, specify page rotation, and revert page sequence.
...
a Python *bytes* object containing a PDF file image. It is created by internally using `tobytes(garbage=4, deflate=True)`. See [`tobytes()`](#Document.tobytes). You can output it directly to disk or open it as a PDF. Here are some examples:
...
- 
Image files: perfect, no issues detected. However, image transparency is ignored. If you need that (like for a watermark), use [`Page.insert_image()`](page.html#Page.insert_image) instead. Otherwise, this method is recommended for its much better performance.
...
get_toc(simple=True)[¶](#Document.get_toc)
...
- 
file: filename if kind is [`LINK_GOTOR`](vars.html#LINK_GOTOR) or [`LINK_LAUNCH`](vars.html#LINK_LAUNCH).

- 
page: target page, 0-based, [`LINK_GOTOR`](vars.html#LINK_GOTOR) or [`LINK_GOTO`](vars.html#LINK_GOTO) only.
...
- 
xref: [`xref`](glossary.html#xref) of the item (0 if no PDF).
...
xref_get_keys(xref)[¶](#Document.xref_get_keys)

- 
New in v1.18.7

PDF only: Return the PDF dictionary keys of the [`dictionary`](glossary.html#dictionary) object provided by its xref number.

Parameters:

**xref** (*int*) – the [`xref`](glossary.html#xref). *(Changed in v1.18.10)* Use `-1` to access the special dictionary “PDF trailer”.

Returns:

a tuple of dictionary keys present in object [`xref`](glossary.html#xref). Examples:
...
xref_get_key(xref, key)[¶](#Document.xref_get_key)

- 
New in v1.18.7

PDF only: Return type and value of a PDF dictionary key of a [`dictionary`](glossary.html#dictionary) object given by its xref.
...
- 
**key** (*str*) – the desired PDF key. Must **exactly** match (case-sensitive) one of the keys contained in [`Document.xref_get_keys()`](#Document.xref_get_keys).
...
xref_set_key(xref, key, value)[¶](#Document.xref_set_key)
...
PDF only: Set (add, update, delete) the value of a PDF key for the [`dictionary`](glossary.html#dictionary) object given by its xref.
...
- 
**xref** (*int*) – the [`xref`](glossary.html#xref). *Changed in v1.18.13:* To update the PDF trailer, specify -1.
...
- 
*:data:`xref`* – must be provided as `"nnn 0 R"` with a valid [`xref`](glossary.html#xref) number nnn of the PDF. The suffix “`0 R`” is required to be recognizable as an xref by PDF applications.
...
- 
If in doubt, we **strongly recommend** to use [`get_pdf_str()`](functions.html#get_pdf_str)! This function automatically generates the right brackets, escapes, and overall format. It will for example do conversions like these:
...
get_page_pixmap(pno: int, *, matrix: matrix_like = Identity, dpi=None, colorspace: [Colorspace](colorspace.html#Colorspace) = csRGB, clip: rect_like = None, alpha: bool = False, annots: bool = True)[¶](#Document.get_page_pixmap)

Creates a pixmap from page *pno* (zero-based). Invokes [`Page.get_pixmap()`](page.html#Page.get_pixmap).
...
get_page_xobjects(pno)[¶](#Document.get_page_xobjects)
...
a list of (non-image) XObjects. These objects typically represent pages *embedded* (not copied) from other PDFs. For example, [`Page.show_pdf_page()`](page.html#Page.show_pdf_page) will create this type of object. An item of this list has the following layout: `(xref, name, invoker, bbox)`, where

- 
*:data:`xref`* (*int*) is the XObject’s [`xref`](glossary.html#xref).

- 
**name** (*str*) is the symbolic name to reference the XObject.

- 
**invoker** (*int*) the [`xref`](glossary.html#xref) of the invoking XObject or zero if the page directly invokes it.
...
get_page_images(pno, full=False)[¶](#Document.get_page_images)
...
- 
**full** (*bool*) – whether to also include the referencer’s [`xref`](glossary.html#xref) (which is zero if this is the page).
...
- 
`referencer` (*int*) the [`xref`](glossary.html#xref) of the referencer. Zero if directly referenced by the page. Only present if *full=True*.

Note

In general, this is not the list of images that are **actually displayed**. This method only parses several PDF objects to collect references to embedded images. It does not analyse the page’s [`contents`](glossary.html#contents), where all the actual image display commands are defined. To get this information, please use [`Page.get_image_info()`](page.html#Page.get_image_info). Also have a look at the discussion in section [Structure of Dictionary Outputs](textpage.html#textpagedict).

get_page_fonts(pno, full=False)[¶](#Document.get_page_fonts)
...
- 
**full** (*bool*) – whether to also include the referencer’s [`xref`](glossary.html#xref). If `True`, the returned items are one entry longer. Use this option if you need to know, whether the page directly references the font. In this case the last entry is 0. If the font is referenced by an `/XObject` of the page, you will find its [`xref`](glossary.html#xref) here.
...
- 
`referencer` (*int* optional) the [`xref`](glossary.html#xref) of the referencer. Zero if directly referenced by the page, otherwise the xref of an XObject. Only present if *full=True*.
...
- 
This list has no duplicate entries: the combination of [`xref`](glossary.html#xref), *name* and *referencer* is unique.
...
- 
Be aware that font names returned by some variants of [`Page.get_text()`](page.html#Page.get_text) (respectively [TextPage](textpage.html#textpage) methods) need not (exactly) equal the base font name shown here. Reasons for any differences include:
...
get_page_text(pno, output='text', flags=3, textpage=None, sort=False)[¶](#Document.get_page_text)

Extracts the text of a page given its page number *pno* (zero-based). Invokes [`Page.get_text()`](page.html#Page.get_text).
...
layout(rect=None, width=0, height=0, fontsize=11)[¶](#Document.layout)

Re-paginate (“reflow”) the document based on the given page dimension and fontsize. This only affects some document types like e-books and HTML. Ignored if not supported. Supported documents have `True` in property [`is_reflowable`](#Document.is_reflowable).
...
- 
On a technical level, the method will always create a new [`pagetree`](glossary.html#pagetree).

- 
When dealing with only a few pages, methods [`copy_page()`](#Document.copy_page), [`move_page()`](#Document.move_page), [`delete_page()`](#Document.delete_page) are easier to use. In fact, they are also **much faster** – by at least one order of magnitude when the document has many pages.

set_metadata(m)[¶](#Document.set_metadata)
...
get_xml_metadata()[¶](#Document.get_xml_metadata)
...
set_xml_metadata(xml)[¶](#Document.set_xml_metadata)
...
set_pagelayout(value)[¶](#Document.set_pagelayout)
...
set_pagemode(value)[¶](#Document.set_pagemode)
...
set_markinfo(value)[¶](#Document.set_markinfo)
...
set_toc(toc, collapse=1)[¶](#Document.set_toc)

PDF only: Replaces the **complete current outline** tree (table of contents) with the one provided as the argument. After successful execution, the new outline tree can be accessed as usual via [`Document.get_toc()`](#Document.get_toc) or via [`Document.outline`](#Document.outline). Like with other output-oriented methods, changes become permanent only via [`save()`](#Document.save) (incremental save supported). Internally, this method consists of the following two steps. For a demonstration see example below.
...
- 
**toc** (*sequence*) – 
A list / tuple with **all bookmark entries** that should form the new table of contents. Output variants of [`get_toc()`](#Document.get_toc) are acceptable. To completely remove the table of contents specify an empty sequence or None. Each item must be a list with the following format.
...
same coordinate system as those returned by [`get_toc()`](#Document.get_toc) (internally they
...
outline_xref(idx)[¶](#Document.outline_xref)

- 
New in v1.17.7

PDF only: Return the [`xref`](glossary.html#xref) of the outline item. This is mainly used for internal purposes.

Parameters:

**idx** (*int*) – index of the item in list [`Document.get_toc()`](#Document.get_toc).

Returns:

[`xref`](glossary.html#xref).

del_toc_item(idx)[¶](#Document.del_toc_item)
...
This also implies that you can reassign the item to a new destination using [`Document.set_toc_item()`](#Document.set_toc_item), when required.

Parameters:

**idx** (*int*) – the index of the item in list [`Document.get_toc()`](#Document.get_toc).

set_toc_item(idx, dest_dict=None, kind=None, pno=None, uri=None, title=None, to=None, filename=None, zoom=0)[¶](#Document.set_toc_item)
...
- 
**idx** (*int*) – the index of the entry in the list created by [`Document.get_toc()`](#Document.get_toc).
...
- 
**kind** (*int*) – the link kind, see [Link Destination Kinds](vars.html#linkdest-kinds). If [`LINK_NONE`](vars.html#LINK_NONE), then all remaining parameter will be ignored, and the TOC item will be removed – same as [`Document.del_toc_item()`](#Document.del_toc_item). If None, then only the title is modified and the remaining parameters are ignored. All other values will lead to making a new destination dictionary using the subsequent arguments.
...
bake(*, annots=True, widgets=True)[¶](#Document.bake)

PDF only: Convert annotations and / or widgets to become permanent parts of the pages. The PDF **will be changed** by this method. If [`widgets`](page.html#Page.widgets) is `True`, the document will also no longer be a “Form PDF”.
...
The method may thus be a viable **alternative for PDF-to-PDF conversions** using [`Document.convert_to_pdf()`](#Document.convert_to_pdf).
...
Use this feature for instance for [`Page.show_pdf_page()`](page.html#Page.show_pdf_page) (which supports neither annotations nor widgets) when the source pages should look exactly the same in the target.
...
can_save_incrementally()[¶](#Document.can_save_incrementally)
...
scrub(attached_files=True, clean_pages=True, embedded_files=True, hidden_text=True, javascript=True, metadata=True, redactions=True, redact_images=0, remove_links=True, reset_fields=True, reset_responses=True, thumbnails=True, xml_metadata=True)[¶](#Document.scrub)
...
- 
**hidden_text** (*bool*) – Remove OCRed text and invisible text [[7]](#f7).
...
- 
**outfile** (*str**,**Path**,**fp*) – The file path, `pathlib.Path` or file object to save to. A file object must have been created before via `open(...)` or `io.BytesIO()`. Choosing `io.BytesIO()` is similar to [`Document.tobytes()`](#Document.tobytes) below, which equals the `getvalue()` output of an internally created `io.BytesIO()`.
...
- 
2 = in addition to 1, compact the [`xref`](glossary.html#xref) table.

- 
3 = in addition to 2, merge duplicate objects.

- 
4 = in addition to 3, check [`stream`](glossary.html#stream) objects for duplication. This may be slow because such data are typically large.
...
- 
**deflate_images** (*bool*) – *(new in v1.18.3)* Deflate (compress) uncompressed image streams [[4]](#f4).

- 
**deflate_fonts** (*bool*) – *(new in v1.18.3)* Deflate (compress) uncompressed fontfile streams [[4]](#f4).

- 
**incremental** (*bool*) – Only save changes to the PDF. Excludes “garbage” and “linear”. Can only be used if *outfile* is a string or a `pathlib.Path` and equal to [`Document.name`](#Document.name). Cannot be used for files that are decrypted or repaired and also in some other cases. To be sure, check [`Document.can_save_incrementally()`](#Document.can_save_incrementally). If this is false, saving to a new file is required.
...
- 
**linear** (*bool*) – Save a linearised version of the document. This option creates a file format for improved performance for Internet access. Excludes “incremental” and “use_objstms”.

- 
**pretty** (*bool*) – Prettify the document source for better readability. PDF objects will be reformatted to look like the default output of [`Document.xref_object()`](#Document.xref_object).
...
- 
**use_objstms** (*int*) – 
*(new in v1.24.0)* compression option that converts eligible PDF object definitions to information that is stored in some other object’s [`stream`](glossary.html#stream) data. Depending on the `deflate` parameter value, the converted object definitions will be compressed – which can lead to very significant file size reductions.
...
Also see [`Document.repair()`](#Document.repair).
...
This is a “lossless” file size reduction. There is a convenience version of this method with these values set by default, [`Document.ez_save()`](#Document.ez_save) – please see below.
...
ez_save(*args, **kwargs)[¶](#Document.ez_save)

- 
New in v1.18.11

PDF only: The same as [`Document.save()`](#Document.save) but with changed defaults `deflate=True, garbage=3, use_objstms=1`.

saveIncr()[¶](#Document.saveIncr)
...
tobytes(garbage=0, clean=False, deflate=False, deflate_images=False, deflate_fonts=False, ascii=False, expand=0, linear=False, pretty=False, no_new_id=False, encryption=PDF_ENCRYPT_NONE, permissions=-1, owner_pw=None, user_pw=None, use_objstms=0)[¶](#Document.tobytes)
...
PDF only: Writes the **current content of the document** to a bytes object instead of to a file. Obviously, you should be wary about memory requirements. The meanings of the parameters exactly equal those in [`save()`](#Document.save). Chapter [FAQ](faq.html#faq) contains an example for using this method as a pre-processor to [pdfrw](https://pypi.python.org/pypi/pdfrw/0.3).
...
search_page_for(pno, text, quads=False)[¶](#Document.search_page_for)

Search for “text” on page number “pno”. Works exactly like the corresponding [`Page.search_for()`](page.html#Page.search_for). Any integer `-∞ < pno < page_count` is acceptable.

insert_pdf(docsrc, *, from_page=-1, to_page=-1, start_at=-1, rotate=-1, links=True, annots=True, widgets=True, join_duplicates=False, show_progress=0, final=1)[¶](#Document.insert_pdf)
...
- 
**links** (*bool*) – Choose whether (internal and external) links should be included in the copy. Default is `True`. *Named* links ([`LINK_NAMED`](vars.html#LINK_NAMED)) and internal links to outside the copied page range are **always excluded**.
...
- 
This is a page-based method. Document-level information of source documents is therefore mostly ignored. Examples include Optional Content, Embedded Files, `StructureElem`, table of contents, page labels, metadata, named destinations (and other named entries) and some more.

- 
If `from_page > to_page`, pages will be **copied in reverse order**. If `0 <= from_page == to_page`, then one page will be copied.
...
insert_file(infile, from_page=-1, to_page=-1, start_at=-1, rotate=-1, links=True, annots=True, show_progress=0, final=1)[¶](#Document.insert_file)

- 
New in v1.22.0

PDF only: Add an arbitrary supported document to the current PDF. Opens “infile” as a document, converts it to a PDF and then invokes [`Document.insert_pdf()`](#Document.insert_pdf). Parameters are the same as for that method. Among other things, this features an easy way to append images as full pages to an output PDF.
...
new_page(pno=-1, width=595, height=842)[¶](#Document.new_page)
...
- 
**pno** (*int*) – page number index (zero-indexed) at which to insert page. Special values -1 and *doc.page_count* insert **after** the last page.
...
insert_page(pno, text=None, fontsize=11, width=595, height=842, fontname='helv', fontfile=None, color=None)[¶](#Document.insert_page)

PDF only: Insert a new page and insert some text. Convenience function which combines [`Document.new_page()`](#Document.new_page) and (parts of) [`Page.insert_text()`](page.html#Page.insert_text).
...
the result of [`Page.insert_text()`](page.html#Page.insert_text) (number of successfully inserted lines).

delete_page(pno=-1)[¶](#Document.delete_page)
...
delete_pages(*args, **kwds)[¶](#Document.delete_pages)
...
copy_page(pno, to=-1)[¶](#Document.copy_page)
...
Only a new **reference** to the page object will be created – not a new page object, all copied pages will have identical attribute values, including the [`Page.xref`](page.html#Page.xref). This implies that any changes to one of these copies will appear on all of them.

fullcopy_page(pno, to=-1)[¶](#Document.fullcopy_page)
...
- 
In contrast to [`copy_page()`](#Document.copy_page), this method creates a new page object (with a new [`xref`](glossary.html#xref)), which can be changed independently from the original.

- 
Any Popup and “IRT” (“in response to”) annotations are **not copied** to avoid potentially incorrect situations.

move_page(pno, to=-1)[¶](#Document.move_page)
...
need_appearances(value=None)[¶](#Document.need_appearances)
...
get_sigflags()[¶](#Document.get_sigflags)
...
embfile_add(name, buffer, filename=None, ufilename=None, desc=None)[¶](#Document.embfile_add)
...
*(Changed in v1.18.13)* The method now returns the [`xref`](glossary.html#xref) of the inserted file. In addition, the file object now will be automatically given the PDF keys `/CreationDate` and `/ModDate` based on the current date-time.

embfile_count()[¶](#Document.embfile_count)
...
embfile_get(item)[¶](#Document.embfile_get)
...
embfile_del(item)[¶](#Document.embfile_del)
...
embfile_info(item)[¶](#Document.embfile_info)
...
- 
`collection` – (*int*) [`xref`](glossary.html#xref) of the associated PDF portfolio item if any, else zero.
...
embfile_names()[¶](#Document.embfile_names)
...
embfile_upd(item, buffer=None, filename=None, ufilename=None, desc=None)[¶](#Document.embfile_upd)
...
*(Changed in v1.18.13)*  The method now returns the [`xref`](glossary.html#xref) of the file object.
...
xref_object(xref, compressed=False, ascii=False)[¶](#Document.xref_object)
...
- 
**xref** (*int*) – the object’s [`xref`](glossary.html#xref). *Changed in v1.18.10:* A value of `-1` returns the PDF trailer source.
...
pdf_catalog()[¶](#Document.pdf_catalog)

- 
New in v1.16.8

PDF only: Return the [`xref`](glossary.html#xref) number of the PDF catalog (or root) object. Use that number with [`Document.xref_object()`](#Document.xref_object) to see its source.

pdf_trailer(compressed=False)[¶](#Document.pdf_trailer)

- 
New in v1.16.8

PDF only: Return the trailer source of the PDF,  which is usually located at the PDF file’s end. This is [`Document.xref_object()`](#Document.xref_object) with an [`xref`](glossary.html#xref) argument of -1.

xref_stream(xref)[¶](#Document.xref_stream)

- 
New in v1.16.8

PDF only: Return the **decompressed** contents of the [`xref`](glossary.html#xref) stream object.

Parameters:

**xref** (*int*) – [`xref`](glossary.html#xref) number.
...
xref_stream_raw(xref)[¶](#Document.xref_stream_raw)

- 
New in v1.16.8

PDF only: Return the **unmodified** (esp. **not decompressed**) contents of the [`xref`](glossary.html#xref) stream object. Otherwise equal to [`Document.xref_stream()`](#Document.xref_stream).
...
update_object(xref, obj_str, page=None)[¶](#Document.update_object)

- 
New in v1.16.8

PDF only: Replace object definition of [`xref`](glossary.html#xref) with the provided string. The xref may also be new, in which case this instruction completes the object definition. If a page object is also given, its links and annotations will be reloaded afterwards.
...
update_stream(xref, data, new=False, compress=True)[¶](#Document.update_stream)
...
Replace the stream of an object identified by [`xref`](glossary.html#xref), which must be a PDF dictionary. If the object is no [`stream`](glossary.html#stream), it will be turned into one. The function automatically performs a compress operation (“deflate”) where beneficial.
...
**ValueError** – if [`xref`](glossary.html#xref) does not represent a PDF `dict`. An empty dictionary `<<>>` is accepted. So if you just created the xref and want to give it a stream, first execute `doc.update_object(xref, "<<>>")`, and then insert the stream data with this method.
...
Example: Let us assume that you no longer want a certain image appear on a page. This can be achieved by deleting the respective reference in its contents source(s) – and indeed: the image will be gone after reloading the page. But the page’s [`resources`](glossary.html#resources) object would still show the image as being referenced by the page. This save option will clean up any such mismatches.

xref_copy(source, target, *, keep=None)[¶](#Document.xref_copy)

- 
New in v1.19.5

PDF Only: Make `target` xref an exact copy of `source`. If `source` is a [`stream`](glossary.html#stream), then this data is also copied.

Parameters:

- 
**source** (*int*) – the source [`xref`](glossary.html#xref). It must be an existing **dictionary** object.
...
- 
This method has much in common with Python’s *dict* method [`copy()`](pixmap.html#Pixmap.copy).
...
- 
If `source` is a [`stream`](glossary.html#stream) object, then these data will also be copied over, and `target` will be converted to a stream object.
...
extract_image(xref)[¶](#Document.extract_image)
...
**xref** (*int*) – [`xref`](glossary.html#xref) of an image object. If this is not in `range(1, doc.xref_length())`, or the object is no image or other errors occur, `None` is returned and no exception is raised.
...
- 
*smask* (*int*) [`xref`](glossary.html#xref) number of a stencil (/SMask) image or zero
...
- 
*xres* (*int*) resolution in x direction. Please also see [`resolution`](glossary.html#resolution).

- 
*yres* (*int*) resolution in y direction. Please also see [`resolution`](glossary.html#resolution).
...
- 
xref 1186 is a JPEG – [`Document.extract_image()`](#Document.extract_image) is **many times faster** and produces a **much smaller** output (2.48 MB vs. 0.35 MB):
...
extract_font(xref, info_only=False, named=None)[¶](#Document.extract_font)

- 
Changed in v1.19.4: return a dictionary if `named == True`.

PDF Only: Return an embedded font file’s data and appropriate file extension. This can be used to store the font as an external file. The method does not throw exceptions (other than via checking for PDF and valid [`xref`](glossary.html#xref)).
...
xref_xml_metadata()[¶](#Document.xref_xml_metadata)

- 
New in v1.16.8

PDF only: Return the [`xref`](glossary.html#xref) of the document’s XML metadata.

has_links()[¶](#Document.has_links)

has_annots()[¶](#Document.has_annots)
...
subset_fonts(verbose=False, fallback=False)[¶](#Document.subset_fonts)
...
Creating font subsets leaves behind a large number of large, now unused PDF objects (“ghosts”). Therefore, make sure to compress and garbage-collect when saving the file. We recommend to use [`Document.ez_save()`](#Document.ez_save).
...
journal_enable()[¶](#Document.journal_enable)
...
journal_start_op(name)[¶](#Document.journal_start_op)
...
journal_stop_op()[¶](#Document.journal_stop_op)
...
journal_position()[¶](#Document.journal_position)
...
journal_op_name(step)[¶](#Document.journal_op_name)
...
journal_can_do()[¶](#Document.journal_can_do)
...
journal_undo()[¶](#Document.journal_undo)
...
journal_redo()[¶](#Document.journal_redo)
...
journal_save(filename)[¶](#Document.journal_save)
...
journal_load(filename)[¶](#Document.journal_load)
...
save_snapshot()[¶](#Document.save_snapshot)
...
is_form_pdf[¶](#Document.is_form_pdf)
...
is_reflowable[¶](#Document.is_reflowable)

`True` if document has a variable page layout (like e-books or HTML). In this case you can set the desired page dimensions during document creation (open) or via method [`layout()`](#Document.layout).
...
`True` if PDF has been repaired during open (because of major structure issues). Always `False` for non-PDF documents. If true, more details have been stored in `TOOLS.mupdf_warnings()`, and [`Document.can_save_incrementally()`](#Document.can_save_incrementally) will return `False`.

Type:

bool

is_fast_webaccess[¶](#Document.is_fast_webaccess)
...
This indicator initially equals [`Document.needs_pass`](#Document.needs_pass). After successful authentication, it is set to `False` to reflect the situation.
...
Contains the permissions to access the document. This is an integer containing bool values in respective bit positions. For example, if *doc.permissions & pymupdf.PDF_PERM_MODIFY > 0*, you may change the document. See [Document Permissions](vars.html#permissioncodes) for details.
...
- 
*encryption* either contains `None` (no encryption), or a string naming an encryption method (e.g. *‘Standard V4 R4 128-bit RC4’*). Note that an encryption method may be specified **even if** *needs_pass=False*. In such cases not all permissions will probably have been granted. Check [`Document.permissions`](#Document.permissions) for details.
...
last_location[¶](#Document.last_location)
...
## `set_metadata()` Example[¶](#set-metadata-example)
...
## `set_toc()` Demonstration[¶](#set-toc-demonstration)
...
## `insert_pdf()` Examples[¶](#insert-pdf-examples)
...
Content streams describe what (e.g. text or images) appears where and how on a page. PDF uses a specialized mini language similar to PostScript to do this (pp. 643 in [Adobe PDF References](app3.html#adobemanual)), which gets interpreted when a page is loaded.

[[2](#id4)]

However, you **can** use [`Document.get_toc()`](#Document.get_toc) and [`Page.get_links()`](page.html#Page.get_links) (which are available for all document types) and copy this information over to the output PDF. See demo [convert.py](https://github.com/pymupdf/PyMuPDF-Utilities/blob/master/examples/convert-document/convert.py).

[[3](#id3)]

For applicable (EPUB) document types, loading a page via its absolute number may result in layouting a large part of the document, before the page can be accessed. To avoid this performance impact, prefer chapter-based access. Use convenience methods and attributes [`Document.next_location()`](#Document.next_location), [`Document.prev_location()`](#Document.prev_location) and [`Document.last_location`](#Document.last_location) for maintaining a high level of coding efficiency.
...
Examples for “Form XObjects” are created by [`Page.show_pdf_page()`](page.html#Page.show_pdf_page).

[[6](#id9)]

For a `False` the **complete document** must be scanned. Both methods **do not load pages,** but only scan object definitions. This makes them at least 10 times faster than application-level loops (where total response time roughly equals the time for loading all pages). For the [Adobe PDF References](app3.html#adobemanual) (756 pages) and the Pandas documentation (over 3070 pages) – both have no annotations – the method needs about 11 ms for the answer `False`. So response times will probably become significant only well beyond this order of magnitude.
...
- [`Document.__init__()`](#Document.__init__)
...
- [`Document.xref_stream()`](#Document.xref_stream)

- [`Document.xref_stream_raw()`](#Document.xref_stream_raw)

- [`Document.update_object()`](#Document.update_object)

- [`Document.update_stream()`](#Document.update_stream)
...
- [`Document.save_snapshot()`](#Document.save_snapshot)
