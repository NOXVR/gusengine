...
- 
from a document page (method [`Page.get_pixmap()`](page.html#Page.get_pixmap))

- 
empty, based on [Colorspace](colorspace.html#colorspace) and [IRect](irect.html#irect) information
...
[`Pixmap.clear_with()`](#Pixmap.clear_with)

clear parts of the pixmap

[`Pixmap.color_count()`](#Pixmap.color_count)

determine used colors

[`Pixmap.color_topusage()`](#Pixmap.color_topusage)
...
[`Pixmap.gamma_with()`](#Pixmap.gamma_with)

apply a gamma factor to the pixmap

[`Pixmap.invert_irect()`](#Pixmap.invert_irect)

invert the pixels of a given area

[`Pixmap.pdfocr_save()`](#Pixmap.pdfocr_save)

save the pixmap as an OCRed 1-page PDF

[`Pixmap.pdfocr_tobytes()`](#Pixmap.pdfocr_tobytes)

save the pixmap as an OCRed 1-page PDF

[`Pixmap.pil_image()`](#Pixmap.pil_image)

create a Pillow Image

[`Pixmap.pil_save()`](#Pixmap.pil_save)

save as a Pillow Image

[`Pixmap.pil_tobytes()`](#Pixmap.pil_tobytes)

write to `bytes` as a Pillow Image

[`Pixmap.pixel()`](#Pixmap.pixel)

return the value of a pixel

[`Pixmap.save()`](#Pixmap.save)

save the pixmap in a variety of formats

[`Pixmap.set_alpha()`](#Pixmap.set_alpha)

set alpha values

[`Pixmap.set_dpi()`](#Pixmap.set_dpi)

set the image resolution

[`Pixmap.set_origin()`](#Pixmap.set_origin)

set pixmap x,y values

[`Pixmap.set_pixel()`](#Pixmap.set_pixel)

set color and alpha of a pixel

[`Pixmap.set_rect()`](#Pixmap.set_rect)

set color and alpha of all pixels in a rectangle

[`Pixmap.shrink()`](#Pixmap.shrink)

reduce size keeping proportions

[`Pixmap.tint_with()`](#Pixmap.tint_with)

tint the pixmap

[`Pixmap.tobytes()`](#Pixmap.tobytes)

return a memory area in a variety of formats

[`Pixmap.warp()`](#Pixmap.warp)

return a pixmap made from a quad inside

[`Pixmap.alpha`](#Pixmap.alpha)

transparency indicator

[`Pixmap.colorspace`](#Pixmap.colorspace)

pixmap’s [Colorspace](colorspace.html#colorspace)

[`Pixmap.digest`](#Pixmap.digest)
...
[`Pixmap.interpolate`](#Pixmap.interpolate)

interpolation method indicator

[`Pixmap.is_monochrome`](#Pixmap.is_monochrome)

check if only black and white occur

[`Pixmap.is_unicolor`](#Pixmap.is_unicolor)

check if only one color occurs

[`Pixmap.irect`](#Pixmap.irect)

[IRect](irect.html#irect) of the pixmap

[`Pixmap.n`](#Pixmap.n)

bytes per pixel

[`Pixmap.samples_mv`](#Pixmap.samples_mv)

`memoryview` of pixel area

[`Pixmap.samples_ptr`](#Pixmap.samples_ptr)

Python pointer to pixel area

[`Pixmap.samples`](#Pixmap.samples)
...
__init__(self, colorspace, irect, alpha=False)[¶](#Pixmap.__init__)

**New empty pixmap:** Create an empty pixmap of size and origin given by the rectangle. So, *irect.top_left* designates the top left corner of the pixmap, and its width and height are *irect.width* resp. *irect.height*. Note that the image area is **not initialized** and will contain crap data – use eg. [`clear_with()`](#Pixmap.clear_with) or [`set_rect()`](#Pixmap.set_rect) to be sure.
...
__init__(self, colorspace, source)[¶](#id0)
...
- 
**colorspace** ([Colorspace](colorspace.html#colorspace)) – desired **target** colorspace. This **may also be** `None`. In this case, a “masking” pixmap is created: its [`Pixmap.samples`](#Pixmap.samples) will consist of the source’s alpha bytes only.

- 
**source** ([Pixmap](#pixmap).) – the source pixmap.

__init__(self, source, mask)[¶](#id2)
...
__init__(self, source, width, height[, clip])[¶](#id3)
...
__init__(self, source, alpha=1)[¶](#id4)
...
__init__(self, colorspace, width, height, samples, alpha)[¶](#id7)
...
- 
**xref** (*int*) – the [`xref`](glossary.html#xref) of an image object. For example, you can make a list of images used on a particular page with [`Document.get_page_images()`](document.html#Document.get_page_images), which also shows the [`xref`](glossary.html#xref) numbers of each image.

clear_with([value[, irect]])[¶](#Pixmap.clear_with)
...
tint_with(black, white)[¶](#Pixmap.tint_with)

Colorize a pixmap by replacing black and / or white with colors given as **sRGB integer** values. Only colorspaces [`CS_GRAY`](vars.html#CS_GRAY) and [`CS_RGB`](vars.html#CS_RGB) are supported, others are ignored with a warning.

If the colorspace is [`CS_GRAY`](vars.html#CS_GRAY), the average *(red + green + blue)/3* will be taken. The pixmap will be changed in place.
...
gamma_with(gamma)[¶](#Pixmap.gamma_with)
...
set_pixel(x, y, color)[¶](#Pixmap.set_pixel)
...
- 
**color** (*sequence*) – the desired pixel value given as a sequence of integers in `range(256)`. The length of the sequence must equal [`Pixmap.n`](#Pixmap.n), which includes any alpha byte.

set_rect(irect, color)[¶](#Pixmap.set_rect)
...
- 
**irect** (*irect_like*) – the rectangle to be filled with the value. The actual area is the intersection of this parameter and [`Pixmap.irect`](#Pixmap.irect). For an empty intersection (or an invalid parameter), no change will happen.

- 
**color** (*sequence*) – the desired value, given as a sequence of integers in `range(256)`. The length of the sequence must equal [`Pixmap.n`](#Pixmap.n), which includes any alpha byte.
...
`False` if the rectangle was invalid or had an empty intersection with [`Pixmap.irect`](#Pixmap.irect), else `True`.

Note

- 
This method is equivalent to [`Pixmap.set_pixel()`](#Pixmap.set_pixel) executed for each pixel in the rectangle, but is obviously **very much faster** if many pixels are involved.

- 
This method can be used similar to [`Pixmap.clear_with()`](#Pixmap.clear_with) to initialize a pixmap with a certain color like this: *pix.set_rect(pix.irect, (255, 255, 0))* (RGB example, colors the complete pixmap with yellow).

set_origin(x, y)[¶](#Pixmap.set_origin)
...
set_dpi(xres, yres)[¶](#Pixmap.set_dpi)
...
set_alpha(alphavalues, premultiply=1, opaque=None)[¶](#Pixmap.set_alpha)
...
- 
**opaque** (*list**,**tuple*) – ignore the alpha value and set this color to fully transparent. A sequence of integers in `range(256)` with a length of [`Pixmap.n`](#Pixmap.n). Default is `None`. For example, a typical choice for RGB would be `opaque=(255, 255, 255)` (white).

invert_irect([irect])[¶](#Pixmap.invert_irect)
...
copy(source, irect)[¶](#Pixmap.copy)

Copy the *irect* part of the *source* pixmap into the corresponding area of this one. The two pixmaps may have different dimensions and can each have [`CS_GRAY`](vars.html#CS_GRAY) or [`CS_RGB`](vars.html#CS_RGB) colorspaces, but they currently **must** have the same alpha property [[2]](#f2). The copy mechanism automatically adjusts discrepancies between source and target like so:

If copying from [`CS_GRAY`](vars.html#CS_GRAY) to [`CS_RGB`](vars.html#CS_RGB), the source gray-shade value will be put into each of the three rgb component bytes. If the other way round, *(r + g + b) / 3* will be taken as the gray-shade value of the target.

Between *irect* and the target pixmap’s rectangle, an “intersection” is calculated at first. This takes into account the rectangle coordinates and the current attribute values [`Pixmap.x`](#Pixmap.x) and [`Pixmap.y`](#Pixmap.y) (which you are free to modify for this purpose via [`Pixmap.set_origin()`](#Pixmap.set_origin)). Then the corresponding data of this intersection are copied. If the intersection is empty, nothing will happen.
...
save(filename, output=None, jpg_quality=95)[¶](#Pixmap.save)
...
tobytes(output='png', jpg_quality=95)[¶](#Pixmap.tobytes)

- 
New in version 1.14.5: Return the pixmap as a *bytes* memory object of the specified format – similar to [`save()`](#Pixmap.save).
...
pdfocr_save(filename, compress=True, language='eng', tessdata=None)[¶](#Pixmap.pdfocr_save)
...
**Will fail** if Tesseract is not installed or if the environment variable “TESSDATA_PREFIX” is not set to the `tessdata` folder name and not provided as parameter.

pdfocr_tobytes(compress=True, language='eng', tessdata=None)[¶](#Pixmap.pdfocr_tobytes)
...
Perform text recognition using Tesseract and convert the image to a 1-page PDF with an OCR text layer. Internally invokes [`Pixmap.pdfocr_save()`](#Pixmap.pdfocr_save).
...
pil_save(*args, unmultiply=False, **kwargs)[¶](#Pixmap.pil_save)
...
pil_tobytes(*args, unmultiply=False, **kwargs)[¶](#Pixmap.pil_tobytes)
...
warp(quad, width, height)[¶](#Pixmap.warp)
...
- 
**quad** (*quad_like*) – a convex quad with coordinates inside [`Pixmap.irect`](#Pixmap.irect) (including the border points).
...
color_count(colors=False, clip=None)[¶](#Pixmap.color_count)
...
- 
**clip** (*rect_like*) – a rectangle inside [`Pixmap.irect`](#Pixmap.irect). If provided, only those pixels are considered. This allows inspecting sub-rectangles of a given pixmap directly – instead of building sub-pixmaps.
...
either the number of colors, or a dictionary with the items `pixel: count`. The pixel key is a `bytes` object of length [`Pixmap.n`](#Pixmap.n).
...
color_topusage(clip=None)[¶](#Pixmap.color_topusage)
...
Contains the length of one row of image data in [`Pixmap.samples`](#Pixmap.samples). This is primarily used for calculation purposes. The following expressions are true:
...
is_monochrome[¶](#Pixmap.is_monochrome)
...
- 
The underlying data is typically a **large** memory area, from which a `bytes` copy is made for this attribute … each time you access it: for example an RGB-rendered letter page has a samples size of almost 1.4 MB. So consider assigning a new variable to it or use the `memoryview` version [`Pixmap.samples_mv`](#Pixmap.samples_mv) (new in v1.18.17).
...
samples_mv[¶](#Pixmap.samples_mv)

- 
New in v1.18.17

Like [`Pixmap.samples`](#Pixmap.samples), but in Python `memoryview` format. It is built pointing to the memory in the pixmap – not from a copy of it. So its creation speed is independent from the pixmap size, and any changes to pixels will be available immediately.
...
samples_ptr[¶](#Pixmap.samples_ptr)
...
X-coordinate of top-left corner in pixels. Cannot directly be changed – use [`Pixmap.set_origin()`](#Pixmap.set_origin).
...
Y-coordinate of top-left corner in pixels. Cannot directly be changed – use [`Pixmap.set_origin()`](#Pixmap.set_origin).
...
Horizontal resolution in dpi (dots per inch). Please also see [`resolution`](glossary.html#resolution). Cannot directly be changed – use [`Pixmap.set_dpi()`](#Pixmap.set_dpi).
...
Vertical resolution in dpi (dots per inch). Please also see [`resolution`](glossary.html#resolution). Cannot directly be changed – use [`Pixmap.set_dpi()`](#Pixmap.set_dpi).

Type:

int

interpolate[¶](#Pixmap.interpolate)
...
## Supported Input Image Formats[¶](#supported-input-image-formats)
...
## Supported Output Image Formats[¶](#supported-output-image-formats)

A number of image **output** formats are supported. You have the option to either write an image directly to a file ([`Pixmap.save()`](#Pixmap.save)), or to generate a bytes object ([`Pixmap.tobytes()`](#Pixmap.tobytes)). Both methods accept a string identifying the desired format (**Format** column below). Please note that not all combinations of pixmap colorspace, transparency support (alpha) and image format are possible.
...
If you need a **vector image** from the SVG, you must first convert it to a PDF. Try [`Document.convert_to_pdf()`](document.html#Document.convert_to_pdf). If this is not good enough, look for other SVG-to-PDF conversion tools like the Python packages [svglib](https://pypi.org/project/svglib), [CairoSVG](https://pypi.org/project/cairosvg), [Uniconvertor](https://sk1project.net/uc2/download/) or the Java solution [Apache Batik](https://github.com/apache/batik). Have a look at our Wiki for more examples.
...
- [`Pixmap.__init__()`](#Pixmap.__init__)
