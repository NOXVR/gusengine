# Source: https://pymupdf.readthedocs.io/en/latest/recipes-common-issues-and-their-solutions.html
...
- 
        Common Issues and their Solutions - PyMuPDF documentation
...
## How To Dynamically Clean Up Corrupt PDFs[¶](#how-to-dynamically-clean-up-corrupt-pdfs)

This shows a potential use of PyMuPDF with another Python PDF library (the excellent pure Python package [pdfrw](https://pypi.python.org/pypi/pdfrw) is used here as an example).
...
With the command line utility *pdftk* ([available](https://www.pdflabs.com/tools/pdftk-the-pdf-toolkit/) for Windows only, but reported to also run under [Wine](https://www.winehq.org/)) a similar result can be achieved, see [here](http://www.overthere.co.uk/2013/07/22/improving-pypdf2-with-pdftk/). However, you must invoke it as a separate process via *subprocess.Popen*, using stdin and stdout as communication vehicles.

## How to Convert Any Document to PDF[¶](#how-to-convert-any-document-to-pdf)

Here is a script that converts any PyMuPDF [supported document](how-to-open-a-file.html#supported-file-types) to a PDF. These include XPS, EPUB, FB2, CBZ and image formats, including multi-page TIFF images.
...
## Changing Annotations: Unexpected Behaviour[¶](#changing-annotations-unexpected-behaviour)
...
- 
When using PyMuPDF to change an “alien” annotation, try to **avoid** [`Annot.update()`](annot.html#Annot.update). The following methods **can be used without it,** so that the original appearance should be maintained:

- 
[`Annot.set_rect()`](annot.html#Annot.set_rect) (location changes)

- 
[`Annot.set_flags()`](annot.html#Annot.set_flags) (annotation behaviour)

- 
[`Annot.set_info()`](annot.html#Annot.set_info) (meta information, except changes to *content*)

- 
[`Annot.set_popup()`](annot.html#Annot.set_popup) (create popup or change its rect)

- 
[`Annot.set_oc()`](annot.html#Annot.set_oc) (add / remove reference to optional content information)

- 
[`Annot.set_open()`](annot.html#Annot.set_open)

- 
[`Annot.update_file()`](annot.html#Annot.update_file) (file attachment changes)

## Missing or Unreadable Extracted Text[¶](#missing-or-unreadable-extracted-text)
...
### Problem: no text is extracted[¶](#problem-no-text-is-extracted)
...
Use an OCR software like [OCRmyPDF](https://pypi.org/project/ocrmypdf/) to insert a hidden text layer underneath the visible page. The resulting PDF should behave as expected.

### Problem: unreadable text[¶](#problem-unreadable-text)
...
- 
The single characters are readable as such (no “<?>” symbols), but the sequence in which the text is **coded in the file** deviates from the reading order. The motivation behind may be technical or protection of data against unwanted copies.
...
- 
Use layout preserving text extraction: `python -m fitz gettext file.pdf`.

- 
If other text extraction tools also don’t work, then the only solution again is OCRing the page.
