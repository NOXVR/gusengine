...
- 
        Appendix 1: Details on Text Extraction - PyMuPDF documentation
...
# Appendix 1: Details on Text Extraction[¶](#appendix-1-details-on-text-extraction)
...
## General structure of a TextPage[¶](#general-structure-of-a-textpage)

[TextPage](textpage.html#textpage) is one of (Py-) MuPDF’s classes. It is normally created (and destroyed again) behind the curtain, when [Page](page.html#page) text extraction methods are used, but it is also available directly and can be used as a persistent object. Other than its name suggests, images may optionally also be part of a text page:
...
Function [`TextPage.extractText()`](textpage.html#TextPage.extractText) (or *Page.get_text(“text”)*) extracts a page’s plain **text in original order** as specified by the creator of the document.
...
Function [`TextPage.extractBLOCKS()`](textpage.html#TextPage.extractBLOCKS) (or *Page.get_text(“blocks”)*) extracts a page’s text blocks as a list of items like:
...
Function [`TextPage.extractWORDS()`](textpage.html#TextPage.extractWORDS) (or *Page.get_text(“words”)*) extracts a page’s text **words** as a list of items like:
...
## Controlling Quality of HTML Output[¶](#controlling-quality-of-html-output)
...
To address the font issue, you can use a simple utility script to scan through the HTML file and replace font references. Here is a little example that replaces all fonts with one of the [PDF Base 14 Fonts](app3.html#base-14-fonts): serifed fonts will become “Times”, non-serifed “Helvetica” and monospaced will become “Courier”. Their respective variations for “bold”, “italic”, etc. are hopefully done correctly by your browser:
...
For a visualization of the dictionary structure have a look at [Structure of Dictionary Outputs](textpage.html#textpagedict).
...
The [`TextPage.extractXML()`](textpage.html#TextPage.extractXML) (or *Page.get_text(“xml”)*) version extracts text (no images) with the detail level of RAWDICT:
...
We have successfully tested [lxml](https://pypi.org/project/lxml/) to interpret this output.

## XHTML[¶](#xhtml)

[`TextPage.extractXHTML()`](textpage.html#TextPage.extractXHTML) (or *Page.get_text(“xhtml”)*) is a variation of TEXT but in HTML format, containing the bare text and images (“semantic” output):

<div id="page0">
<p>Some text on first page.</p>
</div>

## Text Extraction Flags Defaults[¶](#text-extraction-flags-defaults)

- 
New in version 1.16.2: Method [`Page.get_text()`](page.html#Page.get_text) supports a keyword parameter *flags* *(int)* to control the amount and the quality of extracted data. The following table shows the defaults settings (flags parameter omitted or None) for each extraction variant. If you specify flags with a value other than `None`, be aware that you must set **all desired** options. A description of the respective bit settings can be found in [Font Properties](vars.html#textpreserve).

- 
New in v1.19.6: The default combinations in the following table are now available as Python constants: [`TEXTFLAGS_TEXT`](vars.html#TEXTFLAGS_TEXT), [`TEXTFLAGS_WORDS`](vars.html#TEXTFLAGS_WORDS), [`TEXTFLAGS_BLOCKS`](vars.html#TEXTFLAGS_BLOCKS), [`TEXTFLAGS_DICT`](vars.html#TEXTFLAGS_DICT), [`TEXTFLAGS_RAWDICT`](vars.html#TEXTFLAGS_RAWDICT), [`TEXTFLAGS_HTML`](vars.html#TEXTFLAGS_HTML), [`TEXTFLAGS_XHTML`](vars.html#TEXTFLAGS_XHTML), [`TEXTFLAGS_XML`](vars.html#TEXTFLAGS_XML), and [`TEXTFLAGS_SEARCH`](vars.html#TEXTFLAGS_SEARCH). You can now easily modify a default flag, e.g.
...
To show the effect of [`TEXT_INHIBIT_SPACES`](vars.html#TEXT_INHIBIT_SPACES) have a look at this example:
...
To begin with, all methods are **very fast** in relation to other products out there in the market. In terms of processing speed, we are not aware of a faster (free) tool. Even the most detailed method, RAWDICT, processes all 1’310 pages of the [Adobe PDF References](app3.html#adobemanual) in less than 5 seconds (simple text needs less than 2 seconds here).
