# Source: https://pymupdf.readthedocs.io/en/latest/functions.html
...
- 
        Functions - PyMuPDF documentation
...
[`Page.is_wrapped`](#Page.is_wrapped)

check whether contents wrapping is present

[`adobe_glyph_names()`](#adobe_glyph_names)

list of glyph names defined in **Adobe Glyph List**

[`adobe_glyph_unicodes()`](#adobe_glyph_unicodes)

list of unicodes defined in **Adobe Glyph List**

[`Annot.clean_contents()`](#Annot.clean_contents)

PDF only: clean the annot’s [`contents`](glossary.html#contents) object
...
[`ConversionHeader()`](#ConversionHeader)

return header string for *get_text* methods

[`ConversionTrailer()`](#ConversionTrailer)

return trailer string for *get_text* methods

[`Document.del_xml_metadata()`](#Document.del_xml_metadata)

PDF only: remove XML metadata

[`Document.get_char_widths()`](#Document.get_char_widths)

PDF only: return a list of glyph widths of a font

[`Document.get_new_xref()`](#Document.get_new_xref)

PDF only: create and return a new [`xref`](glossary.html#xref) entry

[`Document.is_stream()`](#Document.is_stream)

PDF only: check whether an [`xref`](glossary.html#xref) is a stream object

[`Document.xml_metadata_xref()`](#Document.xml_metadata_xref)

PDF only: return XML metadata [`xref`](glossary.html#xref) number

[`Document.xref_length()`](#Document.xref_length)

PDF only: return length of [`xref`](glossary.html#xref) table

[`EMPTY_IRECT()`](#EMPTY_IRECT)

return the (standard) empty / invalid rectangle

[`EMPTY_QUAD()`](#EMPTY_QUAD)
...
[`get_pdf_now()`](#get_pdf_now)

return the current timestamp in PDF format

[`get_pdf_str()`](#get_pdf_str)

return PDF-compatible string

[`get_text_length()`](#get_text_length)

return string length for a given font & [`fontsize`](glossary.html#fontsize)
...
[`Page.clean_contents()`](#Page.clean_contents)

PDF only: clean the page’s [`contents`](glossary.html#contents) objects

[`Page.get_bboxlog()`](#Page.get_bboxlog)

list of rectangles that envelop text, drawing or image objects

[`Page.get_contents()`](#Page.get_contents)

PDF only: return a list of content [`xref`](glossary.html#xref) numbers

[`Page.get_displaylist()`](#Page.get_displaylist)

create the page’s display list

[`Page.get_text_blocks()`](#Page.get_text_blocks)

extract text blocks as a Python list

[`Page.get_text_words()`](#Page.get_text_words)

extract text words as a Python list

[`Page.get_texttrace()`](#Page.get_texttrace)

low-level text information

[`Page.read_contents()`](#Page.read_contents)
...
[`Page.set_contents()`](#Page.set_contents)

PDF only: set page’s [`contents`](glossary.html#contents) to some [`xref`](glossary.html#xref)

[`Page.wrap_contents()`](#Page.wrap_contents)

wrap contents with stacking commands

[`css_for_pymupdf_font()`](#css_for_pymupdf_font)
...
[`planish_line()`](#planish_line)

matrix to map a line to the x-axis

[`recover_char_quad()`](#recover_char_quad)

compute the quad of a char (“rawdict”)

[`recover_line_quad()`](#recover_line_quad)
...
[`recover_span_quad()`](#recover_span_quad)
...
[`get_tessdata()`](#get_tessdata)

locates the language support of the Tesseract-OCR installation

[`colors_pdf_dict()`](#colors_pdf_dict)

return dict of color names.

[`colors_wx_list()`](#colors_wx_list)

return list of color names.

[`fitz_fontdescriptors`](#fitz_fontdescriptors)

dictionary of available supplement fonts

[`PYMUPDF_MESSAGE`](#PYMUPDF_MESSAGE)
...
paper_rect(s)[¶](#paper_rect)

Convenience function to return a [Rect](rect.html#rect) for a known paper format.

Parameters:

**s** (*str*) – any format name supported by [`paper_size()`](#paper_size).
...
set_messages(*, text=None, fd=None, stream=None, path=None, path_append=None, pylogging=None, pylogging_logger=None, pylogging_level=None, pylogging_name=None)[¶](#set_messages)

Sets destination of PyMuPDF messages to a file descriptor,
a file, an existing stream or [Python’s logging system](https://docs.python.org/3/library/logging.html).
...
- 
**text** (*str*) – A text specification of destination; for details see description of
environmental variable [`PYMUPDF_MESSAGE`](#PYMUPDF_MESSAGE).

- 
**fd** (*int*) – Write to file descriptor.

- 
**stream** – Write to existing stream, which must have methods `.write(text)` and
`.flush()`.

- 
**path** (*str*) – Write to a file.

- 
**path_append** (*str*) – Append to a file.

- 
**pylogging** – Write to Python’s `logging` system.

- 
**pylogging_logger** (*logging.Logger*) – Write to Python’s `logging` system using specified Logger.

- 
**pylogging_level** (*int*) – Write to Python’s `logging` system using specified level.

- 
**pylogging_name** (*str*) – Write to Python’s `logging` system using specified logger name.
Only used if `pylogging_logger` is `None`. Default is `pymupdf`.

If any `pylogging*` arg is not `None`, we write to [Python’s logging system](https://docs.python.org/3/library/logging.html).

sRGB_to_pdf(srgb)[¶](#sRGB_to_pdf)

*New in v1.17.4*

Convenience function returning a PDF color triple (red, green, blue) for a given sRGB color integer as it occurs in [`Page.get_text()`](page.html#Page.get_text) dictionaries “dict” and “rawdict”.
...
glyph_name_to_unicode(name)[¶](#glyph_name_to_unicode)
...
**name** (*str*) – the name of some glyph. The function is based on the [Adobe Glyph List](https://github.com/adobe-type-tools/agl-aglfn/blob/master/glyphlist.txt).
...
A similar functionality is provided by package [fontTools](https://pypi.org/project/fonttools/) in its *agl* sub-package.

unicode_to_glyph_name(ch)[¶](#unicode_to_glyph_name)
...
**ch** (*int*) – 
the unicode given by e.g. `ord("ß")`. The function is based on the [Adobe Glyph List](https://github.com/adobe-type-tools/agl-aglfn/blob/master/glyphlist.txt).
...
css_for_pymupdf_font(fontcode, *, CSS=None, archive=None, name=None)[¶](#css_for_pymupdf_font)
...
To use the font in the Python API for [Story](story-class.html#story), execute `.set_font(fontcode)` (or “name” if given). The correct font weight or style will automatically be selected as required.
...
- 
**fontcode** (*str*) – one of the font codes present in package [pymupdf-fonts](https://pypi.org/project/pymupdf-fonts/) (usually) representing the regular version of the font family.

- 
**CSS** (*str*) – any already existing CSS source, or `None`. The function will append its new definitions to this. This is the string that **must be used** as `user_css` when creating the [Story](story-class.html#story).

- 
**archive** – [Archive](archive-class.html#archive), **mandatory**. All font binaries (i.e. up to four) found for “fontcode” will be added to the archive. This is the archive that **must be used** as [Archive](archive-class.html#archive) when creating the [Story](story-class.html#story).
...
make_table(rect, cols=1, rows=1)[¶](#make_table)

*New in v1.17.4*

Convenience function to split a rectangle into sub-rectangles of equal size. Returns a list of [`rows`](page.html#Table.rows) lists, each containing `cols` [Rect](rect.html#rect) items. Each sub-rectangle can then be addressed by its row and column index.
...
a list of [Rect](rect.html#rect) objects of equal size, whose union equals *rect*. Here is the layout of a 3x4 table created by `cell = pymupdf.make_table(rect, cols=4, rows=3)`:

planish_line(p1, p2)[¶](#planish_line)
...
A dictionary of pre-defines paper formats. Used as basis for [`paper_size()`](#paper_size).
...
A dictionary of usable fonts from repository [pymupdf-fonts](https://pypi.org/project/pymupdf-fonts/). Items are keyed by their reserved fontname and provide information like this:
...
The dictionary keys can be used to define a [Font](font.html#font) via e.g. `font = pymupdf.Font("fimo")` – just like you can do it with the builtin fonts “Helvetica” and friends.
...
- 
`name=` sets the logger name (default is `pymupdf`).
...
Also see [`set_messages()`](#set_messages).
...
get_text_length(text, fontname='helv', fontsize=11, encoding=TEXT_ENCODING_LATIN)[¶](#get_text_length)

- 
New in version 1.14.7

Calculate the length of text on output with a given **builtin** font, [`fontsize`](glossary.html#fontsize) and encoding.
...
- 
**fontname** (*str*) – the font name. Must be one of either the [PDF Base 14 Fonts](app3.html#base-14-fonts) or the CJK fonts, identified by their “reserved” fontnames (see table in [`Page.insert_font()`](page.html#Page.insert_font)).

- 
**fontsize** (*float*) – the [`fontsize`](glossary.html#fontsize).
...
the length in points the string will have (e.g. when used in [`Page.insert_text()`](page.html#Page.insert_text)).
...
The [Font](font.html#font) class offers a similar method, [`Font.text_length()`](font.html#Font.text_length), which supports Base-14 fonts and any font with a character map (CMap, Type 0 fonts).

Warning

If you use this function to determine the required rectangle width for the ([Page](page.html#page) or [Shape](shape.html#shape)) *insert_textbox* methods, be aware that they calculate on a **by-character level**. Because of rounding effects, this will mostly lead to a slightly larger number: *sum([pymupdf.get_text_length(c) for c in text]) > pymupdf.get_text_length(text)*. So either (1) do the same, or (2) use something like *pymupdf.get_text_length(text + “’”)* for your calculation.

get_pdf_str(text)[¶](#get_pdf_str)
...
image_profile(stream)[¶](#image_profile)
...
There is the following relation to **Exif** information encoded in `orientation`, and correspondingly in the [`transform`](point.html#Point.transform) matrix-like (quoted from MuPDF documentation, *ccw* = counter-clockwise):
...
- 
For some “exotic” images (FAX encodings, RAW formats and the like), this method will not work. You can however still work with such images in PyMuPDF, e.g. by using [`Document.extract_image()`](document.html#Document.extract_image) or create pixmaps via `Pixmap(doc, xref)`. These methods will automatically convert exotic images to the PNG format before returning results.

- 
You can also get the properties of images embedded in a PDF, via their [`xref`](glossary.html#xref). In this case make sure to extract the raw stream: `pymupdf.image_profile(doc.xref_stream_raw(xref))`.

- 
Images as returned by the image blocks of [`Page.get_text()`](page.html#Page.get_text) using “dict” or “rawdict” options are also supported.

ConversionHeader("text", filename="UNKNOWN")[¶](#ConversionHeader)
...
ConversionTrailer(output)[¶](#ConversionTrailer)

Return the trailer string required to make a valid document out of page text outputs. See [`Page.get_text()`](page.html#Page.get_text) for an example.
...
Return the XML-based metadata [`xref`](glossary.html#xref) of the PDF if present – also refer to [`Document.del_xml_metadata()`](#Document.del_xml_metadata). You can use it to retrieve the content via [`Document.xref_stream()`](document.html#Document.xref_stream) and then work with it using some XML software.
...
[`xref`](glossary.html#xref) of PDF file level XML metadata – or 0 if none exists.

Page.run(dev, transform)[¶](#Page.run)
...
- 
**dev** ([Device](device.html#device)) – Device, obtained from one of the [Device](device.html#device) constructors.

- 
**transform** ([Matrix](matrix.html#matrix)) – Transformation to apply to the page. Set it to [Identity](identity.html#identity) if no transformation is desired.

Page.get_bboxlog(layers=False)[¶](#Page.get_bboxlog)
...
So this list can be used to detect such situations. An item’s index in this list equals the value of a `"seqno"` in dictionaries as returned by [`Page.get_drawings()`](page.html#Page.get_drawings) and [`Page.get_texttrace()`](#Page.get_texttrace).
...
- 
Information above tagged with “(1)” has the same meaning and value as explained in [TextPage](textpage.html#textpage).

- 
Please note that the font `flags` value will never contain a *superscript* flag bit: the detection of superscripts is done within MuPDF [TextPage](textpage.html#textpage) code – it is not a property of any font.
...
- 
Line width in this context is important only for processing `span["type"] != 0`: it determines the thickness of the character’s border line. This value may not be provided at all with the text data. In this case, a value of 5% of the [`fontsize`](glossary.html#fontsize) (`span["size"] * 0,05`) is generated. Often, an “artificial” bold text in PDF is created by `2 Tr`. There is no equivalent span type for this case. Instead, respective text is represented by two consecutive spans – which are identical in every aspect, except for their types, which are 0, resp 1. It is your responsibility to handle this type of situation - in [`Page.get_text()`](page.html#Page.get_text), MuPDF is doing this for you.

- 
For data compactness, the character’s unicode is provided here. Use built-in function `chr()` for the character itself.

- 
The alpha / opacity value of the span’s text, `0 <= opacity <= 1`, 0 is invisible text, 1 (100%) is intransparent. Depending on `span["type"]`, interpret this value as *fill* opacity or, resp. *stroke* opacity.

- 
*(Changed in v1.19.0)* This value is equal or close to `char["bbox"]` of “rawdict”. In particular, the bbox **height** value is always computed as if **“small glyph heights”** had been requested.

- 
*(New in v1.19.0)* This is the union of all character bboxes.
...
- 
*(New in v1.22.0)* The name of the Optional Content Group (OCG) if applicable or `None`.
...
- 
**Changed in v1.19.3:** Similar to other text extraction methods, the character and span bboxes envelop the character quads. To recover the quads, follow the same methods [`recover_quad()`](#recover_quad), [`recover_char_quad()`](#recover_char_quad) or [`recover_span_quad()`](#recover_span_quad) as explained in [Structure of Dictionary Outputs](textpage.html#textpagedict). Use either `None` or `span["dir"]` for the writing direction.
...
We discourage using [`Page.clean_contents()`](#Page.clean_contents) to achieve this.

Page.is_wrapped[¶](#Page.is_wrapped)

Indicate whether the page’s so-called graphic state is balanced. If `False`, [`Page.wrap_contents()`](#Page.wrap_contents) should be executed if new content is inserted (only relevant in `overlay=True` mode). In newer versions (1.24.1+), this check and corresponding adjustments are automatically executed – you therefore should not be concerned about this anymore.

Return type:

bool

Page.get_text_blocks(flags=None)[¶](#Page.get_text_blocks)

Deprecated wrapper for [`TextPage.extractBLOCKS()`](textpage.html#TextPage.extractBLOCKS).  Use [`Page.get_text()`](page.html#Page.get_text) with the “blocks” option instead.

Return type:

list[tuple]

Page.get_text_words(flags=None, delimiters=None)[¶](#Page.get_text_words)

Deprecated wrapper for [`TextPage.extractWORDS()`](textpage.html#TextPage.extractWORDS). Use [`Page.get_text()`](page.html#Page.get_text) with the “words” option instead.
...
[DisplayList](displaylist.html#displaylist)
...
PDF only: Retrieve a list of [`xref`](glossary.html#xref) of [`contents`](glossary.html#contents) objects of a page. May be empty or contain multiple integers. If the page is cleaned ([`Page.clean_contents()`](#Page.clean_contents)), it will be no more than one entry. The “source” of each `/Contents` object can be individually read by [`Document.xref_stream()`](document.html#Document.xref_stream) using an item of this list. Method [`Page.read_contents()`](#Page.read_contents) in contrast walks through this list and concatenates the corresponding sources into one `bytes` object.

Return type:

list[int]

Page.set_contents(xref)[¶](#Page.set_contents)
...
Page.clean_contents(sanitize=True)[¶](#Page.clean_contents)

- 
Changed in v1.17.6

PDF only: Clean and concatenate all [`contents`](glossary.html#contents) objects associated with this page. “Cleaning” includes syntactical corrections, standardizations and “pretty printing” of the contents stream. Discrepancies between [`contents`](glossary.html#contents) and [`resources`](glossary.html#resources) objects will also be corrected if sanitize is true. See [`Page.get_contents()`](#Page.get_contents) for more details.

Changed in version 1.16.0 Annotations are no longer implicitly cleaned by this method. Use [`Annot.clean_contents()`](#Annot.clean_contents) separately.
...
Return the concatenation of all [`contents`](glossary.html#contents) objects associated with the page – without cleaning or otherwise modifying them. Use this method whenever you need to parse this source in its entirety without having to bother how many separate contents objects exist.

Return type:

bytes

Annot.clean_contents(sanitize=True)[¶](#Annot.clean_contents)

Clean the [`contents`](glossary.html#contents) streams associated with the annotation. This is the same type of action which [`Page.clean_contents()`](#Page.clean_contents) performs – just restricted to this annotation.

Document.get_char_widths(xref=0, limit=256)[¶](#Document.get_char_widths)

Return a list of character glyphs and their widths for a font that is present in the document. A font must be specified by its PDF cross reference number [`xref`](glossary.html#xref). This function is called automatically from [`Page.insert_text()`](page.html#Page.insert_text) and [`Page.insert_textbox()`](page.html#Page.insert_textbox). So you should rarely need to do this yourself.

Parameters:

- 
**xref** (*int*) – cross reference number of a font embedded in the PDF. To find a font [`xref`](glossary.html#xref), use e.g. *doc.get_page_fonts(pno)* of page number *pno* and take the first entry of one of the returned list entries.

- 
**limit** (*int*) – limits the number of returned entries. The default of 256 is enforced for all fonts that only support 1-byte characters, so-called “simple fonts” (checked by this method). All [PDF Base 14 Fonts](app3.html#base-14-fonts) are simple fonts.
...
Document.is_stream(xref)[¶](#Document.is_stream)

- 
New in version 1.14.14

PDF only: Check whether the object represented by [`xref`](glossary.html#xref) is a [`stream`](glossary.html#stream) type. Return is `False` if not a PDF or if the number is outside the valid xref range.
...
Increase the [`xref`](glossary.html#xref) by one entry and return that number. This can then be used to insert a new object.

Return type:

int
:returns: the number of the new [`xref`](glossary.html#xref) entry. Please note, that only a new entry in the PDF’s cross reference table is created. At this point, there will not yet exist a PDF object associated with it. To create an (empty) object with this number use `doc.update_xref(xref, "<<>>")`.

Document.xref_length()[¶](#Document.xref_length)

Return length of [`xref`](glossary.html#xref) table.
...
the number of entries in the [`xref`](glossary.html#xref) table.

recover_quad(line_dir, span)[¶](#recover_quad)

Compute the quadrilateral of a text span extracted via options “dict” or “rawdict” of [`Page.get_text()`](page.html#Page.get_text).

Parameters:

- 
**line_dir** (*tuple*) – `line["dir"]` of the owning line.  Use `None` for a span from [`Page.get_texttrace()`](#Page.get_texttrace).
...
the [Quad](quad.html#quad) of the span, usable for text marker annotations (‘Highlight’, etc.).

recover_char_quad(line_dir, span, char)[¶](#recover_char_quad)

Compute the quadrilateral of a text character extracted via option “rawdict” of [`Page.get_text()`](page.html#Page.get_text).
...
the [Quad](quad.html#quad) of the character, usable for text marker annotations (‘Highlight’, etc.).

recover_span_quad(line_dir, span, chars=None)[¶](#recover_span_quad)

Compute the quadrilateral of a subset of characters of a span extracted via option “rawdict” of [`Page.get_text()`](page.html#Page.get_text).
...
the [Quad](quad.html#quad) of the selected characters, usable for text marker annotations (‘Highlight’, etc.).

recover_line_quad(line, spans=None)[¶](#recover_line_quad)

Compute the quadrilateral of a subset of spans of a text line extracted via options “dict” or “rawdict” of [`Page.get_text()`](page.html#Page.get_text).
...
the [Quad](quad.html#quad) of the selected line spans, usable for text marker annotations (‘Highlight’, etc.).

get_tessdata(tessdata=None)[¶](#get_tessdata)
...
Return the (unique) infinite rectangle `Rect(-2147483648.0, -2147483648.0, 2147483520.0, 2147483520.0)`, resp. the [IRect](irect.html#irect) and [Quad](quad.html#quad) counterparts. It is the largest possible rectangle: all valid rectangles are contained in it.
