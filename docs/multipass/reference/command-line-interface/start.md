<!-- Source: https://documentation.ubuntu.com/multipass/stable/reference/command-line-interface/start/ -->
# start

# start[¶](#start)

The `multipass start` command without any argument will start the [Primary instance](../../../explanation/instance/#primary-instance) (and also create it, if it doesn’t exist). You can also pass one or more instance names or the `--all` option to start more instances at the same time.
...
Only instances in `Stopped` or `Suspended` status can be started. Running instances can be restarted with [`multipass restart`](../restart/), stopped with [`multipass stop`](../stop/), and suspended with [`multipass suspend`](../suspend/).
