<!-- Source: https://documentation.ubuntu.com/multipass/stable/reference/command-line-interface/snapshot/ -->
# snapshot

# snapshot[¶](#snapshot)

See also: [`restore`](../restore/), [`list`](../list/), [`info`](../info/), [`delete`](../delete/)
...
You have the option to specify a snapshot name using the `--name` option, following the same format as the [instance name format](../../instance-name-format/).

If you don’t specify a name, the snapshot will be named “snapshotN”, where N is the total number of snapshots that were ever taken of that instance. You can also add an optional comment with `--comment`. Both of these properties can be later modified with the [`set`](../set/) command, via the snapshot settings keys documented in [Settings](../../settings/).
