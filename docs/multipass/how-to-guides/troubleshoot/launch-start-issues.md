<!-- Source: https://documentation.ubuntu.com/multipass/stable/how-to-guides/troubleshoot/troubleshoot-launch-start-issues/ -->
...
- 
When you launch a new instance, it fails to load the required image due to stale network cache.

- 
The VM didn’t manage to boot properly and didn’t get to the point where it requests an IP address.

- 
The VM requested an IP address, but didn’t obtain one.

- 
The VM obtained an IP address, but:

- 
Multipass can’t find it.

- 
Multipass finds an IP that doesn’t match the one that was assigned to the instance.

- 
SSH doesn’t function properly in the VM, or Multipass is blocked from accessing it.

- 
When you launch a new instance, it times out waiting for initialisation to complete.

## Diagnose your issue[¶](#diagnose-your-issue)

Follow these steps to diagnose your issue and identify the most likely scenario:

- 
If the `multipass launch` command fails with the message “Downloaded image hash does not match”, see [Stale network cache](#launch-start-issues-stale-network-cache).

- 
*(Windows, Hyper-V driver)* Inspect the file `C:\WINDOWS\System32\drivers\etc\hosts.ics` and see if there is more than one entry with your instance name in it. If that’s the case, see [[Windows, with Hyper-V] Stale internet connection sharing lease](#launch-start-issues-stale-sharing-lease).

- 
*(Linux/macOS, QEMU driver)* Inspect the Multipass logs and look for a message mentioning `NIC_RX_FILTER_CHANGED`. This message indicates that the network interface has been initialised.

- 
If you don’t find it, it means that the VM didn’t manage to bring up the interface; see [VM boot failure](#launch-start-issues-vm-boot-failure).

- 
If the message is present, proceed to check DHCP traffic in the next step.

- 
*(Linux/macOS, QEMU driver)* Check DHCP traffic from your host to the instance, to find out if there are requests and replies. Adapt and run the following command *right after starting/launching* the instance:
...
- 
If you see `NIC_RX_FILTER_CHANGED`, you should also see DHCP requests. If you don’t, see [VM boot failure](#launch-start-issues-vm-boot-failure) and please [let us know](https://github.com/canonical/multipass/issues/new/choose).

- 
If you see a DHCP request, but no reply, it means that the VM is still waiting for an IP address to be assigned; see [No IP assigned](#launch-start-issues-no-ip-assigned).
...
### VM boot failure[¶](#vm-boot-failure)
...
- 
Look for the `qemu-system-*` command line corresponding to the failing VM in Multipass logs.

- 
Copy it to an editor and modify it:

- 
Remove `-serial chardev:char0 -nographic`.

- 
Escape any spaces in paths (e.g. `Application Support` should become `Application\ Support`).

- 
Run the edited line in a terminal, with `sudo`. Here is an example:
...
- 
Run the VirtualBox GUI as admin/root:

- 
[macOS] `sudo VirtualBox`

- 
[Windows] Run with `psexec.exe` as explained in [[Windows, VirtualBox driver] Running VirtualBox with the system account](#launch-start-issues-windows-run-virtualbox).

- 
Start or launch the instance with `multipass start|launch`.

- 
Select and attach to the VM in the VirtualBox GUI and observe the boot output. If it eventually arrives at a login screen, it means that the instance should’ve started correctly.

- 
*(Windows, Hyper-V driver)*

- 
Open the Hyper-V Manager GUI (look for it in your Start menu).

- 
Start or launch the instance with `multipass start|launch`.

- 
Select the VM in Hyper-V manager and click “Connect” on the Actions pane, at the right-hand side. Observe the boot output.

#### VM image corruption[¶](#vm-image-corruption)
...
- 
Use the method appropriate for your driver to access the VM’s console, as described in the [VM boot failure](#launch-start-issues-vm-boot-failure) section.
...
- 
**Alternatively, Drop to a Root Shell**
...
- 
*(Linux/macOS)* Alternatively, run `fsck` over a mounted image on the host (see [Reading data from a QCOW2 image](#launch-start-issues-reading-data-from-an-image)).
...
- 
See [Locating Multipass binaries](#launch-start-issues-locate-multipass-binaries) below.

- 
See [Locating Multipass images](#launch-start-issues-locate-multipass-images) below.
...
- 
If none of the above works, you can still try to mount the image manually to recover data (see [Reading data from a QCOW2 image](#launch-start-issues-reading-data-from-an-image)).

### No IP assigned[¶](#no-ip-assigned)
...
#### [macOS, QEMU driver] Firewall blocks `bootp`[¶](#macos-qemu-driver-firewall-blocks-bootp)
...
See also: [How to troubleshoot networking](../troubleshoot-networking/).

#### [macOS, QEMU driver] `bootp` not coming up[¶](#macos-qemu-driver-bootp-not-coming-up)
...
### [Windows, with Hyper-V] Stale internet connection sharing lease[¶](#windows-with-hyper-v-stale-internet-connection-sharing-lease)
...
- 
Mount the instance’s image file on your host (see [Reading data from a QCOW2 image](#launch-start-issues-reading-data-from-an-image)) and make necessary changes through the filesystem.
...
- 
Back up your existing `cloud-init-config.iso`.

- 
Make a new instance by running `multipass launch --cloud-init config.yaml`, the contents `config.yaml` are shown below.

- 
Replace your existing `cloud-init-config.iso` with the newly generated `cloud-init-config.iso`.

- 
Restart the VM and use the password `ubuntu`. The instance’s password will remain `ubuntu` unless it is changed again
...
### Cloud-init tarries during an instance launch[¶](#cloud-init-tarries-during-an-instance-launch)
...
### [Windows, VirtualBox driver] Running VirtualBox with the system account[¶](#windows-virtualbox-driver-running-virtualbox-with-the-system-account)

To run the VirtualBox GUI with the system account, you will need a Windows tool called PsExec:

- 
Install [PsExec](https://docs.microsoft.com/en-us/sysinternals/downloads/psexec).
...
- 
Execute `psexec.exe -s -i "C:\Program Files\Oracle\VirtualBox\VirtualBox.exe"` (adapt the path to the VirtualBox executable as needed).

When successful, you should see Multipass’s instances in VirtualBox

### Stale network cache[¶](#stale-network-cache)
...
### Reading data from a QCOW2 image[¶](#reading-data-from-a-qcow2-image)
...
### Locating Multipass binaries[¶](#locating-multipass-binaries)
...
### Locating Multipass images[¶](#locating-multipass-images)
