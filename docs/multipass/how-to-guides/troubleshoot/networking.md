<!-- Source: https://documentation.ubuntu.com/multipass/stable/how-to-guides/troubleshoot/troubleshoot-networking/ -->
...
## Troubleshoot networking on macOS[¶](#troubleshoot-networking-on-macos)
...
- 
Provide DHCP and DNS resolution on this switch at 192.168.64.1 (via bootpd & mDNSResponder services running on the host). This is configured by an auto-generated file (`/etc/bootpd.plist`), but editing this is pointless as macOS regenerates it as it desires.
...
### Tools known to interfere with Multipass[¶](#tools-known-to-interfere-with-multipass)
...
- 
macOS updates can make changes to the firewall and leave instances in unknown state; see [Issues caused by macOS update](#troubleshoot-networking-issues-caused-by-macos-update) below.
...
- 
see [Generic networking problems](#troubleshoot-networking-generic-networking-problems)

- 
`multipass shell <instance>` fails

- 
see [Network routing problems](#troubleshoot-networking-routing-problems)

- 
`multipass shell <instance>` works but the instance cannot connect to the internet

- 
see [DNS problems](#troubleshoot-networking-dns-problems)

- 
extra IPs not reachable between instances

- 
see [ARP problems](#troubleshoot-networking-arp-problems)

#### Generic networking problems[¶](#generic-networking-problems)
...
See also: [How to troubleshoot launch/start issues](../troubleshoot-launch-start-issues/).
...
#### Potential workaround for VPN conflicts[¶](#potential-workaround-for-vpn-conflicts)
...
#### Configure Multipass to use a different subnet[¶](#configure-multipass-to-use-a-different-subnet)
...
- 
Use a [custom cloud-init to set /etc/resolv.conf](https://cloudinit.readthedocs.io/en/latest/reference/yaml_examples/resolv_conf.html) for you on first boot.

#### ARP problems[¶](#arp-problems)
...
#### Issues caused by macOS update[¶](#issues-caused-by-macos-update)
...
## Troubleshoot networking on Windows[¶](#troubleshoot-networking-on-windows)
...
#### Default switch going awry[¶](#default-switch-going-awry)
...
#### Anti-virus / security software blocking instances[¶](#anti-virus-security-software-blocking-instances)
