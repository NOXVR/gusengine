<!-- Source: https://documentation.ubuntu.com/multipass/stable/explanation/driver/ -->
# Driver
...
See also: [How to set up the driver](../../how-to-guides/customise-multipass/set-up-the-driver/), [`local.driver`](../../reference/settings/local-driver/), [Instance](../instance/), [Platform](../platform/)
...
Multipass relies on a driver to operate. It supports multiple drivers, but it runs with a single driver at a time. There is a [Multipass setting](../../reference/settings/) to select the driver: [`local.driver`](../../reference/settings/local-driver/).
...
## Instance scopes[¶](#instance-scopes)

In general, Multipass instances are tied to a single driver, with the [Exceptions](#explanation-driver-exceptions) listed below. The set of instances that were launched with one driver are available only while that driver is in use.
...
This affects the `--type` option in the [`mount`](../../reference/command-line-interface/mount/) command).
...
This affects the [`networks`](../../reference/command-line-interface/networks/) command, as well as the `--network` and `--bridged` options in [`launch`](../../reference/command-line-interface/launch/).
...
This affects the [`clone`](../../reference/command-line-interface/clone/) command.
...
This affects the [`suspend`](../../reference/command-line-interface/suspend/) command.

Note

There are also feature disparities depending on the host platform. See [Platform](../platform/) for more details.